...
function replaceImage(target, altURLs)
{
	...
	{	
		var subPromise = new Promise(function(resolve,reject) {
			...
			{
				testURL(url).then(function(secondResp) {
					if(!secondResp.responseHeaders.includes("Content-Type: text/html")) resolve(secondResp.finalUrl);
					else return testURL("/web/1/" + originalURL).then(function(lastResort) {
						if(!lastResort.responseHeaders.includes("Content-Type: text/html")) resolve(lastResort.finalUrl);
						else resolve(null);
					});
				})
			}
...
{
	url = url.replace("/http","im_/http").replace("http://web.archive.org","");
...
function evaluateImage(pic)
{
	...
	GM_xmlhttpRequest({
		...
		onload: function(response) {
			...
			if(response.responseHeaders.includes("Content-Type: text/html") || response.responseText.toUpperCase().includes("<BODY>"))
			{
