...
// @exclude     http://forum.fussballcup.de/newattachment.php*
// @exclude     http://forum.fussballcup.de/inlinemod.php*
// @exclude     http://forum.fussballcup.de/login.php*
...
function getFreundschaftsspiele()
{
    ...
    GM_xmlhttpRequest({
        method: "POST",
        url: "http://fussballcup.de/index.php?w=301&area=user&module=Calendar&action=friendly&complex=0&squad=&club=",
    ...
    GM_xmlhttpRequest({
        method: "POST",
        url: "http://fussballcup.de/index.php?w=301&area=user&module=Calendar&action=friendly&complex=0&m="+ month +"&y="+ year +"&squad=0&club=",
        headers: {"Content-Type": "application/x-www-form-urlencoded"},
        onload: function(responseDetails){
            var inhalt = responseDetails.responseText;
            vorhanden += (inhalt.split("tt_dayFree\"").length-2);
...
function createHeader()
{
    ...
    GM_xmlhttpRequest({
        method: "POST",
        url: "http://fussballcup.de/index.php?w=301&area=user&module=main&action=home&_=",
        headers: {"Content-Type": "application/x-www-form-urlencoded" ,
    ...
    GM_xmlhttpRequest({
        ...
        onload: function(responseDetails){
            ...
            if(for_count != -1)
            {
                document.getElementById("tool_verein_name").innerHTML += "<a href='http://fussballcup.de/#/index.php?w=301&area=user&module=mail&action=index&_=1384196361' target='_blank'><img src='https://abload.de/img/icon-1332772_640jjuxp.png' alt='ingame-Mail' height='20px' width='31px' style='margin-left: 5px;' title='Ungelesene Nachricht' border='0'></a>";
                // document.getElementById("tool_header").innerHTML += "<div style='display:inline-block; opacity:0.95; text-align: left;border-bottom-right-radius: 10px; border-bottom-left-radius: 10px; filter:alpha(opacity=95); margin-left: 10px; height: 100%; border: 1px solid #93C971; border-top: 0px; background-color:#DAEAD0; color: black; padding: 5px; width: 150px;'><a href='http://fussballcup.de/#/index.php?w=301&area=user&module=mail&action=index&_=1384196361' target='_blank'>Du hast ungelesene Nachrichten in deinem Postkasten!</a></div>";
            }
        }	
    });
}
