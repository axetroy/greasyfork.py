...
// May need to add a fix for the copyright bug if HQ don't fix it: https://www.waze.com/forum/viewtopic.php?f=1524&t=274510

(function()
{
// global variables
var wmefu_version = "2.26";
var oldVersion;
var prefix = "WMEFU";
var tabAttempts = 0;
var wmeFUAddon;
...
function init2() {
	...
	getId("_cbDisableSaveBlocker").onclick = disableSaveBlocker;
	...
	//event to re-hack toolbar buttons when exiting HN mode
	...
	WMEPC_div.style.margin = "0 5px";
	WMEPC_div.style.fontSize = "16px";
	WMEPC_div.style.cursor = "grab";
	WMEPC_div.title = "Number of segments listed in the URL\nClick to reselect them.";
	...
	WMEPC_div.innerHTML = "<span class='fa fa-link'></span>";
	WMEPC_div.innerHTML += "<span style = 'font-size: 16px'>&nbsp;" + URLSegments + "</span>";
	getId('brand').appendChild(WMEPC_div);
	...
	if (oldVersion != wmefu_version) {
		...
		ChromeWarning() +
		"\n" +
		"Version 2.26 - 2019-02-26\n" +
		"* New Feature: Disable map blocking on save\n" +
		"* New Feature: Control disabled WME warnings\n" +
		"* Bug fix: Fixed clash with WME Chat Addon\n" +
		"* Removed: Fix Bridge button (no longer needed)\n" +
		"\n" +
		"Previous V2 highlights (in the last 12 months):\n" +
		"* 2.25 New Feature: New Permalink Checker\n" +
...
function loadSettings() {
	...
	getId('_cbDisableSaveBlocker').checked = ( options.disableSaveBlocker !== undefined ? options.disableSaveBlocker : false);
}
...
function saveSettings() {
	...
	if (localStorage) {
		...
		options.disableSaveBlocker = getId('_cbDisableSaveBlocker').checked;
...
function shrinkTopBars() {
	...
	if (_cbShrinkTopBars.checked) {
		...
		if (compress > 0) {
			...
			styles += '#edit-buttons > div > .toolbar-button .item-container { padding-left: ' + ['','10px','5px'][compress] + '; padding-right: ' + ['','10px','5px'][compress] + '; }';
			//fix trashcan icon
			styles += '.toolbar .toolbar-button.delete-feature .item-icon { top: ' + ['','5px','2px'][compress] + '; }';
...
function PSclicked() {
	...
	if (W.selectionManager.getSelectedFeatures().length > 0) {
		if (getId("user-info").style.display == "none") {
			...
			getId("edit-panel").style.display = "none";
		} else {
			getId("user-info").style.display = "none";
			getId("edit-panel").style.display = "block";
		}
	}
}
...
function PCclicked() {
	var fname = arguments.callee.toString().match(/function ([^\(]+)/)[1];
	if (q=location.search.match(new RegExp("[?&]segments?=([^&]*)"))) {
		s=q[1].split(',');
		o=[];
...
function disableSaveBlocker() {
	...
	logit("function " + fname + " called", "debug");
	var styles = "";
	if (_cbDisableSaveBlocker.checked) {
