// ==UserScript==
// @name           ✪ AUTO FARM REVOLUTION ✪ 
// @author         Marcos v.s Marques + (Equipe Internacional DS-TW)
...
// @description    Script ECMA5, Revolução dos Auto Farm, modelo de auto farm interativo, com painel de comandos e configurações facilitado, Modelo Somente Saques No ("C")
// @Opções         Totalmente configuravel no painel do aplicativo opção (config)
// @Utilização     Utilizar Unica e Exclusivamente no assistente de saque
// @name_opções    Equipe Tw 100 ainda está em processo de escolha de nomes adequados para tais funções, para colaborar com dicas de nomes adequados para tais funções entre em contato com (tribalwarsbr100@gmail.com) lembrando que os nomes deve ser o menor possivel pois tag grandes para as opções deixa um aspecto feio no conteudo.
// @project        Projeto Piloto, confecção em andamento (Esse conteudo e apenas um (DEMO) 
// @model          Modelo DEMO - Demonstrativo, Não Oficial.
// @security       Conteudo Seguro, Livre de Risco de Banimento
// @Recognition    Agradecimentos a Toda Equipe Ponte (Br-Pt-De-Nl) e toda conecção CMD INTERNATIONAL 
// @grant          GM_getValue
// @grant          GM_setValue
// @grant          unsafeWindow
// @match          https://pt*.tribalwars.com.pt/game.php?*screen=am_farm*
// @include        https://*screen=am_farm*
// @icon           https://imgur.com/41THrXw.png
// ==/UserScript==
...
/* Funções do Scripts Descritas Abaixo
- Passa por todas as aldeias
- passa por todas os farm
- Pula para a próxima aldeia se não houver mais unidades
- informa o usuário quando a proteção do bot vem (recaptcha), o script pára a execução
* Deslocando o título da guia
* Som de advertência
* Notificação na área de trabalho
- A classificação da tabela pode ser definida (Distância / Data, Ascendente / Descendente)
- ID- Aldeia pode ser configurado, onde o script para
- O script pode ser iniciado e parado a qualquer momento
*/
...
$(function(){ // inicia o documento jquery pronto
	...
	storageSet("wait_time_grey2",storageGet("wait_time_grey2",1800000));
	storageSet("nospy_active",storageGet("nospy_active",0));
    ...
    function startRunning() {   
        ...
        (function tick() {
            ...
            //quando acabar unidades da aldeia, mude para o próximo
            current ++;
            if(current>=length) {
                setTimeout(
					nextPage
