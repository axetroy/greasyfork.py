...
JavaScript:var doc=(window.frames.length>0)?window.main.document:document;url = doc.URL;as = doc.getElementsByTagName("a");urlSplit = url.split("screen");theScreen = urlSplit[1];villageId = as[0].href.match(/village=(\d+)/i);t=as[0].href.match(/t=(\d+)/);if(t)t="&"+t[0];else{t=''}nextVillageLink = "village=n" + villageId[1] + "&screen" + theScreen + t;location.search = nextVillageLink;