...
(function() {
    ...
    function initializeRestrictionManager() {
        ...
        sheet.append('h3.modal-title img.icon {float:right;height:20px;width:20px}');
                ...
                if("childList" == mutation.type && mutation.addedNodes.length) {
                    ...
                    if (restrictionsModal) {
                        ...
                        if ("Time based restrictions" == title) {
                            var summaryGroup = $(restrictionsModal).find("modal-body.restriction-summary-group").first().find("div.restriction-summary-group-name");
                            var summaryTitle = summaryGroup.text().replace(/[\x00-\x1F\x7F-\x9F]/g, "");

                            if (summaryTitle.indexOf("Turning") == -1) {
