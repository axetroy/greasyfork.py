...
// @description    Lists first 3 replies after your posts and quotes of your posts up to 20 replies later on the top of the page. Contributions by newtonseple.
...
(function() {
	...
	MalTracker.prototype.loadSettings = function() {
		...
		if(this.settings === null) {
			...
			this.settings.quoteDistance = this.postsPerPage;
			this.saveSettings();
		}
	};
	...
	MalTracker.prototype.getPageCountInfo = function(dom) {
		var lineDivs = x$(dom).find("#content > div:nth-of-type(3) > div");
		
		if(lineDivs.length < 1 || x$(lineDivs[0]).children("a").length === 0) {
			return { current: 1, max: 1 };
		}
		
		var result = x$(lineDivs[0]).text().match(/^Pages \(([\d]+)\).*\[([\d]+)\].*$/);
	...
	MalTracker.prototype.collectPostData = function(dom) {
		...
		x$(dom).find(".forum_border_around").each( function(index, element) {
			...
			var username = x$(element).find("tr:nth-of-type(1) .forum_boardrow2 strong").text();
			var quotedYou = false;
	...
	MalTracker.prototype.processPagePosts = function(topicInfo, postsInfo, pagingInfo) {
		...
		x$.each(postsInfo, function(index, post) {
			...
			if(post.username === this.currentUser) {
				...
				self.safeLog("Found quote for topic " + topicInfo.topicId + ".");
			}
				...
				self.safeLog("Found reply for topic " + topicInfo.topicId + ".");
			}
			
			if(topicInfo.replyCount >= trackMaxDistance) {
				deleteCache = true;
				self.safeLog("Maximum tracked replies reached for topic " + topicInfo.topicId + ", untracking.");
				return false;
			}
		});
		...
		if(postsInfo.length === self.postsPerPage) {
			if(newReplies === 0) {
	...
	MalTracker.prototype.untrackTopic = function(topicId) {
		delete this.topicCache[topicId];
		this.saveTopicCache();
	};
	
	MalTracker.prototype.topicDeleted = function(topicInfo) {
		this.fireTrackerEvent("deleted", topicInfo.replyCount, topicInfo, null, null);
		this.untrackTopic(topicInfo.topicId);
	};
	
	MalTracker.prototype.markTopicChecked = function(topicInfo) {
		topicInfo.lastCheck = (+new Date());
		this.saveTopicCache();
	};
	...
	MalTracker.prototype.analyzePage = function(dom, topicInfo) {
		...
		if(this.topicCache.hasOwnProperty(topicInfo.topicId)) {
			...
			if(deleteCache) {
				...
				this.markTopicChecked(newTopicInfo);
			}
		}
	};
	
	MalTracker.prototype.checkIfTopicExists = function(topicInfo, callbacks) {
		var self = this;
		var topicCheckLink = "/forum/?topicid=" + topicInfo.topicId;
		
		x$.ajax(topicCheckLink, {dataType: "text", timeout: 5000}).done( function() {
			callbacks.exists();
	...
	MalTracker.prototype.queryTopic = function(topicInfo) {
		var self = this;
		var topicCheckLink = "/forum/?topicid=" + topicInfo.topicId + "&show=" + ((topicInfo.checkPage - 1) * self.postsPerPage);
		...
		x$.ajax(topicCheckLink, {dataType: "text", timeout: 5000}).done( function(data) {
			...
			if(response.status == 404) {
				self.safeLog("Received 404 for topic " + topicInfo.topicId + ", checking if exists.");
				
				self.checkIfTopicExists(topicInfo, { exists: function() {
					self.safeLog("Topic still exists, but no posts on new page yet.");
					
					self.markTopicChecked(topicInfo);
					self.scheduleCheckNextTopic(false);
				}, deleted: function() {
					self.safeLog("Topic has been deleted.");
					
					self.topicDeleted(topicInfo);
					self.scheduleCheckNextTopic(false);
				}, error: function() {
					self.safeLog("Received " + response.status + " for topic " + topicInfo.topicId + ", rescheduling with a delay.");
	...
	MalTracker.prototype.findMessageFromLastPage = function(topicId, messageId) {
		var self = this;
		var lastPageLink = "/forum/?topicid=" + topicId + "&goto=lastpost";
		...
		x$.ajax(lastPageLink, {dataType: "text", timeout: 5000}).done( function(data) {
			...
			if(messageElement.length > 0) {
				...
				var isLastPostOnPage = allMessages.length === self.postsPerPage && x$(allMessages[self.postsPerPage - 1]).attr("id") === messageElement.attr("id");
			...
			self.safeLog("Failed to query last page of " + topicId + ".");
		});
	};
	...
	MalTracker.prototype.checkCurrentTopic = function() {
		...
		if(yourMessageId !== null) {
			var isLastPostOnPage = posts.length === self.postsPerPage && postsAfterYou === 0;
	...
	MalTracker.prototype.renderOneEvent = function(event) {
		...
		if(event.type !== "deleted") {
			...
			topicLink.attr("href", "http://myanimelist.net/forum/?topicid=" + event.topicId + "&show=" + ((event.page - 1) * this.postsPerPage) + "#msg" + event.post.id);
			...
			eventDiv.css("background-color", "#c58080");
			eventDiv.html("Topic has been deleted: ");
			
			var topicSpan = x$("<span/>").css("width", "100%").css("border-bottom", "1px solid #c0c0c0");
			topicSpan.text(event.title);
			
			eventDiv.append(topicSpan);
		}
	...
	MalTracker.prototype.createEventsBoxSettingLink = function(linkText, secondary) {
		...
		var settingsBox = $("<div/>", { id: secondary ? "trackerTopics" : "trackerSettingsToggle" })
			.css("cursor", "pointer")
			.css("width", "60px")
			.css("position", "absolute")
			.css("top", "0")
			.css("right", secondary ? "61px" : "0")
	...
	MalTracker.prototype.renderTrackedTopics = function() {
		...
		for(var topicId in this.topicCache) {
			if(this.topicCache.hasOwnProperty(topicId)) {
				...
				var topicLink = x$("<a/>", { id: "rawr" + topicId ,href: "http://myanimelist.net/forum/?topicid=" + topic.topicId + "&show=" + ((topic.startPage - 1) * self.postsPerPage) + "#msg" + topic.messageId });
