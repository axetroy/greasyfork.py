...
// @homepage       https://greasyfork.org/en/scripts/39066
...
(function() {
    ...
    var version = '2.23';
    var idn = 'work_find';
    var str_url = 'https://greasyfork.org/en/scripts/39066-hwm-work-find';
    ...
    function show_dialog_base(ASettingsHTML)
    {
        ...
        if( !bg ) {
            ...
            bgc.innerHTML =
                ...
                '      <tr>' +
                '        <td class="small_text" >' +
                '          <a class="small_text" href="'+str_url+'" target="_blank">Обновить</a>&nbsp;&nbsp;' +
                '          <a class="small_text" href="'+str_url_aut+'" target="_blank">Нашли ошибку?</a>&nbsp;&nbsp;' +
                '          <a class="small_text" href="'+str_url_sps+'" target="_blank">Сказать спасибо</a>&nbsp;&nbsp;' +
                '          <a class="small_text" style="left:calc(50% - 100px);" href="javascript:void(0);" id="settings_reset'+idn+'">Скинуть все настройки</a> ' +
                '        </td>' +
                '      </tr>' +
                '      <tr>' +
                '        <td class="small_text" >' +
                '          Автор: <a class="small_text" href="/pl_info.php?id=30547" target="_blank">Pahan.</a>' +
                '        </td>' +
                '      </tr>' +
                '    </table>' +
                '  </center>' +
    ...
    var Pahan_locArr = new Array(
        ...
        [ 6 ,   50 ,  48 ,  1 ,  'Peaceful Camp' ,      'Мирный Лагерь' ,     172,205,210],
...
*/
    ...
    function show_settings()
    {
        ...
        var LStr =
            ...
            '      <tr>' +
            '        <td>' +
            '          <label style="cursor:pointer;"><input type="checkbox" id="id_return_after_work" title=""> Возвращаться на страницу после устройства на работу</lable>' +
            '        </td>' +
            '      </tr>' +
            '      <tr>' +
            '        <td>' +
            ...
            '      <tr>' +
            '        <td>' +
            '          <label style="cursor:pointer;"><input type="checkbox" id="id_scan_other_locations" title=""> Искать в других локациях (только с транспортом)</lable>' +
            '        </td>' +
            '      </tr>' +
            '      <tr>' +
            '        <td>' +
            ...
            '            Показывать после устройства окно с надписью "Вы устроены на работу."</label">' +
            '        </td>' +
            '      </tr>' +
            '      <tr>' +
            '        <td>' +
    ...
    function ObjectProcess(ALocation, AObjLink, AObjHTML)
    {
        ...
        if (ALocation == 0)
        {
            ...
            if ((res = re.exec(AObjHTML)) != null)
            {
                ...
                if (LLastCode != '') {
                    LLastCodeInfo = '<div><font color="blue"><b>Последний введённый код: ' + LLastCode + '</b></font></div>';
                ...
                var Sform = res[0];
                Sform = Sform.replace("return false;} return true;}",
                                      "return false;} " +
                                      "localStorage['HWMlastWorkCode'] = document.getElementById('code').value; " +
                                      "var currentMoment = new Date(), " +
                                      "currentMomentOnServer = new Date(Date.now() + currentMoment.getTimezoneOffset()*60000 + 10800000); " +
                                      "localStorage['HWMlastWorkCodeSavingMoment'] = currentMomentOnServer.getTime(); " +
                                      "return true;}");
                if (GM_getValueExt('last_code_buffer_copy', 0) == 1) {
                    var lastCode = localStorage["HWMlastWorkCode"];

                    var currentMoment = new Date(),
                        currentMomentOnServer = new Date(Date.now() + currentMoment.getTimezoneOffset()*60000 + 10800000),
                        HWMlastWorkCodeSavingMoment = new Date(parseInt(localStorage["HWMlastWorkCodeSavingMoment"]));

                    if (lastCode && !(
                        (currentMomentOnServer.getHours() !== HWMlastWorkCodeSavingMoment.getHours())||
                        (currentMomentOnServer.getTime() - HWMlastWorkCodeSavingMoment.getTime() > 3600000)
                       )){
                        LLastCodeInfo = "<div><b>Последний введённый код: <font color='blue'>" + lastCode + "</b></font> "+
                            "<button onclick='insertCode()'>Вставить</button></div>";
                        Sform = Sform.replace("function obj_c(){",
                                          "function insertCode() {document.getElementById('code').value = localStorage['HWMlastWorkCode'];}function obj_c() {");
                    }
                }
                ShowFindWorkHTML(GetObjectInfo(AObjHTML) + LLastCodeInfo + Sform, true);
                AddRealZPInfo();

                var obj_c = document.createElement("script"),
                    change_tr = document.createElement("script"),
                    obj_c_old = document.querySelector("td.wb[width='140']").nextSibling,
                    change_tr_old = obj_c_old.nextSibling.nextSibling;
                obj_c.innerHTML = obj_c_old.innerHTML;
                change_tr.innerHTML = "function change_tr(){ var r = 'йцукенгшщзфывапролдячсмитьЙЦУКЕНГШЩЗФЫВАПРОЛДЯЧСМИТЬ', e = 'qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM', u = 'йцукенгшщзфівапролдячсмитьЙЦУКЕНГШЩЗФІВАПРОЛДЯЧСМИТЬ'; var s = document.getElementById('code').value; var l = s.length; var i = 0; var j = 0; var c = ''; for (i < 0; i < l; i++) { c = s.substr(i, 1); j = r.indexOf(c); if (j >= 0) { s = s.substr(0, i) + e.substr(j, 1) + s.substr(i + 1, l); } else { j = u.indexOf(c); if (j >= 0) s = s.substr(0, i) + e.substr(j, 1) + s.substr(i + 1, l); } } document.getElementById('code').value = s; }";
                document.getElementsByTagName("body")[0].appendChild(obj_c);
                document.getElementsByTagName("body")[0].appendChild(change_tr);
                obj_c_old.parentNode.removeChild(obj_c_old);
                change_tr_old.parentNode.removeChild(change_tr_old);
    ...
    {
        var LCoefsGR = new Array(1.0, 1.3, 1.4, 1.5, 1.6, 1.8, 2.0, 2.2, 2.4, 2.6, 2.8, 3.0, 3.2, 3.4, 3.6, 3.8, 4.0);
