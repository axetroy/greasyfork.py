// ==UserScript==
// @name         Slither.io-bot 1.0.0 by Wortex
// @namespace    http://slither.io/
// @version      1.0.1(Скрипт-написал-Wortex)(https://vk.com/prostowinstonvk)
// @description  Slither.io bot 1.0.0
// @author       Wortex
...
var userInterface = window.userInterface = (function() {
    ...
    return {
        ...
        onPrefChange: function() {
            ...
            oContent.push('[T] Вкл/выкл бот: ' + ht(bot.isBotEnabled));
            oContent.push('[C] обнаружение столкновений вкл/выкл: ' + ht(window.collisionDetection));
            oContent.push('[O] Мобильная версия вкл/выкл: ' + ht(window.mobileRender));
            oContent.push('[A/S] Радиус множетель(S-меньше,A-Больше): ' + window.collisionRadiusMultiplier);
            oContent.push('[I] Авто спавн вкл/выкл: ' + ht(window.autoRespawn));
            oContent.push('[Y] Визуальная откладка вкл/выкл: ' + ht(window.visualDebugging));
            oContent.push('[U] журнал отладки: ' + ht(window.logDebugging));
            oContent.push('[Колесо(мышки)] Зум (верх увелечить, вниз уменьшить');
            oContent.push('[Z] Сброс Зума.');
            oContent.push('[ESC] Респавн');
            oContent.push('[Q] Выйти в меню.');
        ...
        if (login) {
            ...
            stmenu.innerHTML = "Подпишись не ленись <a style='color:#000000;' target='_blank' href='https://vk.com/idtimakarsavin'>Vk![Клик на меня]</a>";
            login.appendChild(stmenu);
            stmenu.appendChild(div);
			stmenu.innerHTML += '<a href="https://vk.com/prostowinstonvk" target="_blank" style="color:#0E0C8B;opacity:2;text-decoration:none;">Мой Вк!(Клик) </a> | <a href="https://vk.com/id134915738" target="_blank" style="color:#1FD41F;opacity:2;text-decoration:none;">Вк помощника!(Клик) </a>'; 
    }
	}
	setMenu();
