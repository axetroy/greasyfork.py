...
GM_addStyle("#tas {position:fixed; top:10%; left:10%; z-index:60000; display:none;\
...
if( document.getElementsByClassName('senior-search-link')[0] === undefined ){
	...
	}else if(elmt=id('search_baidu_promote')){
  ...
  <table style="margin-top:15px; border="0" cellpadding="0" cellspacing="0" width="100%">\
    <tbody>\
      <tr>\
        ...
        <td class="padft"><input id="tas-kw" name="kw" class="i" value="'+document.getElementById("wd1").value+'" size="35" maxlength="100">\
...
let updateCB = function() id("tas-cb").checked = GM_getValue("openintab");
updateCB();

id("tas-cb").addEventListener("click", function(){
...
(function(){
	...
	id("tas-rv").firstChild.nodeValue = value;
  range.addEventListener("change",function(){
    if(value==range.value)return; //防止重複觸發
    id("tas-rv").firstChild.nodeValue = range.value; //TODO	
    value=range.value; //記錄當前值
  });
  //滑鼠操作相關事件處理
...
id("tas-f").onsubmit = function(){
	let	c = id("tas-kw").value.trim(),
		e = id("tas-qw").value.trim(),
		a = id("tas-un").value.trim(),
		d = id("tas-rn").value,
		g = id("tas-sm").value,
		b = "res";
	if (c == "" && e == "" && a == "") {
...
elmt.addEventListener("click", function(){
	updateCB();
	id("tas").style.display = "block";
}, false);
id("tas-close").addEventListener("click", function(){
	id("tas").style.display = "none";
}, false);
