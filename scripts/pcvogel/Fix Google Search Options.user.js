...
(function() {
    ...
    function get_form_imagesearch( type ){
		...
		str = "<div style=\"position:fixed;margin-left:0px; margin-top:100px; display\"><form>" +
				...
				bstr +
				get_form_iXX(/(qdr%3A[^&^,]+)[&]?/ , 'qdr' , ['', 'qdr%3Ad','qdr%3Aw','qdr%3Am','qdr%3Am3','qdr%3Ay','qdr%3Ay3',],['期間指定なし','24 時間以内','1 週間以内','1 か月以内','3 か月以内','1 年以内','3 年以内']) +
				"</form></div>";
		return str;
    }
	...
	if( document.URL.indexOf("sout=1") >=0 && ( document.URL.indexOf("tbm=isch") >= 0 || document.URL.indexOf("/images?") >=0 ) ){
		...
		e = document.getElementById('logo').parentNode;
		e.innerHTML = e.innerHTML + get_form_imagesearch(1);
        ...
        if( document.getElementById('hdtb-msb-vis') ){
            e = document.getElementById('hdtb-msb-vis');
            e.innerHTML = e.innerHTML + get_form_imagesearch(1);
        }else{
            e = document.getElementById('logo').parentNode;
            e.innerHTML = e.innerHTML + get_form_imagesearch(1);
		...
		// search for firefox
		e = document.getElementById('logo').parentNode;
		e.innerHTML = e.innerHTML + "<div style=\"position:fixed;margin-left:0px; margin-top:100px; display\"><form>" + get_form_lr() + "<br><br>" + get_form_qdr() + "</form></div>";
	}else if( document.getElementById('logocont') ){
		e = document.getElementById('logocont');
		e.innerHTML = e.innerHTML + "<div style=\"position:fixed;margin-left:0px; margin-top:100px; display\"><form>" + get_form_lr() + "<br><br>" + get_form_qdr() + "</form></div>";
		...
		document.getElementsByTagName("head")[0].appendChild(style);
		style.sheet.insertRule("#center_col {margin-left:130px !important;}" , style.sheet.cssRules.length );
		e = document.getElementById('gbq1');
		e.innerHTML = e.innerHTML + get_form_imagesearch(0);
		...
		e = document.getElementById('gbq1');
		e.innerHTML = e.innerHTML + "<div style=\"position:fixed;margin-left:0px; margin-top:100px; display\"><form>" + get_form_lr() + "<br><br>" + get_form_qdr() + "</form></div>";
    }else if( document.getElementById('hdtb-msb-vis') ){
		e = document.getElementById('hdtb-msb-vis');
		e.innerHTML = e.innerHTML + "<div style=\"position:fixed;margin-left:10px; margin-top:100px; display\"><form>" + get_form_lr() + "<br><br>" + get_form_qdr() + "</form></div>";
	}else{
    }
})();
