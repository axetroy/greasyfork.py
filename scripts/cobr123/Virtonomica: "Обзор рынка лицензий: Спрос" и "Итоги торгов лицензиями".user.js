...
if (matches == null){
    ...
    //
    if (/\w*virtonomic\w+.\w+\/\w+\/main\/globalreport\/technology_target_market\/total/.test(window.location)) {
		setTechList(set_color_teh);
		var link = null;
		$('table[class="list"] > tbody > tr > td > a').each(function(){
        	link = $(this);
			link.attr('href', link.attr('href')+'/ask');
			link.attr('onclick', 'return doWindow(this, 1000, 600);');
			//console.log(link.attr('href'));
		});
	}
    if (/\w*virtonomic\w+.\w+\/\w+\/main\/globalreport\/technology_market\/total/.test(window.location)) {
		var cell = null;
		var svNewVal = '';
		$('table[class="list"] > tbody > tr > td[title^="$"]').each(function(){
        	cell = $(this);
