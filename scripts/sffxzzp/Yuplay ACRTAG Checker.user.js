...
(function() {
    ...
    function addInfo(subid, res) {
        ...
        var prCountries = /PurchaseRestrictedCountries .*[\s\S].*countries-list">(.*?)<\/td>/ig.exec(res);
        var orCountries = /onlyallowrunincountries .*[\s\S].*countries-list">(.*?)<\/td>/ig.exec(res);
        if (prCountries) {
