...
function getSegmentMidPoint(seg, end) {
	...
	for(var i=0; i<points-1; i++) {
		...
		if (midlen >= length1 && midlen < length2) {
			...
			var x = p1.x + (p2.x - p1.x) * proc;
			var y = p1.y + (p2.y - p1.y) * proc;
			return OpenLayers.Layer.SphericalMercator.inverseMercator(x, y);
		}
	}
...
function addLabel(lines, speedtekst, routespeedsoption2, odctime, odclen, routespeedsoption4, id) {
	...
	if (numlines >= 2) {
		...
		var p1 = lines[ps].components[0];
		var p2 = lines[ps].components[1];
		var proc = 0.5;
		...
		var sx = p1.x + (p2.x - p1.x) * proc;
		var sy = p1.y + (p2.y - p1.y) * proc;

		if (routespeedsoption2) speedtekst = odctime + "s ";

		var pt = new OpenLayers.Geometry.Point(sx, sy);
		var textFeature = new OpenLayers.Feature.Vector( pt, {labelText: speedtekst, fontColor: kolor1, pointRadius: 0 } );
		return textFeature;
	}
	else if (numlines==1) {
		var p1 = lines[0].components[0];
		var p2 = lines[0].components[1];

		var sx = (p1.x + p2.x) * 0.5;
		var sy = (p1.y + p2.y) * 0.5;
...
function createMarkers(lon1, lat1, lon2, lat2, disp) {
	...
	if (markerA == undefined && markerB == undefined) {
		...
		var p1 = new OL.Geometry.Point(lon1, lat1).transform(epsg4326, epsg900913);
		var p2 = new OL.Geometry.Point(lon2, lat2).transform(epsg4326, epsg900913);
		...
		var wh = require("Waze/Handler/DragElement");
		...
		markerA.dragging.done = function(e) {
			...
			var lon1 = parseInt(lonlatA.lon * 1000000.0 + 0.5) / 1000000.0;
			var lat1 = parseInt(lonlatA.lat * 1000000.0 + 0.5) / 1000000.0;
			var lon2 = parseInt(lonlatB.lon * 1000000.0 + 0.5) / 1000000.0;
			var lat2 = parseInt(lonlatB.lat * 1000000.0 + 0.5) / 1000000.0;
...
function loopWMERouteSpeeds() {
	...
	if(rlayers.length == 0) {
		...
		I18n.translations.en.layers.name["__DrawRouteSpeeds1"] = "Route Speeds";
		I18n.translations.en.layers.name["__DrawRouteSpeeds2"] = "Route Speeds 2";
		I18n.translations.en.layers.name["__DrawRouteSpeeds3"] = "Route Speeds 3";
		I18n.translations.en.layers.name["__DrawRouteSpeeds4"] = "Route Speeds 4";
		I18n.translations.en.layers.name["__DrawRouteSpeeds5"] = "Route Speeds 5";
	...
	if (mlayers.length == 0) {
		...
		I18n.translations.en.layers.name["__DrawRouteSpeedsMarkers"] = "Route Speeds Markers";
...
function createRouteFeatures(id, routewsp, routeodc) {
	...
	for(var i=0; i<routewsp.length-1; i++) {
		var wsp1 = routewsp[i + 0];
		var wsp2 = routewsp[i + 1];

		var dlon = Math.abs(wsp1.x - wsp2.x);
		var dlat = Math.abs(wsp1.y - wsp2.y);
	...
	for(var i=0; i<routewsp.length-1; i++) {
		...
		var dx = Math.abs(wsp1.x - odcx);
		var dy = Math.abs(wsp1.y - odcy);
		...
		if (dx < 0.000001 && dy < 0.000001) {
			...
			if (!routespeedsoption3) {
				var label = addLabel(lines, speedtekst, routespeedsoption2, odctime, odclen, routespeedsoption4, id);
	...
	if (routespeedsoption7) {
		for(var i=0; i<routeodc.length; i++) {
...
function requestRouteFromLiveMap(x1, y1, x2, y2)
{
	...
	var numPaths = (routespeedsoption5 && routespeedsoption12)
			? numRoutes * 10
			: numRoutes;
			
	var routeType = (routespeedsoption13 == 2)
			? "DISTANCE"
			: (routespeedsoption13 == 3)
					? "TIME"
					: "HISTORIC_TIME";
...
function switchRoute()
{
	...
	if (z1 > z2) { var z = z1; z1 = z2; z2 = z; }
	if (z1 > z3) { var z = z1; z1 = z3; z3 = z; }
	if (z1 > z4) { var z = z1; z1 = z4; z4 = z; }
	if (z1 > z5) { var z = z1; z1 = z5; z5 = z; }
	if (z2 > z3) { var z = z2; z2 = z3; z3 = z; }
	if (z2 > z4) { var z = z2; z2 = z4; z4 = z; }
	if (z2 > z5) { var z = z2; z2 = z5; z5 = z; }
	if (z3 > z4) { var z = z3; z3 = z4; z4 = z; }
	if (z3 > z5) { var z = z3; z3 = z5; z5 = z; }
	if (z4 > z5) { var z = z4; z4 = z5; z5 = z; }
