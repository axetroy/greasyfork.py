// ==UserScript==
// @name        贴吧提醒界面快速回复（牛牛自用）
// @namespace 
// @include     http://tieba.baidu.com/i/*/*
// @version     2.3
// @grant		unsafeWindow
// @require http://static.hdslb.com/js/jquery.min.js
// @description:zh-cn 在I贴吧里点“回复”可快速回复，发表快捷键：【ctrl+回车】或者【alt+s】
// @author      小血~表情插入by妖夏小绯——应用范围、表情分辨率优化：牛牛的约定
// @description ss
// ==/UserScript==
...
$(document).ready(function () {
	...
	zz += '<div style=\'background:rgba(222,222,222,0.5);border:1px solid #ddd; width:260px;height:100px;padding:20px 30px 20px 30px;position:absolute; margin:-150px;border-radius:6px;box-shadow: 0 0 10px #333;text-align:center\'>';
	zz += '<p id=\'msgContent\' style=\'width:100%;text-align:left;line-height:25px;min-height:60px;\'>'
	zz += '</p><span id=\'msgClose\' style=\'background:rgba(222,222,222,0.5);border:1px solid #ddd;color:#000;cursor:pointer;height:24px;text-align:center;width:51px;line-height:24px;display:block;margin:0 auto;\'>关闭</span></div></div></div>';
	$('body').append(zz);
	...
	$('#feed').on('click', '.reply a[href=\'javascript:void(0)\']', function (e) {
		...
		var div = '<div class="edui-dialog-container lzlRecentImgDiv" style="display: none;background:none repeat scroll 0% 0%;padding: 2px;">\
...
function insert_lzl_emoi_td(cc_div) {
	...
	for (var i = 0; i < recentImgData.length; i++) {
		if (i % 14 == 0)
			html += '<tr>';
		html += '<td class="s_face j_emotion lzlrecentImg" border="1" style="border-collapse:collapse;" data-value="0" data-sname="face" data-type="normal" data-class="s_face" data-stype="img"  data-surl="' + recentImgData[i] + '" data-posflag="0" align="center" bgcolor="#FFFFFF" height="40" width="40"><a class="img" href="javascript:void(0)" style="width:30px;height:30px;display:block;color:#000;font-size:14px;text-decoration:none;background-size:contain;background-image:url(\'' + recentImgData[i] + '\');background-repeat:no-repeat">&nbsp;</a></td>';
		if (i % 14 == 13)
			html += '</tr>';
	}
