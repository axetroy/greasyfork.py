...
// @description    Wave builder for Travian Legends and Travian Path to Pandora
// @author         phamduy
...
switch(sLang) {
	case "it-it": //Italian
		langStrings = ["Aggiungi attacco", "Rimuovi l'attacco", "Sposta l'attacco in alto", "Sposta attacco in basso", "Aggiungi più attacchi (1-12 attacchi).", "Intervallo tra attacchi, in millisecondi. L'intervallo minimo è 100 ms.", "Tipo di attacco", "Intervallo", "ms"];
		break;
	case "fr-fr": //French
...
function sendTroops (x) {
	...
	if( x == wCount-1 ) {
		setTimeout(function(){ document.location.href = fullName +'build.php?tt=1&id=39'; }, getRandom(500));
	}
...
var multi = $e('INPUT',[['id','twb_multi'],['type','number'],['value',1],['size','2'],['title',langStrings[4]],['min',1],['max',12],
	['style','width:36px;margin:0 8px;']]);
var hrow = $ee('TR',$am($e('TD'),[multi,plus]));
for( var i=1; i<11; i++ ) {
