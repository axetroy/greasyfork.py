...
(function () {
  new MutationObserver(function () {
    ...
    var vkAbuseButton = document.body.querySelector('a.page_actions_item[onclick*="abuse"]');
    var vkProfileId;
    if (vkAbuseButton) {
      vkProfileId = (vkAbuseButton.getAttribute('onclick').match(/oid: (\d+)/i) || [])[1];
    } else {
      var vkProfileWall = document.body.querySelector('a.ui_tab_sel[href*="/wall"]');
      if (!vkProfileWall) return;
      vkProfileId = (vkProfileWall.href.match(/wall(\d+)/i) || [])[1];
    }
