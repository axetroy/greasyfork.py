...
// @require     https://cdn.jsdelivr.net/gh/gnadelwartz/Enstyler@4.02.181/translations.js
// @require     https://cdn.jsdelivr.net/gh/gnadelwartz/GM_config/GM_config-min.js
// @require      https://cdn.jsdelivr.net/gh/pieroxy/lz-string/libs/lz-string.min.js
// @require     https://cdn.jsdelivr.net/gh/gnadelwartz/sjcl/sjcl.js
// @require     https://cdn.jsdelivr.net/gh/delight-im/ShortURL/JavaScript/ShortURL.min.js
...
function EnstylerDealActions() {
  if (enTransTags = 5, myDealAction) {
    u("footer ul").length && (enDealFooter = "%0D%0A%0D%0A-- %0D%0A" + u("footer ul li p").first().innerHTML.replace(/<br>/g, "%0D%0A").replace(/<.*?>/g, ""));
    ...
    enDealAdd = enLangLocalize(enDealAdd + enDealAction[3] + enDealAction[5], enDealLang, enLANG), enSocialAdd = enLangLocalize(enDealAction[6] + enDealAction[4], enDealLang, enLANG);
  }
...
function EnstylerDealActionsDo() {
  ...
  enMyCSSID = enUserKey(""), (myDealAction || myTouch || myFixHtml || myVotebar || myPrice || enBlackTemp == enValOff) && (s = u(".thread--type-list, .cept-listing--card").length, u("article").not(".enClassActionDone, ." + enClassHidden).each(function(g) {
    if (u(g).addClass("enClassActionDone"), !u(g).hasClass("threadWidget--type-card--item")) if (null !== (e = u(g).attr(enID))) {
      if (n = u(g).find(".thread-title a"), c = "c" != e[0], enBlackTemp != enValOff && (c ? s ? (d = (u(g).find(".thread-title a").text() + " @" + u(g).find("a.user").text()).replace(unwantedRegex[1], " "),
      l = null !== (l = u(g).find(".vote-temp")).text() ? parseInt(l.text()) : 9999) : (l = 9999, d = "") : (d = (u(g).find(".userHtml").text() + " @" + u(g).find("a.user").text()).replace(unwantedRegex[1], " "),
      l = 0), (!enWhiteTrue || !d.match(enWhite)) && (enBlackTemp >= l || enBlackTrue && d.match(enBlack)))) {
        var f = u(g).parent();
        f.hasClass("threadCardLayout--card") ? f.addClass(enClassHidden) : u(g).addClass(enClassHidden), enBlacklisted++;
      }
      myFixHtml && u(g).find(".userHtml").each(function(e) {
        u(e).html(u(e).html().replace(/[^ -~¡-ÿ✘►○●✰€≠]+|(&nbsp;)+|(\n\r)+|<\/p>|<div>/g, " ").replace(/<\/div>/g, "<br>").replace(/<p>|<br>( *<br>)+/g, "<br><br>").replace(/(<li>)(<br>)+|<br>*(<br><\/li>)/g, "$1"));
      }), myPrice && c && (d = u(g).find(".thread-price.text--b")).length && ((o = u(g).find(".cept-dealBtn")).hasClass("ico--type-redirect-white") ? o.html(d.html()) : o.html(d.html() + '<span class="ico ico--type-redirect-white size--all-xl space--l-1"></span>'),
      u(g).find(".cept-tb").html(d.html())), myDealAction && c && (s || (n = u(g).find(".thread-title"), enTransTags = 20), enTranslateGoogle(n), (i = u(g).find(".vote-temp")).length && (i = parseInt(i.text()),
      myVotebar && (l = u(g).find(".vote-temp").attr("class").replace(/.*charcoal|.*vote-temp--/i, ""), 0 > (r = i / (myVotescale / 70) + 5) && (r *= -3), p = g, s || (p = u(u(g).find("div").first())), u(p).prepend('<div class="votebar vote-progress voteBar--' + l + '" style="width: ' + r + '%;"></div>'),
      u(g).hasClass("thread--type-card") && (i = 0), i > myVotescale / 2.51 ? u(p).prepend(enDealFlame) : -myVotescale / 10.1 > i && u(p).prepend(enDealIce))), s ? (u(g).find("span.meta-ribbon.hide--toW3").removeClass("hide--toW3"),
      u(g).find(".threadGrid-title .thread-title.lineClamp--2").removeClass("lineClamp--2"), a = n.first().outerHTML.replace(/\n|\r|\t/g, "").replace(/^.*href="/, "").replace(/".*/, "").replace(/(.*)\/.*-/, "$1/md-"),
      o = enDealComposeAction(u(g), n.text(), a, enDealAdd.replace(enPATTERN[enTITLE], n.text())), u(g).find(".cept-comment-link").append(o)) : (o = enDealComposeAction(u(g), n.text(), enLocParser.toString().replace(/(.*)\/.*-/, "$1/md-"), enSocialAdd.replace(enPATTERN[enTITLE], n.text())),
      u(g).find("a.btn--twitter").parent().append(o))), (t = u(g).find(".userHtml")).length && enTranslateGoogle(t);
    } else u(g).remove();
  }), EnstylerBlacklistShow());
}

function enDealComposeAction(e, t, n, a) {
  100 > (t = encodeURIComponent(("Gnadelwartz" == enUserName ? "KayDealz" : enInterName) + ": " + t.replace(/\r|\n|\t/g, " ").replace(/  */g, " "))).length && e.find("span.thread-price").length && (t += encodeURIComponent(" -> " + e.find("span.thread-price").text().replace(/ |\t/g, "")));
  var o = n.replace(/(.*)\/.*-/, "$1/md-");
  return enCCMail && (o = n.replace(/^https:\/\/.*?\//, "https://dealz.rrr.de/" + enInterName + "/") + "?ID=" + enMyCSSID), a.replace(enPATTERN[enHREF], "\n\r" + n).replace(enPATTERN[enTEXT], truncStringWord(t, 160, "%20") + "&body=" + t + "%0D%0A%0D%0A" + o + enDealFooter);
}
...
function EnstylerPagePickerCreate() {
  if (GM_config.get("enCPageP")) {
    var e, t, n, a = 1, o = 1;
    u(selectList).empty();
    var i = u("nav .cept-last-page");
    if (i.length) {
      var r = u("nav .pagination-page .hide--fromW2");
      isNaN(a = parseInt(r.html())) && (a = 1), isNaN(o = parseInt(i.html())) && (o = a);
    } else o = a = enLocParser.search.length ? parseInt(enLocParser.search.replace(/.*=/, "")) : 1;
    for (var l = 1; o >= l; ) (n = document.createElement("option")).text = l, selectList.add(n), t = l, e = Math.abs(a - l), 10 > l || 5 > e ? l++ : l += 1e3 > l && e > 600 ? e / 3.5 : Math.floor(e / 2),
    e > 9 && l > 9 && (l = 50 > e || 50 > l ? 2 * Math.floor(l / 2) : 5 * Math.floor(l / 5));
    a > o && (o = a), o > t && ((n = document.createElement("option")).text = o, selectList.add(n)), selectList.value = a;
    var s = [ ".js-navDropDown-messages", enPEnum + " js-navDropDown-messages vAlign--all-m" ];
    u(".test-loginButton").length && (s[0] = ".test-loginButton"), u(".voteBar").length && (s = [ ".voteBar--sticky-off--hide.hAlign--all-r", enPEnum + " subNavMenu-link subNavMenu-btn voteBar--sticky-off--hide" ]),
    selectList.setAttribute("class", s[1]), u(s[0]).before(selectList);
  }
}

function EnstylerPagePickerDo() {
  enLocParser.search.length ? selectList.value = parseInt(location.search.replace(/.*=/, "")) : selectList.value = 1;
}

function EnstylerPageAction() {
  var e = document.getElementById(enPEnum), t = "page=" + e.options[e.selectedIndex].value, n = enLocParser.toString().replace(/page=.*|#.*/, "");
  n.endsWith("?") || n.endsWith("&") ? n += t : n += "?" + t, "/deals/" == enSection && (n += "#thread-comments"), window.location = n;
}
...
function EnstylerMenuActions() {
  enInter || EnstylerAddNav("Main", "<EN-LANG:discussion>", EnstylerSiteConfig("discussion"), "enMainDiscussion"), EnstylerAddNav("Main", "<EN-LANG:enstyler>", '<EN-LANG:enhref>" target="_blank', "enMainHomepage", "home"),
  EnstylerAddNav("Main", "Enstyler Discussion", "https://www.mydealz.de/diskussion/enstyler-856062", "enMainHomepage", "page"), EnstylerAddNav("Main", "Enstyler Deal-O-Mat", "https://dealz.rrr.de/deal-O-mat/", "enMainHomepage", "home"),
...
function enAmazonMobileRedirect() {
  ...
  return !!e.startsWith("https://www.amazon") && (GM_config.get("enCRedirect") && (e.includes("/gp/aw/d/") ? window.location.replace(e.replace("/gp/aw/d/", "/dp/")) : e.includes("/gp/aw/ol/") ? window.location.replace(e.replace("/gp/aw/ol/", "/gp/offer-listing/")) : (enDebugLog("AMAZON redirect failed ... redirect to main page"),
  window.location = enLocParser.protocol + enLocParser.host)), !0);
}
...
function EnstylerRedo() {
  EnstylerShowPage(), EnstylerLastSeenDo(), EnstylerPagePickerDo(), EnstylerDealTimeDo(), EnstylerDealActionsDo();
}
...
function EnstylerDelayedInit() {
  ...
  EnstylerMenuActions(), EnstylerPagePickerCreate(), isMobile ? u("#main").on("DOMSubtreeModified", debounce(300, function() {
...
function MAIN() {
  ...
  EnstylerStart(), setTimeout(EnstylerDelayedInit, 300);
}

function enUserstyleDo() {
  u("#EnstylerButton").length || (document.body.appendChild(enGMFrame), addStyleString(enCSS + "#buttons {visibility: hidden;} #ownedButtons {visibility: visible; border: 1px solid red;}"), addStyleString(GM_getValue("Enstyler2_CSS", ""), "url(https://userstyles.org"),
  setTimeout(createEnstylerButton, 1e3));
}
