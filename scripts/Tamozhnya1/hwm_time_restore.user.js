...
CalculateUserName("92571_");
var heartCanvas = document.getElementById("heart");
if(heartCanvas) {
	army_percent = Number(/var heart=(\d+);/.exec(heartCanvas.parentNode.querySelector("script").innerText)[1]);
	heart_scale = Number(/var time_heart=(\d+);/.exec(heartCanvas.parentNode.querySelector("script").innerText)[1]);
	time_l = Math.floor( ( heart_scale * 1000 / 100 ) * ( 100 - army_percent ) );
	time.h = Math.floor( time_l / 1000 );
}
var manaCanvas = document.getElementById("mana");
if(manaCanvas) {
	var mana1 = /var mana=(\d+|"-");/.exec(manaCanvas.parentNode.querySelector("script").innerText)[1];
	var max_mana1 = Number(/var max_mana=(\d+|"-");/.exec(manaCanvas.parentNode.querySelector("script").innerText)[1]);
	var time_mana1 = Number(/var time_mana=(\d+);/.exec(manaCanvas.parentNode.querySelector("script").innerText)[1]);
	if(mana1 == '"-"') {
		manaCurrent = 0;
		manaMax = 0;
	} else {
		manaCurrent = Number(mana1);
		manaMax = Number(max_mana1);
	}
	time_l = Math.floor( ( time_mana1 * 1000 / 100 ) * ( manaMax - manaCurrent ) );
...
function CalculateUserName(nickPrefix) {
	...
	if(enterButton && loginTextBox) {
		enterButton.addEventListener("click", function() { GM_setValue("UserName", nickPrefix + loginTextBox.value + "_"); })
	} else {
