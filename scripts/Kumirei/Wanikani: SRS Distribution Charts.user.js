...
(function() {
		// SETTINGS
		const aggregateApprenticeAndGuru = false;
		...
		if (href == ($('.dropdown-menu a[href^="/users/"]')[0] || {href: "nah"}).href) location = "profile";
		else if (href == "https://www.wanikani.com/dashboard" && displayOnDashboard) location = "dashboard";
		if (location !== false) {
				if (location == "dashboard" && !displayOnDashboard) return;
