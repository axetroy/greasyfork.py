...
function addButton() {
		waitForKeyElements('.usercard-controls', function(){
				...
				$('.usercard-controls').append("<li><button onclick=\"toggleKoichi('"+userName+"', "+userID+")\" class='btn koichiButton' title='Toggle flair for this user'>Toggle Koichi!</button></li>");
...
function koichify(userID, userName) {
		if ($('head style[user-id="'+userID+'"]').length == 0) {
				$('head').append(
						...
						'   [href="/u/'+userName.toLowerCase()+'"] img.avatar,'+
						'	[data-user-id="'+userID+'"] img.avatar {'+
						'       content: url("https://discourse-cdn-sjc1.com/business2/user_avatar/community.wanikani.com/vanilla/45/111900_1.png");'+
						'	}'+
						'   [href="/u/'+userName+'"] .avatar-flair::after,'+
						'   [data-user-id="'+userID+'"] .avatar-flair::after {'+
						'       content: "∞" !important;'+
						'	}'+
						'   [href="/u/'+userName+'"] .avatar-flair,'+
						'   [data-user-id="'+userID+'"] .avatar-flair {'+
						'       color: white;'+
						'       background: #dd0093 !important;'+
						'	}'+
						'   .names .username [href="/u/'+userName+'"] {'+
						'       width: 40px;'+
						'       display: flex;'+
						'   }'+
						'   .names .username [href="/u/'+userName+'"]::before {'+
						'       content: "koichi";'+
						'   }'+
						'</style>'
				);
