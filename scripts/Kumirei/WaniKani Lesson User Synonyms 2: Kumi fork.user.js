// ==UserScript==
// @name          WaniKani Lesson User Synonyms 2: Kumi fork
// @namespace     Kumirei
// @author        Kumirei
// @namespace     https://www.wanikani.com
// @description   Adds User Synonyms to the lessons page. Adds User Synonyms and Notes to unlearned item pages. This is an update of the version by irrelephant (0.2.3), and adds the function to add synonyms during the quiz too. Irrelephant's version was in turn an update of the version that was originally provided by @kobayashi (Takuya Kobayashi) and has been adapted to work with a newer version of WaniKani. The original "WaniKani Lesson User Synonyms" in version 0.1.2 has been broken since November 2017.
// @version       0.3.0
...
(function () {
		...
		waitForKeyElements('#related-items ul', function(e){
				var KFelem = $('#quizLessonSynonyms');
				if (!KFelem.length) {
						$('#item-info-col1').append('<h2>User Synonyms</h2><section id="quizLessonSynonyms" class="user-synonyms"></section>');
						KFelem = $('#quizLessonSynonyms');
				}
				KFelem = KFelem[0];
				flushManagedNotes();
				var currentLesson = $.jStorage.get("l/currentLesson");
				var KFtype;
				if (currentLesson.rad) KFtype = 'radical';
				else if (currentLesson.kan) KFtype = 'kanji';
				else if (currentLesson.voc) KFtype = 'vocabulary';
				UserSynonyms2.load(KFtype, getSynListItem(currentLesson).join(), currentLesson.id, $('#quizLessonSynonyms'));
		});

		$('body').on('keydown', '.user-synonyms-add-form input', function(key){
				if (key.keyCode == 13) {
						key.preventDefault();
						key.stopPropagation();
						$('.user-synonyms-add-form button[type="submit"]').click();
				}
		});
