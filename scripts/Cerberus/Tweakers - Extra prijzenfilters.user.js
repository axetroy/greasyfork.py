...
- Optie toevoegen om bij sorteren overige_producten (zonder prijs, b.v. "wordt verwacht") ook onderdaad te zetten bij weggefilterde producten.
...
var mogelijke_levertijden = [ "one_day_247", "one_day_245", "three_days", "five_days", "eight_days", "twelve_days", "over_twelve_days", "unknown", "preOrder" ];
...
var positie_vinden = setInterval(function(){
        if (menu_geladen){
            ...
            if (document.querySelector('div#filterContainer form#filterForm')) {
                clearInterval(positie_vinden);
                menu_geladen = true;
                l('positie gevonden met interval');
                menu_maken();
            }
        }
}, 100);
...
function menu_maken(){
    ...
    toepasknop.setAttribute("class", 'fancyButton');
    ...
    toepasknop.addEventListener('click', function() {
        if (toepassen_bezig || !document.querySelector('#listingContainer')) {
            l('toepassen_bezig of #listingContainer niet gevonde.')
            return;
        }
        if (!observer) {
    ...
    if (positie){
        positie.insertBefore(formulier, positie.childNodes[0]);
    } else {
    ...
    for (var i = producten.length - 1; i >= 0; i--) {
        ...
        if (prijsetiket && url) {
            ...
            */
            ...
            aantal_producten_tot_nu_toe = aantal_producten_tot_nu_toe + 1;
            // l('aantal_producten_tot_nu_toe = ' + aantal_producten_tot_nu_toe + '; aantal_producten = ' + aantal_producten + '; plaats = ' + plaats);
...
function prijslijst_ophalen(product, prijsetiket, plaats, url){
    // l(typeof GM);
    ...
    if ((typeof GM === 'object')) {
        GM.xmlHttpRequest({
            ...
            onload: function (response) {
                antwoord_verwerken_tot_prijslijst(product, prijsetiket, plaats, url, response);
            }
        });
...
function antwoord_verwerken_tot_prijslijst(product, prijsetiket, plaats, url, response) {
    // l(url);
    ++ firstxr;
    if (firstxr == 1){
        date = new Date();
    ...
    var regex_prijslijst = /<td class="shop-name\b[^<>]*>\s*<p\b[^<>]*>\s*<a\b[^<>]*>([^<>]+)<\/a>\s*(?:<span\b[^<>]*>[^<>]+<\/span>\s*)?<\/p>\s*<\/td>\s*<td[^<>]*>\s*<p class="score\b[^<>]*>\s*<a[^<>]*>\s*<span[^<>]*>(Score: \d+(?:\.\d+)?|Geen score)<\/span>\s*<span class="reviewCount\b[^<>]*>\(\d+\)<\/span>\s*<\/a>\s*<\/p>\s*<\/td>\s*<td[^<>]*>\s*<p\b[^<>]*>\s*<span class="sprite deliveryTime (\w+)"[^<>]*>[^<>"]*<\/span>\s*<\/p>\s*<\/td>\s*<td class="shop-bare-price\b[^<>]*>\s*<p\b[^<>]*>\s*<a\b[^<>]*>\s*(?:&euro;|€) (\d+(?:\.\d+)*(?:,\d+)?)(?:,-)?<\/a>\s*<\/p>\s*<\/td>\s*<td class="shop-price\b[^<>]*>\s*<p\b[^<>]*>\s*<a\b[^<>]*>\s*(?:&euro;|€) (\d+(?:\.\d+)*(?:,\d+)?)(?:,-)?<\/a>\s*<\/p>/gi;
    ...
    if (aantal_winkels > prijsindex) {
        console.warn('De regex kon ' + (aantal_winkels - prijsindex) + ' van de prijzen niet vinden op de prijzenpagina. De regex dient waarschijnlijk aangepast te worden. Het is ook mogelijk dat Uw cookies niet bewaard worden, of dat U ingelogd dient te zijn.\nAantal winkels = ' + aantal_winkels + '; prijsindex = ' + prijsindex + '; ' + url);
    } else if (aantal_winkels < prijsindex) {
        console.warn('De regex heeft op de prijzenpagina ' + (prijsindex - aantal_winkels) + ' prijzen meer gevonden dan stond aangeven op de resultatenpagina; dit zou niet mogelijk moeten zijn.\nAantal winkels = ' + aantal_winkels + '; prijsindex = ' + prijsindex + '; ' + url);
    }
    ...
    laagste_prijs_bepalen(product, prijsetiket, plaats, productobject);
}


function laagste_prijs_bepalen(product, prijsetiket, plaats, productobject){
    ...
    winkelcontrole: for (var i = 0; i < prijslijst.length; i++) {
        ...
        if (score < minimumscore){
            // l('Te lage score voor ' + winkel + ': ' + prijsresultaat[2]);
        ...
        if (verplichte_winkels && verplichte_winkels.length > 0) {
            ...
            for (var s = verplichte_winkels.length - 1; s >= 0; s--) {
                if ( (winkel.indexOf(verplichte_winkels[s]) !== -1) && winkel && verplichte_winkels[s]) {
                    ...
                    var verplichte_winkel_gevonden = true;
                    break;
                }
            }
    ...
    if (laagste_prijs){    // Als er een laagste prijs is (moet sowieso al getal zijn door eerdere filtering).
        ...
        // l(url + ' - Geen prijzen die voldoen aan filters.');
...
function sorteren(){
    ...
    if (sorteren_op_prijs == 'oplopend') {
        ...
        // l('alles sorteren op plaats...');
        var geprijsde_en_overige_producten = geprijsde_producten.concat(overige_producten);
        objecten_sorteren(geprijsde_en_overige_producten, 'plaats', 'laag naar hoog');
        objecten_sorteren(weggefilterde_producten, 'plaats', 'laag naar hoog');
...
function oude_producten_opschonen(){
    ...
    //         // return b.plaats - a.plaats;  // van hoog naar laag
    //         return a.plaats - b.plaats;  // van laag naar hoog
    //     });
...
function l(text, extra) {
    if (debug == true) {
        if ( (trace == true && !(extra == 'notrace') ) || extra == 'trace'){
            console.log(text);
            console.group();
