...
DogMachine.prototype.init = function () {
    ...
    _this.list = $.grep(_this.list, function (v, i) {
        ...
        if (_this.wantTo.test(name)||!_this.shield.test(name)&&price < prc) {
            return v;
        }
    });
...
DogMachine.prototype.bowwow = function (codes) {
    ...
    $.ajax({
        ...
        success: function (r) {
            ...
            } else if (r.code - 0 == -104 || r.code - 0 == -100 || "bottom" in r || r.code - 0 == -117||r.code - 0 == -113) {

            } else if (r.code - 0 == -110) {
                location.href = location.href;
            } else {
                console.log("若该消息多次出现，请至脚本发布页反馈。");
                console.log(r);
...
$(function () {
    ...
    function runAsync() {
        ...
        let stop = setInterval(function () {
            if ($("#pager").children().length>0&&$(".try-button.show:contains('我要申请'),.try-button.show:contains('已申请'),.try-button.show:contains('我要试用')").length>0) {
                clearInterval(stop);
                def.resolve();
            }
        }, 500);
