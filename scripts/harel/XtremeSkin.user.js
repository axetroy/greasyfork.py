// ==UserScript==
// @name         XtremeSkin
// @namespace    XtremeSkin
// @version      1.0
// @description  XtremeSkin- Skin changer for agar.io
// @author       XtremePlayzYT
// @license      MIT
// @match        http://agar.io/*
...
setTimeout(function() {
    ...
    function emitPosition() {
        ...
        socket.emit("pos", {
            "x": x,
            "y": y,
            "dimensions": agar.dimensions,
            "suicide_targets": agar.myCells
        });
    }
    function emitSplit() {
        socket.emit("cmd", {
    ...
    function changeSkin() {
        // Made by Max from StackOverflow (http://stackoverflow.com/users/1891133/max)
        var values = ["%kraken", "%shuttle", "%shark", "%radar", "%nuclear", "%space_dog", "%t_rex", "%spy", "%astronaut", "%meteor", "%toxic", "%evileye", "%raptor"]
        var i = 0;
    ...
    document.addEventListener('keydown', function(e) {
        ...
        switch(key) {
            case 65: // A (Toggle position)
                moveToMouse = !moveToMouse;
                break;
                
            case 68: // D (Toggle Movement)
                toggleMovement();
                break;
                
            case 69: // E (Split bots)
                emitSplit();
                break;
