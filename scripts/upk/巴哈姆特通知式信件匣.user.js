...
if(location.host === 'mailbox.gamer.com.tw' && (document.location.hash.slice(1) === 'notification' || document.location.hash.slice(1) === 's_notification')){
    ...
    window.addEventListener('message', function(e) {
        ...
        m_list.setAttribute('style', 'max-height:' + list_height + 'px;');
        let readU = 0;
        for(let i=1; i<m_data.length; i++){
            let m_div = document.createElement('div');
            let m_nickname = document.createElement('a');
            let m_nickname_outer = document.createElement('p');
            let m_time = document.createElement('span');
            let m_title = document.createElement('a');
            let m_avatar_outer = document.createElement('a');
            ...
            m_div.appendChild(m_avatar_outer);
            m_nickname.setAttribute('href', 'https://home.gamer.com.tw/homeindex.php?owner=' + m_data[i].id);
            m_nickname.setAttribute('target', '_blank');
            ...
            s_to.innerText = '寄給';
            m_div.appendChild(m_nickname_outer);
            m_title.innerText = m_data[i].title;
            m_title.setAttribute('target', '_blank');
            m_div.appendChild(m_title);
            if(m_data[0] === 0){
                m_list.setAttribute('id', 'mailListA');
                m_title.setAttribute('href', 'https://mailbox.gamer.com.tw/read.php?sn=' + m_data[i].sn);
            }else{
                m_nickname_outer.insertBefore(s_to, m_nickname);
                m_list.setAttribute('id', 'mailListB');
                m_list.style.display = 'none';
                m_title.setAttribute('href', 'https://mailbox.gamer.com.tw/reply.php?fw=1&from=sendbox&sn=' + m_data[i].sn);
            }
            ...
            m_time.innerHTML = '<span class="time">' + time_str + '<i class="fa fa-clock-o"></i></span>';
            m_div.appendChild(m_time);
    ...
    function outer(){
        ...
        m_top_btn.setAttribute('href', 'javascript:TOPBAR_show(\'light_3\', \'topbm\')');
