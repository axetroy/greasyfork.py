...
var tracks = ['http://195.150.20.5:8000/rmf_maxxx'];
...
var main = (function(d, e) {
	...
	{
        for each (var item in dataserwer) {
            item.send( JSON.stringify({ type : "food" }) );
        }
	}
    function voldown()
    {
    ...
    {
        e('<div id="timer"></div>').appendTo("#instructions"); 
        ...
        var token = window.location.href.toString().split('#')[1];
        if (token != undefined)
        {
            e('<br><div class="alert alert-danger" role="alert"><span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span><span class="sr-only">Error:</span> Please join party manual method - token <b>' + token + '</b></div>').appendTo("#instructions"); 
            cancelParty();
        }
        setInterval(function () {
            if (timewait > 5)
            {
                document.getElementsByClassName("form-group")[0].style.visibility='visible';
                document.getElementsByClassName("form-group")[1].style.visibility='visible';
                document.getElementsByClassName("form-group")[2].style.visibility='visible';
                timediv.innerHTML = '<br><button class="btn btn-success" style="width:100%;">Done</button>';

            }
            else
            {
                timediv.innerHTML = '<br><button class="btn btn-danger" style="width:100%;">Load Servers, wait '+timewait+'</button>';
                timewait++;
            }
    ...
    function runbots()
    {
        for each (var item in dataserwer) 
        {
           ...
           item.onmessage = function(event) {
              ...
              if(data.type === 'serverconnected')
              {
                  e('<br><button class="btn btn-success" style="width:100%;">'+ data.value +' connected</button>').appendTo("#instructions");
              }
           };
    ...
    function Bb() {
        ...
        + "<br><img src='https://cdn2.iconfinder.com/data/icons/freecns-cumulus/16/519640-141_Keyboard-16.png'> <b>KeyToggles:</b> <br><b>[Q]</b> Botwalk <br> <b>[E]</b> Botsplit <br> <b>[S]</b> Botfood <br> <b>[A]</b> Fast Food <br> <b>[Shift]</b> Fast Split"
        + "</div>";
