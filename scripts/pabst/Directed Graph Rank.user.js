...
// @description    Rank GLB teams based on a directed graph built from each team's wins and losses.
// @namespace      pbr/dgr
...
*/
		var addr = "/game/team.pl?team_id="+ids[i];
...
console.log("calculating ...");
    ...
    for (var i=0; i<ratings.length-1; i++) {
	  ...
	  var textOutput = "# ... Score ... Win ... Loss ... Rec ... Team\n";
...
//    	console.log(i+") "+ratings[i][0]+" -- "+ratings[i][1]+" -- "+ratings[i][2]+"/"+ratings[i][3]);
        ...
        textOutput += (ct)+" ... ";
        textOutput += ratings[i][1].toFixed(2)+" ... ";
        textOutput += ratings[i][2].toFixed(2)+" ... "+ratings[i][3].toFixed(2)+" ... ";
        textOutput += ratings[i][4]+"-"+ratings[i][5]+" ... ";
			  textOutput += ratings[i][0];
			  textOutput += "\n";
    }
    str += "</table>";
