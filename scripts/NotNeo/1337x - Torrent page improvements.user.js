...
// @include      http*://1337x.eu/account
// @include      http*://1337x.eu/torrent/*
// @include      http*://1337x.se/account
// @include      http*://1337x.se/torrent/*
// @include      http*://1337x.is/account
...
if(AreInAccount()) { //if on settings page
    ...
    if(usingIMDBLinker) {
        ...
        if (foundIMDB != null) { //if code was found...
            foundIMDB = foundIMDB.substr(0, 9); //...get it.
            if (!/^tt[0-9]{7}$/.test(foundIMDB)) { //if code is invalid...
        ...
        if(document.getElementsByClassName("torrent-detail")[0]) {
            ...
            document.querySelectorAll(".torrent-category-detail > .list")[0].innerHTML += imdbThing;
            addGlobalStyle(`
.list span.imdbRatingPlugin > .rating .ofTen {padding: 0; line-height: 9px;}
.list span.imdbRatingPlugin > .rating .ofTen::after { content: ""; }
.list span.imdbRatingPlugin > .rating { padding: 0; background: none; height: unset; position: unset; max-width: unset; font-size: 15px; line-height: 15px;}
.list span.imdbRatingPlugin > .rating::after { content: ""; }
.list span.imdbRatingPlugin::after { content: ""; }
.list span.imdbRatingPlugin {color: #000; display: inline-block; font-size: 15px; line-height: unset; padding: 2px; position: unset; background-color: transparent; margin: 3px 0 0 0;}
`);
            ActivateIMDBScript();
        }
    }
    ...
    if(hideStreamButt || hideAnonButt || hideDirectButt) {
        try {
            document.querySelectorAll("div.torrent-category-detail > ul.download-links-dontblock > li > a").forEach(function(el){
