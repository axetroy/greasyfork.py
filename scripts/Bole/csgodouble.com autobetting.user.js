// ==UserScript==
// @name            csgodouble.com autobetting
// @description     A script that automates csgodouble.com betting using martingale system.
// @namespace       automated@Bole
// @version         2.6.5
// @author          Bole
...
function Automated() {
    ...
    menu.innerHTML = '' +
        '<div class="row">' +
            '<div class="col-lg-9">' +
                '<h2>CSGODouble.com Automated <small>by Bole (76561198179454953)</small> <i id="automated-theme-switch" class="fa fa-lightbulb-o" style="cursor: pointer;"></i></h2>' +
            ...
            '</div>' +
        '<div class="form-group">' +
        '<label>Tip the script author for good luck <3</label>' +
                    '<div class="btn-group">' +
                        '<button type="button" class="btn btn-info" id="tip1">Tip 1</button>' +
                        '<button type="button" class="btn btn-info" id="tip10">Tip 10</button>' +
                        '<button type="button" class="btn btn-info" id="tip100">Tip 100</button>' +
                        '<button type="button" class="btn btn-info" id="tip1000">Tip 1000</button>' +
                    '</div>' +
                '</div>' +
        '</div>' +
        '<div class="form-group">' +
        ...
        '</div>' +
        '<div class="checkbox">' +
            '<label><input class="" id="automated-calculate-safe-bet" type="checkbox" ' + (this.calculate_safe_bet ? 'checked' : '') + '> Calculate base bet so you can always double it for 10 times in a row (you need at least 2048 coins for it)</label>' +
        '</div>' +
        '<div class="checkbox">' +
        ...
        '</div>' +
        '<label>Use the referral code "BOLECOINS" for 500 free coins</label>';
    ...
    this.menu = {
        ...
        'tip100': document.getElementById('tip100'),
        'tip1000': document.getElementById('tip1000'),
        ...
        if (!self.running) {
            if (self.updateAll()) {
				if (self.calculate_safe_bet) {
					self.base_bet = Math.floor(self.balance / 2048);
    ...
    this.menu.tip1.onclick = function() {
        self.tip(1);
        self.log('Thank you for the tip of 1 coin *.*');
    };
    
    this.menu.tip10.onclick = function() {
        self.tip(10);
        self.log('Thank you for the tip of 10 coins *.*');
    };
    
    this.menu.tip100.onclick = function() {
        self.tip(100);
        self.log('Thank you for the tip of 100 coins *.*');
    };
    
    this.menu.tip1000.onclick = function() {
        self.tip(1000);
        self.log('Thank you for the tip of 1000 coins *.*');
    };
...
Automated.prototype.tip = function(amount) {
        send({
            "type": "chat",
            "msg": '/send 76561198179454953 ' + amount,
            "lang": LANG
        });
};
