...
// ==UserScript==
// @name           RatingR2 beta - season 43 (darkxuy EDIT)
// @version        4.5
// @description    REREC(+b), Season TI, TrExMa, RatingR2 -facebook compatible
...
if (location.href.indexOf("/players/") != -1){
	...
	recbth.innerHTML = "<b style=\"color: gold;\">REREC v2</b>";
	...
	function seasonTI (playerID, gettr, SI) {
		...
		age = year*1 + month/12;
		var check = today.getTime()-s43.getTime();
		...
		if (wage == 30000 || (playerID > 109044808 && count == 0)) {	// s43 player ID
			sitd.innerHTML = "---";
		}
		else {
		...
		if (wage > 30000 && playerID > 108779964 && age < ageMax) {	// s43 player ID
			wage_rate = 23.75;
