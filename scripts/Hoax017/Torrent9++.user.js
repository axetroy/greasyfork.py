...
// ==/UserScript==
(function () {
	if (location.host.search(/torrents?9\./) < 0 && location.host.search(/t9\.pe/) < 0) {
		return;
	}
				...
				if (option.enable_direct_download || option.enable_description) {
					$.get(Current_Link.attr('href'), function(data) {
						...
						if(option.enable_description) {
							var description = $('div.movie-information p', html);
							description = description[description.length - 2];
							if(!description) {
								description = $('div.movie-information ol', html).text().trim().replace(/\n/g,' ');
							} else {
	...
	if ($("div.header-top a.link_logo").length != 1) {
		return;
	}
    BASE_URL = location.origin.replace('ww1', 'wvw')+'/';
