...
// @version      0.3
// @description  fix nga.blocker一些bug
// @author       OpenGG
...
(function() {
    ...
    var css =
        ...
        "{display:none}." +
        clsBtn +
        "{position:absolute;top:100px;right:30px;opacity:0.6}." +
        clsBtn +
        ":hover{opacity:1}." +
        clsOptions +
        "{position:absolute;top:130px;right:30px;width:600px;padding:10px;background:#abc}." +
        clsOptions +
        " label{display:block}";
    ...
    var showOptions = function showOptions() {
        ...
        container.innerHTML =
            '<label>屏蔽标题关键字（每行一个）：<textarea placeholder="一行一个关键字"></textarea></label><label>屏蔽楼主UID（每行一个）：<textarea placeholder="一行一个UID"></textarea></label><button>保存</button>';
