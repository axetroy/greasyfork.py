...
// @require             https://cdnjs.cloudflare.com/ajax/libs/viewerjs/1.2.0/viewer.min.js
// @resource css        https://cdnjs.cloudflare.com/ajax/libs/viewerjs/1.2.0/viewer.min.css
// @resource weui       https://cdnjs.cloudflare.com/ajax/libs/weui/1.1.3/style/weui.min.css
    ...
    function thumbnailsBack() {
        ...
        function appendImageList(parent, tid, items) {
            ...
            for (const el of items) {
                ...
                img.setAttribute("bpic", `//imgsrc.baidu.com/forum/pic/item/${picId}`);
                a.appendChild(img);
                li.appendChild(a);
                ul.appendChild(li);
            }
        ...
        function appendSmallWrap(tid, items) {
            const thread = document.querySelector(`#thread_list li[data-field^='{"id":${tid},']`);
            if (!thread) {
                return;
            }
        ...
        // function appendUList(parent: HTMLElement, tid: string, items: NodeListOf<HTMLImageElement>): void {
        //   const ul = document.createElement("ul");
        //   ul.className = "threadlist_media j_threadlist_media clearfix";
        //   ul.id = `fm${tid}`;
        //   ul.style.cssFloat = "left";
        //   const regThumb = /2F([\da-f]{40})\.jpg/i;
        //   const regVideo = /%2Ftieba-smallvideo-([a-z]+)%2F(\d*_[\da-f_]{32,})(_\d+)?\.mp4/i;
        ...
        //       // 视频帖
        //       GM_log(el.outerHTML);
        //       const vHref = el.getAttribute('v_href') as string;
        ...
        //       a.dataset.isfive = "0";
        //       if (vHref.includes('product=tieba-movideo')) {
        //         const thumbnailArrary = regThumb.exec(vHref);
        //         const videoArrary = regVideo.exec(vHref);
        //         if (!thumbnailArrary || !videoArrary) {
        //           GM_log(vHref);
        //           continue;
        //         }
        //         const thumbnail = thumbnailArrary[1];
        //         const video = videoArrary[2];
        //         // let video = videoArrary[1];
        //         // if (video.length > 32) {
        //         //   video = video + videoArrary[2];
        //         // }
        //         if (thumbnail && video) {
        //           img.src = `https://imgsa.baidu.com/forum/pic/item/${thumbnail}.jpg`;
        //           const imgBaseURI = "https://gss3.baidu.com/6LZ0ej3k1Qd3ote6lo7D0j9wehsv/";
        //           a.dataset.video = `${imgBaseURI}tieba-smallvideo-${videoArrary[1]}/${video}.mp4`;
        //           const href = "http://tieba.baidu.com/mo/q/movideo/page";
        //           a.href = `${href}?thumbnail_url=http://imgsrc.baidu.com/forum/pic/item/${thumbnail}.jpg` +
        //             "&video_url=http://tb-video.bdstatic.com/" +
        //             `tieba-smallvideo-${videoArrary[1]}/${video}.mp4&product=tieba-movideo`;
        //           a.dataset.type = "movideo";
        //         }
        //       } else {
        //         img.src = el.getAttribute('src') as string;
        //         a.dataset.video = el.getAttribute('href') as string;
        //         a.dataset.vsrc = vHref;
        //         a.href = '#';
        //       }
        //       div.appendChild(a);
        //       li.appendChild(div);
        ...
        //       const src = el.getAttribute("data-url") as string;
        //       const arr = src.split("/");
        //       const pid = arr.pop();
        //       img.src = src;
        //       img.dataset.original = src;
