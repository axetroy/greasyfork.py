...
// @namespace      https://greasyfork.org/en/users/112150
// @grant          none
// @grant          GM_info
// @version        2018.09.25.01
// @include        /^https:\/\/(www|beta)\.waze\.com\/(?!user\/)(.{2,6}\/)?editor\/?.*$/
// @author         Original script by Rick Zabel '2014, modified for Southeast Region by itzwolf
// @license        MIT/BSD/X11
...
var URCommentUSA_SoutheastVersion = GM_info.script.version;
var URCommentUSA_SoutheastUpdateMessage = "yes"; // yes alert the user, no has a silent update.
var URCommentUSA_SoutheastVersionUpdateNotes = "UR Comments USA_Southeast has been updated to v" + URCommentUSA_SoutheastVersion;
URCommentUSA_SoutheastVersionUpdateNotes = URCommentUSA_SoutheastVersionUpdateNotes + "\n" + "This list will no longer function from the dropdown as \"custom\". You will need to select the USA_Southeast list from the dropdown.";

if (URCommentUSA_SoutheastUpdateMessage === "yes") {
	if (localStorage.getItem('URCommentUSA_SoutheastVersion') !== URCommentUSA_SoutheastVersion) {
		alert(URCommentUSA_SoutheastVersionUpdateNotes);
		localStorage.setItem('URCommentUSA_SoutheastVersion', URCommentUSA_SoutheastVersion);
	}
}
...
window.UrcommentsUSA_SoutheastCloseNotIdentifiedPosistion = 3;
...
window.UrcommentsUSA_SoutheastArray2 = [
	...
	"Volunteer Responding. It appears that you followed the route Waze suggested. Please let us know what went wrong, and tell us your destination as you entered it into Waze. Thank You!",
	"Open", //reworded rickzabel 12/7/2014

	"Alley Interference",
	"Volunteer Responding. Waze does not let the us know where you were going, although it was probably adjacent to the alley. Please tell us your destination as you entered it into Waze. Thank You!",
	"Open", //rickzabel 12/9/14

	"Pave Road",
	"Volunteer Responding. You can pave the road from the app by tapping the orange pin icon > Map Issue > Pave Road tab. After leaving the paved road, tap Start paving. Once done, tap the Steamroller > Stop paving. You can provide information about the new road such as its name by tapping on the orange pin icon > Map Issue > Missing Road. Thank You!",
	"Open", //itzwolf 1/12/18

	"Camera Report",
	"Volunteer Responding. Red light cameras are supported separate of the signal on a post before the intersection so they can photograph the signal and your plate simultaneously. Surveillance cameras mounted on the signal masts are not red light infraction cameras. Is this a red light infraction camera or is this a surveillance camera?",
	"Open", //t0cableguy 7/25/16, itzwolf 1/12/18
	...
	"Closures - Temporary Road Closure",
	...
	"Thank you for reporting a missing place.  Anytime you find a place that is missing from the Waze app you can add it from the app by tapping the orange pin icon, then the Place icon. After taking a picture of the place please add as many details as you can. Thank You!",
	"NotIdentified", //itzwolf 1/12/18

	"Places - Address Fishing",
	...
	"Routing - SunPass Required (Express Ln)",
	"The Express Lanes require adding a SunPass to Waze before you will be routed on them. To do this, please open \"Settings\", \"Navigation\", \"Toll & HOV Passes\", then add SunPass. If you do not see SunPass, please tap \"Show All Passes\". Thank you!",
	"NotIdentified", //itzwolf 3/28/18

	"Routing - SunPass Required (General)",
	"This road requires adding a SunPass to Waze before it will be offered as a route. To do this, please open \"Settings\", \"Navigation\", \"Toll & HOV Passes\", then add SunPass. If you do not see SunPass, please tap \"Show All Passes\". Thank you!",
	"NotIdentified", //itzwolf 3/28/18

	"Routing - Peach Pass Required (Express Ln)",
	"The Express Lanes require adding a Peach Pass to Waze before you will be routed on them. To do this, please open \"Settings\", \"Navigation\", \"Toll & HOV Passes\", then add Peach Pass. If you do not see Peach Pass, please tap \"Show All Passes\". Thank you!",
	"NotIdentified", //itzwolf 8/16/18

	"Routing - Peach Pass Required (General)",
	"This road requires adding a Peach Pass to Waze before it will be offered as a route. To do this, please open \"Settings\", \"Navigation\", \"Toll & HOV Passes\", then add Peach Pass. If you do not see Peach Pass, please tap \"Show All Passes\". Thank you!",
	"NotIdentified", //itzwolf 8/16/18

	"Routing - Add Toll Pass to App",
	"Waze has a new feature for toll pass users. If you have a toll pass subscription, please add it to your settings so the app can use the roads that require a pass. To do this, please open \"Settings\", \"Navigation\", \"Toll & HOV Passes\", then add the pass. If you do not see the pass, please tap \"Show All Passes\". Thank you!",
	"NotIdentified",
	
	"Routing - Toll Route Given (Avoid Tolls)",
	"Volunteer Responding. The avoid tolls option will attempt to avoid tolls unless there is no reasonable route available. When this happens Waze will then route on a toll road to ensure you reach your destination. If the route offered is not acceptable, you may view possible alternates by choosing \"Routes\" in the app. Thank you!",
	"NotIdentified", //DCLemur, itzwolf 9/7/18

	"Routing - Valid Route",
	"We reviewed the issue and did not find any map errors. It looks like Waze provided you with a valid route. Try the Waze suggested route a few times, as it may turn out to actually be faster. If not you'll be teaching Waze that the route is slower, and the faster route will become preferred.",
	"NotIdentified", //GizmoGuy, t0cableguy, rickzabel 1/14/2015, itzwolf 1/12/18

	"Routing - Signal Avoidance",
	"Currently this intersection is mapped properly. Waze has calculated that it may take less time to take the suggested route instead of the more direct route. You can choose to take the direct route or attempt the suggested Waze route to help contribute to Waze's database and possibly save some time. Thank You!",
	"NotIdentified", //t0cableguy 7/25/16, fish1552 11/26/17, itzwolf 2/13/18

	"Routing - Gated Community",
	...
	"Routing - Through Neighborhood",
	"Volunteer Responding. We understand that you are frustrated. Please contact your municipality to determine if there is a way to restrict traffic on a legal public right of way. With respect to liability, drivers (not Waze) are legally responsible for following traffic laws and being actively engaged in the driving process. Thanks.",
	"NotIdentified", //DCLemur 9/25/18
	...
	"NotIdentified", //rickzabel 4/18/2015

	"Routing - Double Yellow Turns",
	...
	"Routing - No User Transponder (Avoid Tolls)",
	...
	"Routing - HOV Route Not Used",
    "Volunteer responding. Waze thought you were in the HOV instead of the main lanes during this route. In the future, if you do not have the required number of passengers for HOV or do not wish to take the HOV Toll, please check your routes prior to departure and select a route that does not include the HOV lanes. Also, you can temporarily disable this by enabling the \"Avoid Tolls\" option. Otherwise, please proceed with the correct exit and Waze will reroute as necessary.  Thank you!",
	"NotIdentified", //dbsooner 3/14/2018

	"Routing - Not Using HOV",
	...
	"Routing - Already Included Restrictions",
	...
	"SL - Variable limits",
	"Volunteer Responding. Currently Waze does not support Variable Speed Limits (VSLs). Since we cannot enter speed limits in real time, the highest possible speed limit is displayed in Waze. If you would like to see speed limits at all times while using the app, please go to Settings > Speedometer > Show speed limit. Thank You.",
	"Solved", //t0cableguy 7/30/16 , fish1552 11/26/17
	...
	"<br><b>Miscellaneous / Other",
	"",
	"Open",
	// The "problem children" aka We can't do anything about it

	"Misc - Application Bug",
	"Volunteer Responding. Unfortunately, In this situation, there is nothing wrong with the map that we can adjust to prevent issues with the app. Please report this to https://support.google.com/waze/answer/6276841",
	...
	"Misc - Duplicate/Stacked Reports",
	"Volunteer Responding. Based on this report's time-stamp, location, and other data, it appears to be a duplicate of one or more reports submitted nearby. A single report is sufficient with as many details as possible, so we will close this as an accidental resubmission. Please be aware that intentionally sending multiple reports for the same map issue over a short time taxes the volunteer map editors and prevents them from helping other reporters. Sending multiple, intentional, or identical reports may be considered spamming, and potentially result in loss of reporting privileges.",
	"NotIdentified", //fish1552 11/26/17, itzwolf 01/11/18

	"Misc - Overall Waze Complaint",
	"Volunteer Responding. You can help make Waze better by reporting problems as you find them. Please include as many details as possible. Thank You!",
	"NotIdentified", //rickzabel & Pesach 12/22/14

	"Misc - Time Zone",
	"Volunteer Responding. Thank you for your report, but at this time Waze does not track time zones. Your arrival time will change when your phone time changes.",
	"NotIdentified", //n4dog 8/8/16, itzwolf 1/12/18

	"Misc - Report to State or Local Agency",
	"Volunteer Responding. We are only able to help with map issues. This should be reported to the appropriate state or local agency. Please feel welcome to report any map issues you encounter. Thank You!",
	"NotIdentified", //GizmoGuy, t0cableguy, rickzabel 1/14/2015

	"Misc - Stale Traffic Information",
	...
	"Misc - Unmapped Features",
	"Volunteer Responding. Waze does not map stop lights, stop signs, toll booths, or draw bridges. However, Waze does track their impact on traffic speeds to ensure you get the fastest route.",
	"NotIdentified", //fjsawicki 5/17/2018
	...
	"Reminder message (Do NOT use in SER!)", //do not change (rickzabel) t0cableguy 7/25/16, itzwolf 1/12/18
	...
	"Hi, again! We're still willing to help if you're interested! If the issue is closed, may we close this report? Please let us know your thoughts within the next day or so. Thanks.",
	"Open", //itzwolf 1/12/18
