...
StarNames = {
    ...
    findRating: function(){
        ...
        var urlMatch = location.href.replace("http://www.csfd.cz","").match(/(\/film\/.*[0-9]+[^\/]+\/)/);
        if(urlMatch){
            shortURL = urlMatch[1];
        } else {
            return;
