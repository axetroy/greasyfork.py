...
(function() {
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(response){
                if(response.statusText == 'OK' && response.responseText){
                   if(/<title>百度网盘-链接不存在<\/title>/g.test(response.responseText) || />啊哦，邀请链接已失效/g.test(response.responseText)){
                       callback(false);
                   } else {
                       callback(true);
                   }
    ...
    if(/^(http|https):\/\/pan.baidu.com\//g.test(document.location.href)){
        ...
        if(html){
            ...
            if(arrs && arrs.length){
                ...
                for(var i = 0; i < arrs.length; i++){
                    var bp = new BaiduPan(arrs[i]);
                    setTimeout(function(){bp.load();}, 1);
                }
            }
            arrs = html.match(/(http|https):\/\/pan.baidu.com\/share\/init\?surl=[a-zA-Z0-9_-]+/g);
            if(arrs && arrs.length){
                index = 0;
                for(var j = 0; j < arrs.length; j++){
                    var bp2 = new BaiduPan(arrs[j]);
     ...
     function getRemoteIp(){
        if(/^https:/g.test(document.location.href)){
            return;
        }
        var weburl = "https://ip.cn/";
        GM_xmlhttpRequest({
            method: 'GET',
            url: weburl,
            contentType: "text/html",
            onload: function(response){
                if(response.status == 200 && response.responseText){
                   if(response.responseText.indexOf("您现在的 IP") != -1 && /<code>[0-9]+.[0-9]+.[0-9]+.[0-9]+<\/code>/g.test(response.responseText)){
                       var g = response.responseText.match(/<code>[0-9]+.[0-9]+.[0-9]+.[0-9]+<\/code>/g);
                       if(g && g.length){
                           var ip = g[0].replace('<code>', '');
                           ip = ip.replace('</code>', '');
                           test(ip);
                       }
                   }
