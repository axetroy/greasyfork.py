...
// @include     http://k0.dreamhosters.com/*
// @include http://k0.dreamhosters.com/00/*
// @exclude	http://k0.dreamhosters.com/00/del.php*
...
if(conf_flag & 256){
	...
	for(var i in k){
		if(k[i].content == "3;URL=index.htm" || k[i].content == "0;URL=index.htm")
			window.location = "http://"+hostid+".komica.org/"+pageid+"/index.htm";
	}
}
...
j.innerHTML = "<small>- <a target='_blank' href='http://userscripts.org/scripts/show/166746'>Komica Uiharu Package</a> -</small><hr>";
j.id = "panel_url";
j.align = "center";
j.style.display = "none";
k.parentNode.insertBefore(j, k);
...
if((conf_flag & 16) && !(window.location.href.match(/.*res=\d*/))){
    ...
    if(tmp){
        if(window.location == "http://"+hostid+".komica.org/"+pageid+"/index.htm"){
            tmp.previousSibling.innerHTML = tmp.previousSibling.innerHTML.replace("以下省略","<a href='http://"+hostid+".komica.org/"+pageid+"/10.htm'>10</a>");
        }else if(tmp.action == "http://"+hostid+".komica.org/"+pageid+"/index.htm"){
            var page_10 = "window.location='http://"+hostid+".komica.org/"+pageid+"/10.htm'";
            tmp.nextSibling.nextSibling.innerHTML = tmp.nextSibling.nextSibling.innerHTML.replace("以下省略","<a href='http://"+hostid+".komica.org/"+pageid+"/10.htm'>10</a>");
            tmp.nextSibling.nextSibling.nextSibling.innerHTML = '<input type="button" value="下一頁" onclick="'+page_10+'">';
        }else if(tmp.action == "http://"+hostid+".komica.org/"+pageid+"/10.htm"){
...
if(conf_flag & 128){
	...
	for(var i in k){
		if(k[i].color == "#707070"){
			k[i].innerHTML = k[i].innerHTML.replace("要閱讀","").replace("下返信連結","<a href='http://"+hostid+".komica.org/"+pageid+"/index.php?res=" + k[i].previousSibling.id.replace(/.*p/,"") + "'>這裡</a>閱讀");
		}
	}
}
...
function pre_fetch_iframe(e){
	if(!window.location.href.match(/res=/)){
		var ext_url = "http://"+hostid+".komica.org/"+pageid+"/index.php?res="+e.target.parentNode.id.replace(/.*p/,"");
