...
(function($) {
	...
	function init(refresh, user, cache, data) {
		...
		// start core of script if 'my list page' and jikan api are loaded
		$.when(loadPage, loadList).then(function () {
            if (mal.seasons.length === 0) {
                alert("Error: no seasons found");
            } else {
                console.log("script started", mal.user);
                startScript();
            }
		});
	}
	...
	function startScript(later = true) {
        ...
        // update season loading status
        $('.js-loading-message').html(season + ' ' + year);
		...
		if (mal.seasons.length === 0 && later) {
            // update season loading status
            $('.js-loading-message').html('later');
	...
	function retryFailedSeasons() {
        if (mal.fseasons.length > 0) {
            // update season loading status
            $('.js-loading-message').html('failed seasons again in 5s');
            ...
            setTimeout( function() {
                ...
                setTimeout(function() {startScript(false);}, 5000);
            }, 5000);
        } else if (!mal.finished) {
            finishScript();
        }
	}
	...
	function finishScript() {
        ...
        // calculate failed anime
        Object.keys(mal.list).forEach(id => {
            if (mal.list[id].loaded) delete mal.list[id];
        });
		console.log('Failed to load the following anime: ', mal.list);

        mal.failed = Object.keys(mal.list).length;
	...
	function loadUserList(object, page = 1) {
		// get userlist
        $('.js-loading-message').html("animelist page " + page);

        fetch('https://api.jikan.moe/v3/user/' + mal.user + '/animelist/all/' + page).then(function(response) {
            return response.json();
        }).then(function(json) {
            let animelist = json.anime;

            for (let anime of animelist) {
                if (anime.season_name !== null) {
                    mal.seasons.push(anime.season_year + '-' + anime.season_name.toLowerCase());
                } else if (anime.start_date !== null) {
                    var ymd = anime.start_date.split('-');
                ...
                anime.start_date = (anime.start_date ? anime.start_date.split('T')[0].replace(/-/g,'') : null);
                anime.end_date = (anime.end_date ? anime.end_date.split('T')[0].replace(/-/g,'') : null);

                mal.list[anime.mal_id] = anime;
            }
            ...
            if (animelist.length === 0) {
                ...
                mal.seasons.sort();
                mal.total = Object.keys(mal.list).length

                object.resolve();
            } else {
                loadUserList(object, ++page);
            }
        }).catch( err => {
            err.text().then( errorMessage => {
                alert("Something failed when retrieving animelist from jikan api: " + errorMessage);
            })
        });
	}
	...
	function addToPage(objectType, element) {
		...
		if (id in mal.list && !mal.list[id].loaded) {
            var anime = mal.list[id];
		...
		$(element).attr("data-sort", '{"members":"' + members + '","start":"' + start + '","end":"' + end + '","title":"' + removequotes(title) + '", \
						"score":"' + score + '","myscore":"' + myscore + score.replace('.','') + '","studio":"' + studio + '","episodes":"' + episodes + '"}');
	...
	function sort(element, sortType) {
        var reverse = ($(element).hasClass("defaultSort") ? -1 : 1);
		...
		$(element).addClass("selected").toggleClass("defaultSort");
				...
				switch(sortType) {
					case 'title':
						listitems.sort(function(a, b) {
							...
							return (compA < compB) ? -1 * reverse : (compA > compB) ? 1 * reverse : 0;
						});
	...
	function initPage(object, refresh, cache, data) {
		...
		$('#content > div.navi-seasonal.js-navi-seasonal > p').html('<b>Loading <span class="js-loading-message"></span>: <span class="js-visible-anime-count">...</span></b>');
