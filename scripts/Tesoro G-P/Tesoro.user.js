// ==UserScript==
// @name         Tesoro
// @namespace    https://www.youtube.com/channel/UCIpCflcKEN9YgaO9qDahpR
// @version      1.6.10
// @description  Hold down S to freeze. Press K to show/hide skins, N to show/hide names, h to show/hide mass, F to show/hide food, H to show/hide chat. For Team Scrimmage players: auto respawn in Team Scrimmage, "Team 2v2" is selected by default, press Enter to queue a match or leave a match that has ended, L to leave a match at any time. You can change the keys if you wish. You can also set custom keys for triple split, show/hide minimap, score panel, party panel, leaderboard, and auto decline party invites.
// @match        *://gota.io/web/*
// @author       Tesoro.21
// @grant        none
// @run-at       document-end
...
(function(){
    ...
    if (!('code' in KeyboardEvent.prototype)) {
        return alert("Sorry, your browser is incompatible with  Tesoro. You're recommended to install the newest version of Google Chrome or Mozilla Firefox browsers.");
    }
    ...
    importantUpdates.sort (compareVersions); 100 fps
    ...
    else localStorage[' Tesoro-version'] = version;

    var styles = {
        '. Tesoro-features-table': {
            'margin': 'auto',
            'width': 'max-content',
            'border-collapse': 'collapse'
        },
        '. Tesoro-features-table td:nth-child(1)': {
            'text-align': 'left'
        },
        '. Tesoro-features-table th, .donut-features-table td': {
            'padding': '2px 8px'
        },
        '. Tesoro-features-table input[type="text"]': {
            'width': '100px',
            'text-align': 'center'
        },
        '. Tesoro-features-div': {
        ...
        '. Tesoro-channel-link>span': {
            'display': 'inline-block',
            '-moz-box-sizing': 'border-box',
            'box-sizing': 'border-box'
        },
        '. Tesoro-channel-link:hover': {
            'background-color': '#cc181e'
        },
        '. Tesoro-channel-link:active': {
            'background-color': '#b31217'
        }
    };

    if (notify) {
        styles['. Tesoro-features-btn::before'] = {
            'content': '"!"',
            'position': 'absolute',
            'top': '-6px',
            'right': '-6px'
        };
        styles['. Tesoro-features-btn::before, .donut-updated'] = {
            'width': '12px',
            'height': '12px',
            'color': 'white',
            'font-size': '11px',
    ...
    btn.className += '  Tesoro-features-btn';
    ...
    btn.innerText = 'Features by tesoro';
    btn.addEventListener('click', function() {
        ...
        if (notify) {
            localStorage[' Tesoro-version'] = version;
    ...
    win.innerHTML = `<table class='Tesoro-features-table'><tbody><tr><th>Feature</th><th>Default<th<><th>Custom key</th></tr>
...
<label><input type="checkbox" class="tesoro-checkbox" id="donut-hidePartyPanel" data-donut-feature="hidePartyPanel">Hide Party Panel</label><br>
<label><input type="checkbox" class="Tesoro-checkbox" id="donut-hideLeaderboard" data-donut-feature="hideLeaderboard">Hide Leaderboard</label><br>
