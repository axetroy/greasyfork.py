...
var YtNewUIFix = /** @class */ (function () {
    ...
    YtNewUIFix.prototype.fixControls = function (css) {
        ...
        css += ".ytp-chrome-controls .ytp-button { width: 33px!important; }\n";
    ...
    YtNewUIFix.prototype.addExtras = function (css) {
        if (this.showControlsFullscreen) {
            css += "html:not(.floater) .html5-video-player.ytp-fullscreen:not(.ytp-hide-controls) .html5-main-video  { height: calc(100% - 35px)!important; min-height: calc(100% - 35px) !important; max-height: calc(100% - 35px) !important; }\n";
        ...
        if (this.showControlsNonFullscreen) {
            css += "html:not(.floater) .html5-video-player:not(.ytp-fullscreen):not(.ytp-hide-controls) .html5-main-video  { height: calc(100% - 35px)!important; min-height: calc(100% - 35px) !important; max-height: calc(100% - 35px) !important; }\n";
            ...
            css += "html:not(.floater):not(.part_fullbrowser) #movie_player:not(.ytp-fullscreen):not(.ytp-hide-controls) { height: calc(100% + 35px)!important; }\n";
            css += "html.floater .html5-video-player { padding-bottom: 35px; }\n";
    ...
    YtNewUIFix.prototype.addOptions = function () {
        ...
        if (localStorage) {
            ...
            accSection.id = "YoutubeNewUIFix-Options";
            ...
            {
                accSection.appendChild(this.createOption("addWatchLater", "Add the watch later button to the controls"));
                accSection.appendChild(this.createOption("changeColorsNonFullscreen", "Change the colors back to their original gray (in non-full-screen mode)"));
                accSection.appendChild(this.createOption("changeColorsFullscreen", "Change the colors back to their original gray in full-screen mode"));
                accSection.appendChild(this.createOption("showControlsNonFullscreen", "Always show the controls (in non-full-screen mode)"));
                accSection.appendChild(this.createOption("showControlsFullscreen", "Always show the controls in full-screen mode"));
                accSection.appendChild(this.createOption("removeAnimations", "Remove all animations"));
                accSection.appendChild(this.createOption("optionsReversed", "Move the 'go back' button in the settings menus to the bottom"));
                accSection.appendChild(this.createOption("progressBigger", "Make the progressbar take up the whole width (but not when hovering over)"));
                accSection.appendChild(this.createOption("showTitleOnHover", "Have the title show when hovering over the video"));
                accSection.appendChild(this.createOption("alwaysVolume", "Have the volume slider be always visible"));
            }
            ...
            if (this.isSettingsPage) {
                ...
                document.body.appendChild(accSection);
            }
            else if (content && footer && selectedItem.innerHTML.indexOf("Playback") >= 0) {
                content.insertBefore(accSection, footer);
            }
            var exportBtn = document.createElement("button");
            exportBtn.classList.add("yt-uix-button", "yt-uix-button-size-default", "yt-uix-button-primary", "account-action-button");
            ...
            exportBtn.onclick = function () {
                ...
                var hiddenText = accSection.getElementsByTagName("textarea")[0] || document.createElement("textarea");
                hiddenText.textContent = settingsScript;
                accSection.appendChild(hiddenText);
                hiddenText.select();
                document.execCommand("cut");
                accSection.removeChild(hiddenText);
                exportBtn.innerHTML = "Settings Userscipt Copied";
            };
