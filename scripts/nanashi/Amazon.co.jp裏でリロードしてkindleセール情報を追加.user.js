...
(() => {
    ...
    // 設定
    const END_NOTFOUND_COUNT = 20;			// 未発見がこの回数続いたら終了
    const RELOAD_SLEEP = 1000;				// リロードまでの最低スリープ時間（ミリ秒）
    const RELOAD_SLEEP_EXTENSION = 3000;	// 未発見時にスリープ時間に加算する（ミリ秒）
    const POP_TIME = 5000;					// ポップアップが消えるまでの時間
    const TARGET_SELECTOR = "#a-page div.msw-page, #merchandised-content";
    const LIST_SELECTOR = "#a-page div.msw-page > div.a-section.msw-shoveler, #merchandised-content > div[id^=\"carousel\"], #merchandised-content > div.a-row";
