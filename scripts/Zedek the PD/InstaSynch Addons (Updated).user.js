// ==UserScript==
// @name        InstaSynch Addons (Updated)
// @namespace   Bibby
...
// @author      Julian Hangstoerfer and NoirSuccubus
// @contributor fugXD
...
// @require		https://greasyfork.org/scripts/8609-tcap-emotes/code/TCAP%20Emotes.user.js
...
function loadPrefixOnce() {
    events.bind('onSendChat', function(event, message) {
        if(!message.match(/^[\[>#$'/@]/)){
            ...
            prefix = prefix.replace(/\[(?:(?:b(?!l|row)[^\]]+)|(?:spoiler)|(?:r?sanic)|(?:(?:f|r)?(?:alt|marquee)(?:(?:(?:-)?|\+?)?(?:\d+)?)?))\]/gi,'');
