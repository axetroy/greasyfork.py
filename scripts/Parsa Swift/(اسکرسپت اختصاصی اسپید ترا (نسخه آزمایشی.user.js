// ==UserScript==
// @name        (اسکرسپت اختصاصی اسپید ترا (نسخه آزمایشی
// @author      Parsa Swift
// @license     GPL version 3 or any later version; http://www.gnu.org/copyleft/gpl.html
// @description اسکریپت ساخت ساختمان، ساخت نیرو، فارم لیست.
// @include     *://*.speedtra.*
// @include     *://*/*.speedtra.*
// @include     *://www.speedtra.com/*/*
// @include     *://*.speedtra.*
// @exclude     *://analytics.speedtra*.*/*
...
if (init) {
	...
	*  Words that are removed, and appear as the number zero (0), currently have no translation and the english version is used.
	...
	case "ro": // by Atomic(edited by fulga, Pimp Trizkit, adipiciu)
		aLangTasks = ["Clădire", "Upgrade", "Atacă", "Cercetează", "Instruiește", "Petrecere", "Demolează", "Trimite negustori", "Trimite inapoi/retrage"];
						...
						for ( ii = 0 , j = myBuilds.length; ii < j ; ++ii ) {
							...
							if ( tmp.length > 0 ) {
								if (tmp[0].getAttribute('class')) if (tmp[0].getAttribute('class').indexOf("gold builder") > -1)
								{
									...
									printMsg('<span class="ttq_village_name" style="display:block;">' +getVillageName(oldVID)+ '</span>' + buildingName + " Found the button but it would use gold (Master Builder).", true);
								...
								if (tmp[0].getAttribute('class')) if (tmp[0].getAttribute('class').indexOf("disabled") > -1)
								{
									...
									printMsg('<span class="ttq_village_name" style="display:block;">' +getVillageName(oldVID)+ '</span>' + buildingName + " Found the button but its disabled because there are not enough resources", true);
		...
		if (httpRequest.status == 200 && httpRequest.responseText) {
			...
			if ( reqVID == oldVID && (holder.getElementsByClassName("gid22").length == 1 || holder.getElementsByClassName("gid13").length == 1) ) {
				...
				if ( tConract.length > 0) {
					...
					printMsg('<span class="ttq_village_name" style="display:block;">' +getVillageName(oldVID)+ "</span> ["+aTask[3].split("_")[2]+"] " + aLangStrings[69] +" ("+aLangStrings[70]+" 1: "+tReason+")", true);
				...
				printMsg('<span class="ttq_village_name" style="display:block;">' +getVillageName(oldVID)+ "</span> ["+aTask[3].split("_")[2]+"] " + aLangStrings[68] +" ("+aLangStrings[70]+" 2)", true);
			...
			printMsg('<span class="ttq_village_name" style="display:block;">' +getVillageName(oldVID)+ "</span> ["+aTask[3].split("_")[2]+"] " + aLangStrings[73] +" ("+aLangStrings[74]+" "+aLangStrings[11]+" 1)", true);
			addToHistory(aTask, false, aLangStrings[74]+" "+aLangStrings[11]+" 1");
			return;
		}
		switchActiveVillage(currentActiveVillage);
...
function sendGoldClub (aTask) {
	...
	post("ajax.php", sParams, sendGoldClub1, aTask);
	_log(2, "The attack was requested.");
...
function sendGoldClub1(httpRequest,aTask) {
	if (httpRequest.readyState == 4) {
		...
		if (httpRequest.status == 200 && httpRequest.responseText) {
			...
			get("build.php?tt=99&id=39", sendGoldClub2, aTask);
			return;
		}
		_log(1, "Your attack could not be sent. Bad response from server when sending request. (Server: Page Failed 1)");
...
function train(aTask) {
	...
	var httpRequest = new XMLHttpRequest();
	httpRequest.open("GET", "build.php?" + (oldVID>0?"newdid=" + oldVID:"") + residenceTab1 + "&id=" + aTask[2], true);
	httpRequest.onreadystatechange = function() {
		if (httpRequest.readyState == 4) { //complete
			printMsg(aLangStrings[6] + " > 1 > 2<br><br>" + getTaskDetails(aTask));
			if (httpRequest.status == 200 && httpRequest.responseText ) { // ok
				...
				if ( reqVID == oldVID && holder.getElementsByClassName("gid"+oldGid).length == 1 ) {
					if ( k > 3 ) {
						...
						post("build.php"+residenceTab2, sParams, handleRequestTrain, aTask);
						return;
					}
					if ( reqVID != currentActiveVillage ) switchActiveVillage(currentActiveVillage);
	...
	if (httpRequest.readyState == 4) {
		...
		if (httpRequest.status == 200 && httpRequest.responseText ) { // ok
			...
			if ( tInputs.length > 3 && holder.getElementsByClassName("gid17").length == 1 && reqVID == oldVID ) {
				...
				if( tX ) maxM = parseInt(tX.innerHTML.onlyText().match(/\S+\s+\D*?(\d+)\D*?\s*?\//)[1]);
				tX = $gc('max',holder)[0];
				...
				post("ajax.php", sParams, handleMerchantRequest2, aTask);
				return;
			}
			if ( reqVID != currentActiveVillage ) switchActiveVillage(currentActiveVillage);
	...
	if (httpRequest.readyState == 4) {
		...
		if (httpRequest.status == 200 && httpRequest.responseText) { // ok
			...
			if ( tTime.length > 0 && tInputs.length > 4 && reqVID == oldVID) {
				...
				_log(3,"sParams:"+sParams);
				sParams += '&ajaxToken='+getAjaxToken();
				post("ajax.php", sParams, handleMerchantRequestConfirmation, options);
				return;
			}
			if ( reqVID != currentActiveVillage ) switchActiveVillage(currentActiveVillage);
...
//	checkSetTasks();
    ...
    if ( oLoginBtn.snapshotLength < 1 && (oLogout.snapshotLength > 0 || oSysMsg.snapshotLength > 0) ) {
		...
		_log(0, "Initialization failed, Auto-login failed. Speedtra Task Queue is not running");
	}
}
