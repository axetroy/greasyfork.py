...
function getSettings() {
    ...
    if (settings.spotifyApp) {
        ...
        spotURL = 'https://open.spotify.com/search/results/';
    }
    return settings;
}
...
function createLinks (whatArtist, whatAlbum) {
    ...
    s = createLink(spotURL + encodeURIComponent(whatArtist) + "%20" + encodeURIComponent(whatAlbum), spotIMG);
    t = createLink(tidURL + "albums/" + encodeURIComponent(whatArtist) + "%20" + encodeURIComponent(whatAlbum), tidIMG);
    linkSetting();
}
function createArtistLinks (whatArtist) {
    d = createLink(deezURL + encodeURIComponent(whatArtist) + "/artist", deezIMG);
    s = createLink(spotURL + encodeURIComponent(whatArtist), spotIMG);
    t = createLink(tidURL + "artists/" + encodeURIComponent(whatArtist), tidIMG);
    linkSetting();
}
/*****************************************/
