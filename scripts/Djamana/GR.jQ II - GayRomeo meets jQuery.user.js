...
// @icon        http://classic.planetromeo.com/v1/img/touch-icon.png
...
//
// Non-jQuery code for
// /mitglieder/administration/pictures/upload_form.php
    try {
        
        document.getElementsByClassName('inputfile')[0].multiple = true;
        } catch (e) {
    }
...
//          http://patorjk.com/software/taag/#p=display&h=1&v=0&c=c%2B%2B&f=Big&t=Type%20Something%20
    ...
    config = {
        ...
        // Useful in case you are using mouseover-popup-image-viewer
        // http://greasyfork.org/de/scripts/404
        // with
        // {"r":"/img/usr/([^\\/]+\\.jpg)","s":"http://planetromeo.com/auswertung/pix/popup.php/$1",  "q":"img",  "c":"td[style]"}
        ...
        clickable_footprints : 1,
        // Makes Footprints on the visitors list clickable - so you don't need to open the profile to answer
        
        
		repairFootprint : 1 ,
        // reenable footprints on profiles when you did a top footprint search
    ...
    if (DebugLog) {
        console.log("Running GR.jQ script on: '" + location.pathname + "'")
    }
...
// /mitglieder/administration/pictures/upload_form.php
    try {
        ...
        // Bugfix: When adding JQuery that got lost 
         $('FORM[action*="upload"]').widget=GR.widget
        */
        } catch (e) {
    }
    ...
    /*
    var MultiUploadInput,
        MultiUploadButton
    
    function fnMultiUploadButtonClick (event) {
       // var MultiUploadButton = $('#MultiUploadInput');
         debugger;		
        $('#MultiUploadInput')
	       	.focus()
            .click()
        ...
        function fnMultiUploadButtonDone() {      
         debugger;	       
       
        $('.inputfile')
        	.each ( function (inputfile_index) {
                if (inputfile_index < MultiUploadInput.files.length) {
                    
                 // write File into boxes
                    this.files = MultiUploadInput.files;
    ...
    //<input accept="image/*" class="bf-upload-file-input" type="file" name="file" multiple="multiple">
    debugger
    ...
    MultiUploadButton =
		...
		.click ( 
            fnMultiUploadButtonClick
        )
 	  	.append (
            MultiUploadInput
        )
    
    // remove Jquery from object
    MultiUploadInput = MultiUploadInput[0]
    
    $('FORM[action*="upload"]')
    	.prepend( MultiUploadButton )
...
// Config Dialog
function GRjQ_setup() {
    alert ("TODO: implement Setup...")
	$('div', '\
			<h1>GayRomeo meets jQuery Setup</h1>'
     )
     .appendto ("body")
}
...
//GM_registerMenuCommand('Setup GayRomeo meets jQuery', GRjQ_setup);

	// repairFootprint - reenable footprint when you did a top footprint search
    if (config.repairFootprint) {
        ...
        try {
            ...
            .attr( {
                ...
                href: "../../auswertung/setcard/romeo/footprint.php?receiverId=" + location.search.match(/set=(\d*)/)[1] 
                      + "&locationId=" + 0
                      + "&cameFrom=profile",
                title: "FootprintLink \'Repaired\' by GayRomeo meets jQuery" 
                } )
        } catch (e) {
        }
    }
    ...
    // Makes Footprints on the visitors list clickable - so you don't need to open the profile to answer
    if (config.clickable_footprints) {
        ...
        $('img[src*="footprints/"]')
            .each( function() {
               try {
                   ...
                   UID = $(this).parent().parent().prev().prev()
                       .find('[href*="set"]')[0]
                       .href.match(/set=(\d*)/)[1]
                   ...
                   $(this).wrap( $('<a>')
                                .attr({
                                   ...
                                   href:    "/auswertung/setcard/romeo/footprint.php?receiverId=" + UID ,
                                })
                               )
                } catch (e) {
                } 
...
//
   // remove GR-ads
  $('[id|="div-gpt-ad"],.advertisementContainer')
       .remove()
    ...
    $.fn.mylinker = function() {
        ...
        function fn_LinkifyReplaceHandler ( all, prefix, found ) {
            ...
            if (!IsMaybeAlreadyLinks) {
                ...
                // Patch when text starte with http (h ht ww //)
         ...
         */
            ...
            // used for color fix
            var isClub = !!$(".setcard").length
                    
                    // (fix yellow link text on white background )
                    if (isClub) {
                        var pink = "#FF00FF"
                         link.css({
                             color: pink
     ...
     // Linkify comment
        if (config.TextToLink) {
            ...
            //---------------------------------------
			//[hftps]*
            //var RE_TextLinkMatcher = /(?:[^\s]*:\/\/|www\.)[^\s]*/gi;
            var RE_TextLinkMatcherHTML = /([\w]{0,2})((?:[\w]*:(?:<wbr>)?\/(?:<wbr>)?\/(?:<wbr>)?|www\.)(?:<wbr>|&amp;|[^\s&<\]\[])*)/gi;
...
//
    ...
    //https://www.planetromeo.com/search/index.php?action=edit&SkipEdit=true&savedSearchId=8827115
...
debugger
    ...
    // Zoom hack
     // enlarge table columbs
        var picCol = $(
           $('.searchResults  colgroup:last:not(.GRjQ_Zoom_applied) > col')[0]  ||
           $('.user-table     > colgroup:last:not(.GRjQ_Zoom_applied) > col')[1]
        ) .not('.GRjQ_Zoom_applied')
        .width( config.ThumbZoomHack )
        .parent().addClass('GRjQ_Zoom_applied')
        
     // Enlarge Thumbnails
        // avoids zoomhack on profile page
//        if ( !$('.profilePic')[0] ) {
            Thumbs
            .not('#attachedPictures  img')
            .not('.msgList img')
               .width ( config.ThumbZoomHack )
