// ==UserScript==
// @name           Travian Resource bar plus for EGYPTIANS , HUNS servers
// @namespace      https://greasyfork.org/en/scripts/33154-travian-resource-bar-plus-for-egyptians-huns-servers
// @description    Shows travian resources---updated to support EGYPTIANS , HUNS servers
// @author         hany_mamdoh known as AMETAP BATCHAN , EL COACH ,winner of the first ancient europe server
// @copyright      hany_mamdoh (Orginally by © Serj_LV)
// @license        GPL version 3 or any later version; http://www.gnu.org/copyleft/gpl.html
// @contributionURL https://www.paypal.me/HanyGhoneim
// @youtube         https://www.youtube.com/watch?v=9--evRtXXS8&list=PLFDxOOyu_5MCP3w2Bn4rpT-yTQGm79hil
// @include        *://*.travian.*
// @include        *://*/*.travian.*
...
var homepageurl = 'https://greasyfork.org/en/scripts/33154-travian-resource-bar-plus-for-egyptians-huns-servers';
...
function offsetPosition(el) { var rect = el.getBoundingClientRect(); return [rect.left + window.pageXOffset, rect.top + window.pageYOffset, rect.width]; }
...
if( RB.bodyH[0] == 1 ) {
	if( timerN.length < 4 ){
		...
		stockBar.appendChild(timerN[j].divme1);
	}
	...
	stockBar.removeChild(timerN[j].divme1);
...
function distanceToMyVillages() {
	...
	var kirURL = $a('(kirilloid.ru)',[['href','http://travian.kirilloid.ru/distance.php#cc='+id2xy(village_aid).join()+','+id2xy(curD).join()+'&srv='+sM+'.'+'4'+(RB.Setup[46]==1?3:4)],['target','_blank'],['style','font-size:10px;']]);
	var sel2 = $e('SELECT');
	var sO2 = ['-','+25%','+50%','+75%'];
...
function addlinkT4() {
	...
	if( vilView ) {
		...
		if( h1 ) {
			...
			newLink = 'd='+xy;
			var newName = h1.innerHTML.firstText() + ' ' + printCoords(xy);
			newName = prompt(gtext("linkname"), newName);
			if( newName == null ) return;
			var newOneLink = newLink + "\/@_" + esc(newName);
			RB.ln3.push(newOneLink);
...
function viewMessageIWDisplay( aLink, tV, xy ) {
	...
	var viewPref = [
		[messCr,'messages','width:570px;padding-top:10px;padding-bottom:15px;padding-'+docDir[0]+':20px;text-align:'+docDir[0]+';'],
		['.//*[@id="report_surround"]','reports','width:570px;padding-top:10px;padding-bottom:10px;padding-'+docDir[0]+':20px;']];
...
function bigQuickLinks () {
	...
	var bigIconsHeader = $xf('.//div[contains(@class,"innerBox header")]','l', sidebarBoxActiveVillage).snapshotItem(0);
	var bigIconsFooter = $xf('.//div[contains(@class,"innerBox footer")]','l', sidebarBoxActiveVillage).snapshotItem(0);

	if ($gt('button',bigIconsHeader)[0].className.search('green') == -1 ) { //Plus account active
	...
	bigIconsFooter.appendChild($e('button',[['type','button'],['class','layoutButton disabled'],['style','position:relative;box-shadow:none;background-image:none;border:0;width:46px']]));
...
function karteDistance () {
	...
	contRight = ltr ? contXY[0] + contXY[2] : contXY[0];
	contTop = contXY[1];
	karteDistance4();
}
...
function detectMapSize () {
	...
	if ( /width:\s(\d+),/.test(fullScr) ) {
		RB.Setup[48] = fullScr.match(/width:\s(\d+),/)[1];
		saveCookie( 'RBSetup', 'Setup' );
	}
}
...
function analyzerBattle () {
	...
	newTR.appendChild($c($a('(kirilloid)',[['href','http://travian.kirilloid.ru/warsim2.php'+kirilloid],['target','_blank'],['style','font-size:11px;']])));
...
function oasisKirilloid (vf) {
	...
	var kirillS = kirillRace[RB.Setup[2]]+'RuUb#d:p500r3#r3u';
	...
	return $a('(kirilloid)',[['href','http://travian.kirilloid.ru/warsim2.php'+kirillS],['target','_blank'],['style','font-size:11px;']]);
}
...
function displayWhatIsNew () {
	if ($g('whatsnew')) { 
		...
		var box = $e('div',[['id','whatsnew'],['style','width:410px;position:fixed;top:50%;left:50%;margin-left:-150px;margin-top:-100px;color:black;background-color:#FFFFFF;padding:5px 5px;border-radius:1em;z-index:999;opacity:0.88;']]);
		...
		var donate = $ee('div',$a('Donate',[['href','https://www.paypal.me/HanyGhoneim'],['target','_blank']]),[['style','display:table-cell;width:33%;text-align:'+docDir[1]+';']]);
		var closeb = $ee('div',$a('X',[['style','font-size:120%;float:'+docDir[1]+';']]),[['style','height:15px;padding:10px;']]);
		header.textContent = "About Resource Bar+";
		content.innerHTML = "What's new in Version "+version+" - Oct 24, 2017:<p></p><ui><li>Fix crop Incoming Resource</li><li>Added new feature show Total Incoming Resources</li><li>Fix for Opera browser with ViolentMonkey</li><li>Fixed troops overview for Travian Fire and Sand</li><li>Fixed show message text display direction for Arabia servers</li><li>Renamed server type Rise of Alliances to Fire and Sand</li></ui>";
