...
function mapChanged() {
    ...
    document.getElementById( 'NewMunzeeLat' ).placeholder = map.getCenter().lat;
    document.getElementById( 'NewMunzeeLng' ).placeholder = map.getCenter().lng;
}
function pinMoved( objDragged ) {
    var intPin = 0;
    for ( var pinNode in mapSandbox.list ) { if ( objDragged.target._element.isSameNode( mapSandbox.list[ pinNode ].marker._element ) ) { intPin = parseInt( pinNode ); } }
...
Function.prototype.removeMunzee = function ( valId ) {
    ...
    mapSandbox.list[ ( intIndex - 1 ) ].marker.remove();

    log( 4, 'log', 'Function.prototype.removeMunzee() splicing %i (%s) from mapSandbox.list', ( intIndex - 1 ), mapSandbox.list[ intIndex - 1 ].title );
    mapSandbox.list.splice( ( intIndex - 1 ), 1 );
...
Function.prototype.addMunzee = function() {
    var objNewMunzee = {
        lng: ( $( 'input#NewMunzeeLng' ).val() || map.getCenter().lng ),
        lat: ( $( 'input#NewMunzeeLat' ).val() || map.getCenter().lat ),
        name: ( $( 'input#NewMunzeeName' ).val() || 'Quick Deploy by ' + username ),
    ...
    mapSandbox.addItem( [ objNewMunzee.lng, objNewMunzee.lat ], objNewMunzee.name, objNewMunzee.own );
    mapSandbox.list[ mapSandbox.length - 1 ].marker.on( 'dragend', objDragged => pinMoved( objDragged ) );
    log( 2, 'log', 'Created move listener for pin #%i', ( mapSandbox.length - 1 ) );
    initilSB = 1;
