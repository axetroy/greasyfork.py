// ==UserScript==
// @name           AutoPagerize [Modified by BladeMight]
...
// ==/UserScript==
//
// modified: BladeMight BladeMight@gmail.com
// version: 0.0.14-BM
//
...
var VERSION = '0.0.66 [0.0.14-BM]'
var DEBUG = true
var PAGE_SEPARATOR = true
var PAGE_SEPARATOR_SIZE = 20
var AUTO_START = true
...
var SITEINFO = [    
    /* sample
    {
        ...
        nextLink:     '//ul[@class="pagination"]/li/a[@id="next_page"]',
        pageElement:  '//div[@class="center" and not(@style)]',
        exampleUrl:   'https://wall.alphacoders.com/by_category.php?id=3&name=Anime+Wallpapers',
    },
    */
    /* template
    {
...
var MICROFORMAT = {
    url:          '.*',
    nextLink:     '//a[@rel="next"] | //link[@rel="next"] | //a[@class="next"] | //*[@class="next"]/a | //a[@class="next_page"]',
...
var AutoPager = function(info) {
    ...
    if (isFirefoxExtension()) {
        ...
        if (isSafariExtension()) {
            safari.self.tab.dispatchMessage('aed', {url: location.href })
        }
...
// Settings
AutoPager.prototype.saveConfigs = function() {
    GM_setValue('PAGE_SEPARATOR', PAGE_SEPARATOR)
    GM_setValue('PAGE_SEPARATOR_SIZE', PAGE_SEPARATOR_SIZE)
    GM_setValue('BASE_REMAIN_HEIGHT', BASE_REMAIN_HEIGHT)
    debug("Saved as: " + PAGE_SEPARATOR + " " + PAGE_SEPARATOR_SIZE + " " + BASE_REMAIN_HEIGHT)
    this.loadConfigs();
}
AutoPager.prototype.loadConfigs = function() {
    var val = GM_getValue('PAGE_SEPARATOR')
    if (val != null)
        PAGE_SEPARATOR = val
    val = GM_getValue('PAGE_SEPARATOR_SIZE')
    if (val != null)
        PAGE_SEPARATOR_SIZE = val
    val = GM_getValue('BASE_REMAIN_HEIGHT')
    if (val != null)
        BASE_REMAIN_HEIGHT = val
    debug("Load as: " + PAGE_SEPARATOR + " " + PAGE_SEPARATOR_SIZE + " " + BASE_REMAIN_HEIGHT)
}
...
AutoPager.prototype.initHelp = function() {
    ...
    helpDiv.setAttribute('style', 'padding:5px;position:fixed;' +
                     'top:-200px;left:3px;font-size:10px;' +
                     'background:#248;color:#fff;border:1px solid #ccc;' +
                     'z-index:99999;text-align:left;font-weight:normal;' +
    ...
    raw.innerHTML = `<div style="clear: both;">    
	<label>Insert page separators:</label>
	<input class="pagesep" style="vertical-align: middle; bottom: -1px;" type="checkbox"` + (PAGE_SEPARATOR ? 'checked=""' : "") + `/><br/>
	<label>Page separator size:</label>
	<input class="sepsiz" style="vertical-align: middle; width: 50px;" type="number" min="0" value="` + PAGE_SEPARATOR_SIZE + `"/><br/>
	<label>Base remain height:</label>
    <input class="baserem" style="vertical-align: middle; width: 50px;" type="number" min="0" value="` + BASE_REMAIN_HEIGHT + `"/>
</div>`
    helpDiv.appendChild(colorDiv)
    helpDiv.appendChild(toggleDiv)
    helpDiv.appendChild(raw)
    ...
    var proc = function(e) {
        var pagesep = raw.getElementsByClassName('pagesep')[0]
        var sepsiz = raw.getElementsByClassName('sepsiz')[0]
        var baserem = raw.getElementsByClassName('baserem')[0]
        PAGE_SEPARATOR = pagesep.checked
        PAGE_SEPARATOR_SIZE = sepsiz.value
        BASE_REMAIN_HEIGHT = baserem.value
        self.saveConfigs()
...
AutoPager.prototype.onScroll = function() {
    ...
    debug("Remain: " + parseInt(remain) + " Point: " + parseInt(BASE_REMAIN_HEIGHT))
    if (this.state == 'enable' && remain < parseInt(BASE_REMAIN_HEIGHT)) {
          this.request()
    }
}
...
AutoPager.prototype.requestLoad = function(res) {
    ...
    try {
        ...
        debug("URL: " + url, "XPath: "  + this.info.nextLink)
        // Retry from MICROFORMAT 
        if (!url) { 
            url = this.getNextURL(MICROFORMAT.nextLink, htmlDoc, this.requestURL) 
       }
       if (!page) {
           page = getElementsByXPath(MICROFORMAT.pageElement, htmlDoc)
       }
       // Retry from SITEINFO
        if ((!page || !url) && SITEINFO.lenght > 0) {
           for (var t=0; t != SITEINFO.lenght; t++) {
               if (!url) {
                 url = this.getNextURL(SITEINFO[t].nextLink, htmlDoc, this.requestURL) 
               }
               if (!page) {
                 page = getElementsByXPath(SITEINFO[t].pageElement, htmlDoc)
               }
           }
       }
    }
...
AutoPager.prototype.addPage = function(htmlDoc, page) {
    ...
    p.setAttribute('style', 'height: ' + PAGE_SEPARATOR_SIZE + 'px; ' + (PAGE_SEPARATOR_SIZE < 15 ? 'display:none;' : ''));
    var self = this
...
AutoPager.prototype.getNextURL = function(xpath, doc, url) {
    ...
    if (nextLink) {
        ...
        if (nextValue.match(/^http(s)?:/)) {
        	debug("nextLink="+nextValue)
            return nextValue
        }
        if (nextValue.match(/^\/.*/)) {
            var next = window.location.protocol + '//' + document.domain + nextValue
            debug("nextLink(/)=" + next)
            return next
        } else {
            var base = getFirstElementByXPath('//base[@href]', doc)
            debug("nextLink(ELSE)=" + resolvePath(nextValue, (base ? base.href : url)))
            debug('base ' + (base ? base.href : url) + ' nV' + nextValue)
    ...
    for (var i = 0; i < list.length; i++) {
        try {
            if (ap) {
                ...
                var nextLink = getFirstElementByXPath(list[i].nextLink)
                if (!nextLink) {
                    debug(">>nextLink not found => " + nextLink + ", X: " + list[i].nextLink)
                    if (TIMER_TRIES < 1) {
                        debug("Wait for load page elements from js, retry in 1200ms.")
                        setTimeout(function() { launchAutoPager(list) }, 1200);
                        TIMER_TRIES++
                    }
...
if (isChromeExtension()) {
    ...
    port.onMessage.addListener(function(res) {
        ...
        debug('Running as chrome extension.')
        ...
        safari.self.addEventListener('message', function(event) {
            if (event.name === 'settings') {
                ...
                if (!settings['exclude_patterns'] || !isExclude(settings['exclude_patterns'])) {
                    debug('Running as safari extension.')
                    launchAutoPager(SITEINFO)
    ...
    onMessage = function(message) {
        if (message.name == 'siteinfo') {
            debug('Running as firefox jetpack extension.')
            // launchAutoPager(SITEINFO)
    ...
    debug('Running as user.js extension.')
    debug(location.href)
    launchAutoPager(SITEINFO)
