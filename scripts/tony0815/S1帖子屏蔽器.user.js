...
function main_process(){
    ...
    if(switch_post_class){
        ...
        document.querySelectorAll('#threadlist tbody th em>a:not(.censored),#threadlist tbody td.new em>a:not(.censored)').forEach(ele => {
    ...
    if(switch_post_id){
        ...
        document.querySelectorAll('#threadlist tbody td.by cite a:not(.censored)').forEach((ele,numofa) => {
            if (numofa%2 == 0){
                for (let i=0;i<NG_pool_id.length;i++){
                    if (ele.innerText == NG_pool_id[i]) {
                        count.id++;
                        ele.parentElement.parentElement.parentElement.parentElement.style.display = 'none';
                        ele.parentElement.parentElement.parentElement.parentElement.classList.add("作者过滤","censored");
                    } else {
                        //console.log(ele.innerText);
                    }
                };
