    ...
    oItems.forEach(function(oItem)
    {
      try {
        if (!oItem.equipped && !oItem.hidden)
        {
            ...
            oElContainer.append
            (
                "<div class=\"item-icon-container item icon-box " + sElName + "\" style=\"position: relative\">" +
            ...
            if ((!oEl._tippy) && (jQ(`#${sTooltipName}`).length === 0))
            {
                ...
                {
                    if (IsValid(oItem.stats.damage) && (CDbl(oItem.stats.damage) != 0.0))
                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-damage extra-small-icon mr-1\"></i>${oItem.stats.damage.toFixed(1)}</div>`;
                    if (IsValid(oItem.stats.energyStorage) && (CDbl(oItem.stats.energyStorage) != 0.0))
                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-energyStorage extra-small-icon mr-1\"></i>${oItem.stats.energyStorage.toFixed(1)} energy storage</div>`;
                    
                    if (IsValid(oItem.stats.attack) && (CDbl(oItem.stats.attack) != 0.0)) {
                      if (IsValid(oItem.stats.attackMax) && (CDbl(oItem.stats.attackMax) != 0.0)) {
                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-attack extra-small-icon mr-1\"></i>${oItem.stats.attack.toFixed(1)} - ${oItem.stats.attackMax.toFixed(1)} damage</div>`;
                      } else {
                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-attack extra-small-icon mr-1\"></i>${oItem.stats.attack.toFixed(1)} damage</div>`;
                      }
                    }
                    if (IsValid(oItem.stats.attackSpeed) && (CDbl(oItem.stats.attackSpeed) != 0.0))
                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-attackSpeed extra-small-icon mr-1\"></i>${oItem.stats.attackSpeed.toFixed(1)} attack speed</div>`;
                    if (IsValid(oItem.stats.healthMax) && (CDbl(oItem.stats.healthMax) != 0.0))
                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-healthMax extra-small-icon mr-1\"></i>${oItem.stats.healthMax.toFixed(1)} health</div>`;
                    if (IsValid(oItem.stats.accuracy) && (CDbl(oItem.stats.accuracy) != 0.0))
                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-accuracy extra-small-icon mr-1\"></i>${oItem.stats.accuracy.toFixed(1)} accuracy</div>`;
                    if (IsValid(oItem.stats.magicPower) && (CDbl(oItem.stats.magicPower) != 0.0))
                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-magicPower extra-small-icon mr-1\"></i>${oItem.stats.magicPower.toFixed(1)} magic power</div>`;
                    if (IsValid(oItem.stats.magicArmor) && (CDbl(oItem.stats.magicArmor) != 0.0))
                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-magicArmor extra-small-icon mr-1\"></i>${oItem.stats.magicArmor.toFixed(1)} magic armor</div>`;
                    if (IsValid(oItem.stats.healingPower) && (CDbl(oItem.stats.healingPower) != 0.0))
                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-healingPower extra-small-icon mr-1\"></i>${oItem.stats.healingPower.toFixed(1)} healing power</div>`;
                    if (IsValid(oItem.stats.defense) && (CDbl(oItem.stats.defense) != 0.0))
                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-defense extra-small-icon mr-1\"></i>${oItem.stats.defense.toFixed(1)} defense</div>`;
                    if (IsValid(oItem.stats.armor) && (CDbl(oItem.stats.armor) != 0.0))
                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-armor extra-small-icon mr-1\"></i>${oItem.stats.armor.toFixed(1)} armor</div>`;

                    if (IsValid(oItem.stats.energyRegen) && (CDbl(oItem.stats.energyRegen) != 0.0))
                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-energyRegen extra-small-icon mr-1\"></i>${oItem.stats.energyRegen.toFixed(1)} energy regen</div>`;
                }
                
                jQ.makeArray(oItem.enchantments).forEach(function(oThisEnchant)
                {
                    sExtraText = `${sExtraText}<div>${oThisEnchant}</div>`;
                });
                
                jQ(`#tooltip-${sTooltipName}`).html
                (
                    "    <h3 class=\"popover-title text-capitalize\">" +
                    "        " + oItem.name + ((oItem.enhanced) ? " (E)" : "") + "<br />" + 
                    "    </h3>" +
...
ET_LayoutTweaksMod_AbilityModalList = function(sSlot, bIsEmpty)
{
    ...
    oElContainer.html("");
    if (sSlot === 'companion') {
      jQ("h5#LTModAbilityModal").parent().find("small").html("Equip a new companion ability.");
    } else {
    ...
    oAbilities.forEach(function(oAbility)
    {
        if (!oAbility.equipped)
        {
            ...
            for (let j = 1; j <= 5; j++)
                sAbilityFriendlyName = sAbilityFriendlyName.replace(` (${j})`, "");
            ...
            if (jQ(".lobby-container").find("button.battle-btn").length > 0)
            {
                if (jQ("#LayoutTweaksSpellCasts").length === 0)
                {
                    ...
                    for (let i = 0; i < 6; i++)
                    {
                        if (oAbilitySlots[i].equipped)
                        {
                            ...
                            for (let j = 1; j <= 5; j++)
                            {
                                if (oAbilitySlots[i].isSpell)
                                {
                                    if (sAbilityFriendlyName.indexOf(` (${j})`) !== -1)
                                        sAbilityFriendlyName = ChopperBlank(sAbilityFriendlyName, "", ` (${j})`).trim();
                                }
                                else
                                    sAbilityFriendlyName = sAbilityFriendlyName.replace(` (${j})`, ` L.${j}`);
                            }
                    ...
                    for (let i = 0; i < 6; i++)
                    {
                        if (oAbilitySlots[i].equipped)
                        {
                            if (oAbilitySlots[i].isSpell) jQ("#LayoutTweaksSpellCasts2").append("<button class=\"btn-sm d-flex justify-content-center LayoutTweaksSpellCastsCraft" + i.toFixed(0) + "\" style=\"width: 96px; margin: 4px;\">Craft Casts</button>");
                            else                          jQ("#LayoutTweaksSpellCasts2").append("<div class=\"d-flex\" style=\"width: 96px; margin: 4px;\"></div>");
                        }
                        else                              jQ("#LayoutTweaksSpellCasts2").append("<div class=\"d-flex\" style=\"width: 96px; margin: 4px;\"></div>");
                    }
                    ...
                    for (let i = 0; i < 6; i++)
                    {
                        jQ(".LayoutTweaksSpellCastsClick" + i.toFixed(0)).css("cursor", "pointer").on('click', function()
                        {
                            ...
                            if (i === 5) ET_LayoutTweaksMod_AbilityModalList('companion', !oAbilitySlots[i].equipped);
                        });
                    ...
                    for (let i = 0; i < 6; i++)
                    {
                        if (oEquipSlots[i].equipped)
                        {
                            ...
                            if ((!oEl._tippy) && (jQ(`#${sTooltipName}`).length === 0))
                            {
                                ...
                                {
                                    if (IsValid(oEquipSlots[i].stats.damage) && (CDbl(oEquipSlots[i].stats.damage) != 0.0))
                                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-damage extra-small-icon mr-1\"></i>${oEquipSlots[i].stats.damage.toFixed(1)}</div>`;
                                    if (IsValid(oEquipSlots[i].stats.energyStorage) && (CDbl(oEquipSlots[i].stats.energyStorage) != 0.0))
                                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-energyStorage extra-small-icon mr-1\"></i>${oEquipSlots[i].stats.energyStorage.toFixed(1)} energy storage</div>`;
                                    
                                    if (IsValid(oEquipSlots[i].stats.attack) && (CDbl(oEquipSlots[i].stats.attack) != 0.0)) {
                                      if (IsValid(oEquipSlots[i].stats.attackMax) && (CDbl(oEquipSlots[i].stats.attackMax) != 0.0)) {
                                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-attack extra-small-icon mr-1\"></i>${oEquipSlots[i].stats.attack.toFixed(1)} - ${oEquipSlots[i].stats.attackMax.toFixed(1)} damage</div>`;
                                      } else {
                                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-attack extra-small-icon mr-1\"></i>${oEquipSlots[i].stats.attack.toFixed(1)} damage</div>`;
                                      }
                                    }
                                    if (IsValid(oEquipSlots[i].stats.attackSpeed) && (CDbl(oEquipSlots[i].stats.attackSpeed) != 0.0))
                                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-attackSpeed extra-small-icon mr-1\"></i>${oEquipSlots[i].stats.attackSpeed.toFixed(1)} attack speed</div>`;
                                    if (IsValid(oEquipSlots[i].stats.healthMax) && (CDbl(oEquipSlots[i].stats.healthMax) != 0.0))
                                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-healthMax extra-small-icon mr-1\"></i>${oEquipSlots[i].stats.healthMax.toFixed(1)} health</div>`;
                                    if (IsValid(oEquipSlots[i].stats.accuracy) && (CDbl(oEquipSlots[i].stats.accuracy) != 0.0))
                                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-accuracy extra-small-icon mr-1\"></i>${oEquipSlots[i].stats.accuracy.toFixed(1)} accuracy</div>`;
                                    if (IsValid(oEquipSlots[i].stats.magicPower) && (CDbl(oEquipSlots[i].stats.magicPower) != 0.0))
                                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-magicPower extra-small-icon mr-1\"></i>${oEquipSlots[i].stats.magicPower.toFixed(1)} magic power</div>`;
                                    if (IsValid(oEquipSlots[i].stats.magicArmor) && (CDbl(oEquipSlots[i].stats.magicArmor) != 0.0))
                                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-magicArmor extra-small-icon mr-1\"></i>${oEquipSlots[i].stats.magicArmor.toFixed(1)} magic armor</div>`;
                                    if (IsValid(oEquipSlots[i].stats.healingPower) && (CDbl(oEquipSlots[i].stats.healingPower) != 0.0))
                                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-healingPower extra-small-icon mr-1\"></i>${oEquipSlots[i].stats.healingPower.toFixed(1)} healing power</div>`;
                                    if (IsValid(oEquipSlots[i].stats.defense) && (CDbl(oEquipSlots[i].stats.defense) != 0.0))
                                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-defense extra-small-icon mr-1\"></i>${oEquipSlots[i].stats.defense.toFixed(1)} defense</div>`;
                                    if (IsValid(oEquipSlots[i].stats.armor) && (CDbl(oEquipSlots[i].stats.armor) != 0.0))
                                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-armor extra-small-icon mr-1\"></i>${oEquipSlots[i].stats.armor.toFixed(1)} armor</div>`;

                                    if (IsValid(oEquipSlots[i].stats.energyRegen) && (CDbl(oEquipSlots[i].stats.energyRegen) != 0.0))
                                        sStatLines = `${sStatLines}<div class=\"d-flex align-items-center\"><i class=\"lilIcon-energyRegen extra-small-icon mr-1\"></i>${oEquipSlots[i].stats.energyRegen.toFixed(1)} energy regen</div>`;
                                }
                                ...
                                (
                                    "    <h3 class=\"popover-title text-capitalize\">" +
                                    "        " + oEquipSlots[i].name + ((oEquipSlots[i].enhanced) ? " (E)" : "") + "<br />" + 
                                    "    </h3>" +
                                    "    <div class=\"popover-content\">" +
                                    "        " + sStatLines +
                                    "        " + sExtraText + 
                        ...
                        else
                        {
                            jQ(`.LayoutTweaksLobbyEquipmentClick${i}`).css("cursor", "pointer").on('click', function()
                            {
                                ...
                                if (i === 2) ET_LayoutTweaksMod_EquipmentModalList("head", "helm or hat");
                                if (i === 3) ET_LayoutTweaksMod_EquipmentModalList("neck", "amulet");
