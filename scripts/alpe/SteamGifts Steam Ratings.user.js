...
function processpage(req, hlink, change, gaid, steamstore){
  ...
  if (req.title.indexOf('Site Error') === -1 && (!!steamstore || (!!ratingtest.length || !!ratingtest2.length || !!ratingtest3.length))){
    ...
    features = $('a.name, div.DRM_notice div:first, div.DRM_notice:not(:has(*))', req).map(function() { return ((this.href && this.href.indexOf("vrsupport")===-1)?this.href.match(/=([0-9]+)\&/)[1]:'0') + '::' + this.textContent.replace(/^\s{2,}/,'').replace(/\s{2,}/g,' '); }).get().join('\n');
