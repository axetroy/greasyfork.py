// ==UserScript==
// @name       淘宝、闲鱼无刷新加载翻页(预读)
// @namespace  https://greasyfork.org/zh-CN/users/6065-hatn
// @version    0.2.5
// @description  淘宝、闲鱼: 搜索列表改无刷新(预读)翻页：双击【←→】键快速翻页
...
var config = {
    mode_status: 1,			// 工作模式 [1]闲鱼、淘宝(默认) [2]闲鱼 [3]淘宝
	act_type: 1,			// 加载模式 [1]底部自动载入(默认) [0]翻页
	search_flag: 1,			// 闲鱼恢复搜索框 [1]开启(默认) [0]关闭
