...
// @homepage       https://greasyfork.org/en/scripts/39066-hwm-work-find
// @icon           https://dcdn.heroeswm.ru/avatars/30/nc-5/30547.gif
// @version        2.22.1
// @encoding       utf-8
...
(function() {
    ...
    var Pahan_locArr = new Array(
        ...
        [ 14 ,  52 ,  50 ,  1 ,  'Magma Mines' ,        'Магма Шахты' ,       177,200,210],
    ...
    function ObjectProcess(ALocation, AObjLink, AObjHTML)
    {
        ...
        if (ALocation == 0)
        {
            ...
            if ((res = re.exec(AObjHTML)) != null)
            {
                ...
                change_tr.innerHTML = 'function change_tr(){ var r = "йцукенгшщзфывапролдячсмитьЙЦУКЕНГШЩЗФЫВАПРОЛДЯЧСМИТЬ"; var e = "qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM"; var u = "йцукенгшщзфівапролдячсмитьЙЦУКЕНГШЩЗФІВАПРОЛДЯЧСМИТЬ"; var s = document.getElementById("code").value; var l = s.length; var i = 0; var j = 0; var c = ""; for (i < 0; i < l; i++) { c = s.substr(i, 1); j = r.indexOf(c); if (j >= 0) { s = s.substr(0, i) + e.substr(j, 1) + s.substr(i + 1, l); } else { j = u.indexOf(c); if (j >= 0) s = s.substr(0, i) + e.substr(j, 1) + s.substr(i + 1, l); } } document.getElementById("code").value = s; }';
