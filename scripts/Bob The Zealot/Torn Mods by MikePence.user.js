...
// @version      1.5
// @description  Makes a lot of pages cooler
// @author       Mike Pence
...
$(document).ready(function(){
    if(window.location.href.indexOf("competition.php") > -1){
    ...
    else if(window.location.href.indexOf("newspaper.php") > -1){
        window.setInterval(function(){
            $('.price').first().html('<i class="star m-hide"></i>Price - ' + randomIntFromInterval(10,99) + ' cents');
        }, 100);
    }
    else if(window.location.href.indexOf("jailview.php") > -1){
        var peopleinjail = randomIntFromInterval(10, 250);
        window.setInterval(function(){
            if(randomIntFromInterval(1, 100) <= 75){
                peopleinjail += randomIntFromInterval(1, 5);
            }
            else{
                peopleinjail = Math.max(peopleinjail + randomIntFromInterval(1, 3), 0);
            }
            $('.total').first().html(peopleinjail);
        }, 100);
    }
    else if(window.location.href.indexOf("hospitalview.php") > -1){
        var peopleinhospital = randomIntFromInterval(500, 2500);
        window.setInterval(function(){
            if(randomIntFromInterval(1, 100) <= 75){
                peopleinhospital += randomIntFromInterval(1, 10);
            }
            else{
                peopleinhospital = Math.max(peopleinhospital + randomIntFromInterval(1, 5), 0);
            }
            $('.total').first().html(peopleinhospital);
        }, 100);
    }
    else if(window.location.href.indexOf("forums.php") > -1){
        if(randomIntFromInterval(1, 100) <= 25){
            var forumbarwidth = randomIntFromInterval(0, 100);
            window.setInterval(function(){
                if(randomIntFromInterval(1, 100) <= 50){
                    forumbarwidth = Math.min(forumbarwidth + randomIntFromInterval(1, 5), 100);
                }
                else{
                    forumbarwidth = Math.max(forumbarwidth - randomIntFromInterval(1, 5), 0);
                }
                $('.pg-bar').first().css('width', forumbarwidth + '%');
            }, 100);
        }
    }
    else if(window.location.href.indexOf("profiles.php") > -1){
        var ids =    ["2029670",        "2077158",     "2092622",              "14757",        "2092133",  "2161965"];
        var nids =   ["MikePence",      "KarenPence",  "ROYALTY",              "TomDickHarry", "Vesimere", "Get_cucked"];
        var ranks =  ["Super Epic",     "Super Epic",  "Highly Distinguished", "Supreme",      "Supreme",  "Above Average"];
        var titles = ["Vice President", "Second Lady", "Airhead",              "Leader",       "Overlord", "Cuckenator"];
        window.setInterval(function(){
                   ...
                   window.location.href.indexOf("NID=" + nids[i]) >= 0){
                    ...
                    $('.two-row').first().children().each(function(index){
                        if(index == 0){
                            ...
                            if(ids[i] == 14757 && randomIntFromInterval(1, 10) == 1){
                                $(this).text("Dick");
                            }
                            else {
                                $(this).text(titles[i]);
