...
if(AnchorLink!=null){
    ...
    GM_xmlhttpRequest({
        ...
        onload: function(responseDetails){
            ...
            var saveTime = localStorage.getItem('saveTime_recom');
            modTime = "" + modTime;
            if(modTime!=saveTime){					//Cache time check
                localStorage.setItem('saveTime_recom', modTime);
            ...
            if(cacheCheck==0){
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: function(responseDetails){
                        ...
                        localStorage.setItem('list_recom', key); //Store Cache string
                        recom(ListLinks);
                    }
                });
            }
                ...
                var key = localStorage.getItem('list_recom');					//Fetch link from Cache
                var exactlink=/\d{1,}/g;
                var tagcount = 5000;
                for (var i = 0; i < tagcount; i++){
...
function recom(ListLinks){
    ...
    var checkboxmem = (localStorage.getItem('checkboxmem_recom') === "true"); //Get chceckbox status
    var checkboxmem2 = (localStorage.getItem('checkboxmem2_recom') === "true"); //Get chceckbox status
	var checkboxmem3 = (localStorage.getItem('checkboxmem3_recom') === "true"); //Get chceckbox status
    if(checkboxmem==null || checkboxmem2==null || checkboxmem3==null){
        checkboxmem=false;
        checkboxmem2=false;
		checkboxmem3=false;
        localStorage.setItem('checkboxmem_recom', checkboxmem);
        localStorage.setItem('checkboxmem2_recom', checkboxmem2);
		localStorage.setItem('checkboxmem3_recom', checkboxmem3);
    ...
    checkbox3.addEventListener('change',function () {
        ...
        localStorage.setItem('checkboxmem3_recom', checkbox3.checked);	
    },false)
    ...
    checkbox2.addEventListener('change',function () {
        ...
        localStorage.setItem('checkboxmem_recom', checkbox.checked);
        localStorage.setItem('checkboxmem2_recom', checkbox2.checked);		
    },false)
