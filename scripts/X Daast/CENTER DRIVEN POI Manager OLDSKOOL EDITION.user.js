// ==UserScript==
// @name 			CENTER DRIVEN POI Manager OLDSKOOL EDITION
...
// @version 		1.00
// @author 			Peluski17, REWRITE by XDaast
			...
			qx.Class.define("CDPOI", 
			{
				...
				construct: function()
				{
					try
					{
						...
						this.managerBtn = new qx.ui.form.Button("CD POI").set({alignX: "center"});
						this.managerBtn.set
						({
							alignY: "middle",
...
/*						//var scanBtn = new qx.ui.form.Button("", "webfrontend/ui/icons/icon_mainui_base_button.png").set
						...
						scanBtn.addListener("click", this._openScanner, this);webfrontend/battleview/neutral/gui/icn_mutants.png
*/						
						var poiBtn = new qx.ui.form.Button("", "https://eaassets-a.akamaihd.net/cncalliancesgame/cdn/data/108bc33ab106c1ae9c05b4365e7f361e.png").set
						({
							center: true,
							show: "icon",
							alignY: "middle",
...
*/						
					...
					{
						console.log("Error initializing CDPOI Class: " + e.toString());
					}
				},
...
/*					_openScanner: function()
					{
						if (CDPOI.BaseScanner.getInstance().isVisible())
							CDPOI.BaseScanner.getInstance().close();
						else
						{
							CDPOI.BaseScanner.getInstance().open();
...
*/					
					...
					{
						if (CDPOI.POIWindow.getInstance().isVisible())
						{
							CDPOI.POIWindow.getInstance().close();
						}
						else
						{
							CDPOI.POIWindow.getInstance().open();
							this.managerPopup.hide();
						}
					},
...
/*					_openUpgradeWindow: function()
					{
						if (CDPOI.UpgradeWindow.getInstance().isVisible())
						{
							CDPOI.UpgradeWindow.getInstance().close();
						}
						else
						{
							CDPOI.UpgradeWindow.getInstance().open();
							this.managerPopup.hide();
						}
					},
*/					
...
/*			qx.Class.define("CDPOI.BaseScanner", 
			{
				type: "singleton",
				...
				construct: function()
				{
					try
					{
						...
						scanBtn.addListener("click", function()
						{
							...
							CDPOI.BaseScanner.getInstance()._waitForPlayerCity(ownCity);
						}, this);
				...
				members: 
				{
					...
					{
						if (CDPOI.BaseScanner.TerrainLayout.getInstance().isVisible())
						{
							CDPOI.BaseScanner.TerrainLayout.getInstance().close();
						}
						else
						{
							CDPOI.BaseScanner.TerrainLayout.getInstance().open();
							CDPOI.BaseScanner.TerrainLayout.getInstance().getLayouts();
						}
					},
					...
					_waitForCity: function(city, visCity)
					{
						...
						if ((visCity.get_CurrentCityId() <= 0 || city.m_Level <= 0) && this.loopCount <= 10)
						{
							...
							(function(city, visCity) 
							{
								setTimeout(function() 
								{
									CDPOI.BaseScanner.getInstance()._waitForCity(city, visCity);
								}, 1000);
							}(city, visCity));
						}
					...
					_waitForResources: function(city, visCity)
					{
						...
						if (this.gotResources == false)
						{
							(function(city, visCity) 
							{
								setTimeout(function() 
								{
									CDPOI.BaseScanner.getInstance()._waitForResources(city, visCity);
								}, 1000);
							}(city, visCity));
						}
						else
			...
			qx.Class.define("CDPOI.BaseScanner.TerrainLayout", 
			{
				type: "singleton",
				extend:	qx.ui.window.Window,
				...
				members: 
				{
					...
					getLayouts: function()
					{
						var tableData = CDPOI.BaseScanner.getInstance().getTableData();
...
*/
			qx.Class.define("CDPOI.POIWindow", 
			{
				type: "singleton",
				extend:	qx.ui.window.Window,
				...
				{	
					try
					{
						...
						this.set({
							...
							width: 500,
							caption: "CENTER DRIVEN POI Manager _CLICK UPDATE FOR RESULTS_",
							padding: 5,
							allowMaximize: false,
							showMaximize: false,
							allowMinimize: false,
						...
						this.selectBox.addListener("changeSelection", function(e) {
							var numRows = CDPOI.POIWindow.getInstance().tableModel.getRowCount();
							CDPOI.POIWindow.getInstance().tableModel.removeRows(0, numRows, true);
						});
				...
				members:
				{
					...
					updatePOIStats: function(poiList, totalScore, rankData, baseValue)
					{
						...
						//Set Values
						//Total
						this.poiData["counts"]["total"].score.setValue(CDPOI.getInstance().numberWithCommas(totalScore));
						this.poiData["counts"]["total"].qty.setValue(CDPOI.getInstance().numberWithCommas(totalQty));
						if (this.currPOIType < 5)
							this.poiData["counts"]["total"].bonus.setValue(CDPOI.getInstance().numberWithCommas(totalBonus) + "/hr");
						else
							this.poiData["counts"]["total"].bonus.setValue(totalBonus + "%");
						this.poiData["counts"]["total"].nextTier.setValue(CDPOI.getInstance().numberWithCommas(toNextTier));
						if (rankData.rank != 1)
							this.poiData["counts"]["total"].nextRank.setValue(CDPOI.getInstance().numberWithCommas(toNextRank));
						else
							this.poiData["counts"]["total"].nextRank.setValue(toNextRank);
						
						//Tiers
						this.poiData["counts"]["tier"]["prev"].lower.setValue(CDPOI.getInstance().numberWithCommas(prevLower));
						this.poiData["counts"]["tier"]["prev"].upper.setValue(CDPOI.getInstance().numberWithCommas(prevUpper));
						this.poiData["counts"]["tier"]["curr"].lower.setValue(CDPOI.getInstance().numberWithCommas(currLower));
						this.poiData["counts"]["tier"]["curr"].upper.setValue(CDPOI.getInstance().numberWithCommas(currUpper));
						this.poiData["counts"]["tier"]["next"].lower.setValue(CDPOI.getInstance().numberWithCommas(nextLower));
						this.poiData["counts"]["tier"]["next"].upper.setValue(CDPOI.getInstance().numberWithCommas(nextUpper));
						
						if (this.currPOIType < 5)
						{
							this.poiData["counts"]["tier"]["prev"].bonus.setValue(CDPOI.getInstance().numberWithCommas(prevBonus) + "/hr");
							this.poiData["counts"]["tier"]["curr"].bonus.setValue(CDPOI.getInstance().numberWithCommas(currBonus) + "/hr");
							this.poiData["counts"]["tier"]["next"].bonus.setValue(CDPOI.getInstance().numberWithCommas(nextBonus) + "/hr");
						}
						else	
						{
							this.poiData["counts"]["tier"]["prev"].bonus.setValue(CDPOI.getInstance().numberWithCommas(prevBonus) + "%");
							this.poiData["counts"]["tier"]["curr"].bonus.setValue(CDPOI.getInstance().numberWithCommas(currBonus) + "%");
							this.poiData["counts"]["tier"]["next"].bonus.setValue(CDPOI.getInstance().numberWithCommas(nextBonus) + "%");
						}
						this.poiData["counts"]["tier"]["prev"].diff.setValue(CDPOI.getInstance().numberWithCommas(prevDiff));
						this.poiData["counts"]["tier"]["curr"].diff.setValue(CDPOI.getInstance().numberWithCommas(currDiff));
						this.poiData["counts"]["tier"]["next"].diff.setValue(CDPOI.getInstance().numberWithCommas(nextDiff));
						
						//Ranks
						this.poiData["counts"]["rank"]["prev"].alliance.setValue(prevAlliance);
						this.poiData["counts"]["rank"]["prev"].score.setValue(CDPOI.getInstance().numberWithCommas(prevScore));
						this.poiData["counts"]["rank"]["prev"].multi.setValue(CDPOI.getInstance().numberWithCommas(prevMulti) + "%");
						this.poiData["counts"]["rank"]["prev"].diff.setValue(CDPOI.getInstance().numberWithCommas(prevRankDiff));
						
						this.poiData["counts"]["rank"]["curr"].alliance.setValue(currAlliance);
						this.poiData["counts"]["rank"]["curr"].score.setValue(CDPOI.getInstance().numberWithCommas(currScore));
						this.poiData["counts"]["rank"]["curr"].multi.setValue(CDPOI.getInstance().numberWithCommas(currMulti) + "%");
						this.poiData["counts"]["rank"]["curr"].diff.setValue(CDPOI.getInstance().numberWithCommas(currRankDiff));
						
						this.poiData["counts"]["rank"]["next"].alliance.setValue(nextAlliance);
						
						if (rankData.rank != 1)
						{
							this.poiData["counts"]["rank"]["next"].score.setValue(CDPOI.getInstance().numberWithCommas(nextScore));
							this.poiData["counts"]["rank"]["next"].multi.setValue(CDPOI.getInstance().numberWithCommas(nextMulti) + "%");
							this.poiData["counts"]["rank"]["next"].diff.setValue(CDPOI.getInstance().numberWithCommas(nextRankDiff));
						}
						else
						{
...
//						
					...
					{		
						...
						this.poiData["counts"]["simulation"]["prev"].score.setValue(CDPOI.getInstance().numberWithCommas(prevScore));
						this.poiData["counts"]["simulation"]["curr"].score.setValue(CDPOI.getInstance().numberWithCommas(currScore));
						if (this.currPOIType < 5)
						{
							this.poiData["counts"]["simulation"]["prev"].bonus.setValue(CDPOI.getInstance().numberWithCommas(prevBonus) + "/hr");
							this.poiData["counts"]["simulation"]["prev"].totalBonus.setValue(CDPOI.getInstance().numberWithCommas(prevTotalBonus) + "/hr");
							this.poiData["counts"]["simulation"]["curr"].bonus.setValue(CDPOI.getInstance().numberWithCommas(currBonus) + "/hr");
							this.poiData["counts"]["simulation"]["curr"].totalBonus.setValue(CDPOI.getInstance().numberWithCommas(currTotalBonus) + "/hr");
						}
						else
						{
					...
					*/
					onKeyPress: function(event)
					{
						...
						if (event.altKey && key == "L")
						{
							...
							{
								var msg = "";
								var msgList = CDPOI.POIWindow.getInstance().getMsgList();
...
/*			qx.Class.define("CDPOI.UpgradeWindow", 
			{
				type: "singleton",
				extend:	qx.ui.window.Window,
				...
				members:
				{
					...
					waitToMaximizeAgain: function(buildings, currOwnCity)
					{
						(function(buildings, currOwnCity)
						{
							setTimeout(function()
							{
								CDPOI.UpgradeWindow.getInstance().doMaximizeUpgrading(buildings, currOwnCity);
							}, 500);
					...
					if (app.initDone == true) 
					{
						try
						{
							...
							if (typeof phe.cnc.Util.attachNetEvent == 'undefined')
								CDPOI.getInstance().attachNetEvent = webfrontend.gui.Util.attachNetEvent;
							else
								CDPOI.getInstance().attachNetEvent = phe.cnc.Util.attachNetEvent;
                        
							//Current Server patch (World 52 - US East Coast) uses webfrontend
							if (typeof phe.cnc.gui.util == 'undefined')
								CDPOI.getInstance().formatNumbersCompact = webfrontend.gui.Util.formatNumbersCompact;    
							else
								CDPOI.getInstance().formatNumbersCompact = phe.cnc.gui.util.Numbers.formatNumbersCompact;   
							
							CDPOI.BaseScanner.getInstance();
						}
						catch(e)
						{
