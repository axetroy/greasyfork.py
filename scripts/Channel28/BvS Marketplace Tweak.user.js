...
// @include        http*://*animecubedgaming.com/billy/bvs/villagemarketplace.*
// @licence        MIT; http://www.opensource.org/licenses/mit-license.php
// @copyright      2009, Daniel Karlsson
// @description    Calculates and shows price per unit in the marketplace.
// @grant          none
// ==/UserScript==
...
div#marketinfowin {background-color: rgb(91%, 79%, 62%);
...
function state_Change()
{
	...
	if (infoHTTPReq.status == 200) {
		...
		node = div.getElementsByTagName("table")[1];
	}
	if (!node) {
		node = document.createElement("div");
		node.textContent = "Problem loadinging item info";
	}
...
function showGraph(name, days){
	...
	object.style.width = "522px";
	object.style.height = "274px";
	d=new Date();
	d.setDate(d.getDate()-days);
	object.data="http://bvs-garyzx.appspot.com/bvs/marketplacegraph.svg?item="+name+"&starttime="+
		Math.floor(d.getTime()/60000)+"&scale=0.45";
	div.appendChild(object);

}
//End modifications

function itemInfo(name, lnk)
{
	name = unescape(name);
	lnk = unescape(lnk);
...
"src='data:image/png,%89PNG%0D%0A%1A%0A%00%00%00%0DIHDR%00%00%00%10%00%00%00%10%08%03%00%00%00(-%0FS%00%00%00%01sRGB%00%AE%CE%1C%E9%00%00%00%84PLTE%16Q%AA%19S%AB%1BT%AB-a%B22e%B47i%B6%3Al%B7%40p%B9Ds%BAHv%BCJw%BDR%7D%C0S~%C0U%80%C1W%81%C1%60%88%C5c%8A%C5f%8C%C7r%96%CC~%9F%D0%80%A0%D0%97%B1%D9%9A%B3%D9%9C%B5%DB%A6%BC%DE%AD%C1%E0%B6%C8%E4%B6%C9%E4%BC%CD%E6%CA%D8%EC%D1%DC%EE%D8%E2%F1%DE%E6%F2%DE%E6%F3%E3%EA%F4%E4%EA%F5%E7%ED%F6%EC%F1%F8%EE%F2%F9%F9%FB%FD%FA%FB%FD%FD%FD%FE%FC%FE%FE%FF%FF%FF%CDU%9D%18%00%00%00dIDAT%18%D3%5D%CFE%0E%C0P%08%04%D0Rwww%E3%FE%F7k%17M%3Et%96%2F%04%18%09%7F%91%10X%3E%90u%D3%D4(d%C3%BEu%A9%00u%E8%CB%16%8FXL%D8%1A(%2B%16%02%DE8x%26%14%A2%13%1B%B2%14%DC%FB%AA%E8%15%C8%E6%91%FD%01V%18p%C8%97%89C%FD%96%60%60%F8%1E%9F%A0%E5~y%00%81%D7%0F%95%FE%E9%BB%BF%00%00%00%00IEND%AEB%60%82'/>" +
	...
	var prices = BMA ? BMA.getPriceList(name) : null;
	if (prices && prices.units > 0) {
		...
		table.appendChild(tr);
		prices = BMA.getPriceList(name, false, 25);
		...
		tr.setAttribute("class", "header");
		table.appendChild(tr);
		tr.innerHTML = "<td colspan='2'>2 Weeks</td>";
		
		prices = BMA.getPriceList(name, 14 * 24 *60);
		appendTableRow(table, "Average", BMA.prettyPrice(BMA.average(prices.list)));
		appendTableRow(table, "Sales", prices.list.length);
		...
		tr.setAttribute("class", "header");
		table.appendChild(tr);
		tr.innerHTML = "<td colspan='2'>1 Month</td>";
		
		prices = BMA.getPriceList(name, 4 * 7 * 24 *60);
		appendTableRow(table, "Average", BMA.prettyPrice(BMA.average(prices.list)));
		appendTableRow(table, "Sales", prices.list.length);
		...
		tr.setAttribute("class", "header");
		table.appendChild(tr);
		tr.innerHTML = "<td colspan='2'>3 Months</td>";

		prices = BMA.getPriceList(name, 12 * 7 * 24 *60);
		appendTableRow(table, "Average", BMA.prettyPrice(BMA.average(prices.list)));
		appendTableRow(table, "Sales", prices.list.length);
...
function priceString(p)
{
	...
	return str.replace(/\d{1,3}(?=(\d)+(?![\d\w]))/g, "$&,");
}
...
// Price per Unit for Post Listing form
var askingPrice = document.getElementById("post_cost");
var qtyField = document.getElementsByName("post_qty")[0];
if (askingPrice && qty) {
	var priceRow = askingPrice.parentNode.parentNode;
	var ppuRow = document.createElement("tr");
	priceRow.parentNode.insertBefore(ppuRow, priceRow.nextSibling);
	ppuRow.innerHTML = "<td>Price per Unit:</td><td id='bmptppu' style='font-weight: bold'/>";
	var ppu = ppuRow.childNodes[1];

	var ppuCalc = function() {
			var val = parseInt(askingPrice.value);
			var qty = parseInt(qtyField.value);
			if (val && qty)
				ppu.textContent = Math.round(val / qty) + " Ryo";
			else
				ppu.textContent = "Invalid";
		}
	ppuCalc();
	qtyField.addEventListener('keyup', ppuCalc, false);
	askingPrice.addEventListener('keyup', ppuCalc, false);
}