...
// @namespace https://greasyfork.org/pt-BR/users/150179
...
function waitForCountryTop () {
    ...
    if (myWaze && myWaze.model && myWaze.model.countries && myWaze.model.countries.top && myWaze.model.countries.top.id) {
        ...
        switch(myWaze.model.countries.top.id) {
            case 30:  // Brazil
            unsafeWindow.mainDictionnaryKey = '1MzMS2LThf8J_pwg_wfpNPoDbzU1H7WR1KbnuK2xcuUA';
            unsafeWindow.publicDictionnarykey = '1MzMS2LThf8J_pwg_wfpNPoDbzU1H7WR1KbnuK2xcuUA';
            break;
            case 73:  // France
            ...
            default:
                ...
                unsafeWindow.WME_VdV_onload = "Error";
                delete WME_VdV_1_mainDictionaryTxt;
                delete WME_VdV_1_publicDictionaryTxt;
                return ;
        }
        ...
        GM_xmlhttpRequest({             //alert ("On démarre le chargement");
            ...
            onload: function (mainDictionary) {
                unsafeWindow.WME_VdV_1_mainDictionaryTxt = mainDictionary.responseText;
            }
        });
        ...
        GM_xmlhttpRequest({
            ...
            onload: function (publicDictionary) {
                unsafeWindow.WME_VdV_1_publicDictionaryTxt = publicDictionary.responseText;
            }
        });
...
if ('undefined' == typeof WME_VdV_onload) {        // le script n'est a pas encore chargé
    unsafeWindow.WME_VdV_onload = "In Progress";
    unsafeWindow.WME_VdV_1_mainDictionaryTxt = "In Progress";
    unsafeWindow.WME_VdV_1_publicDictionaryTxt  = "In Progress";
...
function Check_Road_Name() {
    WME_VdV_Dictionary = [];                            // déclaration Hors "loadFiles" pour compatibilité Chrome
    WME_VdV_MainCountry = "";
    ...
    function loadFiles () {                  // Passage en variable locales
        if (WME_VdV_1_mainDictionaryTxt != "In Progress" && WME_VdV_1_publicDictionaryTxt != "In Progress") {
            traiteDictionary(WME_VdV_1_mainDictionaryTxt,1);                           // main directory line 1 +
            traiteDictionary(WME_VdV_1_publicDictionaryTxt,1001);                         // public directory line 1001 +
            delete WME_VdV_1_mainDictionaryTxt; delete WME_VdV_1_publicDictionaryTxt;                            // supprime les variables d'import de fichiers
            WME_VdV_onload = "Done";
            WME_VdV_MainCountry = W.model.countries.top.id;
            insertButton();
        }
        else {
    ...
    function traiteDictionary(texte,N_ligne) {
        ...
        for(var i = 0; i < lignes.length; i++) {
            ...
            WME_VdV_Dictionary.push({line :i+N_ligne, toVerify: toverified.replace (/@/g,",") , flags: flag.replace (/"/g,""),  corrected : correct});
        }

        //alert (WME_VdV_Dictionary.length)
        return ;
    }

    function insertButton() {

        if(document.getElementById('WME_VdV_All') === null) {
			var chk1 = $('<Label style="font-weight:normal;"><input type="checkbox"; style="vertical-align: middle;margin: 0px;" id="WME_VdV_enable" title="Enable or Disable WME CRN"> On/Off</input></Label>');
            var chk2 = $('<Label style="font-weight:normal;height:20px;margin:0px 5px 0px 0px"><input type="checkbox"; style="vertical-align: middle;margin: 0px;" id="WME_VdV_CheckRoadName" title="Marque para identificar automáticamente as vias (E places) por onde passa"> Automático </input></Label>');
			var url1 = $('<div style="font-size:12px;display: inline;"> <u><i><a href="https://greasyfork.org/scripts/3776-wme-check-road-name" target="_blank">Verificador Nomes ' + WME_VdV_version+ '</a></i></u>');
			var btn0 = $('<div id="WME_VdV_Dictionary" style="display: inline;"  > <u><i><a style= "float:right; font-size:12px;padding:0 7px 0 3px;border-width:1px;border-color: #e30c0c; border-style:solid;border-radius:6px;" href="https://docs.google.com/spreadsheets/d/'+publicDictionnarykey+'/edit#gid=0" target="_blank" title="Acessar dicionário ativo"> '+searchIdOTAN(WME_VdV_MainCountry)+' </a> </i></u>');

            var btn1 = $('<button class="waze-btn waze-btn-small waze-btn-white" style="padding:0px 6px; height:20px; margin-right:5px;" id="WME_VdV_chk"  title="Click this button to Check Road Names">Iniciar</button>'); btn1.click(rename_Road2);
            var btn2 = $('<button class="waze-btn waze-btn-small waze-btn-white" style="padding:0px 6px; height:20px; margin-right:5px;" id="WME_VdV_stop" title="Click this button to stop">Parar</button>'); btn2.click(stop_check); //           var chk2 = $('<Label class="controls-container" style="font-weight:normal"><input type="checkbox"; style="vertical-align: middle;margin: 0px;" id="WME_VdV_CheckRoadName" title="Click for automatic check of road name (and landmark if layer On)"> Auto </input></Label>');
            var btn3 = $('<button class="waze-btn waze-btn-small waze-btn-white" style="padding:0px 6px; height:20px; margin-right:5px;" id="WME_VdV_raz"  title="Cluqie para limpar o histórico de estradas">RAZ</button>'); btn3.click(RAZ);
            ...
            var btn5 = $('<button class="waze-btn waze-btn-small waze-btn-white" style="padding:0px 6px; height:20px; margin-right: px;"id="WME_VdV_StreetID" title="Click this button to see Street IDS">Str ID</button>');btn5.click(ShowPrimaryStreetID);

			var cnt1 = $('<section id="WME_VdV_link"           style="padding-top:2px; margin:2px;"/>');
			var cnt2 = $('<section id="WME_VdV_rename"         style="padding-top:2px; margin:2px;"/>');
			var cnt3 = $('<section id="WME_VdV_ChangeToAltern" style="padding-top:2px; margin:2px; display : none"/>');
			...
			var WME_VdV_Menu = $('<div id="WME_VdV_All" style="height: auto;width:100%;"/>');
            WME_VdV_Menu.append(cnt1);
            WME_VdV_Menu.append(cnt2);
            WME_VdV_Menu.append(cnt3);
            ...
            var WME_VdV_MenuFlag = false, myAlertBoxFlag = false, myDialogBoxFlag = false;

            function put_WME_VdV_Menu() {        // wait for 'sidebar'

                if (document.getElementById('sidebar')!== null) {
                    if (document.getElementById('WME_JCB_All')!== null) {   // si mon menu existe

                        $("#WME_JCB_All").append('<hr style="margin-bottom:5px; margin-top:5px;width=100%;color:#e30c0c; background-color:#e30c0c; height:1px;">'); }     // on ajoute une barre
                    else {
                        var WME_JCB_Menu = $ ('<div id="WME_JCB_All" style="padding:2px 2px 2px 5px;margin:5px 0px 35px 10px;width:295px; border-width:3px; border-style:double;border-color: #e30c0c; border-radius:10px"/>');
                        $("#sidebar").append(WME_JCB_Menu); // sinon on le créé
                    }
                    $("#WME_JCB_All").append(WME_VdV_Menu);    // on ajoute le menu CRN
                    WME_VdV_MenuFlag = true;
                }
                else {
                    setTimeout (function () {put_WME_VdV_Menu();}, 500);
                }
            }
            put_WME_VdV_Menu();
            ...
            function start_init_WME_VdV() {         // si tous les boutons sont chargés on démarre le script
                if (WME_VdV_MenuFlag && myAlertBoxFlag) {init_WME_VdV();}
                else {setTimeout(function () {start_init_WME_VdV();}, 501);}
            }
            start_init_WME_VdV();
        }
    ...
    var WME_VdV_badStreet = [];
    var WME_VdV_goodStreet = [];
    var WME_VdV_badAlternateStreet = [];
    var listSegIDs = [];
    var listLmrkIDs = [];
	var myStreets = [];
 ...
 //                   add_alternative_street_city (lieuDit,cityName);										// on ajoute le lieudit en alternate de la ville
    ...
    function ShowPrimaryStreetID(ev) {
		...
		if (selectedItems.length == 1) {
            ...
            if ((sel.type == "segment") && (sel.attributes.primaryStreetID !== null)) {
                ...
                var message =  "StreetID = "+ completeWithSpaces (sel.attributes.primaryStreetID,max_length) +"  Nome da Rua = "+ Street.name;
                message += "\nCityID = " + completeWithSpaces(Street.cityID,max_length) +"      Nome da Cidade = "+ City.attributes.name;
                message += "\nStateID = " + completeWithSpaces(City.attributes.stateID,max_length) + "   Estado = "+ states.name;
                message += "\nCountryID = " + completeWithSpaces(City.attributes.countryID,max_length) + "País = "+ country.name;

                alert (message);
            }
        }
        else { alert ("Seleção incorreta"); }
    }
    ...
    function RAZ (ev) {
        WME_VdV_badStreet.length = 0;
        WME_VdV_goodStreet.length = 0;
        WME_VdV_badAlternateStreet.length = 0;
		myStreets.length = 0;
        document.getElementById('WME_VdV_raz').disabled = true;
        return;
    }
    ...
    function stop_check (ev) {
        ...
        document.getElementById ('WME_VdV_CheckRoadName').checked = 0;
        document.getElementById ('WME_VdV_CheckRoadName').disabled = false;
        document.getElementById ('WME_VdV_chk').disabled = false;
        return;
    }
...
//        if (findPending().length=== 0  && W.map.zoom > 1 && limitForSaveNotReach() )  {  // wait for loading
		if (W.map.zoom > 1 && limitForSaveNotReach() ) {
            ...
            document.getElementById('WME_VdV_chk').disabled = true;            // on désactive le bouton "check" durant le check
            document.getElementById('WME_VdV_CheckRoadName').disabled = true;

            myStreets.length = 0;
            if (WME_VdV_goodStreet.length > 1500) {WME_VdV_goodStreet.splice(0,750);}    // on vide les plus anciennes  goods streets s'il y en a trop de stocker
			...
			for (var streetID in W.model.streets.objects) {
				...
				if (notInArray(street_ID,WME_VdV_goodStreet)
                    && notInArray(street_ID,WME_VdV_badStreet)
                    ...
                    && W.model.cities.objects[street.cityID].attributes.countryID == WME_VdV_MainCountry){
			...
			for (var i = 0; i < myStreets.length; i++) {
                ...
                if (newName == name) {WME_VdV_goodStreet.push(myStreets[i].id);}
                if (!limitForSaveNotReach()){myAlert ("<FONT color='red'><b>Please save and retry</b></FONT>");}
                if (newName!=name && limitForSaveNotReach()){
					mySegs = searchSegWithStreetID (myStreets[i].id);
					var myLdmkToCheckStreetID  = searchLdmkToCheckStreetID(myStreets[i].id);
                    ...
                    if (mySegs.main.length + mySegs.alternate.length + myLdmkToCheckStreetID.length > 0) {
                        ...
                        var message = "";
                        if (mySegs.main.length>0)           {message += mySegs.main.length+" rua(s) para alterar o nome principal\n";}
                        if (mySegs.alternate.length>0)      {message += mySegs.alternate.length+" rua(s) para alterar o nome Alternativo\n";}
                        if (myLdmkToCheckStreetID.length>0) {message += myLdmkToCheckStreetID.length+"Place(s) para alterar\n";}
                        message += "\nCidade: "+city.attributes.name+"\n"
                        message += "Estado: "+ W.model.states.objects[city.attributes.stateID].name+"\n"
                        message += "Nome anterior: "+name+"\n\n"
                        message += "Confirme a alteração ou edite manualmente";
                        newName = prompt (message,newName);
                        if (newName === null) {
                            WME_VdV_badStreet.push (myStreets[i].id);
                            document.getElementById('WME_VdV_raz').disabled = false;}
                        else {
							if (newName != name) {
								var multiaction = new WazeActionMultiAction();
								multiaction.setModel(W.model);
                ...
                for (ldmkID in myWazeVenues) {
                    ...
                    if (ldmk!== null && ldmk.state !== "Delete" && onScreen(ldmk) && (ldmk.attributes.approved) &&  (ldmk.attributes.streetID!== null) && ldmk.isAllowed(ldmk.PERMISSIONS.EDIT_GEOMETRY)) {
                        ...
                        if (street && street.cityID !== null) {
                            ...
                            if (city && city.attributes.name!== null && city.attributes.countryID == WME_VdV_MainCountry && oldName && oldName!== "" && notInArray(oldName,WME_VdV_badStreet) && notInArray(oldName,WME_VdV_goodStreet)) {              //  Le segment remplit toutes les conditions pour analyse ultérieure
    ...
    function checkLandmarkName (listLmrkIDs) {
        ...
        for (var i = 0; i < listLmrkIDs.length; i++) {
            ...
            if (ldmark!==null && onScreen(ldmark)) {
                ...
                if (oldName !==null && oldName !=="" && !isInArray(oldName,WME_VdV_badStreet)) {
                    ...
                    if (newName !=oldName && limitForSaveNotReach() ) {
                        ...
                        newName = prompt (message,newName);
                        if (newName === null) {
                            WME_VdV_badStreet.push (ldmark.attributes.name);
                            document.getElementById('WME_VdV_raz').disabled = false;
                        }
   ...
   function changeVenueAdress (sel, country_ID, state_ID,city_Name, street_Name) {
    ...
    function rename2(old_name) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            //         console_log("WME_VdV line " + list[i].line + ' OldName="'+new_name+'" ==> New_Name="'+name+'"');                // trace modification
            //     }
    ...
    function manage_WME_VdV(ev) {
        localStorage.WME_VdV_enable = document.getElementById ('WME_VdV_enable').checked == 1;
        if(document.getElementById('WME_VdV_enable').checked == 1) {
            document.getElementById ('WME_VdV_rename').style.display = "block";
            document.getElementById ('WME_VdV_Dictionary').style.display = "inline";
            if (userIsFrenchCM() && WME_VdV_MainCountry == 73) {document.getElementById ('WME_VdV_ChangeToAltern').style.display = "block";}
            else {document.getElementById ('WME_VdV_ChangeToAltern').style.display = "none";}
        }
        else {
            document.getElementById ('WME_VdV_rename').style.display = "none";
            document.getElementById ('WME_VdV_CheckRoadName').checked = 0;
            document.getElementById ('WME_VdV_ChangeToAltern').style.display = "none";
            document.getElementById ('WME_VdV_Dictionary').style.display = "none";
        }
        manage_CheckRoadName();
        WME_VdV_goodStreet.length = 0;                                              // vide le tableau des rues correctes
        return;
    }

    function manage_CheckRoadName() {
        if (document.getElementById ('WME_VdV_CheckRoadName').checked == 1) {
            document.getElementById('WME_VdV_CheckRoadName').disabled = true;        // on désactive la case
            document.getElementById('WME_VdV_chk').disabled = true;                   // on désactive le lancement manuel
            rename_Road2();
        }
        else {
            document.getElementById('WME_VdV_CheckRoadName').disabled = false;        // on désactive la case
            document.getElementById('WME_VdV_chk').disabled = false;
        }
        if (WME_VdV_badStreet.length===0 && WME_VdV_goodStreet.length===0) {          // gestion du bouton raz
            document.getElementById('WME_VdV_raz').disabled = true;}
        else {
            document.getElementById('WME_VdV_raz').disabled = false;}
        return;
    ...
    function init_WME_VdV() {
        localStorage.removeItem('WME_VdV_CheckLdmkName');                        // Remove old item in LocalStorage
        ...
        localStorage.removeItem('WME_VdV_CheckCityName');
        localStorage.removeItem('WME_VdV_CheckRoadName');

        if (localStorage.WME_VdV_enable == "true") {                              // restaure old Values (if exist)
            document.getElementById ('WME_VdV_enable').checked = 1; }
		document.getElementById ('WME_VdV_CheckRoadName').checked = 0;
		document.getElementById('WME_VdV_enable').onclick = manage_WME_VdV;
		document.getElementById('WME_VdV_CheckRoadName').onclick = manage_CheckRoadName;
		document.getElementById ('WME_VdV_StreetID').style.display = "none";
		W.selectionManager.events.register("selectionchanged", null, manage_streetID);
		manage_streetID ();
		manage_WME_VdV ();

		myAlert("Verificador de Vias Iniciado!");
		myAlert("Dicionário ativo: " + W.model.countries.objects[WME_VdV_MainCountry].name);
    }

    function manage_streetID() {
		var selectedItems = W.selectionManager.getSelectedFeatures();
		if (selectedItems.length== 1) {
            document.getElementById ('WME_VdV_StreetID').style.display = "inline";}
        else {
            document.getElementById ('WME_VdV_StreetID').style.display = "none";}
    }
    return;
    ...
    function searchIdOTAN(countryID){
        switch (countryID) {
			case 30 : return " BRA "; break; // Brazil
    ...
    function myAlert (message) {
        if (document.getElementById('edit-buttons')!==null && !document.getElementById ('WME_JCB_AlertTxt')) {       // verif (et réafffichage) de l'alerteBox
            var myAlertBox = $('<div id="WME_JCB_AlertBox" class="form-search" style="opacity : 1;display :none; padding :4px 10px; height: auto;min-height: 18px; position: absolute;top :6px; margin-left: 370px; margin-right: auto; background-color: #0ce343;border-radius: 20px;"/>');
            var myAlertTxt = $('<div id="WME_JCB_AlertTxt" style=" opacity : 1;color : #e30c0c; display:inline;padding:0px 0px">City ID/');	$("#edit-buttons").append(myAlertBox);
        ...
        if (document.getElementById ('WME_JCB_AlertTxt')){
            ...
            setTimeout(function() {endAlert();}, 3750 + 1000*Math.random());
        }
    }
