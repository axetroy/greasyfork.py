...
(function() {
    function activiteLink($dom, linkList) {
        ...
        for (var i = 0; i < linkList.length; i++) {
            docu = docu.replace(new RegExp(linkList[i].url+"(?=[^#])", "gm"),"<a target='_blank' class='btn btn-url' color:blue' href='" + linkList[i].url + '#' + linkList[i].password + "'>一键提取</a> ");
        }
        $dom.html(docu);
    }
    function Link() {
    ...
    var urlExp = /((?:https?:\/\/)?(?:yun|pan|eyun|share).(?:baidu|weiyun)?.com\/(?:s\/\w*|share\/\w*)?(?:[a-zA-Z0-9-]+)?)/g;
    ...
    if (isInSharePage) {
        ...
        var $script = $("script");
        var regRuleLink = "((?:https?:\\/\\/)?(?:yun|pan|eyun|share).(?:baidu|weiyun)?.com\\/(?:s\\/\\w*|share\\/\\S*\\d)?(?:[a-zA-Z0-9-]+)?(#\\w{4})?)";
