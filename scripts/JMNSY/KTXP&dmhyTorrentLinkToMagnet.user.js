    ...
    if(link != null){ // 不可使用!==进行两者的比较，因为undefined !== null的值为true
        link.each(function(){
            if(switchy == 1){
                ...
                else if(JSON.parse(getItemByDefault("hasTracker","false"))){
...
function dmhySaveAndClose(){
    ...
    jQuery(".download-arrow[title='磁力下載']").each(function(){
        ...
        //如果勾上“复制种子链”
        if('true'==TRFlag){
            temp.attr("href",temp.next().next().attr("href"));
        }
        //如果勾上“磁鏈帶Tracker”
        else if('true'==HTFlag){
            //在磁链后带上tracker
...
//update log:
//1.增加了动漫花园的只复制种子链设置(目前脚本可支持只复制磁链和只复制种子链)
//2.增加了只复制种子链被勾选时，勾选显示种子链的联动配置
//3.增加了显示种子链被取消勾选时，取消勾选只复制种子链的联动配置
//4.调整设置界面的元素堆叠顺序
//5.避免未安装的用户以为该脚本年久失修，无法正常使用