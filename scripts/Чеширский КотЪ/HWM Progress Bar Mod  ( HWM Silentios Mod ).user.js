// ==UserScript==
// @name 	       HWM Progress Bar Mod  ( HWM Silentios Mod )
// @author 	       Silentio (update ElMarado)
// @collaborator   style: sw.East
// @namespace      https://openuserjs.org/users/chesheerk/scripts
// @description    RU_ru: Показывает уровень Гильдий, Левела, Умелки Фракций  в виде прогресс бара; EN_en: Add progress bar for Combat, Faction and Guilds levels. (2018.10.16) Dual_Language
// @encoding 	   utf-8
// @version 	   1.95.4
...
function main(e){
    ...
    var combat_exp_lvl = [
        ...
        3000000,5000000,8500000,14500000,25000000,43000000,70000000,108000000,160000000,230000000,325000000,500000000
    ];

    //Racial skill table (13 levels)
    var racial_skill_lvl = [
        20,50,90,160,280,500,900,1600,2900,5300,9600,17300,35000
    ];
    ...
    var thieves_guild_lvl = [
        50,120,240,400,600,840,1200,2000,3000,4300,6000,8000,10800,14000,17600,21600,26000,30800,36600,43600,52600
    ];
	...
	150,350,750,1400,2200,3200,4300,5600,7000,8500,10000,11700
    ];

	//Watchers' Guild (7 levels)
    var watchers_guild_lvl = [
        60,200,450,850,1500,2700,4500
    ];
    ...
    var leaders_guild_lvl = [
        80, 180, 300, 440, 600, 780, 990, 1230, 1500
    ];
...
//'
    ...
    function makeProgressBar(exp_attuale, lvl_attuale, exp_lvls){
        ...
        var progress_bar_html = "<div class=\"silentios_progress tooltip\">"+
                                    "<span class=\"tooltiptext\">" + perc_text2 + "%</span>"+
                                    "<div class=\"silentios_progress_bar\" style=\"width:" + perc_text + "%\"></div>"+
                                    "<div class=\"silentios_bar_perc color_bar_perc\" style=\"margin-left:"+ perc_text +"%\">" + perc_text +" %</div>"+
                                "</div>";
            ...
            for (var i = 25; i < tabelle.snapshotLength; i++){
                ...
                if (tabelle.snapshotItem(i).childNodes[0].childNodes[0].childNodes[0]) {
                    if (tabelle.snapshotItem(i).childNodes[0].childNodes[0].childNodes[0].innerHTML.indexOf(T('Combat level') +":") > 0) {
                        ...
                        /** === Style === */
                        GM_addStyle ('.color_bar_perc {background-color: #FFF;}'); // подставляем белый фон
                        /* Style End */
                    }
                }
                ...
                if (tabelle.snapshotItem(i).childNodes[0].childNodes[2]) {
                    if (tabelle.snapshotItem(i).childNodes[0].childNodes[2].childNodes[0]) {
                        if (tabelle.snapshotItem(i).childNodes[0].childNodes[2].childNodes[0].textContent.indexOf(T('Combat level') +":") > 0) {
                            ...
                            /** === Style === */
                            GM_addStyle ('.color_bar_perc {background-color: #F5F3EA;}'); // подставляем светло-бежевый фон
                            /* Style End */
                        }
                    }
                }
