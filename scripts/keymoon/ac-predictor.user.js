...
SideMenu.Datas.Update.History = (() => {
    ...
    try {
        $.ajax({
            url: `https://atcoder.jp/users/${userScreenName}/history/json`,
            type: "GET",
            dataType: "json"
        }).done(x => {
            SideMenu.Datas.History = x;
            d.resolve();
        }).fail(() => { d.reject(); });
    }
    catch (e) {
...
SideMenu.Datas.Update.Standings = (() => {
    ...
    try {
        $.ajax({
            url: `https://atcoder.jp/contests/${contestScreenName}/standings/json`,
            type: "GET",
            dataType: "json"
        }).done(standings => {
...
SideMenu.Elements.Estimator = (async () => {
	await SideMenu.appendToSideMenu(/atcoder.jp/,'Estimator',getElem);
	...
	var js =
	`(() => {
	    var estimator_state = parseInt(localStorage.getItem("sidemenu_estimator_state")) || 0;
	    \$("#estimator-input").val(localStorage.getItem("sidemenu_estimator_value"));
	    updateInputs();
	    ...
	    function updateInputs() {
	        ...
	        var perfhistory = SideMenu.Datas.History.filter(x => x.IsRated);
			perfhistory.sort(function (a, b) {
				if (a.EndTime < b.EndTime) return 1;
				if (a.EndTime > b.EndTime) return -1;
				return 0;
			})
			perfhistory = perfhistory.map(x => x.Performance)
			...
			if (estimator_state === 0) {
				...
				for (var i = 0; i < 100; ++i) {
					var mid = (hi + lo) \/ 2;
					var r = calc_rating([mid].concat(perfhistory));
					if (r >= goal_rating) {
						hi = mid;
					} else {
						lo = mid;
	            ...
	            res = calc_rating([input].concat(perfhistory));
...
SideMenu.Elements.Predictor = (async () => {
	await SideMenu.appendToSideMenu(/atcoder.jp\/contests\/.+/,'Predictor',getElem);
	async function getElem() {
	//NameSpace
	SideMenu.Predictor = {};
	var maxDic =
