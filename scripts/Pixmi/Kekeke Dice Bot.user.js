...
if (localStorage.getItem('kekeke_dice_config') === null) {
    ...
    var config = JSON.parse(localStorage.getItem('kekeke_dice_config'));
    if (config.Auto_Logs == null) { config.Auto_Logs = false; }
    if (config.Logs_Time == null) { config.Logs_Time = false; }
...
function Observer() {
    ...
    var rootObserver = new MutationObserver(function (mutations) {
        ...
        if (chatElement) {
            ...
            console.log('Kekeke Dice Bot 1.1.4 已在 ' + document.title + ' 啟動。');

            var dd_check = /^([1-9][0-9]?)[d]([1-9][0-9]{0,2})([\+\-\*\/])?([1-9][0-9]{0,1})?([?])?/i,
                cc_check = /^cc\(([0-9]{1,2})\)(([\+\-])([1-9]))?/i,
                na_check = /^1na([\+\-\*\/])?([1-9][0-9]*)?/i,
                nc_check = /^([1-9][0-9]?)[n][c]([\+\-\*\/])?([1-9][0-9]{0,1})?/i;
            
            var url = /<a class="GlobalCssResource-external" target="_blank" ref="noopener" href="((http|https)(:\/\/)[\w\+\-\*\/\.\&\:\(\)\<\>\?\=]*)">((http|https)(:\/\/)[\w\+\-\*\/\.\&\:\(\)\<\>\?\=]*)<\/a>/ig,
            ...
            // 檢查自動下載
            if (config.Download_Time !== '0' && config.Auto_Logs) {
                        ...
                        if (mutation.addedNodes[0].classList.contains('SquareCssResource-chatContent') && BotNameCheck()) {
                            ...
                            if (cc_check.test(Message[0])) {
                                ...
                                if (symbols) {
                                    ...
                                    content = '[' + Post_Name.text() + '] ' + match[0] + ' 骰出了 [' + first_value + '](取最' + bonus + 'x10) + [' + last_value + '] = ' + result + '，判定為 [' + judge + ']' ;
                                } else {
                                    content = '[' + Post_Name.text() + '] ' + match[0] + ' 骰出了 [' + result + ']，判定為 [' + judge + ']';
                                }
                            }
                            // 確認是否為NA骰
                            ...
                            if (content !== '') { DiceRespond(content); }
                            // 將聊天紀錄輸出到console.log
                            if (config.Auto_Logs) {
                                let arr = JSON.parse(localStorage.getItem("kekeke_logs")),
                                ...
                                if (logs_content !== '') {
                                    ...
                                    localStorage.setItem('kekeke_logs', JSON.stringify(arr));
                                }
                            }
                        }
                    }
            ...
            $('#DiceConfig').click(function () {
                ...
                if ($(this).hasClass('init')) {
                    ...
                    $('<tr><td></td><td><label><input type="checkbox" id="cb_Logs_Clear" checked="checked">下載後清除舊紀錄 <small style="color: #777">(降低Storage使用量)</small></label></td></tr>').appendTo('#DiceConfig_panel > table');
