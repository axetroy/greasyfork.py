...
(function() {
                ...
                tabset_id: {
                    ...
                    content: {
                        ...
                        page_id3: {type: 'page', label:  'Number of tables', hover_tip:'', content: {
                            'numberOfLeaderboardTables': {type:'dropdown', label:'Number of leaderboard tables', hover_tip: 'The amount of tables the added users will be split between', default:'0', full_width: true,
                                         on_change: changeNumberofTables,
                                         content:{0:'--table nr.--', 1:'1 (Min)', 2:'2'}},
                        }}
                    }
                }
            }
    ...
    const adminNamesInfinity = ['viet', 'Kristen', 'kristen', 'koichi', 'sam', 'oldbonsai', 'arpit.jalan', 'arpit'];//∞
    ...
    function changeNumberofTables(){
        if(wkof.settings.Leaderboard.numberOfLeaderboardTables !== '0'){
            numberOfLeaderboardTables = wkof.settings.Leaderboard.numberOfLeaderboardTables;//save user chosen number of tables
        }
    ...
    async function assignLevelAndAvatarFromWkProfile(item)
    {
        ...
        item.totalBurnPercentage = Math.round((srsCountsLabeled[0].burnTotal/8804*100) * 100) / 100;
    }
    ...
    function startup() {
        ...
        getNumberOfLeaderboardTablesFromCache().then(function(result) {
            ...
            //console.log('numberOfLeaderboardTables from cache: '+numberOfLeaderboardTables);
        });
    ...
    function createLeaderboard() {
        ...
        for (var i = 0; i < numberOfLeaderboardTables; i++){
            ...
            let endNumberTable = Math.floor((usersInfoList.length/numberOfLeaderboardTables)*(i+1))-1;

            //if userlist has an odd number of users and this is not the final table, add a user to the end
            if (usersInfoList.length%2 != 0 && endNumberTable != usersInfoList.length){
                endNumberTable++;
            }
            ...
            //for (var j = 0; j < usersInfoList.length; j++){
