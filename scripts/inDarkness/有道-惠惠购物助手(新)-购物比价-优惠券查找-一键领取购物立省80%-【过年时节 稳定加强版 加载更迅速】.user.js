// ==UserScript==
// @name 有道-惠惠购物助手(新)-购物比价-优惠券查找-一键领取购物立省80%-【过年时节 稳定加强版 加载更迅速】
// @description 有道购物助手，浏览商品页面的时候，自动对比电商同款商品价格，包含【淘宝\天猫\京东\易贝\亚马逊\当当\苏宁各种商城的价格对比】，可以查看【往期商品价格，价格销售曲线】，方便查看用户评价，轻松网购不吃亏，更可以加收藏栏跟随商品销售观察商品价格变化情况，更有【降价提醒】，快速购买折扣商品，更有各种优惠信息对比
// @update 2019-03.24
// @version 3.16
// @note   2019.03.24 继续修复预加载的问题，上次只是更新并修复了一部分的样式内容
...
// @resource guzs https://baidu.ntaow.com/outcss/guzs.css?t=1.0
...
setInterval(function(){
	...
	if(typeof(youdao) != "undefined"){
		var e = youdao.require_module("youdao.consts");
		if ((typeof(e) != "undefined" && e != null) && hasNotInit == false) {
			hasNotInit = true;
			console.log(typeof(e));
			if (canGetResourceText() == true) {
				console.log(e);
				var gmRet = GM_getResourceText('guzs');
				addStyle(gmRet.replace(/hui-shoppingtool2/g, e.ShoppClass).replace(/hui-plugin1/g, e.elemPluginClass).replace(/hui-plugin-not-promo/g, e.elemPluginPromoClass).replace(/hui-clearfix/g, e.elemPluginClearFixClass).replace(/hui-plugin-logo/g, e.elemLogoId.replace("#", "")).replace(/hui-plugin-history-new/g, e.elemHistoryNew.replace(".", "")).replace(/hui-plugin-history/g, e.elemHistory.replace("#", "")).replace(/hui-btn-disabled/g, e.elemDisabled).replace(/hui-btn/g, e.elemBtn).replace(/hui-plugin-collection/g, e.elemCollecId.replace("#", "")).replace(/hui-plugin-qrcode/g, e.elemQrcodeId.replace("#", "")).replace(/hui-plugin-promotions/g, e.elemPromoId).replace(/daigou-promotions/g, e.elemPromoClass).replace(/hui-promotions-in/g, e.elemPromoInClass).replace(/hui-promotion-list/g, e.elemPromoListClass).replace(/hui-shopping-nav/g, e.ShoppNavClass).replace(/hui-shopping-cont/g, e.ShoppContClass).replace(/hui-shopping-ext/g, e.ShoppExtClass).replace(/hui-shopping-faq/g, e.ShoppFaqClass).replace(/hui-fz12/g, e.ShoppFzClass).replace(/hui-bar-qrcode/g, e.BarQrcoderClass).replace(/hui-sp11/g, e.ShoppSpClass.replace(".", "")).replace(/hui-sp/g, e.ShoppHuiSpClass).replace(/hui-shopping-pricestate/g, e.ShoppPriceClass.replace(".", "")).replace(/hui-taobao-history/g, e.ShoppTaobaoHistClass).replace(/hui-shopping-lightbox-bd/g, e.ShoppLightBoxBdClass).replace(/hui-shopping-lightbox-hd/g, e.ShoppLightBoxHdClass).replace(/hui-shopping-lightbox-title/g, e.ShoppLightBoxTitleClass).replace(/hui-shopping-lightbox/g, e.ShoppLightClass).replace(/hui-shopping-datacont/g, e.ShoppDatacontClass).replace(/hui-shopping-datalist/g, e.ShoppDatalistClass).replace(/hui-shopping-readmore/g, e.ShoppReadmoreClass).replace(/hui-shopping-logo/g, e.ShoppLogoClass).replace(/hui-button-bar-wrapper/g, e.ShoppBarWrapperClass).replace(/hui-content/g, e.ShopContentClass).replace(/hui-button-bar-tip/g, e.ShopBarTipClass).replace(/hui-button-bar-title/g, e.elemBarTitleClass).replace(/hui-button-bar/g, e.ShopBarClass).replace(/hui-shopping-img-ad/g, e.ShopImgAdClass).replace(/hui-collection-collected/g, e.HuiCollectionEdClass).replace(/hui-collection-no-collect/g, e.HuiCollectionNoClass).replace(/hui-collection-under/g, e.HuiCollectionUnderClass).replace(/hui-collection-closet-list-positionfix/g, e.HuiCollectionClosetListClass.replace(".", "")).replace(/hui-collection-title/g, e.HuiCollectionTitleClass).replace(/hui-collection-btns/g, e.HuiCollectionBtnsClass), "ac-hui-Style");
			} else {
				// 再说吧，新的GM不好兼容，数据反正也会自动获取成功，预加载不影响效果
			}
		}
	}
}, 50);
