...
function deal_fatherNode(node, faviconUrl){
    ...
    if(II <= 5){
        ...
        var imgHTML = "<img class=\"faviconT\" style=\"vertical-align:sub;\" src='https://api.byi.pw/favicon/?url="+faviconUrl+"' onerror=\"this.src='http://code.remix.ac.cn/favicon.ico'\" height=20 width=20>&nbsp;";
...
function replaceAll(sbefore){
	...
	send = sbefore.replace(/(\/[^/]*|\s*)/g ,"").replace(/<[^>]*>/g, ""); //.replace(/<?[^>]*?\s*>?/g ,"");
	// console.log(send);
	return send;
}
