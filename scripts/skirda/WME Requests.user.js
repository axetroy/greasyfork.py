...
//W.model.streets.get(GetWMESelected()[0].model.attributes.primaryStreetID).cityID
			...
			if (sID)
			{
				var streetsObj = W.model.streets.get(sID);
...
//W.model.streets.get(GetWMESelected()[0].attributes.streetID).cityID
			...
			if (sID == null)
				...
				cityID=W.model.streets.get(sID).cityID;
			...
			if (cityID !== "")
			{
				var cityObj=W.model.cities.get(cityID);
				if (typeof cityObj !== "undefined")
				{
					...
					var stateObj=W.model.states.get(stateID);
					if (typeof stateObj !== "undefined")
						stateName=stateObj.getAttributes().name;

					var countriesObj=W.model.countries.get(countryID);
			...
			{
				var name=W.model.streets.get(segment.attributes.streetID).name;
		...
		if (!(/*typeName === "camera" || */typeName === "node" || cityID === "")) // BUGBUG!!!
		{
			...
			let cityObj=W.model.cities.get(cityID);
			if (typeof cityObj !== "undefined")
			{
				countryID=cityObj.getAttributes().countryID;
				stateID=cityObj.getAttributes().stateID;
				...
				let stateObj=W.model.states.get(stateID);
				if (typeof stateObj !== "undefined")
					stateName=stateObj.getAttributes().name;

				let countriesObj=W.model.countries.get(countryID);
				if(wmer_Debug) console.log(countriesObj);
				if (typeof countriesObj !== "undefined")
					countryName = countriesObj.name;
			}
...
function click_WMERequest() {
	...
	var urPos;
	if(!wmer_MapCenter) //???
		urPos=new OpenLayers.LonLat(W.map.center.lon,W.map.center.lat);
	else
	{
      ...
      if (wmeSelected[0].model.type === "venue")
      {
      	...
      	if(!area_poi)
      	{
			...
			if(area_poi)
			{
				...
				if (typeof W.model.venues.get(v_id).geometry.getGeodesicArea === "undefined")
					area_poi.innerHTML="";
				else
				{
					var square=W.model.venues.get(v_id).geometry.getGeodesicArea(W.map.getProjectionObject());
