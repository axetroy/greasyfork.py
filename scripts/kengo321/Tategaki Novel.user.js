...
;(function() {
  ...
  var Narou = (function() {
    ...
    Parser.prototype.getAuthor = function() {
      var l = document.querySelectorAll('.contents1 > a')
      if (!l.length) return null
      if (l.length >= 2) return Line.parse(l[1].childNodes)
      var a = [].filter.call(l[0].parentNode.childNodes, function(n) {
        return n.nodeType === Node.TEXT_NODE
            && n.data.indexOf('作者：') >= 0
      })
      return a.length
           ? Line.newByStr(a[0].data.slice(a[0].data.indexOf('作者：') + 3))
           : null
    }
    Parser.prototype.parse = function() {
      ...
      , nextURL: getHRef(navSelector, '次へ\xA0>>')
      , prevURL: getHRef(navSelector, '<<\xA0前へ')
      , position: getLine('#novel_no')
      })
    }
    ...
    ShortParser.prototype.getAuthor = function() {
      return getLine('.novel_writername > a')
    }
