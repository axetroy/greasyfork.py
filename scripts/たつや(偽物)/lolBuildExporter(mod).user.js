// ==UserScript==
// @name        lolBuildExporter(mod)
// @name:en     lolBuildExporter(mod)
// @namespace   https://github.com/xwavex/lol-Build-Exporter
// @description    League of Legends Item exporter by Dennis Leroy Wigand Modified tatuya00
// @description:en League of Legends Item exporter by Dennis Leroy Wigand Modified tatuya00
...
function processData(itemSite) {
	if (itemSite != null) {
			...
			for (var u = 0; u < allNamesplit.length; u++) {
					...
					for (var v1 = 0; v1 < listItemBlocks.length; v1++) {
						...
						for (var v2 = 0; v2 < block.items.length; v2++) {
							...
							//エンチャントアイテムは諦めて元アイテムに
							var enc;
							if(block.getItem(v2).name.lastIndexOf(" - ") != -1) {
								enc = block.getItem(v2).name.substr(0,block.getItem(v2).name.lastIndexOf(" - "));
							}else {
								enc = block.getItem(v2).name;
							}
							
							
							if (allNamesplit[u].indexOf(enc) != -1 && allNamesplit[u].indexOf(enc) < 50) {
								// find id:
								if (u > 0) {
		...
		saveTextAsFile(jsonString, champion+"_"+title+'.json');
}
	...
	if (allBuildBoxes.length > 0) { // only do further stuff, when button can be added properly!
		...
		btn.onclick = function() {
			...
			var activeIndex = 0;
			  ...
			  //最新版アイテム情報URL
				doAjax("http://ddragon.leagueoflegends.com/cdn/6.9.1/data/en_GB/item.json");
