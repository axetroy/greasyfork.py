...
// @version        $Id: pfandflaschensammler.user.js 201 2018-04-15 17:01:44Z mkl $
...
if (host == "spenden.hitfaker.net")
    host = GM_getValue("hitfakerhost", "").split(";")[0].split("/").pop();
else {
...
var PageNames = ['Verbrechen', 'Aktionen', 'Apotheke', 'Zubehör', 'Supermarkt',
                           'Weiterbildung', 'Pfandflaschen', 'Kommunikation', 'Kronkorken', 'Inventar'];
...
if(window.location.hostname == "spenden.hitfaker.net"){
    function resethitfaker(hhost) {
        trace("resethitfaker: " + hhost, 2);
        GM_deleteValue("hitfakerlink");
        if (hhost == "")
            window.history.back(8);
        else {
            GM_deleteValue("hitfakerhost", "");
            window.location.href = hhost + "/overview/";
        }
        return;
    }

    var hhost = GM_getValue("hitfakerhost", "").split(";")[0];
    var don = GM_getValue("hitfakerlink", "");
    trace("Spendenlink: " + don, 2);
    if (don == "xxx")
        window.setTimeout(resethitfaker, 20000, hhost);
    else if (don != "") {
        function checklink(link, start) {
            trace("checklink", 2);
            if (document.getElementById(link).value == "")
                window.setTimeout(checklink, 1000, link, start);
            else if (document.getElementById(link).value == don) {
                GM_setValue("hitfakerlink", "xxx");
            ...
            if (content.indexOf("<strong>Mein Penner</strong>") != -1) {
                ...
                for (var i = 1; i < districts.length; i++) {
                    ...
                    if (districts[i].indexOf('value="Kaufen"') == -1) {
                        ...
                        if (bonus.indexOf("schneller Sammeln") != -1) {
                            ...
                            if (bonus.indexOf("mehr Flaschen") != -1)
                ...
                if (hdist.length < 2)
                    ...
                    for (var i = 0; i < hdist.length; i++) {
                        ...
                        if (hdist[i][3].indexOf("mehr Flaschen") == -1)
                            bonus = "";
        ...
        if (this.checked) {
            if (PGu_getValue("AutoCrimePic", "") == "") {
                if (TOWNEXTENSION == "VT")
                    ...
                    alert("Bitte erst Sabotage festlegen !!");
                else
...
function insertCheckBox3(button) {
    ...
    var newtd = document.createElement("td");
    newtd.setAttribute('style', 'padding-top: 10px; padding-bottom: 10px');
    newtd.innerHTML = '<input name="CbCrime" id="' + name +'" type="checkbox"><span style="vertical-align: bottom">&nbsp;diese' + (TOWNEXTENSION=="VT"?" Sünde":(TOWNEXTENSION=="AT"?" Sabotage":"s Verbrechen")) + '&nbsp;&nbsp;</span><input name="CbCrimeFkt" style="width:30px" id="' + name +'Fkt"><span style="vertical-align: bottom">mal automatisch starten</span>';
...
function insertCheckBox8() {
    ...
    if (TOWNEXTENSION == "VT" || TOWNEXTENSION == "AT") {
        tr2.getElementsByTagName("td")[0].innerHTML = 'Mindestverkaufspreis ' + (TOWNEXTENSION == "AT" ? "Perlen" : 'Flaschen') + ':';
    ...
    if (TOWNEXTENSION == "VT" || TOWNEXTENSION == "AT") {
        ...
        tr3.getElementsByTagName("td")[0].innerHTML = 'Maximaler ' + (TOWNEXTENSION == "AT" ? "Perlen" : 'Flaschen') + 'füllgrad:';
...
function insertCheckBox15() {
    ...
    var town = ["HH", "B", "MU", "HR", "K", "SY", "VT", "AT"];
    if (town.indexOf(TOWNEXTENSION) == -1)
        return;
...
function updDFList() {
	...
	PG_log("updDFList");
    var town = ["HH", "B", "MU", "HR", "K", "SY", "VT", "AT"];
    var DFTownCode = ["hamburg", "berlin", "muenchen", "reloaded", "koeln", "sylt", "vatikan", "atlantis"];
...
function CheckSalary() {
    ...
    getOverviewPage(function() {
        ...
        if (overviewcontent.indexOf(" jetzt einsacken") != -1)
            var salary = Number(overviewcontent.split(" jetzt einsacken")[0].match(/[\d,.]+/g).pop().replace(/[.,]/g, ''));
        else
            var salary = 0;
        ...
        var hhost = GM_getValue("hitfakerhost", "").split(";");
        if (hhost.length < 2)
            var hhostset = 0;
        else
            var hhostset = Number(hhost[1]);
        if (hhostset + 60000 < Number(today.getTime())) {
            GM_setValue("hitfakerhost", prothost + ";" + today.getTime());
            trace("hitfakerhost set to <" + prothost + "/" + today.getTime() + ">", 8);
            setTimeout(CheckSalary, 2000);
            return;
        }
        hhost = hhost[0];
        if (hhost != prothost) {
            trace("hitfakerhost nicht für diesen Host", 2);
            return;
        }
        ...
        GM_xmlhttpRequest({method:"GET", url: prothost + '/stock/plunder/ajax/?c='+pltab, onload:function(responseDetails) {
            ...
            PGu_setValue("spendenrest", rest);
            trace("Hole Spenden: " + rest, 1);
            GM_setValue("hitfakerlink", link);
            window.location.href = "http://spenden.hitfaker.net";
        }
        });
    });
}
...
function getOverviewPage(callfkt) {
