...
(function() {
    ...
    var forumSection,
        ...
        ".altHost  {display: inline-block;text-align: center;font-weight: bold; overflow: hidden;background-image: url(http://s23.postimg.org/b8lsdn453/Not_Listed_2.png);background-size: cover;}\n" +
    ...
    var IMDb = {
        search: function(arg){
            ...
            GM_xmlhttpRequest({
                method: "GET",
                url: "http://akas.imdb.com/find?ref_=nv_sr_fn&s=all&q=" + encodeURIComponent(arg.title + arg.year) + ((arg.type === "tv")? "&ttype=tv":"&ttype=ft"),
                onload: function(response) {
    ...
    $(document).on('click','.genre a',function(e){
        if($(this).attr('href').match(/imdb.com\/title\/tt.+$/i)){
            if(! e.ctrlKey){
                ...
                if(typeof(Info.Genre) !== 'undefined')			$('#genre',IMDB).text(Info.Genre);						  	 else $('#genre',IMDB).remove();
                ...
                Info.Stars.forEach(function(actor){
                    var $actor = $('#actorsRow',IMDB).append('<a target="_blank"><img id="profielpic"><span></span></a>').find('a:last'),
                        res = /([US][XY])(\d+)_CR(\d+),(\d+),(\d+),(\d+)_AL_\.jpg$/.exec(actor.Image),
                        url = "";
                    $actor.attr('href',actor.URL);
