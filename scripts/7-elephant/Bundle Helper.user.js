	...
	, classOwned, classNotInterested, classWished, getCountCallback)
{
	...
	var rgxId = /[0-9]{3,}/g;
	var rgxApp = /((:\/\/(store\.steampowered\.com|steamcommunity\.com|steamdb\.info)\/app|\/steam\/apps)\/[0-9]+|^[0-9]{3,}$)/i;
				...
				if (ids)
				{
					...
					if (rgxApp.test(attrHref))
					{
						...
						else
						{
							...
							else if (dataRes["rgIgnoredApps"].indexOf(valId) > -1)
							{
...
function main()
{
	...
	else if (url.indexOf("humblebundle.com") > -1)
	{
			...
			attachOnLoad(function()
			{
				setTimeout(function()
				{
					...
					if (eleFocus)
					{
                        ...
                        var eleClick = document.querySelector(".keyfield-value");
                        if (eleClick)
                        {
                            eleClick.click();
                        }
		...
		else if (url.indexOf("/receipt") > -1)
		{
			setTimeout(function()
			{
				var eleClick = document.querySelector(".success-redirect a");
				if (eleClick)
				{
					eleClick.click();
				}
