...
{	
	...
	if (url.indexOf("/giveaway/") > -1)
	{
		...
		// Train detector
		{
			...
			if (trainUrls.length > 0)
			{
				var rgxBack = /(back|prev|last|past|before|<|◄|⬱|←|⏮|⏪|«|⇦|⬅|↩|⇠|↤|⤶|🔽)/i;
				var rgxNext = /(next|>|↪|⇢|↦|⤷|🔼)/i;
				var rgxNotNum = /[^0-9]/g;
				...
				if (trainUrl !== "")
				{
					...
					if (!isSelf)
					{
						setTimeoutCustom(function(trainUrl)
						{
							...
							if (eleOwned)
							{
								if (document.querySelector(".sidebar__error.is-disabled, .sidebar__entry-delete.is-hidden"))
								{
									// Skip owned
									isRide = true;
								}
