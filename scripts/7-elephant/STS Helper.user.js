...
function initStyle()
{
	...
	(" \
		...
		.stsh_text_org, .stsh_text_trn { min-height: 21px; display: block; max-width: 470px !important; } \
		...
		.stsh_discussion_text { display: inline; } \
...
';
		...
		// Add shortcut keys
		{
			...
			if (elesInputApprove.length > 0)
			{
				...
				addKeyAlt(document, eleBind, ["BracketLeft", 219, "KeyO", 79, "KeyA", 65]); // [ O A
			...
			if (eleInputDecline)
			{
				...
				addKeyCtrl(document, eleBind, ["Delete", 46], "Del", 1|2);			// DEL
				disableAfterClick(eleBind);
				
				var eleInpueDeclineNext = eleInputDecline.nextElementSibling;
				if (eleInpueDeclineNext && eleInpueDeclineNext.value.indexOf("Next") > -1 
					&& !eleInpueDeclineNext.disabled)
				{
					var eleBind = eleInpueDeclineNext;
		...
		// Add remove & next
		{
			...
			for (var i = 0; i < elesRemove.length; i++)
			{
				...
				// Add remove & next
				{
					...
					var eleNext = document.createElement("input");
					eleNext.value = "Next";
			...
			if (!/edit/i.test(getQueryByName("action")))
			{
				if (eleTextOrg)
				{
					...
					if (textOrg === GM_getValue("textLastOrg", ""))
					{
						...
						}, timingInit.pasteLastSuggestion);
					}
				}
			}
			...
			if (/reconsider|approve|decline/i.test(getQueryByName("action")) || url.indexOf("#stsh_modcomment") > -1)
			{
				setTimeoutCustom(function()
				{
					if (eleTextNew && eleTextOrg)
					{
						...
						else if (eleTextNew.scrollHeight > 110 || eleTextOrg.scrollHeight > 200)
						{
