// ==UserScript==
// @name        WaniKani additional resource links + Stroke order
// @namespace   wanikaniALCLRV
// @description Adds a link to alc/goo/weblio/ on vocab/kanji review/lesson/info pages, and stroke order for kanji.
...
// Type of ALC account to use
// 1 = Normal, no log in.
// 2 = Pro lite
// 3 = ALC Pro is under maintenance, so maybe when it is back.
var iAlcLoginType = 1;

// Show stroke order by default
// 0 = Don't show until icon is clicked.
// 1 = Show when page loads.
var iAutoLoadSO = 1;
...
if ($(location).attr('href').indexOf("review") != -1 || $(location).attr('href').indexOf("lesson") != -1) {
	...
	switch (iAlcLoginType)
	{
		case 1: // no login
			var alcURL = 'http://eow.alc.co.jp/search?q=' + encodeURIComponent(strLinkContent) + '&ref=sa';
			strAlcLinkString = "<a target='_alcLink' href='"+alcURL+"'>alc</a>";
			break;		
		case 2:
			var alcURL = 'http://eowpf.alc.co.jp/search?q=' + encodeURIComponent(strLinkContent) + '&ref=sa';
			strAlcLinkString = "<a target='_alcLink' href='"+alcURL+"'>alc*</a>";
			break;
	}
	// goo link
	...
	if ($(location).attr('href').indexOf("kanji") != -1) {
		...
		SOLI.innerHTML = "<a  id='strokeOrderToggle' href=''>画</a>";
...
function addLink(strKey, strLinkElement) {
    ...
    if ("kan" in objCurItem)
	{
		...
		switch (iAlcLoginType)
		{
			case 1: // no login
				var alcURL = 'http://eow.alc.co.jp/search?q=' + encodeURIComponent(strLinkContent) + '&ref=sa';
				strLinkString += "<a class='hvAdResLink' target='_alcLink' href='"+alcURL+"'>alc</a>";
				break;		
			case 2:
				var alcURL = 'http://eowpf.alc.co.jp/search?q=' + encodeURIComponent(strLinkContent) + '&ref=sa';
				strLinkString += "<a class='hvAdResLink' target='_alcLink' href='"+alcURL+"'>alc*</a>";
		...
		// Stroke order button
		strLinkString += " | <a class='hvAdResLink' id='strokeOrderToggle' href=''>画</a>";
		
		
		if ($(strLinkElement + " #hvAlcLinkDisplaySection").length) {
			$(strLinkElement + " #hvAlcLinkDisplaySection").html(strLinkContent + " on: " +strLinkString);
			clearStrokeORderDiagram();
		} else {
			$(strLinkElement).prepend( "<p><span id='hvAlcLinkDisplaySection'> "+strLinkContent + " on: " +strLinkString+" </span><div id='strokeOrderDiagram'></div></p>");
		}
...
function bSOVisible()
{
	console.log("bSOVisible: " + $("div#strokeOrderDiagram").html() == "");
