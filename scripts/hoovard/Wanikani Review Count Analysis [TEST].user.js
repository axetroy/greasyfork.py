// ==UserScript==
// @name        Wanikani Review Count Analysis [TEST]
...
function getMainOutputString(iCr, iIncr, iAccCr, iAccIncr)
{
	...
	strReturn += strTodayDate + " " + "TOTAL: "+ iCorrectTotal + " Accuracy: "+ (dPercentage.toFixed( $( "#accuracyDecimals" ).val() )) + "% "+ g_iDateDiff +" Days, Avg. " + iDailyAverage;
...
{
	// Disable button to avoid unwanted multiple data returns
	//$( "button#startAnalysis" ).attr('disabled','disabled');
	//$( "button#startAnalysis" ).hide();
	...
	strBaseTable = "<table border='1' style='margin-left: auto; margin-right: auto; width: 80%; border 1px solid #000000;' id='ReviewAnalysisTable'><tr style='background-color: #660000; color: #ffffff;'><td>Level</td><td>Rad</td><td>Kan</td><td>Voc</td><td>Level Tot.</td></tr></table>";
