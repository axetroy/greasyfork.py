...
function replaceListUrl() {
    ...
    if (container !== null) {
        container.innerHTML = replaceAll(container.innerHTML, "list/list.htm", "list");
    }
}
...
(function(){
    ...
    S.innerHTML = '  <form method="get" action="//s.2.taobao.com/list" name="search" target="_top">' +
    ...
    if (s.length > 0) {
        ...
        s[0].parentNode.action = action.replace("list/list.htm", "list");
    }

    // 替换分类过滤的url
    var category = document.getElementsByClassName('J_HiddenAreaContent sub-category-list clearfix');
    if (category.length > 0) {
        category[0].innerHTML = replaceAll(category[0].innerHTML, "list/list.htm", "list");
    }

    // 替换过滤操作按钮的url
    var filters = document.getElementsByClassName('search-filters-block click-filters');
    if (filters.length > 0) {
        filters[0].innerHTML = replaceAll(filters[0].innerHTML, "list/list.htm", "list");
    }
    var filterpopup = document.getElementsByClassName('search-filters-popup');
    if (filterpopup.length > 0) {
        filterpopup[0].innerHTML = replaceAll(filterpopup[0].innerHTML, "list/list.htm", "list");
    }
    ...
    if (filtertoggle.length > 0) {
        filtertoggle[0].innerHTML = replaceAll(filtertoggle[0].innerHTML, "list/list.htm", "list");
    }
    ...
    if (paginator !== null) {
        paginator.innerHTML = replaceAll(paginator.innerHTML, "list/list.htm", "list");
    }
