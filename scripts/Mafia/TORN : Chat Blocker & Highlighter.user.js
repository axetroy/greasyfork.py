...
GM_addStyle(`.licolor input { width: 15px; height: 18px; margin: 7px; background-color: transparent; cursor: pointer;}`);
...
(function () {
    ...
    var observer = new MutationObserver(function(mutations) {
        ...
        if($("a:contains('Friends')").closest("li").is("[class*='active']") || $("a:contains('Faction')").closest("li").is("[class*='active']") || $("a:contains('Company')").closest("li").is("[class*='active']")) {
        ...
        if($("div[class^='chat-settings-opts_']").length) {
            if(!$("div:contains(Name Color)").length) {
                $("div[class^='chat-opt-label']:contains(Company)").parent().prepend(`
                <div>
                    <div class="${$("div[class^='chat-opt-label']:contains(Company)").attr("class")}">Name Color</div>
                ...
                <div>
                    <div class="${$("div[class^='chat-opt-label']:contains(Company)").attr("class")}">Highlight</div>
                    <div class="${$("div[class^='chat-opt-value_'").attr("class")}">
                        <div class="${$("div[class^='dropdown-root_']").attr("class")}">
        ...
        $.each($("div[class^='message_'] a"), function() {
            if(chatblocklist.indexOf(parseInt($(this).attr("href").match(/\d+/))) != -1 && $(this).parent().is(":visible")) {
                $(this).parent().hide();
            }

            if(chatblocklist.indexOf(parseInt($(this).attr("href").match(/\d+/))) == -1 && $(this).parent().is(":hidden")) {
                $(this).parent().show();
            }
