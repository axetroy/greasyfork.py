...
// ==UserScript==
// @name         VIP视频在线解析破解去广告-爱看影院-看遍全网
// @namespace    http://www.ikanyy.cn
// @version      1.0
...
(function() {
    ...
    var vipBtn = '<a id="sonimeiVipBtn" style="cursor:pointer;text-decoration:none;color:red;padding:0 5px;border:1px solid red;">观看视频</a>';
    var mSearchBtn = '<a id="ikanSearch" target="_blank" style="cursor:pointer;text-decoration:none;color:red;padding:0 5px;border:1px solid red;">搜索电影</a>';
    ...
    if(reAqy.test(videoSite)){
        ...
        $('#ikanSearch').css({'font-size':'17px','display':'inline-block','height':'24px','line-height':'24px','margin':'0 5px'});
        if($('#drama-series-title').length !== 0){
        	curWords = $('#drama-series-title').find('a').text();
        }else{
        	curWords = iqiyiTitle.text();
        }
        $('#ikanSearch').attr('href','http://www.ikanyy.cn/search/' + curWords + '.html');
    }
    // 乐视
    if(reLS.test(videoSite)){
        ...
        $('#ikanSearch').css({'font-size':'16px','display':'inline-block','height':'20px','line-height':'20px','margin':'0 5px'});
       	if($('.Info').find('.title').find('h3').length !== 0){
        	curWords = $('.Info').find('.title').find('h3').text();
        }else{
        	curWords = lsTitle.text();
        }
        $('#ikanSearch').attr('href','http://www.ikanyy.cn/search/' + curWords + '&p=2');
    }
    // 腾讯
    if(reTX.test(videoSite)){
        ...
        $('#ikanSearch').css({'font-size':'24px','display':'inline-block','height':'36px','line-height':'36px','margin':'0 5px'});
    ...
    if(reMG.test(videoSite)){
        ...
        $('#ikanSearch').css({'font-size':'22px','display':'inline-block','height':'40px','line-height':'40px','margin':'0 5px'});
    ...
    if(reSH.test(videoSite)){
        ...
        $('#ikanSearch').css({'font-weight':'bold','font-size':'16px','display':'inline-block','height':'36px','line-height':'36px','margin':'0 5px'});
    ...
    if(rePP.test(videoSite)){
	 ...
	 if(reYk.test(videoSite)){
        ...
        if(youkuTitle.length !== 0){
	        ...
	        $('#ikanSearch').css({'font-size':'17px','display':'inline-block','height':'22px','line-height':'22px','margin':'0 5px','vertical-align':'bottom'});
	        if($('.tvinfo').length !== 0){
    ...
    $('#sonimeiVipBtn').on('click',function(){
        ...
        window.location.href = 'http://api.ta96.cc/?url=' + curPlaySite;
    });
})();
