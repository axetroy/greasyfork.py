// ==UserScript==
// @name        CSDN/iteye自动展开/解决CSDN强制登录问题-CSDN自动评论
// @author      Dragon 原作:AC 修改了 King.Sollyu，不支持bbs.csdn.net和iteye.com
// @namespace   Dragon
// @description CSDN/iteye自动展开/【解决CSDN强制登录问题】-CSDN自动评论 ,返还下载积分
// @version     1.1
...
// @note        2018-12-10 V3.8    支持bbs.csdn.net和iteye.com  ----只有本版开始是本人新增的，其余归原作者所有
...
if(location.host == "download.csdn.net"){
    ...
    var acCSDNT = setInterval(function(){
        if(document.querySelector("div") != null){
            ...
            // 处理bbs.csdn.net
            addStyle(".hide_topic_box{height:auto !important;max-height:unset !important;}");
            addStyle(".hide_topic_box{display:none !important;}");

        }
    }, 100);
}else if(location.host.indexOf("iteye.com") > -1){
    var iteye = setInterval(function(){
        // 处理iteye.com,模拟点击
        document.getElementById('btn-readmore').click()
    }, 100);
 }else if(location.host == "bbs.csdn.net"){
    var bbs = setInterval(function(){
        // 处理bbs.csdn.net,模拟点击
        document.getElementsByClassName('hide_topic_box')[0].children[0].children[0].children[0].click()
    }, 100);
 }