...
// @homepage    https://greasyfork.org/en/scripts/12743-hwm-def-notifier
...
// fix by CheckT
// old homepage       https://greasyfork.org/en/scripts/12743-hwm-def-notifier
...
(function(){
  try {
    ...
    var _notificationSound = 'https://notificationsounds.com/message-tones/and-a-happy-new-year-sms-513/download/mp3';

    var _updateInterval = 1000 * 60 * 1;
    var _defPage = url+'mapwars.php';
    ...
    if (_bkSign == 'null')
      _bkSign = prompt('Вас приветствует мастер настройки скрипта уведомления о защитах!\n\n'
    ...
    if (_bkSign)
      GM_setValue(_gmVars.bkSign, _bkSign);
    else {
  ...
  function findActiveDefs(doc){
    ...
    for(var i = 0; i < activeDefs.length; i++){
      count = 0;
      while(res = regexp2.exec(doc.querySelectorAll('td.wb:nth-child(3)')[i].innerHTML))
        count++;
      ...
      unclosed = 0;
      if ( activeDefs[i].playersEnrolled % 3 > 0 || Math.ceil((activeDefs[i].totalRows * 3 - activeDefs[i].playersEnrolled) / 3) < document.querySelectorAll('td.wb:nth-child(2)')[i].innerHTML.split('[Вступить]').length - 1)
        unclosed = 1;
  ...
  function notifyCase(id, timeUntil, playersEnrolled, totalRows, percLost, unclosed){
    ...
    if (_settings.getSetting('dn_rule_modifier') == 'true')
      playersEnrolledReq = Math.max(0, playersEnrolledReq - 3 * Math.floor(percLost / 15));

    if(timeUntil > parseInt(_settings.getSetting('dn_timeUntilDef' + id)))
      return false;
    if(totalRows * 3 - playersEnrolled <= playersEnrolledReq)
      return false;
    if(parseInt(_settings.getSetting('dn_totalRows' + id)) != 0 && parseInt(_settings.getSetting('dn_totalRows' + id)) != totalRows)
      return false;
    if(parseInt(_settings.getSetting('dn_percLost' + id)) > percLost)
      return false;
    if(parseInt(_settings.getSetting('dn_uncloosed' + id)) == 1 && unclosed == 1)
      return false;
    return true;
  }
  ...
  function notify(message){
    ...
    if (_settings.getSetting('dn_sound_alert'))
      new Audio(_notificationSound).play();
    setTimeout(function(){
      alert(message);
  ...
  function main(){
    if (parseInt(_settings.getSetting('dn_notify_interval')) == 0)
      return;
    if (parseInt(GM_getValue(_gmVars.timeSinceLastNotify, '0')) < new Date().getTime() - parseInt(_settings.getSetting('dn_notify_interval'))
          && parseInt(GM_getValue(_gmVars.timeSinceLastUpdate, '0')) < new Date().getTime() - _updateInterval){
    ...
    var timeout_time = Math.max(parseInt(GM_getValue(_gmVars.timeSinceLastUpdate, '0'))
      - new Date().getTime() + (1 + 0.1 * Math.random()) * _updateInterval, parseInt(GM_getValue(_gmVars.timeSinceLastNotify, '0'))
      - new Date().getTime() + (1 + 0.1 * Math.random()) * parseInt(_settings.getSetting('dn_notify_interval')));
    setTimeout(main, timeout_time);
  ...
  function addInterface(){
    ...
    if (navigator.userAgent.toLowerCase().indexOf('chrome') == -1)
      GM_addStyle('#settingsInner {border-collapse: collapse;}');
    else
      GM_addStyle('#settingsInner td {top: 21px!important; right: 0px!important;}');
  ...
  function drawSettingsBox(drawAgain){
    var table = document.querySelector('td[align="center"][width="50%"]').parentElement.parentElement;
    var t;
    if(t = document.getElementsByClassName('dn_settingBox')[0])
      table.removeChild(t);
    ...
    for (var i = 1; i < parseInt(GM_getValue(_gmVars.settingRows, _defaultSettingRows)); i++)
      inputRows += '<tr align="center"><td colspan="9"><i>или</i></td></tr>' + row;
  ...
  function settingsRow(){
    ...
    for (i = 0; i < settings.length; i++){
      if (settings[i].inputType != 'select')
        output += '<td align="center"><input type="' + settings[i].inputType + '" class="' +  settings[i].inputClass + '"></td>';
      else
        output += '<td align="center"><select class="' +  settings[i].inputClass + '"></td>';
      if (i != settings.length - 1)
        output += '<td style="width: 2%;"> </td>';
    }
  ...
  function setDefaults(e){
    ...
    setTimeout( function (){ GM_setValue('dn_settings', settings) }, 300); // Рианти: Затычка бага, найти баг в будущем.
  }
  ...
  function fetchValuesToSettings(){
    for(var i = 0; i < document.getElementsByClassName('dn_timeUntilDef').length; i++){
      ...
      el = document.getElementsByClassName('dn_playersEnrolled')[i];
      el.id = el.className + i;
      for (j = 0; j <= 20; j++){
      ...
      el = document.getElementsByClassName('dn_totalRows')[i];
      el.id = el.className + i;
      var newEl = document.createElement('option');
      newEl.value = 0;
      ...
      el = document.getElementsByClassName('dn_percLost')[i];
      el.id = el.className + i;
      for (j = 0; j <= 50; j += 15){
          newEl = document.createElement('option');
          newEl.value = j;
          if(j == 0)
            ...
            newEl.innerHTML = 'больше ' + j + '%';
          el.appendChild(newEl);
      }
      el = document.getElementsByClassName('dn_uncloosed')[i];
      el.id = el.className + i;
      newEl = document.createElement('option');
    ...
    el = document.getElementById('dn_notify_interval');
    for (j = 0; j <= 15; j++){
        newEl = document.createElement('option');
        newEl.value = j * 1000 * 60;
        if(j == 0)
          newEl.innerHTML = 'Не уведомлять';
        else
          newEl.innerHTML = j + ' мин';
        el.appendChild(newEl);
    }
  ...
  function ScriptSettings(settingsKey, formId){
    ...
    function load(){
      ...
      if (!stringSettings)
        ...
        _settings = JSON.parse(stringSettings);
    }
      ...
      try{
        ...
        for(var setting in _settings){
          ...
          if(element.type == 'checkbox')
            element.checked = _settings[setting];
          else if (element.type == 'radio')
            element.selected = _settings[setting];
          else
            element.value = _settings[setting];
        }
    ...
    function enableGMvars() {
      if (!this.GM_getValue || typeof this.GM_getValue != 'function') {
        if (typeof(Storage) === "undefined")
          console.log( 'Local storage must be enabled to use scriptSettings.' );
        else {
