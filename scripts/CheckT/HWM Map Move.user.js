...
// @author        xo4yxa
// @homepage      https://greasyfork.org/en/scripts/374614-hwm-map-move
...
// (c) 2008-2013, xo4yxa (https://www.heroeswm.ru/pl_info.php?id=130)     - оригинальный скрипт
// 2014-2015, Mefistophel_Gr (https://www.heroeswm.ru/pl_info.php?id=2287844)   - последняя модификация
// 2018, CheckT - базовая поддержка HTML, HTTPS, исправления
// old homepage       https://greasyfork.org/ru/scripts/9472-hwm-map-move
...
(function() {
    ...
    var coop = '<center style="font-size:10px;">&#169; <a href="https://greasyfork.org/en/scripts/374614-hwm-map-move" target=_blank style="font-size:10px;">HWM Map Move</a> v.' + ver + '   |    <span style="text-decoration:underline;cursor:pointer;font-weight:bold;" id="hwmmm_options">'+ lng_set +'</span> <br><br> </center>';
    ...
    if ( els ) {
      //---------- Flash
      els = els.parentNode.querySelector("param[name='FlashVars']");
      isFlash = true;
      if ( els ) {
        /*
          <param name="FlashVars" value="param=*hour13*cmbut1*ldbut1*3:3:9:6:24:12:0:4:8:1:2::0:3:0:0:0:1:1541165650::1597:0:200~0~0~1209~76|201~...~43|2733~0~0~15~68|">
        */
        pl = els.value.split('=')[1];
      } else {
          return;
      }
    } else {
      //---------- HTML
      /*
          window.onload = function() {init_map("show_map", "https://dcdn.heroeswm.ru/i/", 450, 450, 1, "cmbut1*ldbut1*3:20...:9:6:24:12:0:4:8:1:2::0:3:0:0:0:1:1541165650::1597:0:200~0~0~1209~76|201~...~43|2733~0~0~15~68|", "4.e2becf", 0);}
      */
      isFlash = false;
      var documentInnerHTML = document.documentElement.innerHTML;
      pl = /ldbut[^"]*/.exec(documentInnerHTML); // под страницу карты
      if(pl == null || pl.length==0){
        return;
      }
      pl=pl[0];
      els = document.getElementById("show_map");
    }
    //---------- HTML or Flash end
    pl=pl.split(':');
    if (pl[0].indexOf('*') != -1) {
  ...
  function get_url_dcdn(){
    switch(location.hostname){
      case '178.248.235.15':
        return 'http://hwm.cdnvideo.ru/';
      case 'www.lordswm.com':
        return 'https://dcdn.lordswm.com/';
      case 'www.heroeswm.ru':
      default:
        return 'https://dcdn.heroeswm.ru/';
    }
  }
})();