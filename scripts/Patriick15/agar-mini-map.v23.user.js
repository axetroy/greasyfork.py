// ==UserScript==
// @name         agar-mini-map.v23
// @namespace    https://www.facebook.com/pato.phiolitha
// @version      0.46
// @description  ß&ω Script configurado por Diaz Patrick 15/11/2015 13:56 Mini map V2
// @author       Patrick15d
// @license      MIT
// @match        http://agar.io/*
            ...
            switch(packet.type) {
                ...
                case 130:
                    if (agar_server != packet.data.url) {
                        ...
                        var content = ('You are now at: <ß&ω>' + agar_server
                            + '</ß&ω><ß&ω>Your team members are all at: <ß&ω>' + packet.data.url + ', ' + region_name + ':' + gamemode_name + packet.data.party
                            + '</ß&ω>.<ß&ω>The minimap server has disconnected automatically.');
        ...
        if ($('#mini-map-options').length === 0) {
            ...
            var help_btn = $('<button>')
             ...
             var help_btn = $('<button>')
                ...
                .text('Help')
                .on('click', function(e) {
                    window.open('https://www.youtube.com/channel/UCoyJtA3IN3xUr-HD4Khancw');
                    help_btn.blur();
                    return false;
                })
                .appendTo(form_group);
