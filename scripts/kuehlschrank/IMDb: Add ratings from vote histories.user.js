...
// @grant       GM_setValue
// @grant       GM_getValue
// @connect-src imdb.com
// ==/UserScript==

if(window.self != window.top) return;

var cacheLifetimeHours = 48;
var lists = loadLists();

var titleId;
try {
	titleId = parseInt(window.location.pathname.match(/\/tt0*(\d+)\//)[1], 10);
} catch(ex) { log('Title ID could not be parsed:' + ex); return; }

var container;
try {
	container = insertContainer();
} catch(ex) { log('Container could not be inserted: ' + ex); return; }
...
function updateRatings(list, onFinished)
{
	...
	GM_xmlhttpRequest({
		...
		onload: function(req) {
			...
			for(var i = 1, len = lines.length; i < len; i++) {
				...
				if(titleId && rating) {
					...
					log('Parse error: ' + lines[i]);
				}
			}
			log(list.name + ': ' + ratings.length + ' ratings, loaded in ' + (parseTime - requestTime) +  ' ms, parsed in ' + (Date.now() - parseTime) + ' ms');
			if(ratings.length) {
		...
		onerror: function(req) {
			log(list.name + ': HTTP Error '+ req.status);
			onFinished(list);
		}
	});
}
