...
if (cfg.get('thumbnails') !== 'off') {
    ...
    $('.tdd th a').remove();
    $('.unveil50').remove();
    $('.tdd a[target]').each(function (i, elem) {
            ...
            $(data).find(PR_SELECTOR).each(function (j, previmg) {
                ...
                $.get(onepage, function (data) {
                    ...
                    if (imglinks.map(function (e) {
                        ...
                        dirlink.name = getNameFile(dirlink.name) + j.pad(4) + getExtensionFile(dirlink.name);
...
function getDirectLink(data) {
    var genlink = data.match(/(var e.+?)(?=<\/)/gms)[0];
    genlink = genlink.replace("document.getElementById(e).src", "$(data).find('#'+e).attr('srcx')");
    genlink = genlink.replace("location.protocol", "'http:'");
    genlink = genlink.replace(/.+getElementById\(\'.+=/gm, "var directlink = ");

    eval(genlink);

    var directlink = directlink.replace(/^\/\//, location.protocol + "//");

    var filename = $(data).find('#b' + e).attr('alt');
