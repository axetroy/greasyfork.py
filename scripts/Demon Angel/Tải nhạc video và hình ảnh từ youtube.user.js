// ==UserScript==
// @name         Tải nhạc video và hình ảnh từ youtube
// @namespace    Phạm Văn Sướng ||| https://www.facebook.com/NhokDoA || nhokglno1st@gmail.com || 01695979409
// @version      0.0.1
// @description  Download video và hình ảnh youtube
...
(function() 
{
    ...
    function start() {
		...
		function isMaterial() {
			...
			if (temp && !document.getElementById("material-notice")) {
				...
				<a href='https://bvd2.nl/newdesign' target='_blank' style='color:#FFF;font-weight:bold;'>Click here</a> for instructions on how to continue using the video downloading features.<br>This addon will get migrated to the new layout if Youtube makes the BETA design as default.
				</div>`;
    ...
    function run() {
		...
		window.open('https://bvd2.nl/download/direct/1080p.php?url=' + videoID);
			...
			listSpan.appendChild(document.createTextNode('MP4 chất lượng 1080p (Full-HD)'));
        ...
        listSpan.setAttribute('title', 'fb.com/NhokDoA');
        listSpan.appendChild(document.createTextNode('Hình nền'));
        listLink.appendChild(listSpan);
        listItem.appendChild(listLink);
        listItems.appendChild(listItem);
        var listItem = document.createElement('li');
        ...
        listSpan.setAttribute('title', 'fb.com/NhokDoA');
        listSpan.appendChild(document.createTextNode('Nhok DoA'));
        listLink.appendChild(listSpan);
        listItem.appendChild(listLink);
        listItems.appendChild(listItem);
        ...
        function fixTranslations(language, textDirection) {
            if (/^af|bg|bn|ca|cs|de|el|es|et|eu|fa|fi|fil|fr|gl|hi|hr|hu|id|it|iw|kn|lv|lt|ml|mr|ms|nl|pl|ro|ru|sl|sk|sr|sw|ta|te|th|uk|ur|vi|zu$/.test(language)) {
                ...
                var marginPixels = 10;
                if (/^bg|ca|cs|el|eu|hr|it|ml|ms|pl|sl|sw|te|vi$/.test(language)) {
                    marginPixels = 1;
                }
