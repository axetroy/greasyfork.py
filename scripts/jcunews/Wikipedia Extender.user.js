...
#toc.sticky:hover > ul {display:block; overflow-y:auto}`;
  ...
  function toggleSection() {
    ...
    if ((htyp === "H2") && ele) hstat = ele.style.display ? "" : "none";
    while (ele) {
      if ((ele.tagName === "H2") || ((ele.tagName === "DIV") && (ele.className === "navbox"))) break;
      if (htyp === "H2") {
  ...
  (function addStickyTblHdr() {
    ...
    if (tables.length) {
      if (!(/\.m\.wikipedia\.org/).test(location.hostname) && Array.from(tables).some(t => t.classList.contains("sortable") && !t.rows[0].cells[0].title)) {
        setTimeout(addStickyTblHdr, 100);
        return;
      }
    } else return;
