...
addEventListener("mousedown", function(ev, popup, mwdrProgress, orgOverflow) {
  ...
  if ((ev.ctrlKey !== shortcutCtrlKey) || (ev.shiftKey !== shortcutShiftKey) || (ev.altKey !== shortcutAltKey) || 
      (shortcutButtons.indexOf(ev.button) < 0) || (ev.target.tagName !== "A") || !(/\.docx$/i).test(ev.target.pathname)) return;
    ...
    onload: function(response) {
      if (response.status < 400) {
        mammoth.convertToHtml({arrayBuffer: response.response}).then(function(result, ele, a) {
          ...
          ele.appendChild(a);
          ele.appendChild(document.createTextNode(ev.target.href));
