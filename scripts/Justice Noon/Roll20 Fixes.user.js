...
// @grant			GM_getValue
// ==/UserScript==
// Changelog: Fix multi-line nameplate crashing on newline characters
...
////_.each(TMp.css, (c,i,l) => l[i] = true);
	...
	//Soon to be deprecated now that functionality is official
	if (TMp.zoom2Alpha) {
		window.superAlpha = 0.55;
		document.getElementById('maincanvas').getContext('2d').save2 = document.getElementById('maincanvas').getContext('2d').save;
		document.getElementById('maincanvas').getContext('2d').save = function() {if (this.globalAlpha == .55) this.globalAlpha = window.superAlpha; this.save2();};
		var e = $("#zoomslider");
		e.slider({
			...
			step: 5,
			slide: function(e, t) {
			window.superAlpha = parseInt(t.value, 10) / 100.0;
			Campaign.activePage().reorderByZ();
			}
		}).addTouch(),
		e.on("dblclick", function() {
			window.superAlpha = .55;
			Campaign.activePage().reorderByZ();
		});
	}
    ...
    if (TMp.perfMode) {
		_.each(['maincanvas', 'upperCanvas'], function(c) {
			document.getElementById(c).getContext('2d').imageSmoothingEnabled = false;
		});
    }
}
