 ...
 */
// ==UserScript==
// @name         Pavel Slither.io-bot
// @namespace    http://slither.io/
// @version      1.2.9.1
// @description  Slither.io bot with server list
...
window.sendData = {
    ...
    ping: function(addr, callback){
        ...
        var now_ms = new Date().getMilliseconds();

        this.connect(addr, 'GET', {}, function(status, text){
            var end_ms = new Date().getMilliseconds();
            if(end_ms<now_ms) end_ms += 999;
            var diff = end_ms - now_ms;
            callback(diff, status, text);
        });
...
window.readServers = {
    ...
    readfile: function(){
        ...
        console.log('Read file servers..');
        window.sendData.connect('http://slither.io/i33628.txt', 'GET', false, function(status, text){
            if(status==200){
                window.readServers.stock = text;
                window.readServers.decodestock();
            }
        });
    },
    decodestock: function(stock){
        var ch = 1;
        if(!stock) stock = window.readServers.stock;
        // step 2
        var rch = {a: 0, b: 1, c: 2, d: 3, e: 4, f: 5, g: 6, h: 7, i: 8, j: 9, k: 10, l: 11, m: 12, n: 13, o: 14, p: 15, q: 16, r: 17, s: 18, t: 19, u: 20, v: 21, w: 22, x: 23, y: 24, z: 25};
        var d = [];
        while(stock[ch]){
            d.push(rch[stock[ch]]);
            ch++;
        ...
        while(((a*2 + 1) in d)){
            if(!(p in dv)) dv[p] = [];
            dv[p].push(d[a*2] * 16 + d[a*2 + 1]);
            if((pch % 11)===0){ p++; pch = 1; }else{ pch++; }
            a++;
        }
        // step 6
        var servers = [];
        for(var i=1; i<=dv.length; i++){
            ...
            s.id = i;
            s.ping = -1;
            s.ip = item[0]+'.'+item[1]+'.'+item[2]+'.'+item[3];
            s.po = item[4]*256^2 + item[5]*256 + item[6];
            s.ac = item[7]*256^2 + item[8]*256 + item[9];
            servers.push(s);
        }
        window.readServers.dstock = servers;
    }
};
...
window.pingServers = {
    ...
    reqping: function(){
        ...
        while(this.list[this.countNow] && ni<=this.sizepack){
            window.pingServers.ping(this.list[this.countNow].ip+':'+this.list[this.countNow].po, window.pingServers.countNow);
            ni++;
    ...
    ping: function(ip, clb){
        var ws = new WebSocket('ws://'+ip+'/ptc');
        ws.binaryType = "blob";
        var st = 0;
        ws.onopen = function(){
            st = new Date().getTime();
    ...
    getcountry: function(){
        ...
        if(i < window.pingServers.countAll){ setTimeout('window.pingServers.getcountry()', 5000); }else{ console.log('All ip have country'); }
    }
};
...
var userInterface = window.userInterface = (function() {
    ...
    return {
        ...
        initOverlays: function() {
            ...
            var listServers = document.createElement('div');
            listServers.style.position = 'fixed';
            listServers.style.right = '200px';
            listServers.style.bottom = '5px';
            listServers.style.width = '150px';
            listServers.style.height = '200px';
            listServers.style.overflow = 'auto';
            listServers.style.color = '#C0C0C0';
            listServers.style.fontFamily = 'Consolas, Verdana';
            listServers.style.zIndex = 999;
            listServers.style.fontSize = '14px';
            listServers.className = 'nsi';
            document.body.appendChild(listServers);
            ...
            userInterface.overlays.listServers = listServers;
