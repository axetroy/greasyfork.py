...
function Tweaker(params) {
  ...
  this.mapF = (arr, text) => {
    return arr.map(function (ind, obj) {
      return ($(obj).text().match(text)) ? obj : undefined
    })[0]
  }
  ...
  var comment = function (list, ind) {
    ...
    this.render = () => {
      ...
      let avatar = (this.getAvatar() === "") ? "" : $("<img />").css({
        height: 44,
        width: 44,
        marginRight: "5px",
        verticalAlign: "middle"
      }).attr("src", this.getAvatar().src)
      let commentHeader = $("<div></div>").append($("<h3></h3>").append(avatar)
        .append($("<span></span>").text(this.getPseudo())))
      let commentContent = $("<div></div>").html(this.getComment())
      let commentFooter = $("<div></div>").css({
        display: "flex"
      }).append($("<div></div>").html(this.getLikes())).append($("<div></div>").css({
        textAlign: "right"
      }).text(this.getDate()))
  ...
  this.Popup = (context) => {
    ...
    let contentComments = $("<div></div>").attr("class", "post-comments").css({
      display: "none",
      minWidth: "250px",
      background: "white",
      padding: "1em",
      fontSize: "0.75em"
    }).append($("<div></div>").addClass("list-comments"))
  ...
  this.loadComments = () => {
    ...
    for (let i = comments.length - 1; i >= 0; i--) {
      let c = (params !== undefined && params.comments !== undefined && params.comments.comment !== undefined) ? Object.assign(new comment(comments, i), params.comments.comment) : new comment(comments, i)
      ...
      if (toolbar !== null) {
        ...
        let eventFS = () => {
          if (getFullscreenElement() !== null) {
            ...
            document.querySelector(".g_header").style.display = "block"
            document.querySelector(".g_header").style.opacity = 1
          }
          setTimeout(() => {
        ...
        document.querySelector(".g_header").addEventListener("mouseover", () => {
          clearTimeout(timeoutFade)
          enableFade = false
        })
        document.querySelector(".g_header").addEventListener("mouseout", () => {
          enableFade = true
        })
