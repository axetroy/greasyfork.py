// ==UserScript==
// @name         腾讯乐视爱奇艺优酷vip视频在线解析网站-4月12号更新可用
// @namespace    http://app.sooyyu.com/
// @version      2.7.0// @description  在视频标题旁上显示“vip解析(去广告)”按钮和“搜索电影”按钮，在线播放vip视频；支持优酷vip，腾讯vip，爱奇艺vip，芒果vip，乐视vip等常用视频...
// @author       sonimei
...
(function() {
    ...
    if(rePP.test(videoSite)){
	 ...
	 if(reYk.test(videoSite)){
        ...
        if(youkuTitle.length !== 0){
	        ...
	        $('#sonimeiSearchBtn').attr('href','http://app.sooyyu.com/index.php?m=vod-search-wd-' + curWords + '&p=1');
        }
    }
    // 音悦台
    ...
    $('#sonimeiVipBtn').on('click',function(){
        ...
        window.location.href = 'http://mlxztz.com/vip/index.php?url=' + curPlaySite;
    });
})();