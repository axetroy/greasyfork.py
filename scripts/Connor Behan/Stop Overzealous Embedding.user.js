...
function timer_callback(risky_elements) {
        init(risky_elements);
}

function init_callback(mutations) {
        var i, j;
        ...
        for (i = 0; i < mutations.length; i++) {
                ...
                for (j = 0; j < mutation_nodes.length; j++) {
                        if (mutation_nodes[j].nodeName && riskyTags.indexOf(mutation_nodes[j].nodeName.toLowerCase()) !== -1) {
                                risky_elements.push(mutation_nodes[j]);
                        }
                }
                
                setTimeout(timer_callback, 200, risky_elements);
        }
}
