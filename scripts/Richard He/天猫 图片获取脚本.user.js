...
// @iconURL      http://p8fvixyr8.bkt.clouddn.com/images/icon/tmallImageDown.ico
   ...
   var d = document;
	...
	for(var i=0;i<thumbList.getElementsByTagName('li').length;i++)
	{
		...
		imgAddress[i] = imgSrc.substr(0,imgSrc.length-13);
            //生成按钮
					...
					thumbA = d.createElement('a');
					thumbA.data = imgAddress[i];
					thumbA.href = 'javascript:void(0)';
					thumbA.innerText = i+1;
					thumbA.onmouseover = function()
					{
						thumbList.getElementsByTagName('li')[this.innerText-1].className = 'tb-selected';
					};
					thumbA.onmouseout = function()
					{
						thumbList.getElementsByTagName('li')[this.innerText-1].className = 'tb-thumb tm-clear';
					};
					thumbA.onclick = function()
					{
                GM_download(this.data,paramsObj.id+'Thumb'+this.innerText+this.data.substr(-4,4));
					};
	...
	if(jAmount.nodeName == 'DD'&&typeof(catImage)!='undefined'&&document.getElementsByClassName('tb-img')[0].childNodes[1].childNodes[1].style.backgroundImage.indexOf('url')>=0)
	{
		...
		for(var i=0;i<catImage.getElementsByTagName('li').length;i++)
		{
			...
			{
				GM_download(this.data,paramsObj.id+'分类图'+this.innerText);
			};
	...
	detailA.onclick = function()
	{
		...
		if(typeof(desLis[0])!='undefined')
		{
			...
			{
				if((desLis[i].align == "absmiddle"|desLis[i].alt!='')&&desLis[i].nodeName == 'IMG')
				{
					imgAdrs.push(desLis[i].src);
				}
				else if(desLis[i].childNodes.length>0)
				{
					...
					{  
						if(desLis[i].childNodes[j].nodeName == 'IMG')
							imgAdrs.push(desLis[i].childNodes[j].src);
					}
				}
				else
				{
			...
			GM_notification({
				...
				title:'友情提示',timeout:0},function()
				{
					for(var k=0;k<imgAdrs.length;k++)
					{
						GM_download(imgAdrs[k],paramsObj.id+'详情图'+(k+1)+imgAdrs[k].substr(-4,4));
						//console.log(imgAdrs[k]);
					}
				});
		}
	};
