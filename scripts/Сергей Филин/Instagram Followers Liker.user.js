...
// @description:ru  Скрипт пройдётся по подписчикам и по подписчикам подписчиков до бесконечности.
// @author       Philins
// @match        https://www.instagram.com/*
// @grant        none
// @contributionURL    https://www.paypal.me/philins/5
// ==/UserScript==
...
(function() {
    ...
    var trans = {"interval": {
        ...
        "ru": "Увеличьте если страница не успевает загрузиться."},
                 ...
                 "listtip": {
                     ...
                     "ru": "При обнаружении хештега из этого списка, лайк не поставится."},
                 "manytag": {
    ...
    if(!inIframe()){
        ...
        settings = (settings!==null ? JSON.parse(settings) : {'daylimit': 500, 'likelimit': 100, 'taglimit': 20, 'fotolimit': 10, 'setlike': 8, 'fol': 1, 'sb': false, 'interval': 5});
...
info.appendChild(dt);
        ...
        if(settings.sb){
            ...
            input8.addEventListener('click', function() {
                Save(document.getElementById("input1").value, document.getElementById("input2").value, document.getElementById("input3").value, document.getElementById("input4").value, document.getElementById("input5").value, area.value, input61.checked, settings.sb, document.getElementById("input8").value);
            }, false);
    ...
    function Save(dl, ll, tl, fl, sl, skiplist, radio, sb, interval){
        var rb = (radio) ? 1 : 2;
        settings = {'daylimit' : dl, 'likelimit' : ll, 'taglimit' : tl, 'fotolimit' : fl, 'setlike': sl, 'fol': rb, 'sb': sb, 'interval': interval};
        localStorage.setItem('settings', JSON.stringify(settings));
