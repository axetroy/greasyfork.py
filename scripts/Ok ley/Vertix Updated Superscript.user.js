// ==UserScript==
// @name         Vertix Updated Superscript
// @namespace    http://vertix.io/
// @version      1.63
// @description  Zoom ('tab' = on ') Enemy Radar Auto Jump ('Z' =on ') Aimbot ('shift' =on , 'CAPS LOCK =off') Show's Camos Right click Secondary (buffed)
// @author       xOKLEYx
...
function wepLength() {
    ...
    if (player.onScreen) {
         setTimeout(player.weapons["0"].length = 100,100);
    player.weapons["0"].width = 33;
...
function deactivate(event) { //'CAPS LOCK' to deactivate
    event.preventDefault();
    if (event.keyCode === 20) {
        active = false;
