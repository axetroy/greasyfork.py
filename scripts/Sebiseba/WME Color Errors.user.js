...
var CErrWaze, CErrorsMap, CErrorsModel, CErrorsI18n, CErrorsOpenLayers, CErrorshandle, CErrorshandleClass, CErrorshandleClass2,
    ...
    parkNamePoi="(Parking[s]?|Parc-Relais|Placette|Aire|Arrêt)",
    religiousPoi="(Abbatiale|Abbaye|Basilique|Calvaire|Carmel|Cathédrale|Chapelle|Cloître|Collégiale|Conjuratoire|Couvent|Crypte|Dôme|Église|Grande Mosquée|Grotte|Mandir|Maison Diocésaine|Monastère|Mosquée|Notre-Dame|Oratoire|Ordre|Pagode|Presbytère|Prieuré|Sanctuaire|Stupa|Synagogue|Temple)",
...
function CErrors_init(){
    ...
    if(typeof(CErrorsModel.countries.top) === 'undefined' || CErrorsModel.countries.top === null){ if (debug) { console.error("WME ColorErrors - CErrorsmodel Countries top : NOK"); } window.setTimeout(CErrors_init, 500); return; }
    ...
    // Phone prefix
    switch(CErrorsModel.countries.top.id) {
            ...
            if ((ls.myLvl && attributes.rank <= myLevel) || ls.myLvl===false) {
                ...
                if (street.name || altname) {
                    ...
                    if (ls.seg_BadAltState) {
                        if (typeof (streetalt)==='undefined') continue;
                        if (CErrorsModel.cities.getObjectById(street.cityID).attributes.stateID != CErrorsModel.cities.getObjectById(streetalt.cityID).attributes.stateID) { // Alt State != Main State
                            newColor="#ff7700"; newWidth=3; newOpacity=0.95; gline=segment.attributes.geometry.getVertices();
                            j++;
                            addIcon("\uf037",newColor,geometry,"segment",j);
            ...
            if ((ls.myLvl && pattributes.rank <= myLevel && pattributes.adLocked===false) || ls.myLvl===false) {
                ...
                if (ls.poi_Transp && new RegExp(stationsPoi).test(categories)===false) { //Bus Subway and Tram Station wihtout others stations
                    if ((
                        ...
                        categories.indexOf("BUS_STATION") == -1 && categories.indexOf("SUBWAY_STATION") == -1 && categories.indexOf("TRAIN_STATION") == -1 &&
