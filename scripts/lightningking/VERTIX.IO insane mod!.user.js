// ==UserScript==
// @name         VERTIX.IO insane mod!
// @namespace    http://vertix.io/
// @version      0.2.3
// @description  Want to win every single vertix.io match and absolutely destroy every pro you come across? Well now you can with this awesome mod!
// @author       lightningking
...
c.addEventListener('keydown', function (event) {
    if (!document.activeElement || document.activeElement.tagName != 'INPUT') {
        ...
        } else if (keyCode == 18 && !aimbot_active) { // Hold `Alt` to activate aimbot.
            aimbot_activate();
        }
...
c.addEventListener('keyup', function (event) {
    if (!document.activeElement || document.activeElement.tagName != 'INPUT') {
        event.preventDefault();
        var keyCode = event.keyCode ? event.keyCode : event.which;
        if (keyCode == 18) { // Release `Alt` to deactivate aimbot.
            aimbot_deactivate();
        }
    }
});
...
var adWrapperHtml = '';
adWrapperHtml += "<h3 class=\"menuHeader\">MOD SETTINGS</h3>";
...
adWrapperHtml += "<br>";
adWrapperHtml += "<h3 class=\"menuHeader\">VERTIX.IO INSANE MOD INFO</h3>";
adWrapperHtml += "  <p style=\"color: rgba(0, 0, 0, 0.4);\"><b>Version:</b> 0.2.3</p>";
adWrapperHtml += "  <p style=\"color: rgba(0, 0, 0, 0.4);\"><b>Greasy Fork update link:</b> <a target=\"_blank\" href=\"https://greasyfork.org/en/scripts/35854-vertix-io-insane-mod\">greasyfork.org/en/scripts/35854</a></p>";
...
// ======================================================================  ENEMY MINIMAP  ======================================================================                                                                                                                                                                                    6Zeq55S15LmL546L
// This isn't my code, credit to the original creator (whoever he / she is, it seems that every mod has this code).
// I allow you to copy the code below (because I didn't write it), but not the code above. But just because _I_ allow you, doesn't mean that whoever wrote it does.
// If you do find the original creator, please let me know so I can ask them if I can use their code.
