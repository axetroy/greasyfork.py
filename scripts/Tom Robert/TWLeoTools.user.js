          ...
          if (!LT.list)
            $.getScript('https://tomrobert.safe-ws.de/forbidN.js').done(function () {
              ...
              while (i--) {
                var si = LT.list[i],
                sb = si.bonus[10];
                if (!forbid.sets.includes(si.key) && !(sb && sb[5] && (sb[5].type == 'speed' || sb[4].job == 'all') && sb[0].value == 20))
              ...
              for (var k in allItems)
                if (k > 9 && k < forbid.maxID && !forbid.IDs.includes(k) && !allItems[k].short.includes('friendset_'))
