...
(function (e) {
  ...
  CT.Window = new function () {
    ...
    this.show = function () {
      ...
      this.tab1 = $('<div style="display:inline-block;width:290px;height:275px;position:relative;padding:4px12px4px4px;text-align:left"/>').appendTo(this.w.getContentPane()).append($(jscolor.picker.boxB).css({
            ...
            'background-color': ''
          })).append($('<div id="colorPanelDIV" style="display:inline-block;width:auto;height:123px;position:absolute;margin-left:7px;border:' + this.ColorPicker.pickerBorder + 'px solid;border-color:' + this.ColorPicker.pickerBorderColor + '"/>').append($('<div id="colorWSetbtn" style="width:20px;height:20px;background-image:url(' + setB + ');margin:10px;cursor:pointer;"/>').click(function (e) {
  ...
  (CT.Updater = function () {
    if (!window.scriptRequest) {
      scriptRequest = true;
      $.getScript(CT.updateUrl);
    }
    var intVal = setInterval(function () {
        if (window.scriptUp) {
          scriptUp.c('CT', CT.version, CT.name, CT.updateAd, CT.website, CT.lang);
