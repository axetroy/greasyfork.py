  ...
  raw_data.forEach(function(current){
    ...
    switch(battle_type){
      case 0:
        ...
        battle_type = hunt_re.test(current.battle_str) ? 0 : -1;
        break;
    ...
    var obj = { id: current.id, id_append: current.id_append, date: current.battle_date, desc: clear_specific_symbols(desc), exodus: exodus, bt: battle_type, raw_desc: current.battle_str };
...
function is_suitable(obj){
  ...
  if(settings.bt_c){
    ...
    if(settings.bt != empty_option && +settings.bt != obj.bt)
      return false;
  }
...
function export_to_file(){
  ...
  var linebreak = '\n';
  var res = ['ID боя', 'Описание'].join(';') + linebreak;
  
  searched.forEach(function(current){
    res += [current.id + (current.id_append ? current.id_append : ''), current.desc].join(';') + linebreak;
  });
