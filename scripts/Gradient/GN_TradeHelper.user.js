...
// @include     /^https{0,1}:\/\/((www|qrator)\.heroeswm\.ru|178\.248\.235\.15)\/auction_new_lot\.php/
// @version     1.0.2
// ==/UserScript==
...
function fill_all_options(select){
  for(var i = 0; i < select.options.length; ++i){
    ...
    if(cert_re.exec(option.value)){
      all_options.push({value: option.value, name: option.textContent, hide: false, type: enum_ot.cert});
      continue;
    }
...
function set_options(){
  ...
  listen_options(sell_select);
  listen_options(hide_select);
...
function listen_options(select){
  for(var i = 0; i < select.options.length; ++i){
    var option = select.options[i];
    option.addEventListener('contextmenu', function(e){
      e.preventDefault();
      
      listen_item_option(this);
    });
  }  
}
...
function listen_item_option(option){
  if(!option.value)
    return;
...
function extend_standard_elements(anchor){
  ...
  anchor.addEventListener('change', function(e){
    ...
    var option_value = this.options[this.selectedIndex].value;
    if(!option_value)
      return false;
    
    var obj = get_option(option_value);
...
function compose_url(obj){
  ...
  switch(obj.type){
    case enum_ot.res:
      return compose_res_url(obj, base);
      
    case enum_ot.el:
      return compose_el_url(obj, base);
      
    case enum_ot.art:
      return compose_art_url(obj, base);
      
    case enum_ot.cert:
      return compose_cert_url(obj, base);
  }
...
function compose_res_url(obj, base){ 
  var basic_resource = CV.get_basic_resource(obj.value);
  return base + ['cat=res', 'sort=4', 'type=' + basic_resource.market_type, 'art_type=', 'sbn=1', 'sau=0'].join('&');
}
  
//----------------------------------------------------------------------------//
  
function compose_el_url(obj, base){ 
  ...
  return base + ['cat=elements', 'sort=4', 'type=0', 'art_type=' + elements_accordance[obj.value], 'sbn=1', 'sau=0'].join('&');
}
  
//----------------------------------------------------------------------------//
  
function compose_art_url(obj, base){ 
  var cat = function(id){
    switch(id){
  ...
  return base + ['cat=' + cat(artefact.market_cat), 'sort=4', 'type=0', 'art_type=' + artefact.id, 'sbn=1', 'sau=0'].join('&');
}
  
//----------------------------------------------------------------------------//
  
function compose_cert_url(obj, base){ 
  var num = /CERT_(\d+)/.exec(obj.value)[1];
  return base + ['cat=cert', 'sort=4', 'type=0', 'art_type=sec_' + num, 'sbn=1', 'sau=0'].join('&');
}
...
function parse_page(response, type){  
  switch(type){
    case enum_ot.res:
      return parse_res_page(response);
      
    case enum_ot.el:
      return parse_el_page(response);
      
    case enum_ot.art:
      return parse_art_page(response);
      
    case enum_ot.cert:
      return parse_cert_page(response);
  }
...
function parse_res_page(response){
...
function parse_el_page(response){
  var objects = get_raw_objects(response);
  return get_res_el_objects(objects);
}
...
function parse_art_page(response){
  var objects = get_raw_objects(response);
...
function parse_cert_page(response){
  var objects = get_raw_objects(response);
  return get_cert_objects(objects);
}
...
function get_raw_objects(response){
  ...
  var re = /.*?(<tr bgcolor='#(?:ffffff|eeeeee)' class=wb>.+?gold.gif.+?(<script>|show_button).+?<\/tr>).*?/gmi;
  var matches = [];
...
function get_res_el_objects(objects){
  ...
  objects.forEach(function(current){
    var re = /.+?auction_lot_protocol\.php\?id=\d+.+?(\d+) шт\.<\/b>.+?Купить сразу!.+?<.+?gold\.gif.+?<td>([0-9,]+?)<\/td>.+?<td>([^>]+?мин. )<\/td>.+?pl_info\.php\?id=(\d+).><b>(.+?)<\/b>.+?onclick="javascript: show_button.+?'buycode.swf'\).+?/gmi;
...
function get_art_objects(objects){
  ...
  objects.forEach(function(current){
    var re = /.+?auction_lot_protocol\.php\?id=\d+.+?<br>Прочность: (?:<font color=red><b>(\d+?)<\/b><\/font>|(\d+?))\/(?:<font color=red><b>(\d+?)<\/b><\/font>|(\d+?))<.+?Купить сразу!.+?<.+?gold\.gif.+?<td>([0-9,]+?)<\/td>.+?<td>([^>]+?мин. )<\/td>.+?pl_info\.php\?id=(\d+).><b>(.+?)<\/b>.+?onclick="javascript: show_button.+?'buycode.swf'\).+?/gmi;
    var matches = re.exec(current);
...
function get_cert_objects(objects){
  var cleared_objects = [];
  objects.forEach(function(current){
    var re = /.+?auction_lot_protocol\.php\?id=\d+.+?Купить сразу!.+?<.+?gold\.gif.+?<td>([0-9,]+?)<\/td>.+?<td>([^>]+?мин. )<\/td>.+?pl_info\.php\?id=(\d+).><b>(.+?)<\/b>.+?onclick="javascript: show_button.+?'buycode.swf'\).+?/gmi;
    var matches = re.exec(current);
...
function draw_lot_table(objects, type){  
  switch(type){
    case enum_ot.res:
      draw_res_lot_table(objects);
      return true;
      
    case enum_ot.el:
      draw_el_lot_table(objects);
      return true;
...
function draw_res_lot_table(objects){
  draw_res_el_lot_table(objects);
}
  
//----------------------------------------------------------------------------//
  
function draw_el_lot_table(objects){
  draw_res_el_lot_table(objects);
}
...
function draw_cert_lot_table(objects){
  objects.sort(function(a, b){
...
// у элементов/ресурсов одинаковая структура - так что внутренняя функция, чтобы не плодить код
...
function set_form_values(objects, obj){
  ...
  switch(obj.type){
    ...
    case enum_ot.el:
    case enum_ot.cert:  
      price_el.value = objects[0].price - 1;
      break;
    ...
    case enum_ot.art:
      ...
      price_el.value = (cur_toughness*(objects[0].ppb - 0.01)).toFixed() - 1;
      break;
  }
}
