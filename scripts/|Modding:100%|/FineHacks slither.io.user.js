...
function zoom(e) {
    ...
    w.onkeyup = function(e) {
        if (w.playing) {
            ...
            if (e.keyCode == 86) w.open("https://greasyfork.org/en/scripts/18794-finehacks-slither-io", "_blank");
            if (e.keyCode == 66) w.open("http://iKnew.xyz/", "_blank");
            if (e.keyCode == 78) w.open("http://www.youtube.com/channel/UCey0eggB1VRtwtLrOLROKag", "_blank");
        }
    };
    ...
    function dt() {
             if (w.playing) {
                 ...
                 if (data) data.innerHTML = '<div id="FineHacks" className="FineHacks" style="font-family: Papyrus; font-size: 20px; color: red;"><b>FineHacks ' + v + '</b></div>' + "<br>" +
                    '<div id="FuckYouStealers" className="MessageToStealers" style="color: yellow;"><b>Steal code = GET DDOSED!</b></div>' + "<br>" +
                    '<div id="slitherio" className="FineHacks" style="color: bisque; font-size: 17px"><sup><b>[Special for slitherio.org]</b></sup></div>' + "<br>" +
                    '<div id="Hotkeys" className="FineHacks" style="color: green;">[SHIFT] - Accelerate<br>[V] - Check official script<br>[B] - Check my website<br>[N] - Check my YouTube channel<br>[Mouse wheel] - Zoom</div>' + "<br>" +
                     "X: " + ~~w.snake.xx + "<br>" +
                     "Y: " + ~~w.snake.yy + "<br>" +
