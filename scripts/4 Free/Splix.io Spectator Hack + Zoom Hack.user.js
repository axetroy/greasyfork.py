// ==UserScript==
// @name         Splix.io Spectator Hack + Zoom Hack 
// @description  Use Y to use the "spectrator" option (Warning you'll be "disconected"!)
// @version      3.6
// @author       TheMnich
// @match        http://splix.io
...
(function() {
      ...
      window.addEventListener("load", function(){
       ...
       function WheelHandler(event) {
        ...
        var myCssText = "color:#DAA520; font-weight: 700; position:fixed; right:  13px; top: 8px; z-index:100; padding: 5px 10px; opacity: .7; transform-orign: right top;";

        var myDiv = document.createElement("div");
        myDiv.className  = "greenBox";
        myDiv.style.cssText = myCssText;
        myDiv.innerHTML = "Spectator Hack by:TheMnich";
        ...
        var loop = setInterval(function(){
                if(paused) {
                    sendDir((myPlayer.dir + 1)%6);
                }
            }, 100);

        window.addEventListener("keyup", function(e){
            if(e.keyCode == 89) paused ^= true;
        });
