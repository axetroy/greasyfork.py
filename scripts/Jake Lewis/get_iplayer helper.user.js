...
//    'use strict';
    ...
    $(document).ready(function() {
        ...
        (async () => {
            ...
            // save the sortType change it using the 'Change sort:" gui on bbc page https://www.bbc.co.uk/iplayer/categories/
...
function parsePage(){
    ...
    if(orbModules != null){
        ...
        var tbdy = document.createElement('tbody');
            var row1 = document.createElement('tr');
                ...
                var dir = document.createElement('td'); row1.appendChild(dir)
                    dir.appendChild( createCheckControl(" append Category Dir ", appendCatLabel, "0"));
                    dir.appendChild( createCheckControl(" append Name Dir ",appendNameLabel, "1"));
     ...
     //on all pages change the sort type
        var catNav = document.querySelector('div.column-section');
        if(catNav!=null){
            var cats = catNav.querySelectorAll('li');
            if(cats.length<20){
                console.error("Only "+cats.length+" found");}
            for( i = 0; i < cats.length; i++){
                var cat = cats[i].firstChild;
                cat.href = cat.href.replace("featured", sortType);
               // console.log("get_ihelper: "+i+" "+cat.innerText+" "+cat.href);
            }
        }else{
            console.error("Cannot find div.column-section__"); }
        ...
        if(window.location.href.includes("www.bbc.co.uk/iplayer/episode/")){
            ...
            if(singleElement!=null ){
                singleElement.insertBefore(createButtons(myLocation, categoryString, nameString, nameString), singleElement.children[0]);
            }
       ...
       // console.log("get_ihelper"+"targetElements.length:"+targetElements.length);
        ...
        for (var i = 0; i < targetElements.length; i++) {
            ...
            if(content_item__link != null){
                ...
                targetElement.insertBefore(createButtons(content_item__link, categoryString, nameString, nameString), content_item__link);
            }else{
                ...
                if(content_item__link!=null){
                    if(content_item__link.innerText.includes('This Episode')){
                       targetElement.insertBefore(createButtons(location,categoryString, nameString, nameString), content_item__link);
                    }
                }
            }
...
function createButtons(videoElement, catDirectory, nameDirectory, searchTerm){
    ...
    buttonSpan.appendChild( createButton(videoURL, safeCatDir, safeNameDir, "", "Add QuickURL to Queue", 'pvr_queue') );
    buttonSpan.appendChild( createButton('', safeCatDir, safeNameDir, searchTerm, "Search Get_iPlayer ", 'search_progs'));
    //console.log("get_ihelper: "+safeCatDir+"/"+safeNameDir+" search:"+searchTerm+" "+videoURL);
    return buttonSpan;
}


function createButton(videoURL, catDirectory, nameDirectory, searchTerm, label, nextPage){
            ...
            addButton.onclick = function buttonClick(evt){
                ...
                (async () => {
                    ...
                    var directoryName = "";
                    if( await GM.getValue(appendCatLabel, "1")=="1" && evt.target.catDirectory.length > 0) directoryName += "/"+evt.target.catDirectory;
                    if( await GM.getValue(appendNameLabel, "1")=="1"&& evt.target.nameDirectory.length >0) directoryName += "/"+evt.target.nameDirectory;
