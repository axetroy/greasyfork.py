// ==UserScript==
// @version         1.9.8 (LICENSED AND CHROME COMPATIBLE)
// @name            YouTube + +
// @namespace       https://github.com/ParticleCore
// @description     YouTube with more freedom [CREDIT TO PARTICLECORE FOR THE SCRIPT. THIS VERSION IS CHROME COMPATIBLE AND HAS FEWER SYNTAX ERRORS]
// @compatible      firefox
// @compatible      opera
// @compatible      chrome
                ...
                while (i--) {
                    ...
                    while (j--) {
                        ...
                        if (thumb) {
                            ...
                            if (user_settings.BLK_ON && list[i] !== "no.channel.id" && !thumb.querySelector(".blacklist") && window.yt.config_.PAGE_NAME !== "channel" && window.location.pathname !== "/feed/subscriptions") {
                                button = document.createElement("template");
                                button.innerHTML = //
                    ...
                    if (temp.href) {
                        ...
                        if (channel_id) {
                            ...
                            if (watch_id) {
                                ...
                                while (temp) {
                                    if (temp.tagName && temp.tagName === "LI" && video_list.indexOf(temp) > -1) {
                                        if (!modThumbs.thumbs["no.channel.id"]) {
                                            modThumbs.thumbs["no.channel.id"] = [temp];
                                        } else if (modThumbs.thumbs["no.channel.id"].indexOf(temp) < 0) {
                                            modThumbs.thumbs["no.channel.id"].push(temp);
                                        }
                                        break;
                                    }
                                    temp = temp.parentNode;
