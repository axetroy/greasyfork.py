...
function WMEURMPT_Injected() {
  ...
  WMEURMPT.PURFilterList = {hideOutOfMyManagedArea:1, hideVisited:2, hideBlacklisted:4, hideWhitelisted:8, hideLimitTo:16, hideArea:32, hideNotKW:64, hideCategorie:128};
  ...
  WMEURMPT.initializeWazeObjects = function() {
    var objectToCheck = [{o:"W.map", s:"wazeMap"}, {o:"W.model", s:"wazeModel"}, {o:"W.loginManager", s:"loginManager"}, {o:"W.controller", s:"wazeController"}, {o:"W.Config.api_base", s:"wazeConfigApiBase"}, {o:"OL", s:"OpenLayers"}, {o:"W.loginManager.user", s:"me"}, {o:"W.loginManager.user.rank", s:"ul"}, {o:"W.loginManager.user.isAreaManager", s:"uam"}];
  ...
  WMEURMPT.registerEvents = function() {
    ...
    WMEURMPT.wazeModel.repos.venues.on("objectschanged", WMEURMPT.newDataAvailableStarts);
    WMEURMPT.wazeModel.repos.venues.on("objectsadded", WMEURMPT.newDataAvailableStarts);
    WMEURMPT.wazeModel.repos.venues.on("objectsremoved", WMEURMPT.newDataAvailableStarts);
  ...
  WMEURMPT.unregisterEvents = function() {
    ...
    WMEURMPT.wazeModel.repos.venues.off("objectschanged", WMEURMPT.newDataAvailableStarts);
    WMEURMPT.wazeModel.repos.venues.off("objectsadded", WMEURMPT.newDataAvailableStarts);
    WMEURMPT.wazeModel.repos.venues.off("objectsremoved", WMEURMPT.newDataAvailableStarts);
  ...
  WMEURMPT.selectPURById = function(PURId) {
    ...
    PURId.attempts++;
    WMEURMPT.log("Cannot select PUR " + PURId.PURId + ". Trying again " + PURId.attempts + "/10...");
  ...
  WMEURMPT.getMPs = function(bounds, filter) {
    var url = "https://" + document.location.host + WMEURMPT.wazeConfigApiBase + "/Features?language=en" + (WMEURMPT.scanUR ? "&mapUpdateRequestFilter=1" : "") + (WMEURMPT.scanMP ? "&problemFilter=1&turnProblemFilter=1" : "") + "&mapComments=" + (WMEURMPT.scanMC ? "true" : "false") + "&venueLevel=3&venueFilter=" + (WMEURMPT.scanPUR ? "2%2C2%2C2" : "0%2C0%2C0") + "&editableAreas=true&bbox=" + bounds.left + "," + bounds.bottom + "," + bounds.right + "," + bounds.top;
  ...
  WMEURMPT.updateListsFromManualScan = function(areaFilter) {
    ...
    if (areaFilter.type == "custom") {
      ...
      for (var i = 0; i < geometry.length; i++) {
        ...
        for (var lat = lonlatFrom.lat; lat < lonlatTo.lat; lat += 0.5) {
          for (var lon = lonlatFrom.lon; lon < lonlatTo.lon; lon += 0.5) {
            WMEURMPT.logBeta("Scan custom from: " + lon + " " + lat + " to: " + (lon + 1) + " " + (lat + 1));
            var tileBounds = new OpenLayers.Bounds;
            tileBounds.extend(new OpenLayers.LonLat(lon.toFixed(6), lat.toFixed(6)));
            tileBounds.extend(new OpenLayers.LonLat((lon + 0.5).toFixed(6), (lat + 0.5).toFixed(6)));
            WMEURMPT.scanAreaBoundsList.push(tileBounds);
