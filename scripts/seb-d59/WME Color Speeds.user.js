...
function CSTestVersion() {
  if (CSpeeds_OldVersion != CSpeeds_Version) {
    ...
    if (CSpeedI18n  == "fr"){
        CSpeeds_Maj +="\nQuoi de neuf:\n";
        CSpeeds_Maj +="petite correction\n";
    }else {
        CSpeeds_Maj +="\nWhat's new:\n";
        CSpeeds_Maj +="Small bug fix\n";
    }
    alert(CSpeeds_Maj);
...
function CSpeeds_init(){
    ...
    //    OpenLayers
    CSpeedOpenLayers = unsafeWindow.OL; 
...
function CSpeeds_Mainhtml() {
    ...
    */
    ...
    if (WMECSpeeds.MultiplePalette==true){
      var stateToSelect = CSpeedsModel.states.top.name.replace(/ /g, '_');
...
function eventUnRegister(){
    ...
    CSpeedsMap.events.unregister("zoomend", null, SCColor);
    CSpeedsMap.events.unregister("moveend", null, SCColor);
...
function LoadSettings(){
  ...
  if (WMECSpeeds.PaletteByCountrie == true && WMECSpeeds.MultiplePalette == false){
    ...
    else selectedCountrie = CSpeedsModel.countries.top.name.replace(/ /g, '_');
...
function updateCountrieList(){
  ...
  if (current === null) current = CSpeedsModel.countries.top.name.replace(/ /g, '_');

  selectCountrie.options.length = 0;
  
  for (var countrie in WMECSpeeds.speedColors.Countries) {
   // create option in select menu
    if (countrie === undefined) countrie=CSpeedsModel.countries.top.name.replace(/ /g, '_');
...
function SCColor(){
    ...
    if (CSpeedsCountries.top === null) return;
