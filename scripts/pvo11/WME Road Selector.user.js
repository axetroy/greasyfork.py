...
// @version 			1.45.2
// @copyright			2014-2019, pvo11 (HTML design by bures)
...
function populateCountries(sel)
{
	...
	for (var countryID in uWaze.model.countries.objects) {
		Countries[countryID] = uWaze.model.countries.getObjectById(countryID).name;
	}
		...
		case "Country":
			...
			if (typeof(sid) === 'undefined' || sid === null) {
				...
				if (typeof(street) === 'undefined') {
					...
					var countryID = uWaze.model.cities.objects[street.cityID].attributes.countryID;
		...
		case "State":
			...
			if (typeof(sid) === 'undefined' || sid === null) {
				...
				if (typeof(street) === 'undefined') {
					...
					var stateID = uWaze.model.cities.objects[street.cityID].attributes.stateID;
					if (typeof(street) === 'undefined') {
						result = false;
					} else {
						var stateName = uWaze.model.states.getObjectById(stateID).name;
						if (stateName === null) {
							stateName = "";
						}
...
//					var cityName = uWaze.model.cities.getObjectById(street.cityID).name;
					var cityName = uWaze.model.cities.objects[street.cityID].attributes.name;
					if (cityName === null) {
						cityName = "";
					}
					var condTxt = tree.txt;
		...
		case "ClReas":
			if (segment.hasClosures()) {
				for (var closure in uWaze.model.roadClosures.objects) {
					if (uWaze.model.roadClosures.objects[closure].segID === segment.attributes.id) {
						var condTxt = tree.txt;
						var reason = uWaze.model.roadClosures.objects[closure].reason;
						if ((!getId("cbRSCaseSens").checked) && (tree.op != "4")) {
							condTxt = condTxt.toLowerCase();
			...
			if (segment.hasClosures()) {
				for (var closure in uWaze.model.roadClosures.objects) {
					if (uWaze.model.roadClosures.objects[closure].segID === segment.attributes.id) {
						...
						if (opType === 0) {
							userId = uWaze.model.roadClosures.objects[closure].createdBy;
						} else {
							userId = uWaze.model.roadClosures.objects[closure].updatedBy;
						}
						if (typeof(userId) === 'undefined' || userId === null) {
							name = '';
						} else {
							name = uWaze.model.users.getObjectById(userId).userName;
		...
		case "Updtd":
			...
			if (typeof(segment.attributes.updatedBy) === 'undefined' || segment.attributes.updatedBy === null) {
				...
				name = uWaze.model.users.getObjectById(segment.attributes.updatedBy).userName;
				if ((typeof(name) === "undefined") || (name === null)) {
					return false;
				}
			}
		...
		case "UpLev":
			...
			lev = uWaze.model.users.getObjectById(segment.attributes.updatedBy).rank;
		...
		case "Crtd":
			...
			if (typeof(segment.attributes.createdBy) === 'undefined' || segment.attributes.createdBy === null) {
				...
				name = uWaze.model.users.getObjectById(segment.attributes.createdBy).userName;
				if ((typeof(name) === "undefined") || (name === null)) {
					return false;
				}
			}
		...
		case "CrLev":
			...
			lev = uWaze.model.users.getObjectById(segment.attributes.createdBy).rank;
			if (typeof(lev) !== "number") {
				return false;
			}
			lev += 1;
		...
		case "Connect":
			...
			switch (tree.op) {
				case "0":
					...
					break;
				default:
					for(var seg2 in uWaze.model._turnGraph._adjacencyList[segment.attributes.id + 'f']) {
					...
					if (!result) {
					for(var seg2 in uWaze.model._turnGraph._adjacencyList[segment.attributes.id + 'r']) {
							seg2 = seg2.substring(0, seg2.length-1);
							var segment2 = uWaze.model.segments.getObjectById(seg2);
							if (typeof(segment2) !== 'undefined') {
								result = checkExpr (tree.L, segment2);
...
function roadSelector_init()
{
	...
	str += '<tr><td><label for="inRSCity">City</label><select id="opRSCity"></select>'
					...
					+ '<tr><td><label><input type="checkbox" id="cbRSInbj">In Junction Box</label></td>'
