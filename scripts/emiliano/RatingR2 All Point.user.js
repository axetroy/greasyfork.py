...
// @version      1.0.9 + EMO'Modify
// @description  REREC, TrExMa, RatingR2  EN + Season TI
...
/*
    ...
    // We need to reposition widgets, adapt most browsers
	...
	document.calculateREREC = function (positionIndex, skills, gettr, num, rou, rou_factor){
		...
		for (i = 0; 2+i <= positionIndex; i += 2) {		// TrExMaWhenRECofweightTable of misalignment correction
			positionIndex--;
		}
		...
		for (var j = 0; j < 9; j++) {		// All position
			...
			var not20 = 0;					// 20The number of skills other than
  ...
  if(isShowNew)
  {
    ...
    function addBt()
    {
       ...
       myscript.innerHTML+="var clubid = new String(gettr2[0].getElementsByTagName(\"a\")[0].getAttribute(\"club_link\")).replace(/,/g, \"\"); \n";  
        ...
        h.innerHTML=h.innerHTML+"  &nbsp; &nbsp;<a href=\"javascript:getDetail()\" class=\"magnify\"><img src=\"/pics/magnifying_glass_normal.png\">Display decimal</a>(Do not leak, the leak completely abolished)";
