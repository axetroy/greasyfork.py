// ==UserScript==
// @name         FMU RatingR2 All
// @version      0.4 + EMO'Modify
// @author       Emiliano
// @description  REREC, TrExMa, RatingR2  C/EN + Season TI
// @include	http://ultra.trophymanager.com/players/*
// @namespace    https://ultra.trophymanager.com/club/217226/
...
var positionFullNamesJ = ["Obrambeni Sredina", "Obrambeni Lijevo", "Obrambeni Desno", "Defenzivni vezni Sredina", "Defenzivni vezni Lijevo", "Defenzivni vezni Desno", "Vezni Sredina", "Vezni Lijevo", "Vezni Desno", "Ofenzivni vezni Sredina", "Ofenzivni vezni Lijevo", "Ofenzivni vezni Desno", "Napadac", "Golman"],
...
/* Ova funkcija je zašticena, smanjiti volumen prijevoda 

    //var trnum = siSearch(gettr);	
	function siSearch (gettr) {
		for (var i = 0; i < gettr.length; i++){
			if (gettr[i].innerHTML.indexOf("vještine indeks") > 0) return i;
			if (gettr[i].innerHTML.indexOf("Skill Index") > 0) return i;
			if (gettr[i].innerHTML.indexOf("Vještine Procjena") > 0) return i;
		}
	}
...
*/
	...
	// to translate
	if(lang=="C")
	div_area.innerHTML="<div id=\"area\" style=\"position: absolute; z-index: 1000; width: 175px; margin-top: 25px; background: #5F8D2D; color: #ff9900; padding-left: 5px; -moz-opacity: .8; text-align: middle; color: gold; border: 2px #333333 outset; display:inline;\"><p style=\"text-decoration: underline;\"><b>PlayerData+:<\p><table style=\"margin-top: -1em; margin-bottom: 1em;\"><tr><td>ukupno Fizicka: </td><td>" + phySum + " / " + Math.round(100 * phySum / 80) +"% </td></tr><tr><td>ukupan broj taktika: </td><td>" + tacSum + " / " + Math.round(100 * tacSum / 80) +"% </td></tr><tr><td>ukupno tehnologija: </td><td>" + tecSum +  " / " + Math.round(100 * tecSum / 120) +"% </td></tr><tr><td>Ukupan broj vještina: </td><td>" + allSum + " + " + remainder + " </td></tr><tr><td>&nbsp;</td></tr><tr><td>procjenaR2: </td><td>" + ratingR2 + " </td></tr><tr><td>Iskustvo bonusa: </td><td>" + rouEffect + " </td></tr><tr><td>Ciste Vještine Evaluacija: </td><td>" + R2Pure + "</td></tr></table></b></div>";
     // Trebate premjestiti clanak, prilagoditi vecinu preglednike
	...
	document.calculateREREC = function (positionIndex, skills, gettr, num, rou, rou_factor){
		...
		for (i = 0; 2+i <= positionIndex; i += 2) {		// TrExMakadaRECodweightTablica korekcije odstupanje
			positionIndex--;
		}
		...
		for (var j = 0; j < 9; j++) {		// All position
			...
			var not20 = 0;					// 20Broj osim sposobnosti
  ...
  if(isShowNew)
  {
	...
	sith2.innerHTML = "Season TI New";
        if(lang=="CHS")
        sith2.innerHTML = "Nova sfera u sezonuTI";
    ...
    function addBt()
    {
       ...
       myscript.innerHTML+="var clubid = new String(gettr2[0].getElementsByTagName(\"a\")[0].getAttribute(\"club_link\")).replace(/,/g, \"\"); \n";  
        ...
        // start ajax
        myscript.innerHTML+="$.post(\"http://ultra.trophymanager.com/ajax/players_get_select.ajax.php\",{type:\"change\",club_id:clubid},function(data){\n";                        
        ...
        h.innerHTML=h.innerHTML+"  &nbsp; &nbsp;<a href=\"javascript:getDetail()\" class=\"magnify\"><img src=\"/pics/magnifying_glass_normal.png\">prikaz decimale</a>(Nemojte propuštati, curenja potpunosti ukinuti)";
