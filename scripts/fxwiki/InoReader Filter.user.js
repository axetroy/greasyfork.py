...
var aNgTextarea = [],
  ...
  nUnreadEx = 0,
  st = {},
  reAd = '^\\s*(?:ad|pr|広告)\\s*[:：]|^\\s*[\\[［【](?:ad|pr|広告)[\\]］】]|[\\[［【](?:ad|pr|広告)[\\]］】]\\s*$|[:：]\\s*(?:ad|pr|広告)\\s*$',
  $id = function(id) {
...
var currentExpandedArticle = function() {
  if (target) return target.getElementsByClassName('article_current article_expanded');
  return [];
};
...
var changedArticles = function() {
  ...
  if (eUct && eUct.textContent && articles.length + nExcludes/* && !nUnread*/) {
    aCnt = eUct.textContent.match(/\d+/);
    if (aCnt && aCnt.length) {
      nCnt = Number(aCnt[0]);
      if (nCnt !== 1000 && nCnt !== 10000 && (!nUnread || nCnt !== nUnread - nUnreadEx)) {
        nUnread = nCnt;
      }
    }
  }
  ...
  loop1: for (var n1 = 0, l1 = articles.length; n1 < l1; n1++) {
    ...
    if (st.hi) {
      for (var i1 = 0, j1 = aHi.length; i1 < j1; i1++) {
        ...
        if (sCa) {
          if (st.log) console.log('highlight: ', oA.sTitle, ' / matching rule ', sCa);
...
var observer1 = new MutationObserver(changedArticles);

var changedNumberOfUnreadArticles = function() {
  if (st.ngcount === 2) return;
  observer2.disconnect();
  var eUct = $id('unread_cnt_top');
  if (!eUct) return;
  var nDeduct = (nUnread) ? nUnread - nUnreadEx : 0,
    nCnt = 0,
    bModify = false,
    aCnt;
  eUct.removeAttribute('data-irf_ngcount');
  if (st.ngcount === 0) {
    if (nUnreadEx) eUct.setAttribute('data-irf_ngcount', '(' + nUnreadEx + ')');
    ...
    if (aCnt && aCnt.length) {
      ...
      if (nCnt !== 1000 && nCnt !== 10000) {
        ...
        eUct.textContent = eUct.textContent.replace(/([^\d]*)\d+([^\d]*)/, '$1' + nDeduct + '$2');
        bModify = true;
      }
    }
  }
...
var observer2 = new MutationObserver(changedNumberOfUnreadArticles);
...
var init = function() {
  ...
  var CSS =
    ...
    '.inoreader_filter_highlight span[id^="at_"], .inoreader_filter_highlight a[id^="article_title_link_"], .inoreader_filter_highlight .article_title_link, .inoreader_filter_highlight.article_unreaded .article_header_title, .inoreader_filter_highlight.article_tile.article_unreaded .article_title_link { color: #B65F06; }' +
    ...
    '#irf_settings { display: none; color: black; padding: 0; position: absolute; top: 48px; left: 48px; z-index: 90300; background: rgba(255, 255, 255, 0.98); border: 1px solid #999999; border-radius: 4px; box-shadow: 2px 2px 4px rgba(0, 0, 0, 0.2); min-width: 20em; -moz-user-select: none; -webkit-user-select: none; }' +
  ...
  if ($id('sb_rp_settings_menu')) {
    CSS += '.inoreader_filter_adarticle:before { font-family: "InoReader-UI-Icons-Font"; content: "\\e684"; padding-right: 5px; }';
  ...
  for (var i = 0, a; i < aAd.length; i++) {
    ...
    if (a.length === 2 && (Date.now() > new Date(Number(a[0]) + (86400000 * 60)).getTime())) {
      if (st.log) console.log('unregister ad: ', aAd[i].slice(0, aAd[i].lastIndexOf('<>')));
      aAd.splice(i, 1);
    }
  }
  saveAddata(aAd);
  ...
  $id('irf_settings').addEventListener('click', function(e) {
    ...
    if (tId === 'irf_ok') {
      var problem = false,
        adfilter = $id('irf_ad_filter').value,
