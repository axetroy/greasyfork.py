...
var init = function() {
  ...
  var s = '<div id="irkc_titlebar"><div id="irkc_title"><a href="https://greasyfork.org/scripts/898-inoreader-key-customize" target="_blank">InoReader Key Customize ' +
  ...
  switch (code) {
    ...
    case key2.n:
      if (st.replacemovearticle) {
        ...
        return;
      } else evt.initKeyEvent('keydown', 1, 1, null, 0, 0, 0, 0, cha, 0);
      break;
    case key2.k:
    case key2.p:
      if (st.replacemovearticle) {
...
var keyUp = function(e) {
  if (/^irkc_key/.test(e.target.id)) {
    ...
    for (let n = 0, l = el.snapshotLength, v; n < l; n++) {
      ...
      if (/[0-9a-fh-zA-Z%&!]/.test(v) && v !== '!') key.push(v);
    }
