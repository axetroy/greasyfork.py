...
FullFeed.checkScroll = function() {
  ...
  scrollInterval = window.setTimeout(function() {
    var e1 = $id('box'), e2 = getFirstElementByXPath(returnXpathEntry()),
      e3 = $id('floatingEntry'), remain;
    if (!e1 || !e2) return;
...
FullFeed.createSettings = function() {
  ...
  if (eTabs) {
    ...
    eItem.className = 'tab';
    eItem.innerHTML = '<div class="header target"><div id="gm_fullfeed_settings-menu" class="label primary iconless">Full Feed ' + loc[21] + '</div></div>';
...
FullFeed.registerWidgets = function() {
  ...
  el = getElementsByXPath(xe + '//div[contains(concat(" ", @class, " "), " metadata ")]/span');
  if (el) {
    el.some(function(i) {
  ...
  if (flag) return;
  var container = getFirstElementByXPath(xe + '//div[contains(concat(" ", @class, " "), " metadata ")]');
  if (!container) return;
  itemSiteInfo = [];
...
FullFeed.registerSocialIcons = function() {
  ...
  if (!c.innerContents) return;
  var xe = returnXpathEntry();
  el = getFirstElementByXPath(xe + '//div[contains(concat(" ", @class, " "), " metadata ")]/span[@class="gm_fullfeed_socialicon"]');
  if (el) return;
  var container = getFirstElementByXPath(xe + '//div[contains(concat(" ", @class, " "), " metadata ")]');
  ...
  var inter, repeat = 0, check = function() {
    var e = getElementsByXPath(xe + '//div[contains(concat(" ", @class, " "), " metadata ")]/span[@class="gm_fullfeed_socialicon"]/a/img'), f = true;
    repeat ++;
    if (e) {
...
var returnXpathEntry = function() {
  ...
  if (document.querySelector('#box.slided')) {
    return 'id("floatingEntry")//div[contains(concat(" ", @class, " "), " floatingEntryContent ")][last()]//div[contains(concat(" ", @class, " "), " floatingEntryScroller ")]';
  }
  //not use the right slider
  return '//div[contains(concat(" ", @class, " "), " inlineFrame ") and contains(concat(" ", @class, " "), " selected ")]';
};
...
initInterval = window.setInterval(function() {
  var e = $id('addContentPlaceholderFX');
