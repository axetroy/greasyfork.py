...
// @compatible  firefox
// @contributionURL bitcoin:1Bu3FeUob9GGEuTJVWXGy3gY16Yq6kLojp
// @basecodeversion 24
// ==/UserScript==
...
function doentry(resp) {
  ...
  if($('#hiddeniframe').contents().find('.sidebar .sidebar__error').length==0 && $('#hiddeniframe').contents().find('.sidebar form').length>0 && $('#hiddeniframe').contents().find('.sidebar form').html().indexOf('Enter Giveaway')>=0) {
    log('Entering giveaway '+$('#hiddeniframe').attr('src'));
    if($('#hiddeniframe').contents().find('.sidebar .sidebar__entry-insert')) {
      $('#hiddeniframe').contents().find('.sidebar .sidebar__entry-insert').click();
    } else {
      log('Unable to find entry button to submit entry!');
    }
    submitted=true;
  }
  ...
  if(submitted==true) {
    ...
    log('Unable to enter giveaway '+$('#hiddeniframe').attr('src')+'  pointsavailable='+pointsavailable);
  ...
  if(possibleentries.length>0) {
    ...
    for(var ti=0; ti<gamelist.length; ti++) {
      for(var ei=0; ei<possibleentries.length; ei++) {
        var rematch=false;
        try {
          var rematch=new RegExp(gamelist[ti].name.replace("(", "\\(").replace(")", "\\)"),"i").test(possibleentries[ei].name); //TESTING added .replace("(", "\\(").replace(")", "\\)")
        } catch(e) {
...
$(document).ready(function() {
  ...
  if($('.nav__left-container').length>0) {
    ...
    $('#autoentrysettings').click(function() { 
      ...
      $('#autoentrysettingsdiv').css('top','50%');
      $('#autoentrysettingsdiv').css('margin-top','-'+$('#autoentrysettingsdiv').height()/2+'px');
    ...
    $('#autoentrybackuprestore').click(function() {
      ...
      $('#autoentrybackuprestorediv').css('top','50%');
      $('#autoentrybackuprestorediv').css('margin-top','-'+$('#autoentrybackuprestorediv').height()/2+'px');
    ...
    $('#autoentryhelp').click(function() {
      ...
      $('#autoentryhelpdiv').css('top','50%');
      $('#autoentryhelpdiv').css('margin-top','-'+$('#autoentryhelpdiv').height()/2+'px');
    ...
    var elementsensor = $('.page__outer-wrap');
