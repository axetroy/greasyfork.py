...
function alternativetitle() {
   for (i = 0; i < rootlength.length; i++) {
       if (rootlength.className == "txt-block") {
           if (rootlength.innerHTML.match(/Also Known As|English title|\(USA\)\(/)) {
               for (y = 0; rootlength.childNodes.length; y++) {
                   if (rootlength.childNodes[y].textContent.match(/Also Known As|English title|\(USA\)/)) {
                       
                       var engishTitle = rootlength.childNodes[y].textContent;
...
//whoManyResults = whoManyResults.replace(/\r|\n/,'');
whoManyResults = whoManyResults.replace(/\n/g," ").replace(/\r/g," ");
                                //console.log(whoManyResults);
                                //res2 = whoManyResults.match(/download-(.*?).html\" title=\"Greek subtitle for (.*?)\">/);
                                res2 = whoManyResults.match(/download-(.*?).html" title="Greek subtitle for (.*?)"><img.*?uploaded by (.*?)" class=.*?<\/a> on (.*?)<\/div><\/td>  /);
...
var movienamereg = new RegExp(moviename);
       ...
       function sb(movieID, host) {
           var link = document.getElementById('sub_results');
           url = 'http://www.' + host + '.com/leftayparxoun/my_hosts_imdb.php?query=' + movieID;
           GM_xmlhttpRequest( {
...
foundresults.innerHTML += "<span id='" + host + "_results_id'>" + (i + 1) + "  <a href=\"http://www." + host + ".com/leftayparxoun/dload.php?action=file&file_id=" + file_id + "\">" + title.replace(/\'/g, ''') + "</a><br/><small>" + release + " " + translator + "</small></span><br/>";
...
foundresults.innerHTML += "<span id='opensubtitles_results_id'>"+ (i + 1) + " . " + "<a href='http://www.opensubtitles.org/" + detail + "'>" + releasename.replace(/\'/g, ''') + "</a> <a href='"+detaildwnloadlink+"'>download</a><br/><small>" + files + " CD " + uploader + " " + subadddate.replace(/ \d\d:\d\d:\d\d/, '') + "</small></span><br/>";
                           }
                       }
                       else {
