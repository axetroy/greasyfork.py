...
var ScriptAd = function () {
  ...
  _createClass(ScriptAd, [{
    ...
    value: function displayScriptAd() {
      ...
      var originHeight = locationPoint.parent().height();
      var afterObject = locationPoint[0];
      msg.insertAfter(afterObject);

      this.checkAdCanDisplayOrNeedRemove(originHeight, locationPoint);
      setTimeout(function () {
        _this17.checkAdCanDisplayOrNeedRemove(originHeight, locationPoint);
      }, 2000); // 2秒後再次檢查, 確認是否被其他外掛影響
    }
  }, {
    key: 'checkAdCanDisplayOrNeedRemove',
    value: function checkAdCanDisplayOrNeedRemove(originHeight, locationPoint) {
      var height = locationPoint.parent().height();
  ...
  _createClass(CloudUpdater, [{
    ...
    value: function checkUpdateTime(url, localUpdateTime, updater) {
      ...
      cloud(function (cloudInfo) {
        ...
        console.log(_this18.version + ' >= ' + conformedVersion + ': ' + (_this18.version >= conformedVersion));
        if (cloudTime === localUpdateTime) {
          console.log('cloud don\'t have new data');
          console.log('');
        } else if (_this18.version >= conformedVersion) {
          console.log('cloud have new data');
          console.log('');
          updater(cloudTime);
        } else {
          console.log('script version(' + _this18.version + ') is too old, can not update');
    ...
    value: function checkScriptVipProductsUpdate() {
      ...
      var updater = function updater(cloudTime) {
        var cloud = _this21.getWebData(dataUrl);
        cloud(function (cloudData) {
          var inputData = cloudData || [];
          var localScriptVipProducts = JSON.parse(window.localStorage.getItem('localScriptVipProducts')) || [];
          var defaultUser = {
    ...
    value: function templateListener(template, templateName, callback) {
      template.onCreated(function () {
        ...
        this.autorun(function () {
          rIsDataReady.set(_this22.subscriptionsReady());
        });
        this.autorun(function () {
          if (rIsDataReady.get()) {
...
var CompanyListView = function (_View2) {
  ...
  function CompanyListView(loginUser) {
    ...
    var _this23 = _possibleConstructorReturn(this, (CompanyListView.__proto__ || Object.getPrototypeOf(CompanyListView)).call(this, 'CompanyListView'));

    _this23.loginUser = loginUser;
  ...
  _createClass(CompanyListView, [{
    ...
    value: function addCardInfo(instance) {
      ...
      instance.autorun(function () {
        ...
        var company = new Company(serverCompany);
        var companyData = _this24.localCompanies.find(function (x) {
          return x.companyId === company.companyId;
        });
        if (companyData !== undefined) {
        ...
        if (Meteor.user()) {
          ...
          if (stockAmount > 0) {
            ...
            var holdC = _this24.loginUser.holdStocks.find(function (x) {
              return x.companyId === company.companyId;
            }) || { vip: null };
...
var BigLogView = function (_View3) {
  ...
  function BigLogView(name) {
    ...
    var _this26 = _possibleConstructorReturn(this, (BigLogView.__proto__ || Object.getPrototypeOf(BigLogView)).call(this, 'create BigLogView'));

    _this26.getDescriptionHtml = Template.displayLog.__helpers[' getDescriptionHtml'];
    _this26.name = String(name);
    return _this26;
  }
...
var CompanyDetailController = function (_EventController2) {
  ...
  function CompanyDetailController(loginUser) {
    ...
    _this28.templateListener(Template.companyDetailNormalContent, 'Template.companyDetailNormalContent', function () {
    ...
    _this28.panelFolderListener('companyDetail_companyBigLog', function () {
...
var AccountInfoController = function (_EventController3) {
  ...
  function AccountInfoController(loginUser) {
    ...
    var _this30 = _possibleConstructorReturn(this, (AccountInfoController.__proto__ || Object.getPrototypeOf(AccountInfoController)).call(this, 'AccountInfoController', loginUser));

    _this30.accountInfoView = new AccountInfoView();
    _this30.logRecorder = new LogRecorder();
    _this30.bigLogView = new BigLogView('accountBigLog');

    _this30.user = null;
    _this30.userId = null;
    _this30.waitList = [];

    _this30.templateListener(Template.accountInfo, 'Template.accountInfo', function () {
      _this30.usersEvent();
    });
    _this30.templateListener(Template.managerTitleList, 'Template.managerTitleList', function () {
      _this30.managersEvent();
    });
    _this30.templateListener(Template.vipTitleList, 'Template.vipTitleList', function () {
      _this30.vipsEvent();
    });
    _this30.templateListener(Template.accountInfoOwnStockList, 'Template.accountInfoOwnStockList', function () {
      _this30.ownStocksEvent();
    });
    _this30.templateListener(Template.accountInfoOwnedProductsPanel, 'Template.accountInfoOwnedProductsPanel', function () {
      _this30.ownProductsEvent();
    });
    _this30.templateListener(Template.accountInfoLogList, 'Template.accountInfoLogList', function () {
      _this30.logEvent();
    });
    ...
    Template.accountInfoBasic.onRendered(function () {
      ...
      _this30.bigLogView.showBigLogFolder();
    });
    _this30.panelFolderListener('accountInfo_holdStocksTable', function () {
      var state = $('a[data-key=\'accountInfo_holdStocksTable\']').find('i[class=\'fa fa-folder-open\']');
      if (state.length > 0) {
        _this30.accountInfoView.displayHoldStocksTable(_this30.holdStocksTableInfo());
      }
    });
    _this30.panelFolderListener('accountInfo_accountBigLog', function () {
      var state = $('a[data-key=\'accountInfo_accountBigLog\']').find('i[class=\'fa fa-folder-open\']');
      if (state.length > 0) {
        var userId = FlowRouter.getParam('userId');
        var localLog = _this30.logRecorder.filter(function (x) {
          if (x.userId) {
        ...
        localLog = _this30.logRecorder.sort(localLog);
        _this30.bigLogView.displayBigLog(localLog);
      }
    });
    return _this30;
  }
  ...
  _createClass(AccountInfoController, [{
    ...
    value: function showHoldStocksTableFolder() {
      ...
      if (intoObject.length > 0) {
        ...
        setTimeout(function () {
          _this31.showHoldStocksTableFolder();
        }, 10);
      }
    }
  }, {
...
var ScriptVipView = function (_View5) {
  ...
  function ScriptVipView(controller) {
    ...
    var _this32 = _possibleConstructorReturn(this, (ScriptVipView.__proto__ || Object.getPrototypeOf(ScriptVipView)).call(this, 'ScriptVipView'));

    _this32.controller = controller;
    _this32.scriptAd = new ScriptAd();
  ...
  _createClass(ScriptVipView, [{
    ...
    value: function displayScriptAdInfo(loginUser) {
      ...
      if (loginUser.vipLevel() < 1) {
        ...
        $('button[name=\'closeAd\']')[0].addEventListener('click', function () {
          ...
          _this33.scriptAd.removeScriptAd();
        });
      }
      ...
      $('button[name=\'openAd\']')[0].addEventListener('click', function () {
        ...
        if ($('a[name=\'scriptAd\'][demo=\'false\']').length < 1) {
          _this33.scriptAd.displayScriptAd();
        }
      });
    ...
    value: function displaySearchTables(loginUser) {
      ...
      $('button[name=\'deleteAllTable\']')[0].addEventListener('click', function () {
        alertDialog.confirm({
          ...
          callback: function callback(result) {
            if (result) {
              _this34.controller.deleteLocalSearchTables();
            }
          }
        });
      });
      ...
      $('button[name=\'createTable\']')[0].addEventListener('click', function () {
        alertDialog.dialog({
          ...
          callback: function callback(result) {
            if (result) {
              _this34.controller.createNewSearchTable(result);
            }
          }
        });
      });
      $('button[name=\'deleteTable\']')[0].addEventListener('click', function () {
        ...
        alertDialog.confirm({
          ...
          callback: function callback(result) {
            if (result) {
              _this34.controller.deleteSearchTable(tableName);
            }
          }
        });
      });
      ...
      $('button[name=\'addTableFilter\']')[0].addEventListener('click', function () {
        ...
        _this34.controller.addSearchTableFilter(tableName, filter);
      });
      $('button[name=\'deleteTableFilter\']')[0].addEventListener('click', function () {
        var tableName = $('select[name=\'dataSearchList\']')[0].value;
        _this34.controller.deleteSearchTableFilter(tableName);
      ...
      $('button[name=\'addTableSort\']')[0].addEventListener('click', function () {
        ...
        _this34.controller.addSearchTableSort(tableName, sort);
      });
      $('button[name=\'deleteTableSort\']')[0].addEventListener('click', function () {
        var tableName = $('select[name=\'dataSearchList\']')[0].value;
        _this34.controller.deleteSearchTableSort(tableName);
            ...
            if (newName) {
              alertDialog.dialog({
                ...
                callback: function callback(newRule) {
                  if (newRule) {
                    _this34.controller.addSearchTableColumn(tableName, newName, newRule);
      ...
      $('button[name=\'outputTable\']')[0].addEventListener('click', function () {
        if (loginUser.vipLevel() > 0) {
          ...
          if (tableName !== '') {
            ...
            _this34.displayOutputTable(tableName);
          }
        } else {
    ...
    value: function displaySearchTablesList() {
      ...
      $('select[name=\'dataSearchList\']')[0].addEventListener('change', function () {
        $('table[name=outputTable]').remove();
        _this35.displaySearchTableInfo();
      });
    ...
    value: function displaySearchTableColumns(tableName) {
          ...
          callback: function callback(newName) {
            if (newName) {
              alertDialog.dialog({
                ...
                callback: function callback(newRule) {
                  if (newRule) {
                    _this36.controller.changeSearchTableColumn(tableName, { name: c.columnName, newName: newName }, newRule);
                    _this36.displaySearchTableColumns(tableName);
                  }
                }
              });
            }
      ...
      var deleteColumn = function deleteColumn(c) {
        alertDialog.confirm({
          ...
          callback: function callback(result) {
            if (result) {
              _this36.controller.deleteSearchTableColumn(tableName, c.columnName);
              _this36.displaySearchTableColumns(tableName);
            }
          }
        });
  ...
  _createClass(SearchTables, [{
      ...
      try {
        if (table.sort) {
          outputCompanies.sort(function (a, b) {
            return _this37.doInputFunction(b, table.sort) - _this37.doInputFunction(a, table.sort);
          });
        }
      } catch (e) {
    ...
    value: function outputTable(tableName) {
      ...
      try {
        if (t.sort) {
          outputCompanies.sort(function (a, b) {
            return _this38.doInputFunction(b, t.sort) - _this38.doInputFunction(a, t.sort);
          });
        }
      } catch (e) {
        alertDialog.alert('計算失敗！排序公式出錯');
...
var ScriptVipController = function (_EventController4) {
  ...
  function ScriptVipController(loginUser) {
    ...
    var _this39 = _possibleConstructorReturn(this, (ScriptVipController.__proto__ || Object.getPrototypeOf(ScriptVipController)).call(this, 'ScriptVipController', loginUser));

    _this39.searchTables = new SearchTables();
    _this39.scriptVipView = new ScriptVipView(_this39);

    Template.softwareScriptVip.onRendered(function () {
      _this39.scriptVipView.displayScriptVipProducts(_this39.loginUser);
      _this39.scriptVipView.displayScriptAdInfo(_this39.loginUser);
      _this39.scriptVipView.displaySearchTables(_this39.loginUser);
    });
    return _this39;
  }
...
var AboutView = function (_View6) {
  ...
  function AboutView() {
    ...
    var _this40 = _possibleConstructorReturn(this, (AboutView.__proto__ || Object.getPrototypeOf(AboutView)).call(this, 'AboutView'));
...
var AboutController = function (_EventController5) {
  ...
  function AboutController(loginUser) {
    ...
    var _this41 = _possibleConstructorReturn(this, (AboutController.__proto__ || Object.getPrototypeOf(AboutController)).call(this, 'AboutScriptController', loginUser));

    _this41.aboutView = new AboutView();
    return _this41;
  }
  ...
  _createClass(AccessedRecorder, [{
    ...
    value: function getAccessedCount() {
      ...
      this.records = this.records.filter(function (t) {
        return Date.now() - t < _this42.interval;
      });
...
var DisconnectReminderController = function (_EventController6) {
  ...
  function DisconnectReminderController(loginUser) {
    ...
    var _this44 = _possibleConstructorReturn(this, (DisconnectReminderController.__proto__ || Object.getPrototypeOf(DisconnectReminderController)).call(this, 'DisconnectReminderController', loginUser));

    _this44.disconnectReminderView = new DisconnectReminderView();

    // this.accountOwnStocksReminder();
    _this44.companyProductCenterInfoReminder();
    _this44.companyMarketingProductsReminder();
    ...
    _this44.accountInfoReminder();
    _this44.employeeListByUserReminder();
    _this44.accountChairmanTitleReminder();
    _this44.accountManagerTitleReminder();
    _this44.accounEmployeeTitleReminder();
    _this44.accountVipTitleReminder();
    _this44.accountInfoTaxReminder();
    _this44.accountInfoLogReminder();
    _this44.userPlacedStonesReminder();

    _this44.companyDataForEditReminder();
    ...
    _this44.userOwnedProductsReminder();
    _this44.companyListReminder();

    // this.adjacentSeasonReminder();
    // this.productListBySeasonIdReminder(); //未限制
    // this.rankListBySeasonIdReminder();

    _this44.companyVipsReminder();
    // this.currentUserCompanyVipReminder();

    _this44.foundationListReminder();
    _this44.foundationDetailReminder();
    _this44.foundationDataForEditReminder();

    _this44.companyMiningMachineInfoReminder();
    _this44.companyStonesReminder();
    _this44.companyCurrentUserPlacedStonesReminder();
    _this44.companyLogReminder();
    _this44.productListByCompanyReminder();
    _this44.companyDetailReminder();
    _this44.employeeListByCompanyReminder();
    _this44.companyDirectorReminder();
    _this44.companyArenaInfoReminder();
    ...
    _this44.arenaInfoReminder();
    // this.adjacentArenaReminder();
    _this44.arenaLogReminder();
    // this.fscMembersReminder();

    _this44.currentUserOrdersReminder();
    _this44.currentUserDirectorsReminder();
    _this44.companyOrdersReminder();
    return _this44;
  }
  ...
  _createClass(DisconnectReminderController, [{
    ...
    value: function createReminder(recorder) {
      ...
      return function () {
        ...
        if (shouldWarning) {
          _this45.disconnectReminderView.displayWarningDialog(recorder.name, count, Math.ceil((firstTime + recorder.interval - Date.now()) / 1000));
        }
      };
    }
  }, {
