...
function fillAnswer(obj, tip) {
    ...
    obj.code == 1 && $input.each(function(index) {
        if (this.value == 'true') {
            ...
            index = $.inArray(setting.TiMu[3][index], data) + 1 || (setting.TiMu[1] == '1' && String(obj.data).indexOf(setting.TiMu[3][index]) + 1);
            Boolean(index) == this.checked || this.click();
        }
    });
    if (setting.TiMu[1].match(/^[013]$/)) {
        tip = $input.filter(':checked').length || setting.none && (($input[Math.floor(Math.random() * $input.length)] || $()).click(), '不会');
    } else if (setting.TiMu[1].match(/^[0-7]$/) && $('#submitTest textarea').length) {
        var $li = $('.Cy_ulTk li');
