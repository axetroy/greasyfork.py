...
// @exclude      https://classic.planetromeo.com/*
// @exclude      https://wish.planetromeo.com/*
// @copyright    braveguy 12.10.2016 / 21.04.2019
...
// ***** jQuery *****
// Taken from https://code.jquery.com/jquery-3.3.1.min.js
...
GM_addStyle (
	...
	//icons
	'a.re-icon {margin-left:0.25em; font-size:0.875em; color:rgba(255,255,255,0.8)}' +
	'a:hover.re-icon {color:rgba(102,215,255,0.8)}' +
	'a.re-link {color:rgba(255,255,255,1)}' +
	'span.re-icon-copy:before {content:"\\f12f"}' +
	'a#contacts-menu span.icon:before {content:"\\f123"}' +

	//text
	'span.re-posts-view {font-family: Gibson Bold; font-weight:500; text-transform:uppercase; background-color:transparent; color:#fff; height:auto; cursor:pointer; padding-right:2rem;}' +
	'span.re-list-view, span.re-list-load {font-family: Gibson Bold; font-weight:500; text-transform:uppercase; background-color:transparent; color:#fff; height:auto; cursor:pointer;}' +
	'span.re-list-load:before {content:"\\f1a0"; display:inline-block; transform:scaleX(-1)}' +

	//bigger emojis on hover
	'span.emoji:hover {font-size:1.8rem; line-height:1.33rem; padding-bottom:.5rem}' +

	//smaller stats in tiles
	'.stat-bar__item--305IN {padding:0 .35rem}' +

	//more dense list view
	'.list-stat-bar__item {width:inherit}' +
	'.list-stat-bar__item:nth-child(3n) {margin-right: 1rem}' +

	//separators
	'.separator--dark, .gallery__info, .footprint-separator, .Caption--18fzb {color:rgba(255,255,255,0.33)}' +

	//photo album captions
	'.Description--1V_OY {font-size:.85rem}' +

	//groups
	'.PostDate--1qfU0, .PostDate--2LVzF, .PostDeleted--2Jlwr {color:rgba(255,255,255,.66)}' +
	'div.Container--2mFBj div.js-groups {display:none}' +
...
var val1 = window.atob('dnVFcDhvOTNiMzRDeFVDbGpTTUZFZGhJNzBxRFd0dWs=');
function ajaxHead(){
...
function timeDiff(loginTime) {
	var timeTag = loginTime.toLocaleDateString();
	...
	$(jNode).parent().on('keyup mouseout', function() {
		...
		if ($(jNode).val().match(/^[1-9]\d{2,8}$/)) {
			$('#search div.js-input').after(
				`<a style="text-align:center; font-size:1rem" class="ml- mr- re-add">Profil-Nr. ${$(jNode).val()} anzeigen</a>`
...
function openByID(id) {
	$.ajax({url: `/api/v4/profiles?expand=items&pick=items.*.(type,id,name)&lang=de&length=1&filter[profile_ids][]=${id}`,
			headers: ajaxHead()
		   })
	.done(function (data) {
		if (data.items[0]) {
			changeUrl('/profile/' + data.items[0].name);
		} else {
			$.ajax({url: `/api/v4/hunqz/profiles?expand=items&pick=items.*.(type,id,name)&lang=de&length=1&filter[profile_ids][]=${id}`,
					headers: ajaxHead()
				   })
			.done(function (data) {
				if (data.items[0]) {
					changeUrl('/hunq/' + data.items[0].name);
				} else {
					$.ajax({url: `/api/v4/groups/${id}?lang=de&pick=id,name`,
							headers: ajaxHead()
						   })
					.done(function (data) {
						if (data.id) {
							changeUrl('/group/' + data.name);
						}
					})
					.fail(function (data) {
						changeUrl('/profile/-');
...
function copyMessageThread() {
	...
	var msgCount = '', msgThread = '', msgTime = '', name = '';
	var profileName = $('#messenger section.js-detail a.plain-text-link.pr--').attr('href').match(/[\w\-]*$/);
	...
	.done(function (data) {
		...
		if (data.items_total) {
			...
			msgThread = 'Message-Verlauf von ' + ownName + ' mit ' + profileName + ' (Profil-Nr. ' + profileId + ') vom ' + loadTime +'\n\n\n';
		...
		$('#messenger div.js-correspondence').after(
			'<div class="re-bubble ui-bubble ui-bubble--dark" style="width: auto; top: 56px; right:40px;">' +
			'<div class="ui-bubble__content [ js-content ] [ js-scrollable ]  scrollable"><div class="confirm-box">' +
			'<div class="confirm-box__label">' +
			'<p>Message-Verlauf in die Zwischenablage kopieren<br>(im Ziel mit Strg+V einfügen)</p>' +
			'</div>' +
			'<div class="confirm-box__actions">' +
			'<button class="re-copy-cancel ui-button ui-button--transparent">Abbrechen</button>' +
			'<button class="re-copy-confirm ui-button ui-button--primary">Kopieren</button>' +
			'</div>' +
			'</div></div>' +
			'<div class="ui-bubble__arrow [ js-arrow ] ui-bubble__arrow--up" style="top: -20px; right:26px;"></div></div>' +
			'<div class="ui-bubble__overlay ui-bubble__overlay--hidden"></div>'
		);
		$('button.re-copy-confirm').click(function() {
			copyToClipboard(msgThread);
			$('div.re-bubble, div.ui-bubble__overlay').hide();
		});
		$('button.re-copy-cancel, div.ui-bubble__overlay').click(function() {
			$('div.re-bubble, div.ui-bubble__overlay').hide();
		});
	});
}
...
// ***** Display selected bookmark in bookmark icon title *****
function bookmarkTitle (jNode) {
	...
	//$('header.js-header h1').attr('title','RomeoEnhancer ' + GM.info.script.version);
	//$('header.js-header h1').attr('title','PlanetRomeo ' + APP_VERSION + '\rRomeoEnhancer ' + GM.info.script.version);
}


// ***** Add forums and classic to radar navigation bar, show users, online, version in header logo title *****
function forumsLink (jNode) {

	//forums, classic
	$('main nav.js-navigation ul:has(a[href^="/radar/"])').append(
		'<li class="Tabbed-nav-item--8Ou8b"><a class="c" style="cursor:default">|</a></li>' +
		'<li class="Tabbed-nav-item--8Ou8b"><a href="/messenger/contacts" class="js-nav-item Tabbed-nav-link--bLtph">Kontakte</a></li>' +
		'<li class="Tabbed-nav-item--8Ou8b"><a target="romeoclubs" href="https://classic.planetromeo.com/myuser/?page=club&amp;subPage=forum" class="js-nav-item Tabbed-nav-link--bLtph">Foren</a></li>' +
		'<li class="Tabbed-nav-item--8Ou8b"><a target="romeoclassic" href="https://classic.planetromeo.com" class="js-nav-item Tabbed-nav-link--bLtph">Classic</a></li>'
	);
	$('div.js-navigation ul:has(a[href^="/groups/member/"])').append(
		'<li class="Item--111ms"><a class="Link--11pi1" style="cursor:default"><div class="Label--jzcum"><span>|</span></div></a></li>' +
		'<li class="Item--111ms"><a class="Link--11pi1" target="romeoclubs" href="https://classic.planetromeo.com/myuser/?page=club&amp;subPage=forum"><div class="Label--jzcum"><span>Foren</span></div></a></li>' +
		'<li class="Item--111ms"><a class="Link--11pi1" target="romeoclassic" href="https://classic.planetromeo.com"><div class="Label--jzcum"><span>Classic</span></div></a></li>'
	);

	//users, online, version
	var users = '', online = '';
	var romeoVersion = '';
	var reVersion = (typeof GM.info.script.version) ? GM.info.script.version : '';
	$.ajax({headers: ajaxHead(), url: '/api/v4/profiles?pick=items_total&lang=de&length=1'}).done(function (data) {
		if (data.items_total) {
			users = data.items_total.toLocaleString() + ' Nutzer • ';
		}
		$.ajax({headers: ajaxHead(), url: '/api/v4/profiles?pick=items_total&lang=de&length=1&filter[online_status][]=ONLINE&filter[online_status][]=DATE&filter[online_status][]=SEX'}).done(function (data) {
			if (data.items_total) {
				online = data.items_total.toLocaleString() + ' online\r';
			}
			var version = romeoVersion + 'RomeoEnhancer ' + reVersion;
...
function statisticsLink (jNode) {
	$('#visits div.js-navigation ul').append(
		'<li class="Tabbed-nav-item--8Ou8b"><a class="Tabbed-nav-link--bLtph" style="cursor:default">|</a></li>' +
		'<li class="Tabbed-nav-item--8Ou8b"><a target="romeostats" href="https://classic.planetromeo.com/settings/visitorStatistics.php" class="Tabbed-nav-link--bLtph">Statistik</a></li>'
	);
}
...
function previewMessage (jNode) {
	var profileId = $(jNode).parent().parent().parent().attr('href').match(/\d{3,}/);
	$(jNode).parent().parent().parent().addClass('re-id' + profileId);
...
function showLoginLocation (jNode) {
	var profileId = location.pathname.match(/\d{3,}/);
	var id = '<span style="opacity:.5">Profil-Nr. ' + profileId + '</span>';
	...
	.done(function (data) {
		...
		if (data.last_login) {
			...
			var loginLocalTime = loginTime.toLocaleString().slice(0,-3);
			var timeTag = timeDiff(loginTime);
			//var timeTag = loginTime;
			$(jNode).not(':has(span.txt-raise)').find('span.icon').removeAttr('title').parent().attr(
				{'title': 'Online seit ' + loginLocalTime, 'style': 'font-size:0.85em'}
			);
			$(jNode).find('span.icon-last-login-hours').removeAttr('title').parent().attr(
				{'title': 'Zuletzt online ' + loginLocalTime, 'style': 'font-size:0.85em'}
			);
			$(jNode).not(':has(span.icon)').append(
				'<span class="icon icon-last-login-hours txt-raise" style="font-size:0.85em" title="Zuletzt online ' + loginLocalTime + '"> ' + timeTag + '</span>'
			);
		}

		//profile id, location
		if (data.location.name) {
			name = '<span style="opacity:.5"> • </span>' + data.location.name;
		}
		...
		$(jNode).after(
			'<div style="text-align:center" class="typo-small lh-heading txt-truncate">' + id + name + distance + sensor + '</div>'
		);
	});

	//thread icon
	if (location.pathname.match(/^\/messenger\/chat/)) {
		$(jNode).parent().parent().find('a.js-confirm-trigger.l-tappable.plain-text-link').before(
			'<a class="plain-text-link pl- pr+" title="Message-Verlauf kopieren\r(im Ziel mit Strg+V einfügen)"><span class="icon icon-larger re-icon-copy"></span></a>'
		).prev().click(function() {
			copyMessageThread();
			return false;
		});
...
function contactsMessage (jNode) {
	var profileId = $(jNode).attr('href').match(/\d{3,}/);
...
function messageContacts (jNode) {
	var profileId = $(jNode).attr('href').match(/\d{3,}/);
	if (!profileId) profileId = location.pathname.match(/\d{3,}/);
	$(jNode).find('span.plain-text-link').not(':has(a)').append(
		'<a class="icon icon-chat re-icon ml--" title="Messages" href="/messenger/chat/' + profileId + '"></a>'
	);
}


// ***** Add message icons to visitor list entries and search results, link contact icons *****
function handleVisitsSearch (jNode) {
	var replacePattern = /(.*\/(profile|hunq|group)\/)([A-Za-z0-9\-_]*)(.*)/;
	var profileLink = $(jNode).attr('href');
	var profileName = profileLink.replace(replacePattern, '$3');

	//add message icon
	var baseUrl = '/messenger/chat/';
	$(jNode).find('div.info, div.Name--3cum4').append(
		'<a class="icon icon-chat re-icon" title="Messages" href="' + baseUrl + profileName + '"></a>'
	).find('a.icon-chat').click(function() {
	...
	//link contact icon
	$(jNode).find('span.icon-save-contact').replaceWith(
		'<a class="tile__badge icon icon-save-contact" title="Kontakt bearbeiten" href="/messenger/contacts/all/' + profileName + '"></a>'
	);
...
// ***** Link user names on discover page to profiles *****
function handleDiscover (jNode) {
	var profileLink = '/profile/' + $(jNode).text();
	$(jNode).not(':has(a)').wrapInner(
		'<a class="re-link" title="Profil anzeigen" href="' + profileLink + '"></a>'
	);
}
...
function handleRadar (jNode) {
	...
	$(jNode).find('div.info__username, span.listresult__name').wrapInner(
		'<a class="re-link" title="Vorschau" href="' + profileLink.replace(replacePattern, '$1' + '$3' + '/preview') + '"></a>'
	);
	...
	//show count of profiles, Plus, and GPS in title tag of selected tab
	...
	if ($('#profiles .search-results--small-tiles').length == 0) {
		profilesPlus = $('#profiles div.js-romeo-badge span img, #profiles .badge--plus--3SFc3').length;
	...
	$('ul.Tabbed-nav--bO30c li.is-selected, div.js-nav-item, li.list__item--active--2myHB').attr(
		'title', loadTime + ' aktualisiert\r' + profiles + ' Profile geladen' + profilesPlus + profilesGPS
	);
}


// ***** Add message icons to non-message Activity Stream entries; hide group related entries *****
function handleActivity (jNode) {
	...
	if (! bodyLink.match('/messenger/chat/')) {
		...
		//hide if group entry
		//$(jNode).not(':has(a)').not(':has(img)').parent().hide();
		$(jNode).not(':has(a)').has('span.listitem__text').has('img.thumbnail').parent().hide();

		//add message icon
		$(jNode).find('div.layout.layout--consume span:last').not(':has(a)').append(
			'<a class="icon icon-chat re-icon ml--" title="Messages" href="' + baseUrl + profileName + '"></a>'
		).find('a.icon-chat').click(function() {
			openByName(baseUrl, profileLink, profileName);
			return false;
		});
...
function handleProfile (jNode) {
	var profileName = $('h2.profile__name').text();
	var profileId = '', visits = '', since = '';
	var albums = 0, pictures = 0, quickShare = 0;
	$.ajax({headers: ajaxHead(), url: '/api/v4/profiles/' + profileName + '/full?lookup_type=NAME&expand=albumsV2.items.*.pictures&lang=de'})
	...
	.done(function (data) {
		...
		if (data.visits_count) {
			visits = '\n' + data.visits_count.toLocaleString() + ' Besucher';
		}
		if (data.creation_date) {
			since = new Date(data.creation_date.slice(0,-5)+'.000Z').toLocaleDateString();
			since = 'Mitglied seit ' + since;
		} else {
			since = $('section.profile__stats .js-section-view .txt-description .txt-preserve').last().text();
		}

		//show pictures count
		if (data.albumsV2.items_total) {
			albums = data.albumsV2.items_total;
			for (var i = 0; i < albums; i++) {
				if (data.albumsV2.items[i].access_policy == 'SHARED') {
					quickShare += data.albumsV2.items[i].items_total;
				} else {
					pictures += data.albumsV2.items[i].items_total;
				}
			}
			$('li a span.profile-nav__link-label').first().text(pictures);
			$('section.js-profile-strip a.strip__placeholder').append(
				'<span style="padding-top:4em; color:#fff">' + quickShare + '</span>'
			);
		}
	})
	...
	.always(function (data) {
		...
		var linkChat = '<a target="_blank" href="https://classic.planetromeo.com/msg/history.php?uid=' + profileId + '">Chat</a>';
		...
		//profile id, visits since
		$('section.profile__stats .js-section-view .txt-description .txt-preserve').last().text(
			since + visits + '\nProfil-Nr. ' + profileId
		);
...
function recentPosts (jNode) {
	...
	$('div.Container--2mFBj div.Container--2r7Kf').not(':has(span.re-add)').after(
		`
<div class="mt- mb- pb-" style="font-size:.85rem; border-bottom:1px solid rgba(255,255,255,.125)">
<span class="layout-item SortBy--3gzoM">Sortierung</span>
<span class="re-list-view ml-" title="Umschalten">${linkText}</span>
<span class="re-list-load icon pl- pr-" title="Aktualisieren"></span>
</div>
`
	);
    if (window.matchMedia("(max-width:783px)").matches) {
		$('div.Container--2mFBj div.js-header').removeClass('l-hidden-sm');
		$('div.Container--2r7Kf').hide();
	}
	if (viewMode !== 'active') {
	...
	$('span.re-list-load').click(function() {
		groupsList.length = 0;
		counter = 0;
		$('div.Container--2mFBj div.js-header div.re-add').remove();
		$('div.Container--2mFBj div.js-groups').hide();
		handlePostsList(viewMode);
	});
	handlePostsList(viewMode);
}
...
function handlePostsList (viewMode) {
	...
	if (viewMode !== 'active') {
		$('div.Container--2mFBj div.js-header div.re-add').remove();
		$('div.Container--2mFBj div.js-groups').show();
	} else {
		$('div.Container--2mFBj div.js-groups').hide();
		if (groupsList.length > 0) {
			insertPostsList(groupsList);
		} else {
			$('div.Container--2mFBj div.js-header').append(
				'<div class="spinner-container re-add"><div class="spinner"></div></div>'
			);
			$.ajax({headers: ajaxHead(), url: '/api/v4/session?expand=groups.items,groups.items.*.activity&pick=groups.items.*.(id,name,display_name,activity.posts.last_accessed,activity.photos.last_accessed,preview_pic.url_token),groups.items_total'})

			.done(function (data) {
				groupsList = [];
				counter = 0;
				var groupsCount = data.groups.items_total;
				var name = '', displayName = '', picToken = '', actPosts = 0, actPhotos = 0;
				console.log ('groups: ', groupsCount);

				for (var i = 0; i < groupsCount; i++) {

					// loop "posts" for all groups
					name = data.groups.items[i].name;
					displayName = data.groups.items[i].display_name;
					picToken = (data.groups.items[i].preview_pic) ? data.groups.items[i].preview_pic.url_token : '';
					if (data.groups.items[i].activity) {
						actPosts = (data.groups.items[i].activity.posts) ? data.groups.items[i].activity.posts.last_accessed : 0;
						actPhotos = (data.groups.items[i].activity.photos) ? data.groups.items[i].activity.photos.last_accessed : 0;
					} else {
						actPosts = 0;
						actPhotos = 0;
					}
					$.ajax({headers: ajaxHead(),
							// /api/v4/groups/13381772/posts?expand=items.*.(owner)&lang=de&length=10&sort_criteria=COMMENTED_AT_DESC
							url: `/api/v4/groups/${data.groups.items[i].id}/posts?pick=items.*.(group_id,date_created,edit_status,comments.items.*.(date_created,edit_status))&expand=items.*.(comments)&lang=de&length=5&sort_criteria=COMMENTED_AT_DESC`,
							custom: {name: name, displayName: displayName, picToken: picToken, actPosts: actPosts, actPhotos: actPhotos} })

					.done(function (data) {

						//save group id, name, and timestamp to array (most recent not deleted post or comment)
						var id = '', name = '', displayName = '', time = 0, editStatus = '', last = '', picToken = '', actPosts = 0, actPhotos = 0;
						if (data.items[0]) {
							id = data.items[0].group_id;
							if (data.items[0].comments.items[0]) {
								time = data.items[0].comments.items[0].date_created;
								editStatus = data.items[0].comments.items[0].edit_status;
								last = 'comment';
							} else {
								time = data.items[0].date_created;
								editStatus = data.items[0].edit_status;
								last = 'post';
							}
						}
						...
						//add to array
						groupsList.push({time: time, name: name, displayName: displayName, picToken: picToken, actPosts: actPosts, actPhotos: actPhotos});
					})
...
function insertPostsList (groupsList) {
	...
	//insert list elements on top of YOUR GROUPS
	$('div.Container--2mFBj div.js-header div.re-add').remove();
	$('div.Container--2mFBj div.js-groups').hide();
	var thumbnail = '';
	for (var i = 0; i < groupsList.length; i++) {
		if (groupsList[i].picToken == '') {
			thumbnail = '/assets/95a535c0070126f52d27820aeab2326c.svg';
		} else {
			thumbnail = `/img/usr/squarish/212x212/${groupsList[i].picToken}.jpg`;
		}
		$('div.Container--2mFBj div.js-header').append(
			`
<div class="Container--1V5ct re-add"><a href="/groups/member/${groupsList[i].name}">
<div class="layout layout--v-center">
<div class="js-tile layout-item Image--2S6h3"><div style="background-image:url(${thumbnail})" class="Image--1BRzA"></div></div>
<div class="layout-item layout-item--consume pl txt-truncate">
<h1 class="Name--b7vyN txt-truncate">${groupsList[i].displayName}</h1>
<div style="color:rgba(255,255,255,.8); font-size:.75rem">${groupsList[i].time.toLocaleString('de-DE').slice(0,-3)}</div>
</div>
</div>
</a>
</div>
...
function groupPostsViewMode (jNode) {
	...
	$('div.Container--1cJVr').prepend(
		'<span class="layout-item SortBy--3gzoM">Ansicht</span>' +
		'<span class="re-posts-view ml-" title="Umschalten">' + linkText + '</span>'
	);
...
function refreshGroupComments (jNode) {
	var thisNode = $(jNode).find('span.CommentCount--3uCNR');
...
function imgInfo (jNode) {
	var eq = ($('#photos div.swipe__element--1TlCJ').length == 1) ? 0 : 1;
	var imgName = $('#photos div.swipe__element--1TlCJ:eq(' + eq + ') div.fit img').attr('src');
	if (imgName) {
		var imgNameTxt = imgName.substr(imgName.lastIndexOf('/')+1, 5) + '...';
		$('#photos div.Container--1MjID').not(':has(a)').prepend(
			'<a target="_blank" style="color:rgba(255,255,255,0.5); font-size:0.85em" class="mr" href="' + imgName + '">' + imgNameTxt + '</a>'
		);
	}

	//URLs in picture caption
	$('#photos div.Description--1V_OY').each(function() {
...
waitForKeyElements ('li.js-bookmarks li.is-selected, header.js-header h1', bookmarkTitle);
waitForKeyElements ('main ul.Tabbed-nav--bO30c li:first, ul.Container--sDaVi li:first', forumsLink);
...
waitForKeyElements ('#messenger div.layer-header__title div.info', showLoginLocation);
waitForKeyElements ('#messenger div.js-chat a.js-preview.listitem__body', contactsMessage);
waitForKeyElements ('#messenger div.js-contacts a.js-preview.listitem__body', messageContacts);
waitForKeyElements ('#visits a.tile__link, #visits a.listresult, #search a.tile__link, /* #search .Tile--20XaP a, */ .js-profiles a.tile__link, .js-profiles a.listresult', handleVisitsSearch);
waitForKeyElements ('li a div.Name--VZAP4', handleDiscover);
waitForKeyElements ('#profiles a.tile__link, #profiles a.listresult', handleRadar);
waitForKeyElements ('div.stream a.listitem__body, #activity-stream a.listitem__body', handleActivity);
waitForKeyElements ('div.is-profile-loaded', handleProfile);
waitForKeyElements ('div.profile__container--error', linksError);
waitForKeyElements ('div.Container--2mFBj', recentPosts);  //div.Sidebar--1Hale div.Labels--2h_QY. //.js-navigation a[href^="/groups/discover"]
...
waitForKeyElements ('#photos div.swipe__element--1TlCJ', imgInfo);
