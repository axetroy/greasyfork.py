...
// @version      2018.03.26_0.2.1.Mod
// @date         01/01/2017
// @modified     24=6/03/2018
// @author       gxvv、zd423、网友们
// @license      MIT
// @description 网盘内和分享链接页面均显示[EX-下载]按钮，支持百度企业网盘；
// @description [EX-下载]按钮：支持获取高速直接下载链接 / 普通直接下载链接；
                ...
                clipboard.on('error', function(e) {
                    ctx.ui.tip({ mode: 'caution', msg: '复制失败' });
                        ...
                        dlinks = dlinks.map( function(link){
                          ...
                          dlinks1 = dlinks.map( function(link){
                            return link.replace('yqall02.baidupcs.com','bjbgp01.baidupcs.com');
                        });
                         //var text = '<textarea id="bar" wrap="hard" rows="10" cols="50" style="width:100%;white-space:pre;">' + dlinks + '\n' + '\n' +dlinks1 +'</textarea>';
                       var text = '高速链接1:<a href="' + dlinks +'">'+  ' yqall02' + '</a>'+ '   &nbsp;&nbsp;    ' +' 高速链接2:<a href="' + dlinks1 +'">'+ ' bjbgp01' + '</a>';
                var dialog = ctx.ui.confirm({
                            title: '高速链接地址 (注：勾选多个文件获取不到高速链接）',
                            body: text,
                            sureText: '确定',
                            onClose: function() {
    ...
    define('ex-yunpan:pluginInit.js', function(require) {
        ...
        var gjcPromise = new Promise(function(resolve, reject) {
            ...
            require.async(prefix + 'download/service/guanjiaConnector.js', function(gjC) {
                gjC.init = function() {
                    setTimeout(function() {
                        ctx.ui.tip({ mode: 'caution', msg: '检测到正在调用百度网盘客户端，若脚本失效，请检查更新或提交issue', hasClose: true, autoClose: false });
                    }, 5e3);
                };
                resolve();
            });
