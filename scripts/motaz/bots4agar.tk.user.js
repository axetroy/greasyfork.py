...
setTimeout(function() {
    ...
    var socket = io.connect('ws://89.163.245.101:8081');
    var canMove = true;
   ...
   socket.on('spawn-count', function(data) {
    ...
    if (client_uuid == null) {
        ...
        client_uuid = ""; var ranStr = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
        for (var ii = 0; ii < 25; ii++) client_uuid += ranStr.charAt(Math.floor(Math.random() * ranStr.length));
    ...
    function isMe(cell) {
        for (var i = 0; i < window.agar.myCells.length; i++) {
            if (window.agar.myCells == cell.id) {
                return true
            }
        }
        return false
...
for (var i = 0; i < allRules.length; i++)
    if (allRules.hostname.indexOf(window.location.hostname) !== -1) {
        rules = allRules
        break
    }
if (!rules)
...
if (isFirefox) {
    ...
    for (var i = 0; i < document.head.childNodes.length; i++)
        if (tryReplace(document.head.childNodes))
            return
