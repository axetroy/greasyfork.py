...
(function main() {
  ...
  const pfb = {
    ...
    selectorsList: {
      currentWorkHeartSelector: 'figure section > div:nth-child(3)',
      ...
      heartButtonSelector: 'button.ulIVIAn',
      heartImgSelector: '._one-click-bookmark',
      placeIllustrationSelector: '#root figure > div > div section',
      ...
      anchorIllust: 'a[href*="/member_illust.php?mode=medium&illust_id="]',
    },
    regexList: {
      novelPath: new RegExp('^\\/novel\\/show\\.php$', 'g'),
      novelPath2: new RegExp('\\/novel\\/show\\.php\\?.*id=\\d+.*'),
      illustPath: new RegExp(
        '\\/member_illust\\.php\\?.*(mode=medium.*illust_id=\\d+)|(illust_id=\\d+.*mode=medium).*',
        'g',
      ),
    },
    urlList: {
      ...
      class Mini extends rc {
        ...
        detectHeart(e) {
          ...
          if (pfb.data.isReactApp) {
            ...
            const prms = new URLSearchParams(href.match(/\?.+/g)[0]);
            const illId = prms.get('illust_id') || prms.get('id');
            id = +illId;
    ...
    runObserver() {
      ...
      const observer = new MutationObserver(() => {
        ...
        if ((elementIllust && isIllust) || (elementNovel && isNovel)) {
          const mc = isIllust ? MAIN_ID : 'pfb-nv';
          if (!document.getElementById(mc)) {
            const illustSearchParams = new URLSearchParams(window.location.search);
            const illustId = +illustSearchParams.get(isIllust ? 'illust_id' : 'id');
