	...
	if(dir.indexOf('|') != -1){
		if(w && w.location.hostname != 'translate.google.com'){
			...
			if(encTxt.length < 1900){
				createFrame(w, 'https://gtl-proxy.appspot.com/translate_t?text='+encTxt+'&hl='+lng+'&langpair='+dir+'&eotf=0&tbb=1&ujs=gtt');
			}
			else{
				var l = dir.split('|');
				createFrame(w, '');
				createForm(w, 'https://gtl-proxy.appspot.com/?sl='+l[0]+'&tl='+l[1]+'&hl='+lng+'&eotf=0&ujs=gtt', 'text', txt);
			}
		}
		else{
			window.open('https://translate.google.com/translate?u='+url+'&hl='+lng+'&langpair='+dir+'&tbb=1'+(document.charset ? '&ie='+document.charset : ''));
		}
	}
	else{
...
// for compatibility
if(window.top == window.self)window.addEventListener('message', function(e){
	if(e.data && e.data.indexOf('ujs_google_translate') == 0)ujs_google_translate(e.data.slice(21));
}, false);

// window.google_translate = ujs_google_translate; 