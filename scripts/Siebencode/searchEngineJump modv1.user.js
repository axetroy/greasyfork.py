// ==UserScript==
// @name           searchEngineJump modv1
// @author         NLF & iqxin and siebencode 
// @contributor    iqxin
// @description    Conveniently jump between various search engines, add visual settings menu, can be more friendly customization, and others. adapted for my use.
// @version        5.17.11
// @created        2011-07-02
// @lastUpdated    2018-09-04

// @namespace      https://greasyfork.org/en/scripts/27752-searchenginejump
...
(function () {
    ...
    function iqxinstart(){
        // Insertar la lista del motor de busqueda en el sitio web especificado segun las reglas.
        var rules = [
             // Busqueda web ////////////// la primera se puede ver como una plantilla
            {name: "google Search",// El nombre del sitio web que desea cargar (para su propia conveniencia)
                // ¿Esta habilitado?
                enabled: true,
                // En que sitio web cargar, regular.
                url: /^https?:\/\/www\.google(?:\.[A-z]{2,3}){1,2}\/[^?]+\?(?!tbm=)(?:&?q=|(?:[^#](?!&tbm=))+?&q=)(?:.(?!&tbm=))*$/,
                // Que tipo de lista cargar:
                // ['web'|'music'|'video'|'image'|'download'|'shopping'|'translate'|'knowledge'|'sociality']
                engineList: 'web',
                // Si esta fijado a la barra superior, ¿se le da una altura?
                // fixedTop: 100,
                // estilo para la lista de motores
                style: '\
                    margin-left: 142px;\
                    z-index: 100;\
                    margin-top:5px;\
                ...
                // insertar el documento, relevante
                // target inserta la barra de herramientas de salto de motor en un elemento del documento
                    // (Utilice xpath para hacer coincidir, por ejemplo: '// * [@ id = "subform_ctrl"]' o css match (agregue el prefijo 'css;'), por ejemplo: 'css; #subform_ctrl');
                // 'Keyword' Use 'xpath' o 'css' para seleccionar un elemento 'form input' o el elemento es una funcion que usa el valor de retorno
                // where cuatro tipos:
                    // 'beforeBegin' (insertado delante del elemento dado);
                    // 'afterBegin' (como el primer elemento secundario del elemento dado);
                    // 'beforeEnd' (como el ultimo elemento secundario del elemento dado);
                    // 'afterEnd' (insertado despues del elemento dado);
                insertIntoDoc: {
                    target: 'css;#appbar',
                    // keyword: function () {
                ...
                // Modificar la pagina de origen para adaptarla a la barra de salto
                stylish: 'body.vasq #hdtbMenus.hdtb-td-o{top:100px !important}'
            },
            {name: "google-hash-query", // No actualice Google de los resultados de busqueda que se muestran en la pagina.
                enabled: true,
            ...
            //Go home without a network, use the 8090 port offline test.
            {
                name:"test",
                enabled:true,
        ...
        // engineList.web[0] El numero del medio indica la clasificacion (el numero no se puede repetir, de lo contrario, este ultimo sobrescribira el frente), cuanto mas pequeño sea el numero, menor sera el numero, menos de 0 el motor no se mostrara en la pagina.
        engineList.web[0] = {
            //Search engine name
            name: 'Google',
            //Search engine address, keyword variable replaced with %s
            url: 'https://www.google.com/search?q=%s&ie=utf-8&oe=utf-8',
            //Search engine site icon
            ...
            favicon: icon.google,
            //Search engine encoding (default UTF-8), if jumbled after garbled, you can fill in'gbk'.
            encoding: 'utf-8',
            // New tab open
            // blank:true,
            // disable
        ...
        // Use this list to hide by default.,
        engineList.mine = [];
        ...
        engineList_plus[0] = {
            "status":3,
            "version":1,
            "message":"App download, organized by runningcheese",
            "name":"application",
            "engineDetails":['application',"app_xin",true],
            "engineList":[
                {
                    name: 'AppStore',
        ...
        engineList_plus[1] = {
            "status":3,
            "version":1,
            "engineDetails":['ebook',"ebook_xin",true],
            "message":"Finished by runningcheese",
            "engineList":[
                // {
                //     name: 'jiumodiary',
        ...
        engineList_plus[2] = {
            "status":3,
            "version":1,
            "engineDetails":['netdisc',"netdisc_xin",true],
            "message":"Finished by runningcheese,https://www.runningcheese.com/resources",
            "engineList":[
                // {
                //     name: 'jiumodiary',
                //     url: 'https://www.jiumodiary.com/$post$q',
...
//
        ...
        var settingData = {
            "status":1,
            "message":"$related notes$(status: This may be very important in the future)..."+
                    "(version: If a new feature is added, it will be identified by this version number)..." +
                    "(addSearchItems: Al actualizar, agregue una nueva busqueda a su lista de busqueda)..." +
                    "(modifySearchItems: Al actualizar, modifique los elementos en su lista de busqueda)..." +
                    "(connectToTheServer: Permitir conectarme a mi servidor (lista de actualizacion, convertir iconos a base64, etc.) for future updates or never)..." +
                    "(closeBtn: Establezca el circulo de 'cierre' en la esquina superior derecha de la pagina para mostrar.True display, False hide)..." +
                    "(newtab: New tab opens.0 for default settings, 1 for new tab pages)..." +
                    "(foldlist: Contraer la lista de categoria de busqueda actual.true contraido,false expandir)..." +
                    "(settingOpacity: Establecer la transparencia del boton,The number between 0-1,0 transparent,1 full display,The middle value is translucent. Note: -1 is a direct closing button,Before closing, please make sure you know how to open it again)..." +
                    "(debug: debug Pattern,After opening,The console will output some information,“Close and save” El boton no actualizara la pagina)..." +
                    "(fixedTop: Repare la barra de busqueda en la parte superior. true open,false close)..." +
                    "(baiduOffset: El menu que el mouse ha cruzado en la pagina de Baidu aparecera como desplazado,Si tienes otros estilos de 'estilo',Puede modificar esto para reparar el desplazamiento del menu de dos niveles)..." +
                    "(getIcon: Como obtener el icono despues de agregar su propia busqueda.0 is automatic，Se puede conectar a Google en caso de acceso a Google，No puede conectarse，Domain name plus favicon.ico get；1 get favicon for domain name，2 get access to Google，3 is obtained for services using dnspot (not recommended).Or add a URL, the keyword is replaced with %s, not tested)...." +
                    "(engineDetails: El primer valor es el nombre del titulo de la lista clasificada,The second value is associated with 'enginelist',Debe coincidir con,El tercer valor 'true' es la lista de visualizacion,false lista deshabilitada.Ordenado en el mismo orden en que aparecen en jump bar，Puede usarlo para ordenar la lista de acuerdo con sus propias preferencias)..." +
                    "(engineList: Relevant information of search)" +
                    "(rules: Insertar el estilo de busqueda en la pagina web de destino,Lo mismo que la configuracion de 'rules' en el script,La prioridad alta.With 360 search, it can be imitated)...",
            "version":3,
            "addSearchItems":true,
            ...
            "baiduOffset":-120,
            "getIcon":0,
            "engineDetails":[['web', 'web',true],['translate', 'translate',true],['knowledge', 'knowledge',true],['image', 'image',true],['video', 'video',true],['music', 'music',true],['scholar', 'scholar',false],  ['sociality', 'sociality',true],['shopping', 'shopping',true],["download","download",true],["news","news",false],['mine', 'mine',false]],
            "engineList":{},
        ...
        if(getSettingData){
            // console.log("Local presence list：",getSettingData); 
            if(!getSettingData.status && confirm("Setup error occurred,The script will restore the factory settings")){
                GM_deleteValue("searchEngineJumpData");
                window.location.reload();
            }

            // Check if the local configuration information is complete.
            for(let value in settingData){
                if(!getSettingData.hasOwnProperty(value)){
                    console.log("Attribute does not exist： ",value);
                    getSettingData[value] = settingData[value];
                    GM_setValue("searchEngineJumpData",getSettingData);
                }
            }

            // Get version for search list update
                // console.log("Current version number and target version number: ",getSettingData.version,settingData.version);
            if(parseFloat(getSettingData.version) < settingData.version){
                console.log("Low version,Start to update,Current version number and target version number: ",getSettingData.version,settingData.version);
                ...
                // update the local version of other relevant information
        ...
        function modifySearchItemsRuleFun(name,value){
            ...
            for(let item in oldRule){
                if(oldRule[item].name == name){
                    console.log("successful compilation, Update rule : ", name);
                    oldRule[item] = value;
                    GM_setValue("searchEngineJumpData",getSettingData);
                }
            }
        ...
        // --------------------Establecer el final del elemento------------------------
        // console.log("engineList: ",engineList);
        //xpath Obtener un solo elemento
        ...
        // Obtiene la cadena de multiples filas de la funcion
        function getMStr(fn) {
        ...
        //Deteccion de soporte de eventos.
        // Por ejemplo eventSupported('fullscreenchange', document);
        function eventSupported(eventName, elem) {
            ...
            while(l --) {
                ...
                } else if (typeof elem.setAttribute == 'function') { // setAttribute //Es el metodo del nodo del elemento
                    setAttr = false;
        ...
        // Guardar datos sobre el objeto especificado
        var data = (function () {
            'use strict';
            ...
            function data(obj, key, value) {
                ...
                if (!cache.data[id]) {	//initialization
                    cache.data[id] = {};
                };
        ...
        // Make compatibility for mouseleave mouseenter event
        ...
        var mouseEventListener = (function () {
            ...
            return {
                add : function (type, ele, callback) {	//event type, element, listener function
                    if (support[type]) {
                        ele.addEventListener(type, callback, false); //'mouseleave', 'enter' no burbujea, asi que escuche los eventos durante la fase de burbujeo, y no se preocupe por lo que sucede cuando los descendientes entran y salen.
                    } else {
                        var listener = data(callback, 'mouseELListener');
                        if (!listener) {
                            listener = function (e) {
                                var relatedTarget = e.relatedTarget; //mouseout,el elemento para ir;mouseover,el elemento de
                                // Cuando 'mouseout' (deja ele) el elemento no es su propio elemento descendiente
                                // Cuando 'mouseover' (enter ele) proviene de un elemento que no es su propio elemento descendiente
                                if (!ele.contains(relatedTarget)) {		//funcion contains, propia. contains (self) devuelve true
                                    callback.call(ele, e);
                                };
                            };
        ...
        function mousedownhandler(e) {
         ...
         //Obtenga el HTML del formulario POST
        ...
        //Primero determine las reglas del usuario
        ...
        if (!iTarget || !iInput) {
            console.log("Script searchEngineJump search engine shortcut jump encountered errors： ");
            console.log("Incorrect target： iTarget：" + iTarget + "\niInput: " + iInput);
            return;
        }
        ...
        engineList.details.forEach(function (item) {
            ...
            engineList[category].forEach(function (engine) {
                // Check whether to search a website site:xxx.xx with search
                ...
                if (!siteMark && matchedRule.url.test(engineUrl)) return;	// remove the engine that jumps to the current engine
            ...
            // Expand the current search category list
        ...
        //Inserte cada lista de busqueda en el documento.
        ...
        // todo: Repita aqui e inserte dos veces en la pagina de Baidu.
        if(/^https?:\/\/www\.baidu\.com\/(?:s|baidu)/.test(url)){
        ...
        // Cambie el estilo de la pagina de origen porque no es compatible con el estilo de la pagina que desea insertar.
        ...
        if(getSettingData.fixedTop){
            ...
            function fixedTopFun(height){
                ...
                if(scrollTop <= objTop){
                    ...
                    //Si no se establecio ningun color antes, la configuracion predeterminada es blanco.
                    ...
                    // Conozca la situacion del sitio web, use 'margin' para centrar
                    if(marginRight === marginLeft && marginRight != 0){
                        obj.style.left = marginLeft + "px";
                    }
                    // La situacion de Taobao y otros sitios web usando text-align centrado
        ...
        SEJsetting.prototype = {
            ...
            init: function () {
                ...
                this.dragEvent();
                this.setDragNode(this.ele); //deslizar
                // input[range]
            ...
            addContent: function(){
                var aPattern = '<span draggable="true" class="drag">' +
                                ...
                                ' <span class="iqxin-set-edit" title="Editar"><img class="sej-engine-icon" src="data:image/png;base64,"/></span>' +
                ...
                // Si lo obtiene de acuerdo con la longitud de la matriz, el numero negativo es el atributo, no la longitud de la longitud, por lo que un numero general, cuando todos estan vacios, finalice el ciclo
                ...
                for (let i=0;i<detailsLength;i++){
                    ...
                    oDivTitle.innerHTML ='<span class="iqxin-pointer-events">' + details[j][0] + '</span>' +
                                        '<span class="iqxin-title-edit" title=" Editar"><img class="sej-engine-icon" src="' + icon.edit + '"/></span>'+
                                        ' <span class="iqxin-set-title-del" title=" Delete"><img class="sej-engine-icon" src="' + icon.del + '"></span>';
                ...
                var btnStr2 = "<div>" +
                            "<span id='xin-reset' title='Attention,Factory reset'>Clear settings</span>" +
                            "<span id='xin-modification' title='edit Comparte tu propia configuracion o borra tu configuracion'>configuration file</span>" +
                            "<span id='xin-importing' title='importing Import a more professional search engine'>Import</span>" +
                            // "<span id='iqxin-debugS' title='It has a certain effect on setting menu.'>" +
                            //     "<label>debug<input id='iqxin-debug' type='checkbox' name='' " +
                            //         debug_checked +
                            //     " style='vertical-align:middle;'></label>" +
                            // "</span>" +
                            "<span id='xin-foldlists'>" +
                                "<label>Plegable categoria de busqueda actual<input id='iqxin-foldlist' type='checkbox' name='' " +
                                    foldlist_checked +
                                " style='vertical-align:middle;'></label>" +
                            "</span>" +
                            "<span id='iqxin-fixedTopS' title='fixedTop Al desplazarse por la pagina,Solucionar (Fixed top) parte superior de la pagina.Hay un problema con el estilo de algunas paginas'>" +
                                "<label>Fixed to the top<input id='iqxin-fixedTop' type='checkbox' name='' " +
                                    fixedTop_checked +
                                " style='vertical-align:middle;'></label>" +
                            "</span>" +
                            "<span id='xin-setBtnOpacity' title='Set button transparency'>Set button transparency <input type='range' step='0.01'  min='0' max='1' value='"+ (getSettingData.setBtnOpacity<0?-getSettingData.setBtnOpacity:getSettingData.setBtnOpacity) +"' id='setBtnOpacityRange'><i style='display:inline-block;width:3em;text-align:center;' class='iqxin-setBtnOpacityRangeValue' title='按钮 显示/隐藏(非透明)),请确定知道自己如何再次打开; 火狐非高级玩家建议别禁用'></i></span>" +
                ...
                var btnStr = "<div class='btnEleLayer'>" +
                            "<span class='feedback'><a target='_blank' href='https://greasyfork.org/es/scripts/27752-searchenginejump'>feedback greasyfork</a></span>" +
                            "<span class='feedback'><a target='_blank' href='https://github.com/qxinGitHub/searchEngineJump'>feedback GitHub</a></span>" +
                            "<span id='moreSet' title='more set'>More settings</span>" +
                            "<span id='xin-newtab' title='open newtab. Do you want to open the new tab page?'>Open mode：" +
                                "<select id='iqxin-globalNewtab'>" +
                                    "<option value='globalDef'>Default page</option>" +
                                    "<option value='globalNewtab'" + (getSettingData.newtab?"selected":"")  + ">New tab page</option>" +
                                "</select>" +
                            "</span> " +
                            "<span id='xin-addDel' title='add & del !Add new or delete existing search!'>Add / Delete</span> " +
                            "<span id='xin-save' title='save & close'>Save and close</span>" +
                            "</div>";
                btnEle.innerHTML = btnStr;
                this.ele.appendChild(btnEle);

                // deslizar la barra superior
                ...
                nSearchList.innerHTML = "Add a new search list";
            ...
            hide: function(){
                this.allBoxClose(); // Close all secondary windows, menus
            ...
            reset: function(){
                if(confirm("La configuracion del usuario sera eliminada！")){
                    GM_deleteValue("searchEngineJumpData");
                    window.location.reload();
                }
            },
            ...
            addDel: function(e){
                if (e.target.classList.contains("iqxin-btn-active")){
                    ...
                    // console.log("Does not exist, increase or increase");
            ...
            // Interface, frame: add new search.
            addItemBox: function(bool){
                ...
                newDiv.innerHTML=""+
                    "<span>Title&nbsp;&nbsp;&nbsp&nbsp&nbsp&nbsp&nbspTitulo : </span><input id='iqxin-newTitle' placeholder='Required' onfocus='this.select()' /> <br/><br/>" +
                    "<span>Link&nbsp;&nbsp;&nbsp&nbsp&nbsp&nbsp&nbspUrl : </span><input id='iqxin-newLink' placeholder='Required' onfocus='this.select()' /> <br/><br/>" +
                    "<span>Icon&nbsp;&nbsp;&nbsp&nbsp&nbsp&nbsp&nbspIcono : </span><input id='iqxin-newIcon' placeholder='Opcional, dejelo en blanco y recibalo automaticamente' onfocus='this.select()' /> <br/><br/>" +
                    "<span>Open mode : " +
                        '<select id="iqxin-newTarget" style="border-radius: 4px;border: none;padding: 2px 0 2px 2px"> ' +
                        '<option value="default">New tab open</option> ' +
                        '<option value="newtab">Abrir en pagina actual - Current page</option> ' +
                        '<select> ' +
                    "</span>" +
                    "<br/><br/>" +
                    "<span style=''><a target='_blank' style='color:#999;' href='https://greasyfork.org/es/scripts/27752-searchenginejump'>Related instructions</a></span>" +
                    "&nbsp;&nbsp;&nbsp&nbsp&nbsp&nbsp&nbsp;" +
                    "<button id='addItemBoxEnter' class='addItemBoxEnter addItemBoxBtn iqxin-enterBtn'>Ok.Aceptar</button>&nbsp;&nbsp;&nbsp&nbsp&nbsp;&nbsp" +
                    "<button id='addItemBoxCancel' class='addItemBoxCancel addItemBoxBtn iqxin-closeBtn'>Cancel</button>" +
                    "";

                this.ele.appendChild(newDiv);
                document.querySelector("#iqxin-newTitle").focus();
            },
            // Internal logic: add new search.
            addItemEnger: function(){
                ...
                var a = '<span class="sej-engine"' +
                            ...
                            '<span class="iqxin-set-edit" title="Editar">' +
                ...
                // Add complete, remove the add box
                this.addItemBoxRemove();
            },
            ...
            getICON: function(olink){
                ...
                debug("Can it connect to Google：",this.online);
                if (this.online){
            ...
            // Interface, box: Add a new search list
            addSearchListBox: function(){
                ...
                newDiv.innerHTML=""+
                    "<span>search List Name: </span><input id='iqxin-newSearchListName' onfocus='this.select()'>" +
                    "<br><br>" +
                    "<span>search List InternalName: </span><input id='iqxin-newSearchListInnerName' onfocus='this.select()' value='" + hash + "'>" +
                    "<br><br>" +
                    "<button id='addSearchListBoxEnter' class='addSearchListBoxEnter addItemBoxBtn'>Ok.Aceptar</button>&nbsp;&nbsp;&nbsp&nbsp&nbsp;&nbsp" +
                    "<button id='addSearchListBoxCancel' class='addSearchListBoxCancel addItemBoxBtn'>Cancel</button>" +
                    "";
                this.ele.appendChild(newDiv);
            ...
            addSearchListEnger: function(){
                ...
                if(innerName.length===0){
                    alert("El nombre interno no puede estar vacio");
                    return;
                }
                if(name.length===0){
                    name = innerName;
                ...
                odiv.innerHTML = "" +
                    '<div class="sejtitle" data-iqxintitle="' + innerName + '" data-xin="99">' +
                        ...
                        '<span class="iqxin-title-edit" title="Editar">' +
                            '<img class="sej-engine-icon" src="' + icon.edit + '">' +
                        '</span> ' +
            ...
            addEditBox: function(e){
                ...
                var strblank;
                if(otarget){
                    strblank = '<option value="default">New TAB opens</option><option value="newtab">Current page open/abrir en pagina actual</option> ';
                } else{
                    strblank = '<option value="default">New TAB opens</option><option value="newtab" selected="selected">Current page open/abrir en pagina actual</option>';
                }
                ...
                var innerHTML=""+
                    "<span>Title&nbsp;&nbsp;&nbsp&nbsp&nbsp&nbsp&nbspTitulo : </span><input id='iqxin-newTitle' placeholder='Required' onfocus='this.select()' value='"+ otitle +"' /> <br/><br/>" +
                    "<span>Link&nbsp;&nbsp;&nbsp&nbsp&nbsp&nbsp&nbspUrl : </span><input id='iqxin-newLink' placeholder='Required' onfocus='this.select()' value='"+ olink +"' /> <br/><br/>" +
                    "<span>Icon&nbsp;&nbsp;&nbsp&nbsp&nbsp&nbsp&nbspIcono : </span><input id='iqxin-newIcon' placeholder='Opcional, dejelo en blanco y recibalo automaticamente' onfocus='this.select()' value='"+ oicon +"' /> <br/><br/>" +
                    "<span>Open mode : " +
                        '<select id="iqxin-newTarget" style="border-radius: 4px;border: none;padding: 2px 0 2px 2px"> ' +
                            '$strblank$' +
                        '<select> ' +
                    "</span>" +
                    "<br/><br/>" +
                    "<span style=''><label>Disabled：<input type='checkbox' name='' id='iqxin-newDisabled' $checked$ style='vertical-align:middle;'></label></span>" +
                    "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" +
                    "<button id='editItemBoxEnter' class='editItemBoxEnter addItemBoxBtn iqxin-enterBtn'>Ok.Aceptar</button>&nbsp;&nbsp;&nbsp&nbsp&nbsp;&nbsp" +
                    "<button id='addItemBoxCancel' class='addItemBoxCancel addItemBoxBtn iqxin-closeBtn'>Cancel</button>" +
                    "";
            ...
            addEditBoxEnger: function(){
                ...
                // Modify to completely, remove add box
                this.addItemBoxRemove();
            },
            ...
            addTitleEditBox: function(e){
                ...
                // Hay un titulo de editor &&; el nodo antes de la edicion es el mismo nodo que el nodo cliqueado.
                if(flag && flag.parentNode == element){
                        element.innerHTML = element.firstChild.value?element.firstChild.value:"empty";
                        element.classList.add("iqxin-pointer-events");
                } else {
                    // Hay un titulo de editor, pero no es el mismo nodo que el que hizo clic.
                    if(flag){
            ...
            addTitleEditBoxRemove:function(){
                ...
                if(odiv){
                    odiv.parentNode.innerHTML = odiv.value?odiv.value:"Vacio";
                }
            },

            // Menu avanzado, interfaz de edicion de perfil
            editCodeBox: function(){
                console.log("Raw data - OriginalCode： ",getSettingData);
                ...
                var innerH = " "+
                    ...
                    "mas opciones de configuracion, mas libertad,</br>"+
                    "Sin embargo, una configuracion incorrecta hara que No se ejecute el script"+
                    "</p>" +
                    "<textarea wrap='off' cols='45' rows='20' style='overflow:auto;border-radius:4px;'>" + JSON.stringify(userSetting,false,4) + "</textarea>" +
                    "<br>" +
                    "<button id='xin-reset'>Reset.Clear Settings</button> &nbsp;&nbsp;&nbsp;" +
                    "<button id='xin-copyCode'>Copy</button> &nbsp;&nbsp;&nbsp;" +
                    "<button id='codeboxclose' class='iqxin-closeBtn'>Close</button> &nbsp;&nbsp;&nbsp;" +
                    "<button id='xin-codeboxsave' class='iqxin-enterBtn'>Ok.Aceptar</button>" +
                "";
            ...
            addImportingBox: function(){
                ...
                var a ="<p>Mas listas de busqueda subdivididas, habra superposicion entre las listas</p>" +
                       "<ul>";
            ...
            addImportingEnger:function(e){
                // console.log(e);
                // console.log("los primeros： ",e.target.getAttribute("xin-importing-id"));
            ...
            setBtnOpacityFun: function(){
                if(~window.navigator.userAgent.indexOf("Chrome")){
                    ...
                    if(getSettingData.setBtnOpacity<0){
                        ...
                        document.querySelector(".iqxin-setBtnOpacityRangeValue").innerHTML = "Disable";
                    ...
                    iqxinShowTip("Sorry, currently only Chrome browsers are supported",2500);
                }
            },

            //Title click (switch search list) (can be incorporated into the click event below)
            titleClick: function(e){
                var target = e.target;
                target.dataset.xin = -parseInt(target.dataset.xin);
            },
            // Click event here if need to be replaced with elseif (switch) according to the actual situation
            domClick: function(e){
                var targetClass = e.target.className;
                var targetid = e.target.id;
                debug("Click on the event：%o, ID: %o, class: %o, e: %o",e.target,targetid,targetClass,e)
                ...
                if(targetid ==="codeboxclose"){
                    ...
                    iqxinShowTip("Successful copy.Ok!");
                }
                ...
                // Close the "Settings Menu Button"
            ...
            // Close all secondary windows and menus.
            allBoxClose: function(){
                this.addItemBoxRemove(); //New search add box
                this.addDelremove();  //  Add/remove interface
                this.editCodeBoxClose(); // Code edit box
                this.addTitleEditBoxRemove(); // Title edit box
                this.addItemBoxRemove("#newSearchListBox"); // Add a new search list
                this.boxClose("#iqxin-sortBox"); // Search list sort
                this.addItemBoxRemove("#importingBox"); //Import box
                document.querySelector("#btnEle2").classList.remove("btnEle2active"); // More settings
            },
            ...
            setDragNode: function(ele) {
                ...
                node.addEventListener("mousedown",function(event){
                    ...
                    // offsetLeft es la posicion de 'body', y la 'dis' resultante es la posicion del mouse en la esquina superior izquierda de la ventana.
                    var disX = event.clientX - ele.offsetLeft;
                    var disY = event.clientY - ele.offsetTop;

                    var move = function(event) {
                        //La posicion del mouse se resta de la posicion en la esquina superior izquierda, es decir, la posicion de la ventana
            ...
            domdrop:function (e) {
                ...
                // Prevent cross-regional movement
                SEJsetting.prototype.domdropend();
                if(dragEl.className != that.className){
                    console.log("Antes de mover objetos, ahora: ", dragEl.className);
                ...
                var width = targetRect.right - targetRect.left; //Ancho del nodo objetivo
                var height = targetRect.bottom - targetRect.top; //Altura del nodo objetivo
                ...
                // Volver a vincular arrastrar y soltar eventos
            ...
            // Determine si puede conectarse a google
            isOnline: function(){
            ...
            // Set button transparency settings
            ...
            saveData: function(){
                //
                this.addTitleEditBoxRemove(); //The title bar is in edit mode.
                ...
                // En caso de cualquier eventualidad, vuelva a adquirir el archivo de configuracion local 
                ...
                debug('Data to be saved：',getData);
            ...
            addGlobalStyle: function(){
                ...
                var css =
                    ...
                    "#titleEdit{" +
                        "width:6em;" +
                    "}" +
                    // Button effect ： OK Cancel button
                    ".iqxin-closeBtn," +
                    ".iqxin-enterBtn{" +
                        "box-sizing: border-box;" +
                    "}" +
                    ...
                    "input[type=range] {" +
                        ...
                        "border-radius: 10px; /*Esta propiedad esta configurada para llenar la barra de progreso con una esquina redondeada*/" +
                    "}" +
                    ...
                    "input[type=range]::-webkit-slider-runnable-track {" +
                        "height: 10px;" +
                        "border-radius: 10px; /*Set the track to rounded*/" +
                        "box-shadow: 0 1px 1px #def3f8, inset 0 .125em .125em #0d1112; /*Seguir el efecto de sombra incorporado*/" +
                    "}" +
                    "input[type=range]::-webkit-slider-thumb {" +
                        "-webkit-appearance: none;" +
                        "height: 18px;" +
                        "width: 18px;" +
                        "margin-top: -5px; /*Haga que el desplazamiento del control deslizante sea igual a la parte del seguimiento*/" +
                        "background: #fff; " +
                        "border-radius: 50%; /*La apariencia se establece en un circulo*/" +
                        "border: solid 0.125em rgba(205, 224, 230, 0.5); /*Establecer borde*/" +
                        "box-shadow: 0 .125em .125em #3b4547; /*Agregar sombra inferior*/" +
                    "}" +
                    // Import
                    "#importingBox{" +
                        "position:fixed;" +
        ...
        // Registration menu
        GM_registerMenuCommand("search jump Settings",setBtnStart);
        ...
        // Obtener informacion de datos almacenados
        function get_data(){
    ...
    // Cuadro de aviso de mensaje
        // Currently only to give users a feedback. - Has it succeeded? - Well, it’s successful.
    var iqxinTimerGlobalTip = null
    function iqxinShowTip(text,duration){
        console.log("iqxin -- Message prompt: ", text);
        ...
        //Prevent multiple triggers from being triggered for the duration
    ...
    // hash-query  Buscar sin refrescar la pagina
        // hashchange y popstate There is no way to detect changes in URLs when Google and Baidu search, and do not understand.
    if (window.self != window.top) return;
    // De la siguiente manera, Baidu no ha podido usar el informe incorrecto.
    // if (true) {
    //     console.log('iqxin Agregar monitor de nodo de titulo: title');
    ...
    //         console.log('iqxin The title has changed', document.title);
    ...
    // }
    // Add a list to the homepage of Google and Baidu search separately.
    ...
    if (hashtag){
        ...
        var timer = setInterval(function(){
             // console.log("En el bucle",newTitle,oldTitle);
            ...
            if (document.querySelector("#appbar") && !document.querySelector("sejspan")){
                ...
                // console.log("No existe para comenzar a insertar");
                iqxinstart();
            }
        },1000)
    } else {
