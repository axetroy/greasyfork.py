...
// @version      0.2
// @description  DUI json 美化
// @author       jason
// @match        *://www.dui.ai/*
// @require      https://code.jquery.com/jquery-3.2.1.min.js
// @require      https://cdn.bootcss.com/lodash.js/4.17.4/lodash.min.js
// @resource     jsonviewCSS https://raw.githubusercontent.com/yesmeck/jquery-jsonview/master/dist/jquery.jsonview.min.css
// @grant        GM_addStyle
...
(function() {
    'use strict';
    const throttleEcho = _.throttle(msg => {
        console.log(msg);
    }, 3000);
    // Copy text as text
    ...
    const jsonviewCSS = GM_getResourceText("jsonviewCSS");
    GM_addStyle(jsonviewCSS);
    GM_addStyle(`
.control:after {
display: block;
content: ' ';
...
#dmJson.fullscreen, #dmJsonView.fullscreen {
position: fixed;
top: 0;
left: 0;
width: 100%;
...
`);
    let jsonObj = null;
    window.requestAnimationFrame(function mySelf (){
        if($('#dmJson').length > 0) {
            try {
                if ($('#dmJsonView').length === 0) {
                    $('#dmJson').after('<div id="dmJsonView"></div>');
                }
                if ($('#jsonview-control').length === 0) {
                    $('#dmJson').before('<p id="jsonview-control" class="control"><button class="view-source el-button float_right el-button--text">源码</button><button class="fullscreen-view el-button float_right el-button--text">全屏</button><button class="copy-json el-button float_right el-button--text">复制</button></p>');
                    $('#jsonview-control').on('click', '.view-source', function () {
                        $('#dmJson').show();
                        $('#dmJsonView').hide();
                        $('#jsonview-control').html('<button class="view-formated el-button float_right el-button--text">格式化</button><button class="fullscreen-view el-button float_right el-button--text">全屏</button><button class="copy-json el-button float_right el-button--text">复制</button>');
                    });
                    $('#jsonview-control').on('click', '.view-formated', function () {
                        $('#dmJson').hide();
                        $('#dmJsonView').show();
                        $('#jsonview-control').html('<button class="view-source el-button float_right el-button--text">源码</button><button class="fullscreen-view el-button float_right el-button--text">全屏</button><button class="copy-json el-button float_right el-button--text">复制</button>');
                    });
                    $('#jsonview-control').on('click', '.copy-json', function () {
                        executeCopy($('#dmJson').text());
                        alert('复制成功');
                    });
                    $('.json-pane').on('click', '.fullscreen-view', function () {
                        $('#topnav').hide();
                        $('.el-dialog').addClass('clear-position');
                        if ($('#dmJson').is(':visible')) {
                            $('#dmJson').addClass('fullscreen');
                            $('#dmJson').prepend('<p class="control"><button class="exit-fullscreen el-button el-button--text">取消全屏</button></p>');
                        } else {
                            $('#dmJsonView').addClass('fullscreen');
                            $('#dmJsonView').prepend('<p class="control"><button class="exit-fullscreen el-button el-button--text">取消全屏</button></p>');
                        }
                    });
                    $('.json-pane').on('click', '.exit-fullscreen', function () {
                        $('.el-dialog').removeClass('clear-position');
                        $('#topnav').show();
                        if ($('#dmJson').is(':visible')) {
                            $('#dmJson').removeClass('fullscreen');
                            $('#dmJson .control').remove();
                        } else {
                            $('#dmJsonView').removeClass('fullscreen');
                            $('#dmJsonView .control').remove();
                        }
                    });
                }
                let nowJsonObj = JSON.parse($('#dmJson').text());
                if (!_.isEqual(jsonObj, nowJsonObj)) {
                    jsonObj = nowJsonObj;
                    $('#dmJson').hide();
                    $('#dmJsonView').JSONView(jsonObj, { collapsed: true });
                };
            } catch (e) {
                throttleEcho(e);
            }
            requestAnimationFrame(mySelf);
        } else {
            requestAnimationFrame(mySelf);
        }
    });
