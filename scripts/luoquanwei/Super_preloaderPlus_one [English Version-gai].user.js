(function() {
// ==UserScript==
// @name         Super_preloaderPlus_one [English Version-gai]
// @namespace    https://github.com/ywzhaiqi
// @description  Endless scrolling with custom preloading ...
...
var scriptInfo = {
    ...
    homepageURL: 'https://greasyfork.org/scripts/29384-super-preloaderplus-one',
    downloadUrl: 'https://greasyfork.org/scripts/294-super-preloaderplus-one/code/Super_preloaderPlus_one.user.js',
    metaUrl: 'https://greasyfork.org/scripts/294-super-preloaderplus-one/code/Super_preloaderPlus_one.meta.js',
};
...
var SITEINFO_D={
    ...
    autopager: {
        ...
        maxpage: 99,            // 最多翻多少页..
        ipages: [false, 5],     // 立即翻页,第一项是控制是否在js加载的时候立即翻第二项(必须小于maxpage)的页数,比如[true,3].就是说JS加载后.立即翻3页.
...
var SITEINFO=[
    {name: 'Google搜索',                                                                                                                               //站点名字...(可选)
        ...
        autopager:{
            ...
            ipages: [false,5],                               //立即翻页,第一项是控制是否在js加载的时候立即翻第二项(必须小于maxpage)的页数,比如[true,3].就是说JS加载后.立即翻3页.(可选)
...
var setup = function(){
    ...
    div.innerHTML = '\
        <div>Super_preloaderPlus_one Settings</div>\
            <ul>\
                <li>The current version is <b>' + scriptInfo.version + ' </b>, The last update time is <b>'+ scriptInfo.updateTime + '</b>\
                    <a id="sp-prefs-homepageURL" target="_blank" href="' + scriptInfo.homepageURL + '"/>Script home page</a>\
                </li>\
                <li><input type="checkbox" id="sp-prefs-debug" /> Debug mode</li>\
                <li><input type="checkbox" id="sp-prefs-dblclick_pause" /> Double-click to pause the page (default Ctrl + left-click)</li>\
                <li><input type="checkbox" id="sp-prefs-enableHistory" /> Add next page to history</li>\
                <li title="The links on the next page are set to open in a new tab"><input type="checkbox" id="sp-prefs-forceTargetWindow" /> New tab Open link</li>\
                <li><input type="checkbox" id="sp-prefs-SITEINFO_D-useiframe" /> In read-ahead mode, iframe is enabled by default</li>\
                <li><input type="checkbox" id="sp-prefs-SITEINFO_D-a_enable" /> Auto paging is enabled by default </li>\
                <li><input type="checkbox" id="sp-prefs-SITEINFO_D-a_force_enable" /> Auto Flip Enables forced stitching by default</li>\
                <li>Customize the exclusion list：\
                    <div><textarea id="sp-prefs-excludes" placeholder="Custom exclusion lists that support wildcards. \nFor example：http://*.douban.com/*"></textarea></div>\
                </li>\
                <li>Custom site rules：\
                    <div><textarea id="sp-prefs-custom_siteinfo" placeholder="Custom site rules"></textarea></div>\
                </li>\
            </ul>\
        <div><button id="sp-prefs-ok">Confirm</button><button id="sp-prefs-cancel">Cancel</button></div>';
    div = null;
            ...
            <div id="sp-fw-content" style="display:none;">\
                <div id="sp-fw-main">\
                    <div id="sp-fw-main-head">\
                        <input type="checkbox" title="Use paging mode, otherwise use read-ahead mode" id="sp-fw-a_enable" name="sp-fw-a_enable"/>Use paging mode\
                        <span id="sp-fw-span-info">Super_preloader</span>\
                    </div>\
                    <fieldset>\
                        <legend title="Read-ahead mode settings" >Read-ahead settings</legend>\
                        <ul>\
                            <li>\
                                <input type="checkbox" title="Use iframe to pre-load the next page to the cache, otherwise use xhr request next page source, remove all the pictures for pre-reading" id="sp-fw-useiframe" name="sp-fw-useiframe"/>Use Iframe\
                            </li>\
                            <li>\
                                <input type="checkbox" title="Look at the pre-read content and display it at the bottom of the page to see whats pre-read." id="sp-fw-viewcontent" name="sp-fw-viewcontent"/>Look at the pre-read content\
                            </li>\
                        </ul>\
                    </fieldset>\
                    <fieldset id="sp-fw-autopager-field" style="display:block;">\
                        <legend title="Auto-page mode settings">Page up settings</legend>\
                        <ul>\
                            <li>\
                                <input type="checkbox" title="Use iframe way to flip, or use xhr way page, you can solve some pages xhr way can not flip the problem, if the xhr page normal, then do not hook this." id="sp-fw-a_useiframe" name="sp-fw-a_useiframe"/>Use Iframe</input>\
                                <input type="checkbox" title="Each next page with a new iframe, you can solve the next picture or button click problem" id="sp-fw-a_newIframe" name="sp-fw-a_newIframe">New Iframe</input>\
                                <ul id="sp-fw-a_useiframe-extend">\
                                    <li>\
                                        <input type="checkbox" title="Wait for the iframe to load completely (after the load event), remove the content, or in the DOM is completed, it is directly taken out .. (hook on, it will be slower, but may solve some problems.)" id="sp-fw-a_iloaded" name="sp-fw-a_iloaded" />Wait for the iframe to fully load\
                                    </li>\
                                    <li>\
                                        <input type="number"  min="0" title="In the data can be taken from the iframe, continue to wait for the set of milliseconds before starting to take out the data (this is a special web page preparation, if normal, set to 0)" id="sp-fw-a_itimeout" name="sp-fw-a_itimeout"/>Ms delay is removed\
                                    </li>\
                                </ul>\
                            </li>\
                            <li>\
                                <input type="checkbox" id="sp-fw-a_manualA" name="sp-fw-a_manualA" title="Will not automatically stitching up, there will be a similar page navigation of the graphics, click page (in the forum post content page, you can consider checking this, which does not affect your Replies)"/>Manual mode\
                            </li>\
                            <li>\
                                 Remaining<input type="number" min="0" id="sp-fw-a_remain" name="sp-fw-a_remain" title="When the height of the remaining pages is several times the height of the browsers visible window, the page is opened"/>Double page height trigger\
                            </li>\
                            <li>\
                                 Up to turn<input type="number" min="0" id="sp-fw-a_maxpage" name="sp-fw-a_maxpage" title="Up to the number of pages, when the number reached this page when the automatic page stop." />page\
                            </li>\
                            <li>\
                                <input type="checkbox" id="sp-fw-a_separator" name="sp-fw-a_separator" title="Split the navigation bar of the main content of the page, the main content of the page can be quickly jump between the positioning and so on."/>Show paging navigation\
                            </li>\
                            <li>\
                                <input type="checkbox" title="The next part of the body part of the contents of the entire stitching up. (When there is no need to turn the page of the site rules, the mandatory check, can not be canceled.)" id="sp-fw-a_force" name="sp-fw-a_force"/>Forced stitching\
                            </li>\
                            <li>\
                                <input type="checkbox" id="sp-fw-a_ipages_0" name="sp-fw-a_ipages_0" title="JS is loaded immediately after the continuous set behind the pages"/>Enabled \
                                Immediately turn<input type="number" min="1" id="sp-fw-a_ipages_1" name="sp-fw-a_ipages_1" title="The number of consecutive pages" />page\
                                <input type="button" value="Start" title="Now it is time to start a continuous page" id="sp-fw-a_starti" />\
                            </li>\
                        </ul>\
                    </fieldset>\
                    <div id="sp-fw-foot">\
                     <input type="checkbox" id="sp-fw-enable" title="Master switch, enable js, otherwise disable." name="sp-fw-enable"/>Enabled\
                     <span id="sp-fw-setup" class="sp-fw-spanbutton" title="Open the Settings window">Settings</span>\
    ...
    function autopager(SSS, floatWO) {
        ...
        function manualAdiv() {
            if (!manualDiv) {
                ...
                manualDiv = div;
                var span = $C('span', { class: 'sp-sp-md-span' }, 'Load');
                ...
                var input = $C('input', {
                    ...
                    value: 1,
                    min: 1,
                    title: 'Enter the number of pages you want to load (must be> = 1) then press Enter.',
                    id: 'sp-sp-md-number'
                });
                ...
                div.appendChild($C('span', { className: 'sp-sp-md-span' }, ' page(s)'));
        ...
        autoPO = {
            startipages: function(value) {
                if (value > 0) {
                    ipagesmode = true;
                    ipagesnumber = value + paged;
                    notice('<b>Status</b>:' + 'Loading pages <b>' + paged + '</b>' + '-<b style="color:red!important;">' + ipagesnumber + '</b>.');
...
function getRalativePageStr(lastUrl, currentUrl, nextUrl) {
    ...
    if (realPageSiteMatch) { //如果匹配就显示实际网页信息
        if (ralativePageNumarray[1] - ralativePageNumarray[0] > 1) { //一般是搜索引擎的第xx - xx项……
            ralativePageStr = ' [ Post:  <font color="red">' + ralativePageNumarray[0] + ' - ' + ralativePageNumarray[1] + '</font> ]';
        } else if ((ralativePageNumarray[1] - ralativePageNumarray[0]) === 1) { //一般的翻页数，差值应该是1
            ralativePageStr = ' [ Post: <font color="red">' + ralativePageNumarray[0] + '</font> page ]';
        } else if ((ralativePageNumarray[0] === 0 && ralativePageNumarray[1]) === 0) { //找不到的话……
            ralativePageStr = ' [ <font color="red">The actual page ends</font> ]';
        }
    } else {
        ralativePageStr = '';
    }
