...
// @name         Pilatinhas 
// @version      0.7
// @description  Exensão para o evento Piratas 
// @author       Miles Tailsko Prower
// @match        http://agar.io
// @match        https://agar.io
// @changes        Nenhuma ;)
...
var debugMonkeyReleaseMessage = "<h1>EAE SEUS PUTO!</h1>"+
    "SAMPLE TEXT <br><p>" +
	"ALEQUIS INUTIL <br><p>" +
	"ANONYMOUS DITADOR <br><p>" +
    "Chega mais parsa, essa extensão é para o evento Piratinhas no domingo (10/01/16) <br><p>" +
	"..................<p>" +
	"FAQ: <p>" +
	"<b> LAG! </b><br>" +
	"Foda-se, a culpa não é minha. <br><p>"+
	"<b> ME DA MOD! </b><br>"+
	"'-' <br><p>"+
	"<b> Piratas virou comunidade de fic. </b><br>" +
	"avá<br><p>"+
	"<b> XBOX um >>>>>>>>>>>>>>>>>>>>>>> PS4 </b><br>" +
	".........................<br><p>"+
	".........................<br><p>"+
	"<i><b> Você sabia que... </b></i><br>" +
	"<i> Enquanto você lia, um corinthiano roubou seu carro? </i><br>" +
	"<i><strike> Alequis já foi útil? </i></strike>..<< Ele pediu pra mim remover, pois é ofensivo :(<br>"
   "<img src='http://omfgdogs.com/omfgdogs.gif'>";
...
// ======================   UI stuff    ==================================================================
    ...
    function displayDebugText(ctx, agarTextFunction) {
        ...
        if(1 <= cobbler.debugLevel) {
            ...
            debugStrings.push("G - Bot: " + (isGrazing ? (1 == isGrazing) ? "Old" : "Ativado" : "Desativado"));
        }
        if(2 <= cobbler.debugLevel) {
            debugStrings.push("M - Suspender Mouse: " + (suspendMouseUpdates ? "Sim" : "Não"));
    ...
    if (!d.agarioNoInit) {
        ...
        if ("undefined" == typeof console || ("undefined" == typeof DataView || ("undefined" == typeof WebSocket || (null == ib || (null == ib.getContext || null == d.localStorage))))) {
            ...
            d.setNick = function(a) {
                ...
                /*new*/console.log("foda se '" + a + "' as nick");
            };
            d.setRegion = ea;
...
'<h4 class="modal-title">AgarPilatas v' + '2.3; By: Miles Tailsko Prower' + '</h4>'+
'</div>'+
...
jQuery("#agario-main-buttons")
    .append('<button type="button" class="btn btn-danger" id="opnZC" onclick="showZCOverlay()" style="margin-top:5px;position:relative;width:100%;">Opções</button>');
 //.append('<button type="button" class="btn btn-safe" id="conect" onclick="connect("ws://" + prompt("Server IP", "XXX.XXX.XXX.XXX:XXX")); void 0;" style="margin-top:5px;position:relative;width:100%;">IP Connect</button>');
jQuery("#agario-main-buttons")
    .append('<button type="button" id="opnBrowser" onclick="openServerbrowser();" style="margin-top:5px;position:relative;width:100%" class="btn btn-success">Servers do AgarioMods</button><br>');
...
jQuery('#ZCOverlayBody').append('<div id="ZCStats" style="position:relative;width:100%; background-color: #FFFFFF; border-radius: 15px; padding: 5px 15px 5px 15px;">'+
    ...
    '<li role="presentation" class="active" > <a href="#page0" id="newsTab"   role="tab" data-toggle="tab">Início</a></li>' +
    //'<li role="presentation">                 <a href="#page1" id="statsTab"  role="tab" data-toggle="tab">Gabriel</a></li>' +
    '<li role="presentation">                 <a href="#page2" id="configTab" role="tab" data-toggle="tab">Coisinhas</a></li>' +
    '<li role="presentation">                 <a href="#page3" id="helpTab" role="tab" data-toggle="tab">AJUDA</a></li>' +
    ...
    '   <li><B>TAB</B> - Quando separado muda a célula principal</li>' +
    '   <li><B>A</B> - Modo Acido</li>' +
    '   <li><B>Q</B> - Macro (Atira massa igual um demente)</li>' +
    '   <li><B>C</B> - Desabilita/Habilita a parte visual da extensão</li>' +
    '   <li><B>G</B> - Desabilita/Habilita o novo grazer (Melhor)</li>' +
    '   <li><B>H</B> - Desabilita/Habilita o antigo grazer' +
    '   <li><B>E</B> - Atira no vírus perto do cursor</li>' +
    '   <li><B>R</B> - Atira no virus perto da célula selecionada (Virus fica vermelho)</li>' +
    '   <li><B>M</B> - Desabilita/Habilita controle pelo mouse</li>' +
    '   <li><B>Z</B> - Zoom</li>' +
    '   <li><B>Click</B> - Trava a célula (Se habilitado nas opções)</li>' +
    '   <li><B>S</B> - Destrava a célula (Se habilitado nas opções)</li>' +
    '</ul></div>' +
...
function DrawStats(game_over) {
    ...
    if (game_over){
        ...
        StopBGM();
        ShowZCStats();
        if(window.cobbler.autoRespawn && window.cobbler.grazingMode){setTimeout(function(){jQuery(".btn-play-guest").click();},500);}
    }
...
var col1 = $("#col1");
col1.append("<h4>Opções</h4>");
AppendCheckbox(col1, 'showZcStats-checkbox', ' Status na morte', window.cobbler.showZcStats, function(val){window.cobbler.showZcStats = val;});
col1.append("<h4>Modos</h4>");
AppendCheckbox(col1, 'litebrite-checkbox', ' Habilitar Modo Lite Brite', window.cobbler.isLiteBrite, function(val){window.cobbler.isLiteBrite = val;});
col1.append("<h4>Visual</h4>");
AppendCheckbox(col1, 'trailingtail-checkbox', ' Cauda', window.cobbler.drawTail, function(val){window.cobbler.drawTail = val;});
AppendCheckbox(col1, 'splitguide-checkbox', ' Mostrar distância de split', window.cobbler.splitGuide, function(val){window.cobbler.splitGuide = val;});
AppendCheckbox(col1, 'rainbow-checkbox', ' Pellets gays', window.cobbler.rainbowPellets, function(val){window.cobbler.rainbowPellets = val;});
AppendCheckbox(col1, 'namesunder-checkbox', ' Nomes nas células', window.cobbler.namesUnderBlobs, function(val){window.cobbler.namesUnderBlobs = val;});
AppendCheckbox(col1, 'gridlines-checkbox', ' Grid', window.cobbler.gridLines, function(val){window.cobbler.gridLines = val;});
col1.append("<h4>Status</h4>");
AppendCheckbox(col1, 'chart-checkbox', ' Alguma coisa', display_chart, OnChangeDisplayChart);
AppendCheckbox(col1, 'stats-checkbox', ' Status no jogo', display_stats, OnChangeDisplayStats);
col1.append("<h4>Extras</h4>");
AppendCheckbox(col1, 'feature-click-fire', ' Clicar para ativar macro do vírus', window.cobbler.rightClickFires, function(val) {window.cobbler.rightClickFires = val;});
AppendCheckbox(col1, 'feature-blob-lock', ' Clicar para travar célula', window.cobbler.enableBlobLock, function(val) {window.cobbler.enableBlobLock = val;});
AppendCheckbox(col1, 'feature-blob-lock-next', ' Mudar célula no click', window.cobbler.nextOnBlobLock, function(val) {window.cobbler.nextOnBlobLock = val;});

var col2 = $("#col2");
col2.append('<h4>Debug</h4><div class="btn-group-sm" role="group" data-toggle="buttons">' +
...
col2.append('<h4>Macro para vírus</h4><h5>Millisegundos por tiro</h5><div id="mspershot-group" class="input-group input-group-sm"> <input type="text" id="mspershot-textbox" class="form-control" placeholder="1-2000 (Default: 100)"' +
    'value=' + cobbler.msDelayBetweenShots.toString() + '><span class="input-group-addon">ms</span></div><h6>145ms = 7 tiros. Valores menores = BUG, BUG, E BUG!</h6>');
...
col2.append('<h4>Tamanho do Minimap</h4>' +
...
var col3 = $("#col3");
col3.append("<h4>Sons</h4>");
col3.append('<p>SFX<input id="sfx" type="range" value=' + window.cobbler.sfxVol + ' step=".1" min="0" max="1" oninput="volSFX(this.value);"></p>');
col3.append('<p>Musica<input type="range" id="bgm" value=' + window.cobbler.bgmVol + ' step=".1" min="0" max="1" oninput="volBGM(this.value);"></p>');
col3.append('<h4>Skins</h4>');
AppendCheckboxP(col3, 'amConnect-checkbox', ' Skins do AgarioMods', window.cobbler.amConnectSkins, function(val){window.cobbler.amConnectSkins = val;});
AppendCheckboxP(col3, 'amExtended-checkbox', ' O mesmo de cima, só que costumizadas', window.cobbler.amExtendedSkins, function(val){window.cobbler.amExtendedSkins = val;});
