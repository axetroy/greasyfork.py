...
(function(){
  ...
  const DEBUG = false;/*
  [update] 2.6.0
  ビデオで右矢印キーの長押しによる高速早送りに対応(通信速度に依存。ブラウザの仕様により巻き戻しは従来通り)。ビデオで[N]で次のエピソードに移動。ビデオCM中のホイール音量に対応。ヌルヌルマウスホイールで音量微調整しやすく。Edge対応(マウスホイールは除く)。ほか、軽微な修正。
  (z-index, cover.wheel, isConnected, html.TimetableViewer, passive:false, indicator.fixed)
  ...
  */
  ...
  let site = {
    elements: {
      ...
      closer: function(){
        /* チャンネル切り替えごとに変わる */
        let viewCounter = $('[class*="tv-ViewCounter__"]');
        if(!viewCounter) return log(`Not found: viewCounter`);
        let button = viewCounter.parentNode.parentNode.parentNode.parentNode.querySelectorAll('div > button')[1];/*アベマの構造にすごく依存する*/
      ...
      timeshiftCover: function(){let node = $('.c-tv-SlotPlayerContainer'); return node ? node : DUMMY;},
      videoCover: function(){let node = $('.c-vod-PlayerContainer-wrapper'); return node ? node : DUMMY;},
      ...
      miniScreenInBrowserButton: function(){let node = $('.com-vod-VideoControlBar use[*|href^="/images/icons/mini_screen_in_browser.svg"]'); return node ? node.parentNode.parentNode : null;},
    ...
    wheel: function(e){
      ...
      switch(e.deltaMode){
        ...
        default:
          switch(true){
            case(d < 0): volume += (volume <  10) ? 1 : 5; break;
            case(0 < d): volume -= (volume <= 10) ? 1 : 5; break;
          }
          break;
      }
    ...
    modifyVolume: function(volume){
      /* 0-100の音量調整に対応する */
      let slider = site.elements.volumeSlider(), rect = slider.getBoundingClientRect();
      volume = between(0, volume, 100);/*四捨五入して0-100に収める*/
      let options = {
      ...
      core.indicateVolume(Math.round(volume));
    },
    assign: function(e){
      switch(true){
  ...
  let html, elements = {}, configs = {}, keyPressing, indicatorTimer;
  let core = {
    initialize: function(){
      html = document.documentElement;
      html.classList.add(SCRIPTNAME);
      configs = Storage.read('configs') || configs;
    ...
    indicateRewind: function(rewinded = true){
      let indicator = elements.indicator;
      while(indicator.firstChild) indicator.removeChild(indicator.firstChild);
      indicator.appendChild(createElement(core.html.rewind(rewinded)));
      indicator.classList.add('active');
      clearTimeout(indicatorTimer);
      indicatorTimer = setTimeout(function(){
        indicator.classList.remove('active');
      }, rewinded ? 20*1000 : 1000);
    },
    realtime: function(e){
      switch(true){
        /* 音量 */
        ...
        case(e.key === 'ArrowLeft'):
          ...
          let videos = document.querySelectorAll('video[src]'), rewinded = false;
        ...
        case(e.key === 'm'):
          ...
          if(site.isMuted()) core.indicateVolume('mute');
    ...
    video: function(e){
      switch(true){
        ...
        case(e.type === 'wheel' && Math.abs(e.deltaX) <= Math.abs(e.deltaY)/*縦ホイールのみ*/):
          /* あらゆる場所でのイベントを拾ってwindow.addEventListenerで一括処理する代償をここで支払う */
          let parents = [site.elements.timeshiftCover(), site.elements.videoCover(), site.elements.adCover()];
          for(let target = e.target; target; target = target.parentNode){
            if(parents.includes(target)){
              site.wheel(e);
              return e.preventDefault();
