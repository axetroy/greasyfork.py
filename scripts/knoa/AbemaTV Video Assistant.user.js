...
// @namespace   knoa.jp
// @description AbemaTVのビデオ視聴を快適にします。
...
ビデオ視聴ページの歯車アイコンから設定できます。

設定項目:
ビデオ再生
　自動で再生を開始する/しない
　ブラウザ全画面かどうかを記憶する/しない
　再生速度を記憶する/しない
次のエピソードへの移動
　次のエピソードへの移動ボタンを出す/出さない
　　ビデオの最後まで再生してから出す/出さない
　　カウントダウンして自動移動する/しない

そのほかの機能:
CM中でも映像クリックで一時停止させる
再生時刻の更新頻度を再生速度に比例させる
再生速度ボタンのマウス判定領域を拡大
音量調整ボタンのマウス判定領域を拡大
  ...
  */
  ...
  let site = {
    ...
    get: {
      ...
      currentPlaybackRate: () => $('input[name="vod-setting-playbackRate"][checked]'),/*現在選択中の再生速度*/
  ...
  let core = {
    ...
    keepSpeed: function(){
      ...
      if(!video.isListeningRatechange){/*ビデオごとに1度だけ*/
        video.isListeningRatechange = true;
