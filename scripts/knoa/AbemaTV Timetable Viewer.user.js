...
(function(){
  ...
  const DEBUG = false;/*
  [update] 2.3.0
  番組検索の対象に見逃し番組とビデオを追加。通知一覧にマイビデオの一覧タブを追加。通知一覧にマイビデオへ登録できるボタンを追加。番組表にアベマ番組表ページへの移動ボタンを追加。初回ページ読み込み時の負荷を軽減。Edge対応。ほか、軽微な修正。
  (progressTime, listPane, tabs.score, isConnected, searchPane=>listPane, groupId)
      ...
      window.addEventListener('click', function(e){
        switch(true){
          ...
          case(e.target === elements.channelButton):
            if(elements.channelPane.attributes['aria-hidden'].value === 'true'){/*チャンネルロード前*/
              /* 表示しようとするときはアベマのローディング完了を待って改変する */
              core.channelPane.observeOnce(function(){
                core.channelPane.modify();
                core.channelPane.scrollToChannel(core.getCurrentChannelId());
              });
            }else if(html.classList.contains('channel')){/*ScreenCommentScrollerによってロード済み*/
              core.channelPane.modify();
              core.channelPane.scrollToChannel(core.getCurrentChannelId());
            }
            return;
