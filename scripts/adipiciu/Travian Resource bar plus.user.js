...
function getTTime(dist, speed, arena, artefact, shoes, leftHand) {
	...
	var abonus = ((RB.Setup[46] == 1 || RB.Setup[49] == 1) ? 0.2 : 0.1); //if Travian Path to Pandora server, or Travian 4.4 2019 version, then double the Tournament Square bonus
...
function getUserID() {
	try {
		var uName = $gt('a',$gc('playerName',$g('sidebarBoxHero'))[0])[1].textContent;
	} catch(e) { return null }
...
acss += "table#vlist td{padding:0;line-height:16px;text-align:"+docDir[0]+";white-space:nowrap;}table#vlist thead td{background-color:#FFF;height:22px;text-align:center;padding:0px 3px;}" +
	...
	acss += "span."+allIDs[48]+" {position:relative;float:"+docDir[0]+";margin-top:-15px;"+docDir[0]+":110px !important;}"+
...
function neededResAdd () {	
	...
	for( var j = 0; j < baseWrap.snapshotLength; j++ ) {
		var baseCosts = $xf('.//div[contains(@class,"contractCosts") or contains(@class,"resourceWrapper")]','l', baseWrap.snapshotItem(j));
		for( var i = 0; i < baseCosts.snapshotLength; i++ ) {
			...
			if( wfl || baseCosts.snapshotItem(i).parentNode.getAttribute("class") != "details" ) {
				var res = $gc('resourceWrapper',baseWrap.snapshotItem(j));
				if (res.length > 0) {
					res[0].parentNode.insertBefore(newD, res[0].nextSibling);
				}
			}
			addNPC(baseWrap.snapshotItem(i));
		}
	}
	...
	for( var i = 0; i < research.snapshotLength; i++ ) {
		...
		var res = $gc('resourceWrapper',base);
		if (res.length > 0) {
			var newD = needed_show( base.innerHTML );
			if( wfl ) res[0].parentNode.insertBefore(newD, res[0].nextSibling);
		}
		addNPC(base);
	}
	
	var heroCost = $xf('//div[@class="regenerateCosts" or @class="hero-dead"]','l',cont);
...
function marketTradeRoutes() {
	function createTrade(userHour,minute) {
		var sParams = '&did_dest='+$g("trade_route_destination",routesForm).value;
		...
		if ($g('trade_route_mode_send').checked) {
			sParams += '&trade_route_mode=send';
		} else if ($g('trade_route_mode_deliver').checked) {
			sParams += '&trade_route_mode=deliver';
		}
		sParams += '&hour='+userHour;
		var mins = routesForm.querySelector('input[name="minute"]').value
		if ( mins ) { sParams += '&minute='+mins; } else { sParams += '&minute=00'; }
		...
		ajaxRequest(fullName+'build.php?show-destination=all', 'POST', sParams, function(ajaxResp) { return function(userHour) { return logTradeRoutes(userHour,1); }(userHour); }, 
			function(ajaxResp) { return function(userHour) { return logTradeRoutes(userHour,0); }(userHour); } );
	}
	...
	p.appendChild($e('BR'));
	var btnText = $g("tradeSaveButton",routesForm);
	var but = $e('INPUT',[['type','button'],['value',btnText.value],['style','margin:10px 0;padding:2px 10px;'],['onclick','marketPlace.validateTradeRouteResourcesSanity() && marketPlace.validateTradeRouteSendTimeSanity();return false;']]);
	p.appendChild(but);
...
function bigQuickLinks () {
	...
	function CreateBigLinkButton (strBuilding, iNo) {
		var pos = '';
		if (iNo == 8) { pos = docDir[0] + ':160px;' }
		if (iNo == 9) { pos = docDir[0] + ':200px;' }
...
function scanTroopsData () {
	function scanTropsDataR ( hn ) {
		ajaxRequest(fullName +'manual.php?typ=1&s='+ hn, 'GET', null, function(ajaxResp) {
			...
			/* commented until travian fixes bug (crop consumption replaced wrong by free crop)
			if( RB.dictFL[18] == "0" ) {
...
function displayWhatIsNew () {
	if ($g('whatsnew')) { 
		...
		content.innerHTML = "What's new in Version "+version+" - Apr 1, 2019:<p></p><ui><li>Minor fix for resources/culturepoint at level 1</li><li>Added info with the number of traders for every market transport</li></ui>";
