...
//************regionEditorsHTML is the table header info
      ...
      function addStatsToTable(element, index, array)
      {
        ...
        editors[index].totalEdits = numberWithCommas(editors[index].totalEdits);
        editors[index].rate7 = numberWithCommas(editors[index].rate7);
        editors[index].rate30 = numberWithCommas(editors[index].rate30);
        editors[index].rate60 = numberWithCommas(editors[index].rate60);
        editors[index].rate90 = numberWithCommas(editors[index].rate90);
          ...
          rowEditorName.forEach(function(element,index,array)
          {
              ...
              if(index > 0) {
                  if ($('.rate-7').prop('checked')) {
                      console.log('rate 7');
                      rows[index].children[7].textContent = editors[whoIndex].rate7;
                  }
                  if ($('.rate-30').prop('checked')) {
                      console.log('rate 30')
                      rows[index].children[7].textContent = editors[whoIndex].rate30;
                  }
                  if ($('.rate-60').prop('checked')) {
                      console.log('rate 60');
                      rows[index].children[7].textContent = editors[whoIndex].rate60;
                  }
                  if ($('.rate-90').prop('checked')) {
                      console.log('rate 90');
                      rows[index].children[7].textContent = editors[whoIndex].rate90;
                  }
              }
          });
